...
- **테스트**: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **971 passed / 48 skipped / 215 subtests**(2026-07-13, v1.6.80 aggregate budget; 현재 환경은 ES Python dependency 미설치라 관련 3개 skip). skip은 대부분 live Mongo/Chroma/embedding 미가용 통합 테스트.
...
- 현재 시스템 전체 스위트: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **971 passed / 48 skipped / 215 subtests**. `git diff --check` clean.
