...
- 정본 SoT는 `docs/system-contract-sot.md`이며 현재 **v1.6.72**(Approved). SoT가 정본 우선순위이고, 미확정 항목은 추측 구현하지 않는다.
...
- 구현된 계층(모두 회귀로 잠김, 아래는 현재 동작하는 표면):
  ...
  - **Writing AI(Phase 5.1~5.5, v1.6.68~72)**: 평문 candidate 생성→별도 typed report extractor→Writing Gate→accept/save→pending Analysis job. report는 Gate JSON 입력과 AnalysisJob advisory copy로 소비되며 snapshot extraction 정본과 direct-mint 금지를 유지한다. side-effect-free inline `POST /writing/report`가 서버 재구성 ContextPackage로 재평가한다. persisted candidate/report 감사 이력·full pointer·부분 revise는 후속.
...
- **테스트**: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **883 passed / 45 skipped / 155 subtests**(2026-07-13, v1.6.72 `/writing/report` 독립 검증 B1 closure+H1~H4 보강; 이 머신은 ES Python dependency가 있어 종전 skip 3개가 실행됨). skip은 대부분 live Mongo/Chroma/embedding 미가용 통합 테스트.
...
1. **Phase 5.2 Writing Gate 구현 완료(v1.6.69)** — 별도 LLM Gate service/prompt/models + `POST /projects/{id}/writing/gate`, 구조화 findings와 다섯 decision/우선순위, project isolation·strict JSON/provider error mapping을 회귀로 잠갔다. 다음 핵심 후보:
   ...
   - **구조적 candidate report + inline 재평가 API 완료(v1.6.71~72)** — 후속 확정: stable context full pointer schema, candidate/report persistence+감사 이력과 id 기반 API(B), finding evidence 기반 부분 revise/retrieve orchestration.
...
- 현재 시스템 전체 스위트: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **883 passed / 45 skipped / 155 subtests**. `git diff --check` clean.
