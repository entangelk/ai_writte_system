...
- **★ Phase 8 Slice 8.0 — billable request 경계 B1~B6**(브리프 [`plans/08-0-billable-request-boundary-decisions.md`](docs/plans/08-0-billable-request-boundary-decisions.md), 2026-08-03). **"서비스 요청 1회"가 코드의 무엇인지**를 정하는 결정 6개다: B1 차감 단위(요청 1건 / provider 호출 / 가중치) · B2 내부 repair·라운드 · B3 fan-out 경로(`compare`는 후보 N건마다 호출) · B4 조회성·임베딩 경로 · B5 "같은 한 번"의 정의 · B6 전수 가드 형태. **추천은 전부 A**이고 근거는 브리프 안에 있다. **결정 전에는 카운터·원장·차감 코드를 만들지 않는다** — 지금까지 만든 것은 실측 인벤토리와 브리프뿐이다. 결정에 영향을 주는 실측 셋: ① `/writing/accept`는 멱등 replay여도 보고서 호출을 이미 써 버린다 ② `/analysis/.../run`의 replay는 LLM 0회다 ③ 한 요청의 provider 호출은 **0~15회**로 변한다(`revise-and-gate` 기본 정책 기준).
...
1. **Phase 8 Slice 8.0 — 오너 결정 대기 중이다**(브리프 제출 완료, 2026-08-03). 머신 전환 체크와 실측 인벤토리는 끝났다. 다음 작업자가 할 일은 **오너가 B1~B6을 확정한 뒤**: ① 결과를 billable-action 표로 확정해 브리프·SoT에 반영 ② (B6=A면) 라우트 전수 대비 **분류 누락을 실패시키는 테스트를 먼저** 작성 — 양방향(분류 누락도, 무료 경로의 오분류도 실패) ③ 기록 갱신 ④ 8.1 정책 브리프로 인계. 결정 전에는 카운터·원장·차감 코드를 만들지 않는다.
