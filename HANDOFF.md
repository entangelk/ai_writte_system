...
>
> 마지막 자가 검수: **2026-09-10 · 340줄** — 계정 탈퇴 Slice 4(세션 55) 반영: 계약 절 SoT v1.8.48→**v1.8.54**(여섯 판 뒤처져 있었다) · **프런트 기준선 467/38 → 460/39 정정**(09-08 슬라이스가 안 고쳐 한 슬라이스 샜다 — 원인과 유도를 그 줄에 적었다) · Slice 4 계약 주의 넷 등재 · Next Tasks 순서 재부여(탈퇴 축이 앞자리에서 빠지고 랜딩·동의 게이트가 1번).
...
정본은 [`docs/system-contract-sot.md`](docs/system-contract-sot.md) **v1.8.58**(Approved). **미확정 항목은 추측 구현하지 않는다.** 아래는 코드를 만지기 전에 알아야 하는 요약이다.
...
**⚠️ 오너 결정이 있어야 움직이는 것 — ✅ 2026-09-09 현재 비어 있다**
...
## 랜딩 축 ② (약관·방침 페이지 + 공개 푸터) — 조건 폐쇄됨, 승격 재검 대기 (2026-09-11)

기록 [`landing_l2_terms_privacy.md`](docs/verifications/2026-09-11/landing_l2_terms_privacy.md) **조건부 합격**. D2=ⓐ의 대조 가드는 **양방향으로 실증**(사본만·정본만·감시 축소 전부 기명 실패)됐고 세션 58 변이 표 재적용 6종·번들 수치·전수 476/41·문서 가드 58/1110 전부 재현 — 보고는 정직했다. 남는 것:

- **LB1 — 푸터의 세 장착 지점 중 둘이 무셀**: SoT v1.8.56 이 *"정문 세 얼굴"* 로 열거한 중 **AuthStatus(세션 확인·오류)와 가입 접수 얼굴**은 각각만 제거해도 전건 초록이었다(MO-6a·6b). 실패 셀 둘 다 로그인 폼 얼굴만 봤다 — **세션 56 B1(배너 장착)과 같은 형태의 재발**. **폐쇄(2026-09-11, 세션 60 · `7be433e` · SoT v1.8.57)**: 세 얼굴을 한 셀에 담고 사이를 `unmount()` 로 끊는다 — MO-6a 1실패 · MO-6b 1실패 · **ML-3b 2실패 = 기존 셀이 폼 얼굴을 계속 지킨다(세 얼굴 분할 점유)**. **하드닝 H3 도 함께 닫았다**(접두 필터가 문서 전체에 걸려 본문의 `근거:` 줄이 조용히 사라지던 축 — 머리말 경계로 제한, MH-3a 1실패 · MH-3b 2실패 · **MO-8 등가 초록**). **판정은 아직 조건부 합격이다** — 승격 재검(변이 재적용 MO-6a·6b·ML-3b + MH-3a·3b·MO-8)은 다음 **검증 세션 몫**이다. **★ H2(ML-10)도 닫혔다 — 오너가 갈래 ⓑ 를 골랐다**(SoT v1.8.58): 머리말 고지에서 **오너·날짜·단계 서술을 걷고**(그 자리는 [`legal/README.md`](docs/legal/README.md)) **금지 어휘 셀**을 세웠다. **계약 주의**: 약관·방침 본문에 `오너`·`브리프`·`HANDOFF`·`SoT`·`work_log`·`커밋`·`저장소`·`포트폴리오`·`구현 중`·`미착수`·`README` 를 쓰면 전수가 빨개진다 — 개발 과정의 서술은 `legal/README.md` 로 가고 **문서에는 회원이 읽을 문장만** 남긴다(`근거:` 머리말 줄만 예외 — 렌더러가 공개면에서 걷는다).
- 하드닝: 편집 메모 재유입(정본·사본 양쪽 일관)은 무가드(ML-10 초록 — 바이트 가드는 "같은가"만 알지 "무엇이"를 모른다) · `근거:`/메타 필터가 문서 전체에 적용(본문 중간의 `근거:` 줄은 조용히 사라짐).
...
>
> **★★ 랜딩 축 ② 의 검증 조건 LB1 은 2026-09-11 에 닫혔다(위 절, `7be433e` · SoT v1.8.57).** 남은 것은 **승격 재검**이고 **검증 세션 몫**이다 — **구현 세션이 다음에 집는 것은 ③ 랜딩 본문과 ④ 가입 동의 게이트**고, 그 뒤가 **Slice 5(문서 승격)** 와 **Slice 4b(관리자 잔여 정리 — 브리프 선행)** 다.
>
> | 순서 | 무엇 | 왜 이 순서인가 |
> |---|---|---|
> | **1** | **랜딩 ② 승격 재검**(검증 세션) | ~~조건 LB1 폐쇄~~ **완료(2026-09-11, `7be433e` · SoT v1.8.57 — 하드닝 H3 동반)**. 남은 것은 변이 재적용(**MO-6a·6b·ML-3b + MH-3a·3b·MO-8**)으로 폐쇄를 재현하고 판정을 올리는 일 — [`landing_l2_terms_privacy.md`](docs/verifications/2026-09-11/landing_l2_terms_privacy.md) §폐쇄 보고. ~~Slice 4 승격 재검~~ **완료(2026-09-11, 세션 59)** |
> | 2 | **12번 랜딩 ③ 본문 + 10번 동의 게이트 ④** | **한 창에서 본다** — 아래 4번 행이 그 순서이고 ~~①②~~ 는 닫혔다 |
> | 3 | 11번 **Slice 5**(정책 §8 → §6 승격) | 화면까지 섰으므로 *"정해졌으나 미시행"* 이 더는 참이 아니다. 한 편집이고 포인터를 단다 |
> | 4 | **12번 랜딩 + 10번 동의 게이트** | **한 창에서 본다.** 순서는 ~~① 약관 대괄호 넷 치환 + 시행 버전 문자열~~ **완료(2026-09-09, 버전 `1.0`)** ~~② `/terms`·`/privacy` + 푸터~~ **완료(2026-09-11, SoT v1.8.56 · 검증 조건 LB1 폐쇄 v1.8.57)** → **③ 랜딩 본문 ④ 동의 게이트**가 남았다. **Phase S-2(nginx 헤더)·AdSense 화면별 제어도 같은 창** — 광고가 랜딩·로그인에 더해 **약관·방침 페이지에도** 뜬다 |
> | 5 | 2번 최종 저장·분석 6차 승격 재검증 | 셀은 이미 있고 MW-D 재적용만 남았다 |
> | 6 | 6번 육안 확인 · 8번 확인용 계정 정리 | 오너·운영 데이터 축 |
> | — | **11번 Slice 4b**(관리자 잔여 정리) | **브리프가 먼저다** — 갈래 셋이 [`slice4-…-decisions.md`](docs/plans/slice4-withdrawal-screen-decisions.md) §후속 고려에 있다. 트리거 없음(S-2 와 같은 분류) |
>
...
6. **육안 확인(누적)** — 프론트 재빌드 선행. 스타일 개선은 [Phase W](docs/plans/frontend-writing-studio-phase.md)를 따른다. **W2 한글 타이포(시스템 서체 축)·W3 서재는 닫았다(2026-09-12)** — W2: 원고 본문 한글은 OS별 명조 폴백, UI 한글은 system-ui 뒤 고딕 고정, 라틴·숫자 불변. W3: 순서 배지 제거·보관 칩·모바일 두 줄 행(메모 행 제외)·320px 넘침 해소. **웹폰트 도입(기기 간 일관성·두자 해소)은 오너 결정 갈림길로 열려 있다**, 제목 등 나머지 serif 표면의 한글 서체도 원고 명조가 자리 잡은 뒤 별도 비교다. **다음은 W4: 보조 표면·마감**이다(폼-목록 사이 평행선 미관도 여기서 본다). 모바일 도구는 48rem 이하에서 원고 앞의 일반 흐름(42rem 이하 2열)에 있고, 데스크톱은 고정 우측 띠다. 모바일을 스티키로 바꾸면 원고 겹침을 다시 검증해야 한다. 기존 블루 팔레트·공통 외곽 폭은 유지한다. Phase 10 마감 다섯(첫 화면 콘텐츠 · 오른쪽 끝 정렬 · 목록 행 간격 · 차트 막대 테두리 · 버튼 hover) · 비활성 버튼 색 · 편집기 드로어와 설정 탭이 좁은 화면에서 겹치는지 · 관측 화면과 비동기 패드 렌더 · **최종 저장·분석 연동의 첫 실사용**(운영 이력 0건) · **장면 메모 행 꼬리줄**(좁은 드로어에서 수정 시각과 "더 보기"가 한 줄에 남는지 · 긴 전문을 펼쳤을 때 목록 스크롤).
...
10. **★ 가입 동의 게이트(오너 결정 2026-09-07)** — 가입 절차에서 약관을 확인하게 하고 **동의 시각 + 약관 버전을 저장**한다. **★ 그 버전은 이제 확정이다 — `1.0`**(2026-09-09, SoT v1.8.50). 핀 셀이 문서의 값을 잠그고 있으니, 코드에 상수를 만들면 **그 핀 셀이 상수를 가리키게** 바꾼다. 기존 계정에 **소급 동의는 받지 않는다**(그래서 *"모든 회원이 고지를 봤다"* 고 말할 수 없다 — 시행일 이후 가입자만이다). **선행은 끝났다** — [`docs/legal/`](docs/legal/README.md) 두 문서가 **버전 `1.0` · 시행일 2026-09-08** 로 시행 표기됐고(머리말·부칙 두 자리를 가드가 함께 잠근다), 남은 것은 게이트 자체다(동의 시각·버전 필드, 가입 확인 단계). **초안의 대괄호 넷은 코드가 못 채운다** — 특히 `[추론 서비스 사업자]` 는 배포 `.env` 가 정한다. 근거: 원고가 이미 스택 밖으로 나간다(정책 문서 §8).
11. **★ 셀프 계정 탈퇴 + 30일 유예 파기(오너 결정 2026-09-07)** — **버튼 하나가 아니다.** 구현 순서는 [`plans/account-withdrawal-implementation-phases.md`](docs/plans/account-withdrawal-implementation-phases.md) 에 있고 **~~D4~~·~~Slice 0~~·~~1~~·~~2~~·~~3~~ 이 닫혔다**(SoT v1.8.43·v1.8.45·v1.8.46·v1.8.48·**v1.8.51**). **~~Slice 4(화면)~~ 도 닫혔다**(2026-09-10, SoT v1.8.54 — 브리프 [`slice4-…-decisions.md`](docs/plans/slice4-withdrawal-screen-decisions.md), D1=ⓐ 전역 배너 · D2=ⓐ 관리자 축 분리). **남은 것은 Slice 4b(관리자 잔여 정리)·5(문서 승격)다.**
    ...
    - **정책 문서 §6 승격은 Slice 5** 다.

12. **★ 랜딩 페이지 기획(오너 2026-09-07·2026-09-08)** — **①(약관 값·시행 표기)은 닫혔다**(2026-09-09, SoT v1.8.50) · **②(`/terms`·`/privacy` + 푸터)도 닫혔다**(2026-09-11, SoT v1.8.56). 약관·방침은 **버전 `1.0` · 시행일 2026-09-08** 로 시행 표기됐고 대괄호 넷이 다 채워졌다([`docs/legal/`](docs/legal/README.md)). **남은 것은 ③ 랜딩 본문 · ④ 동의 게이트(10번)** 다. **★ ② 가 남긴 계약 주의 다섯**: ⓐ **약관·방침 본문을 고치는 쪽은 `docs/legal/` 하나다** — `frontend/src/legal/*.md` 는 **사본**이고 갈라지면 전수가 빨개진다(`legalSource.test.ts` 바이트 대조). 고칠 때 두 파일을 함께 옮긴다. ⓑ **`AuthGate` 는 보호 구간(`path="*"`)만 감싼다** — 공개 라우트를 그 안으로 넣으면 **가입 전에 읽어야 하는 문서가 세션 뒤로 숨는다**(변이 ML-2 가 7셀로 잡는다). 보호 구간을 **여러 자리로 쪼개지 말 것**(라우트마다 세션 확인이 다시 나간다). ⓒ **푸터는 공개 표면에만** 붙는다(정문 세 얼굴 + 약관·방침 + 앞으로 랜딩). 앱 셸에 달면 over 셀이 문다 — 뒤집으려면 그 결정을 먼저 바꾼다. ⓓ **렌더러는 부분집합이다** — 문서에 새 문법(`### `·이미지·표 안 목록)을 쓰면 **문단 텍스트로 떨어진다**(멈추지는 않는다 — 종료 셀이 그것을 잠근다). 새 문법을 쓸 거면 `markdown.tsx` 를 먼저 고친다. **머리말 접두 규칙(`근거:` 걷기 · `상태`·`작성`·`버전` 한 줄씩)은 머리말 안에서만 산다**(경계 = 첫 `---`, 없으면 첫 `## `) — 그 경계를 없애면 본문의 같은 모양 줄이 조용히 사라진다(MH-3a 가 문다). ⓔ **버전 `1.0` 핀이 이제 두 곳**이다(프런트 `legalSource.test.ts` · 백엔드 `test_service_policy_contract.py`) — 동의 게이트가 상수를 만들면 둘이 함께 그 상수를 가리켜야 한다. **★ 버전 문자열 `1.0` 이 10번의 입력**이다(동의 시각과 함께 저장). **★ 시행 전제 둘이 아직 미구현이라는 사실**은 문서가 스스로 적는다 — 계정 탈퇴는 Slice 4~5 가, 동의 게이트는 10번이 닫는다. **★ `[문의 연락처]` 는 개인 이메일이고 공개 문서 7곳에 있다** — 폼·별칭으로 바꾸려면 치환 지점이 그만큼이다. 공개 전 법적 요건 축이라 **Phase S-2(nginx 보안 헤더)·AdSense 화면별 제어와 같은 창**에서 보는 편이 싸다(광고가 랜딩에도 뜬다).
