...
>
> 마지막 자가 검수: 2026-08-01 · 224줄 (**D8-6c 전부 완료(6c-1·6c-1b 백엔드 purge + 6c-2 worker PROJECT_PURGED drain 연결) + SoT v1.7.73**. 6c-2 회귀 3을 _fixture 헬퍼 뒤에 잘못 넣어 수집 제외된 무효 가드를 발견/수정(f8a6ad3) — 뮤테이션 re-fail로 가드 유효 확정. 1817 passed/회귀 0. 남은 6 = 6d(endpoint). 다음 검수 트리거는 300줄)
...
- 정본은 `docs/system-contract-sot.md` **v1.7.73**(Approved). 미확정 항목은 추측 구현하지 않는다.
- **진행 중 페이즈 = 다중 사용자 인증(D8).** 오너 지시로 **슬라이스를 잘게 쪼개 진행 중**이다(한 번에 큰 덩어리 금지). 진행표:
  ...
  | **6 (D8-6a·6b·6c)** | **6a·6b·6c 전부 완료, 6d(endpoint) 대기** | 영구 삭제 — **6a**(core_sot 8) + **6b**(derived 10) + **6c**(vector/index 백엔드 + worker drain) 완료. 6c-1·6c-1b(memory/candidate vector·lexical purge) + **6c-2**(worker `_drain_purge` whole-event all-or-retry + `run_once` PROJECT_PURGED 분기 + source_block archive purge + **깨진 guard `_archive_where` ValueError 차단**) — **5 백엔드(source_block + memory vec/lex + candidate vec/lex) drain consistent 연결**. 전수 가드 `IndexingBackendPurgeCoverageTest` 7 백엔드. **6d**(`POST /admin/projects/{id}/purge` + `_REQUIRE_ADMIN` + boundary matrix ADMIN +1·총 +1)만 남음 — endpoint 는 d 에서만(부분 삭제 고아 위험 D5). 상세 = `daily_logs/2026-08-01/` 6c-2 섹션 |
