...
정본은 [`docs/system-contract-sot.md`](docs/system-contract-sot.md) **v1.8.63**(Approved). **미확정 항목은 추측 구현하지 않는다.** 아래는 코드를 만지기 전에 알아야 하는 요약이다.
...
**⚠️ 오너 결정이 있어야 움직이는 것 — 2026-09-12 현재 한 건**
...
>
> **★★ 랜딩 축 ③(본문)과 ④(가입 동의 게이트)가 2026-09-12 에 구현됐다(SoT v1.8.59 · 아래 10·12번).** **Slice 4b(관리자 잔여 정리)도 2026-09-12 구현이 끝나 계정 탈퇴 구현 축은 전부 닫혔다** — 남은 구현 축은 5번(최종 저장 6차 재검증)뿐이다. Slice 5·4b 의 **판정 승격은 다음 검증 세션 몫**이다.
>
> | 순서 | 무엇 | 왜 이 순서인가 |
> |---|---|---|
...
> | 3 | ~~11번 **Slice 5**(정책 §8 → §6 승격)~~ **완료(2026-09-12, SoT v1.8.60)** — §6 에 30일 행 + 정본 포인터 · 두 문서 머리말의 **거짓이 된 미제공 고지** 정리 · 약관 제8조 3항·방침 제5조 3항의 `30일` ↔ 상수 신규 가드. **독립 검증 조건부 합격 → 조건 셋(B1 취소 경계 · B2 방침 머리말 · B3 §8 자기모순) 같은 세션에 폐쇄 + 하드닝 다섯**(SoT v1.8.61). **판정 승격은 다음 검증 세션 몫이다.** |
...
> | — | ~~11번 Slice 4b~~(관리자 잔여 정리) | **완료(2026-09-12, SoT v1.8.62)** — 브리프 [`slice4b-admin-residual-purge-decisions.md`](docs/plans/slice4b-admin-residual-purge-decisions.md), 오너 결정 ①ⓐ·②ⓐ·③ⓐ. **독립 검증 조건부 합격 → 조건 셋(C1 실행 재확인 · C2 감사 화면 배제 · C3 상속 과교정) 같은 날 폐쇄(v1.8.63) — 판정 승격은 다음 검증 세션 몫** |
>
