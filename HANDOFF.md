...
- 정본 SoT는 `docs/system-contract-sot.md`이며 현재 **v1.6.76**(Approved). SoT가 정본 우선순위이고, 미확정 항목은 추측 구현하지 않는다.
...
- 구현된 계층(모두 회귀로 잠김, 아래는 현재 동작하는 표면):
  ...
  - **Writing AI(Phase 5.1~5.8, v1.6.68~76)**: 평문 candidate 생성→typed report→Writing Gate→accept/save→pending Analysis job. inline report/revise와 `/writing/revise-and-gate` 합성까지 구현. 첫 Gate가 retrieve_more면 follow-up retrieval planner가 필요한 query+canonical needs만 골라 이전 package+targeted delta를 merge하고, report 재호출 없이 Gate를 최대 한 번 재평가한다. retrieval 실패는 candidate+첫 Gate partial artifact를 보존한다. 모델은 호출/응답, Application은 anchor·validation·splice·검색 합성·loop/budget 제어를 소유한다. multi-finding/2회 이상 내부 loop·persisted 감사 이력은 후속.
...
- **테스트**: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **917 passed / 45 skipped / 198 subtests**(2026-07-13, v1.6.76 targeted retrieve_more 검증 B2/H1~H3 closure; 이 머신은 ES Python dependency가 있어 종전 skip 3개가 실행됨). skip은 대부분 live Mongo/Chroma/embedding 미가용 통합 테스트.
...
1. **Phase 5.2 Writing Gate 구현 완료(v1.6.69)** — 별도 LLM Gate service/prompt/models + `POST /projects/{id}/writing/gate`, 구조화 findings와 다섯 decision/우선순위, project isolation·strict JSON/provider error mapping을 회귀로 잠갔다. 다음 핵심 후보:
   ...
   - **report + 부분 revise + report 최신화 + targeted retrieve_more 1회 완료(v1.6.71~76)** — 다음 Writing 후보는 G8 B 내부 loop의 자동 반복 decision/finding·사람 확인·전체 search/provider/token/time budget 브리프. 이후 multi-finding, stable pointer, persisted 감사 API.
...
- 현재 시스템 전체 스위트: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **917 passed / 45 skipped / 198 subtests**. `git diff --check` clean.
