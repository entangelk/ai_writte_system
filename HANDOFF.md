...
- **회귀 기준선**: backend **test-mongo ON 2879 passed / 4 skipped / 7881 subtests**(2026-09-06 **알파**). **전수 판정은 `passed` 가 아니라 `skip` 수를 먼저 본다** — 이 4의 내역은 **live Chroma 1 + 알파에 `elasticsearch` 파이썬 패키지가 없어서 3**이고, 그 셋은 머신-로컬이라 **베타에서는 skip 1 이 정상**이다(함정 절 "호스트 도구 공백"). subtest 가 3189 → 7881 로 뛴 것은 [`test_repo_hygiene.py`](tests/test_repo_hygiene.py) 가 추적 파일 전건을 subtest 로 돌기 때문이다(단독 4665). **★ 최상위 [`README.md`](README.md) 절차 표의 `N passed / N subtests` 는 아무 가드도 안 잡는다** — 기준선을 갱신하는 슬라이스가 그 한 줄도 함께 고친다.
...
- **호스트 도구 공백**: 일부 호스트에 `python` 이 없고 `python3` 만 있다 · `pyflakes` 가 없고 `pip install --user` 가 PEP 668로 거부된다(**`--break-system-packages` 말고 scratchpad venv**) · **Mongo DB 이름은 `ai_writing_system`** 이다(`ai_writing` 아님 — 그 이름으로 조회해 "DB가 비었다"는 거짓 판독을 한 번 냈다) · **★ 알파에는 `elasticsearch` 파이썬 패키지가 없어 lexical retrieval 셀 3개가 조용히 skip 된다**(요약줄은 초록이다). `requirements-dev.txt` 는 `mypy` 한 줄뿐이라 그 설치로도 안 붙는다 — **알파에서 전수 skip 은 4가 정상**이고 회귀가 아니다(2026-09-06 실측).
...
> **★ 2026-09-05 보안 감사 후속(Phase S)이 진행 중이다.** 감사 판단 **critical 0건** + 오너 확인 둘(확인용 계정은 배포에 없었다 · 승인된 계정이 오너뿐이다). **S-3 signup 축과 S-1 quota 우회 체인(HIGH)이 같은 날 닫히고 그대로 배포됐다**(각각 SoT v1.8.30·v1.8.32, 배포 서버 HEAD `d37eb84` — 세부는 work_log 세션 11). **S-0 문서 위생은 2026-09-06 에 닫혔다**(스윕 + 재유입 가드). **저장소 안에 남은 Phase S 작업은 없다** — S-7 은 배포 호스트 설정이고 S-2·S-5·S-6 은 트리거 대기다. **다음은 1번(Slice 5 검증)이다.**

0. **★ Phase S — 보안 감사 후속**(기록 [`security_audit_dual_workflow.md`](docs/verifications/2026-09-05/security_audit_dual_workflow.md), 확정 39건 · critical 0 · HIGH 1). 아래 "🛡 보안 — Phase S" 표가 항목별 정본이다. **~~S-3 signup~~·~~S-1 quota dedupe~~·~~S-0 문서 스윕~~(닫힘) → S-7 토큰 저장 방식(위생, 설정 한 줄 — **저장소 작업이 아니라 배포 호스트 설정이다**) → 나머지는 트리거.** S-2(nginx 보안 헤더)를 할 때는 **nginx 앞단 레이트리밋·`client_max_body_size` 와 한 슬라이스로 묶는 편이 싸다** — nginx 를 한 번만 연다.
