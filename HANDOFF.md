...
>
> 마지막 자가 검수: 2026-08-02 · 236줄 (종료된 컨텍스트 예산 트랙의 Next Tasks 서술을 제거했다. 다음 제품 잔여는 D8-6 영구 삭제 UI이고, 그 뒤 Phase 8 회원별 요청 횟수 제한을 시작한다. 다음 검수 트리거는 300줄)
...
- 정본은 `docs/system-contract-sot.md` **v1.7.81**(Approved). 미확정 항목은 추측 구현하지 않는다.
...
- 회귀 기준선: backend는 **test-mongo ON 1900 passed / 4 skipped / 1608 subtests**(D8-5d 독립 검증 `06dbd16`, D8-5d backend 무변)가 최신 실측이다. frontend는 **234 passed / 17 files**(hardening 후 재실측), build는 **698 modules · 진입 410.08 kB · AdminConsole lazy 5.51 kB · 관측 lazy 386.70 kB**. test-mongo OFF의 과거 89 skip 수치는 현재 기준선이 아니므로 직접 비교하지 말고 같은 환경에서 다시 잰다. **skip 수는 머신·인프라 기동 여부마다 다르다** — 숫자가 안 맞으면 `-rs`로 skip 사유부터 볼 것. **백엔드는 `argon2-cffi`가 설치돼 있어야 한다** — 없으면 auth 모듈이 수집 단계에서 실패한다. 핀은 [`services/application/requirements.txt:1`](services/application/requirements.txt#L1). **프론트는 `npm install`이 선행**돼야 한다.
...
- **관리자 제품 표면(v1.7.81)**: 첫 관리자만 위 부트스트랩 스크립트로 만든다. 로그인 뒤 헤더 `관리` → `/admin`에서 사용자 생성·비활성화, 전 project 메타데이터, 전역 KPI, 사유 필수 1시간 승격과 접근 이력을 쓴다. 관리자가 만든 계정은 첫 로그인 409에서 같은 화면의 **새 비밀번호 설정(12자 이상)** 단계로 넘어가므로 종전 curl 우회가 필요 없다. 소유자는 project 작업공간의 `관리자 접근 이력`에서 사후 조회한다. **함정**: `owner_id=None` project는 승격으로도 안 열리며 화면도 발급을 닫는다. 첫 관리자 없이 자기 가입하는 표면은 정책상 여전히 없다.
...
1. **D8-6 영구 삭제 UI 결정 브리프** — backend purge는 이미 있으나 UI는 불가역 확인 방식·표시 범위·재시도 불가(derived 부분 실패 뒤 두 번째 호출 404) 안내를 정해야 한다. 기존 `plans/multi-user-auth-cms-decisions.md` D5=A와 purge reconciler 한계를 바탕으로 owner decision brief를 먼저 만들고, 결정 전에는 버튼을 임의 구현하지 않는다. purge 감사 로그도 저장 위치·필드 브리프가 먼저다.
