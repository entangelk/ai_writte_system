...
- 정본 SoT는 `docs/system-contract-sot.md`이며 현재 **v1.6.81**(Approved). SoT가 정본 우선순위이고, 미확정 항목은 추측 구현하지 않는다.
...
- **테스트**: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **981 passed / 48 skipped / 217 subtests**(2026-07-14, v1.6.81 B2b harness B1 closure; 현재 환경은 ES Python dependency 미설치라 관련 3개 skip). skip은 대부분 live Mongo/Chroma/embedding 미가용 통합 테스트.
...
- **B2b harness independently re-verified PASS (v1.6.81).** `docs/verifications/2026-07-14/b2b_writing_loop_benchmark_harness.md`의 재검증은 B1(warmup failure raw-run 보존)의 분기 삭제→RED/복원→GREEN을 직접 확인해 conditional pass를 PASS로 올렸다. `scripts/benchmark_writing_loop.py`는 deployed HTTP POST와 opt-in audit aggregate를 분리 기록하고 terminal/retrieve_more/max structural trace를 검증한다. provenance 누락 H1은 현재 harness의 required `--model`/`--quant`/`--compose-revision` metadata로 이미 폐쇄됐으며, 기록의 H1 단락은 그 보강 이전 관찰이다. 다음 owner decision은 live report의 p95/failure rate를 본 뒤 aggregate default를 off→on으로 승격할지와 token/time 여유 ceiling 숫자를 확정하는 것이다.
...
1. **Phase 5.2 Writing Gate 구현 완료(v1.6.69)** — 별도 LLM Gate service/prompt/models + `POST /projects/{id}/writing/gate`, 구조화 findings와 다섯 decision/우선순위, project isolation·strict JSON/provider error mapping을 회귀로 잠갔다. 다음 핵심 후보:
   ...
   - **bounded revise/retrieve loop + persisted audit + aggregate budget 완료(v1.6.71~81)** — B2b harness가 준비됐다. 다음은 full-stack machine에서 `scripts/benchmark_writing_loop.py`를 실행해 report를 만들고, p95/failure rate로 production token/time default·여유 ceiling을 owner가 확정하는 것이다. 이후 multi-finding, stable pointer, persisted 감사 retention(TTL/archive 운영과제)·전체 중간 artifact(P1=C).
...
- 현재 시스템 전체 스위트: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **972 passed / 48 skipped / 215 subtests**. `git diff --check` clean.
