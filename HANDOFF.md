...
>
> 마지막 자가 검수: 2026-08-02 · 239줄 (**D8-5 관리자 트랙이 크게 전진했다**. 승격(5-e)·operation 단위 감사와 사후 조회(5-f)가 서고 **F1=C 하위 결정 C-1~C-5가 전부 시행**됐다(SoT v1.7.78, operation 73). 독립 검증 3건 — 5-e 는 **Blocking(무소유 project 가 승격으로 열림)** 을 잡아 `8f3c52e` 로 닫았고 5-f 는 합격. **오너 결정 대기 페이즈 트랙은 이제 없다** — 남은 D8-5 는 5-b·5-d·C-6 이고 전부 착수 가능하다. 기준선 1883. 다음 검수 트리거는 300줄)
...
- 정본은 `docs/system-contract-sot.md` **v1.7.80**(Approved). 미확정 항목은 추측 구현하지 않는다.
- **진행 중 페이즈 = 다중 사용자 인증(D8).** 오너 지시로 **슬라이스를 잘게 쪼개 진행 중**이다(한 번에 큰 덩어리 금지). 진행표:
  ...
  | **5-d** | **남은 유일한 항목** | 관리자 화면 — ★ **강제 교체 UI가 필수**(아래 공백) · 전 프로젝트 목록 · 사용자 관리 · 승격 발급/접근 이력 |
  | **6 (D8-6)** | **완료·검증 합격(D8-6 종료)** | 영구 삭제 — **6a**(core_sot 8) + **6b**(derived 10) + **6c**(vector/index 백엔드 + worker drain) + **6d**(`POST /admin/projects/{id}/purge`, 204, `_REQUIRE_ADMIN`) 전부 완료. D5 전체 그래프 파기(18컬렉션 + vector/index 5백엔드) 완성. endpoint = core_sot.purge → derived 8 service purge → enqueue_project_purged(worker). operation 카운트 71(ADMIN tier 5번째). 6d 독립 검증은 **조건부 합격**이었고 조건(endpoint→derived fan-out 회귀 부재)은 `_PurgeSpy` 회귀로 해소됐다. `schema.d.ts`도 재생성됨. **남은**: 프론트 purge UI(D8-5 C-1~C-6 후)·감사 로그(별도 브리프 권장). **잔류물 정리는 2026-08-02에 닫혔다** — [`scripts/purge_reconciler.py`](scripts/purge_reconciler.py)가 `projects`에 없는 `project_id`의 잔류 데이터를 찾아 지우고 `PROJECT_PURGED`를 enqueue한다(기본 dry-run, `--apply`로만 삭제). **컬렉션 목록을 코드에 안 적고 DB에서 발견**하므로 새 derived가 생겨도 덮인다. **★ 다만 endpoint 자체는 여전히 재시도가 안 된다** — core_sot이 먼저 지워져 두 번째 호출이 404다. docstring·SoT의 "재시도(멱등)"는 **derived 단계 실패에는 성립하지 않는다**. 고치려면 순서 변경(ⓑ)이나 `core_sot.purge` 멱등화(ⓒ, **404 계약 변경**)가 필요해 오너 결정 사안이다(아래 Owner Decisions). 상세 = `daily_logs/2026-08-01/` 6d + `2026-08-02/` reconciler |
...
- **관리자 경계(D8-5a·5-b·5-c·5-e)**: `/admin/*` **7개 operation**이 `require_admin_user` 뒤에 있다. 인증된 비관리자는 **403**(401 아님 — 세션은 살아 있다). **관리자는 여전히 소유권 403을 받는다** — 그것을 지나는 **유일한** 통로가 **승격(D8-5e, 2026-08-02 구현)**이다: 사유를 적고 `POST /admin/projects/{id}/access-grants`로 발급하면 **1시간 동안 그 project를 GET만** 할 수 있다. **★ 알아야 하는 넷**: ① 읽기 전용 판정은 **HTTP method**다(`GET`/`HEAD`) — 목록이 아니라서 새 endpoint는 자동으로 닫힌 쪽에서 시작한다 ② `is_admin` **AND** 승격이라 강등되면 즉시 죽는다 ③ **만료는 판정이지 삭제가 아니다** — 만료 행이 감사 기록이라 이 컬렉션엔 **TTL 인덱스가 없다**(`sessions`와 의도적으로 다르며 회귀가 단정한다. "일관성"을 이유로 붙이지 말 것) ④ 승격은 D8-6 파기에 물려 있다. **★ ⑤ `owner_id=None` 은 승격으로도 안 열린다**(E1=A) — 첫 판에서 이 검사가 **승격 분기에만 빠져** 무소유 project 가 관리자 승격으로 200 이었다(2026-08-02 독립 검증 Blocking, `8f3c52e` 로 폐쇄). **원인은 빈 셀**이었다: 승격 셀 11개가 전부 *소유자가 있는* project 만 썼다. 같은 함정을 다시 밟지 않으려면 **인가 셀을 쓸 때 `owner_id=None` 축을 넣는다**. **★ ⑥ 승격 아래 요청은 전부 기록된다**(C-3, v1.7.78) — `access_grant_uses` 에 method·경로·사유까지. **기록은 fail-closed 다: 감사 write 가 실패하면 읽기도 실패한다**(503). **이것은 LLM 호출 감사와 정반대이고 의도적이다** — 그쪽은 보안 하중이 없어 격리가 옳고, 이쪽은 하중을 받아 격리가 틀리다. **`try/except` 로 "안정화"하면 계약 위반**이며 뮤테이션 회귀가 잡는다. 소유자 자신의 사용은 기록하지 않는다(섞이면 접근 이력이 활동 로그가 된다). 소유자는 `GET /projects/{id}/access-log` 로 읽는다(C-4). 첫 계정은 여전히 아래 `create_user.py`로 만든다 — `POST /admin/users`를 쓰려면 이미 관리자여야 한다.
...
- **인증 경계를 건드릴 사람이 알아야 하는 것**: (a) 가드는 `tests/test_auth_api.py`에 세 겹이다 — `AuthenticationBoundaryTest`(인증 축) · `ProjectAuthorizationTest`(소유권 축, dependency 내부 Mongo 장애→503 포함) · `CombinedBoundaryMatrixTest`(**71개** tier 분할 = public 4 · 인증 전용 2 · 관리자 5 · project 60 + 축이 만나는 칸. **숫자는 코드가 권위**이며 문서와 어긋나면 코드가 옳다 — 2026-08-02 실측 정정). 새 endpoint는 **세 클래스가 모두** 본다. (b) `POST /projects`·`GET /projects`는 특정 project를 지목하지 않아 인증만 요구하고 403을 선언하지 않는다 — 인증 전용 tier 리터럴에 등재돼 있으니 세 번째 항목이 생긴다면 그것은 결정이다. (b') **403의 생산자는 정확히 둘**(소유권·관리자)이고 그 밖의 operation이 403을 선언하면 거짓 선언으로 잡힌다. 두 인가 dependency는 **대안이지 스택이 아니다**. (c) 도메인 스위트는 `tests/auth_support.py`에서 두 dependency의 **해석만 override**한다. **경계 테스트는 override 없는 앱으로만** 쓴다 — 그 성질은 `TestSeamStaysAnOverrideTest`가 같은 모듈에서 잠그므로 경계 테스트를 다른 모듈로 옮기면 성질이 사라진다. (d) 워커는 HTTP가 아니라 Mongo 직접 접근이라 서비스 계정은 D8-7 사안이다.
...
- **[★★ 제품 공백 — 2026-08-02 C-6 이후 더 좁아졌다] 관리자가 만든 계정은 *브라우저로 로그인할 수 없다.*** C-6 시행으로 관리자가 정한 비밀번호는 **1회용**이고 첫 로그인에 `new_password`를 함께 보내야 세션이 나오는데(안 보내면 **409**), **프론트 로그인 화면이 그 필드를 보내지 못한다.** 즉 계정을 만들 수는 있어도 **화면으로는 못 쓴다** — C-6 이전에는 그냥 로그인됐으므로 이 경로는 **일시적으로 후퇴**했다(오너 결정의 정직한 대가이며 D8-5d의 필수 항목이다). **지금 우회**: `curl -X POST :8520/auth/login -d '{"username":…,"password":…,"new_password":"12자 이상"}'`를 한 번 치면 그 뒤로는 화면으로 정상 로그인된다.
...
- **[해소됨 2026-08-02] D8-5 관리자 C-1~C-6 전부 결정됐다** — C-1~C-5는 구현자 추천 일괄 채택(**TTL만 1시간**, 추천 30분보다 길게), C-6은 **최초 로그인 시 변경 강제**. C-1~C-5는 **5-e로 구현 완료**이고 **C-6은 미착수**(별개 축 — 비밀번호 정책 부재도 함께 닫는다). 종전 서술은 아래에 남긴다: 브리프 [`plans/auth-d8-5-admin-decisions.md`](docs/plans/auth-d8-5-admin-decisions.md) §7. C-1~C-5는 F1=C가 연 승격 메커니즘(수명·쓰기 허용·감사 대상·소유자 통지·사유 필수)이고 서로 얽혀 있어 한 번에 정하는 편이 낫다. C-6은 별개 축(관리자가 사용자 초기 비밀번호를 평문으로 아는 상태 + 비밀번호 정책 부재)이며 **"계정을 만들 표면이 없다"는 제품 공백과 같은 축**이다. 2026-08-02 오너 판단으로 이번 차례에서는 미루었다.
- **purge endpoint의 재시도 가능성(D8-6 잔여, 2026-08-02 제기)** — 지금은 core_sot이 먼저 지워져 **derived 단계에서 실패하면 재시도가 불가능**하다(두 번째 호출 404). 잔류물은 [`purge_reconciler.py`](scripts/purge_reconciler.py)로 수습되지만 endpoint 자체는 그대로다. 선택지: **ⓐ 현행 유지 + reconciler로 수습**(지금, 추가 비용 0) · **ⓑ derived를 먼저 지워 재시도 가능하게** — 대신 실패를 방치하면 *"살아 있는데 기억만 없는 project"*가 남아 지금보다 나쁠 수 있다 · **ⓒ `core_sot.purge`를 멱등화** — 없는 project도 204가 되어 **404 계약이 바뀐다**. ⓑ·ⓒ는 데이터 안전성·공개 계약 트레이드오프라 브리프감이다. **급하지 않다**(발생 조건이 드물고 수습 수단이 생겼다).
...
- **다중 사용자 인증 D0~D8 + E1~E4 = 전부 결정됨**. HTTP 시행(3-a·3-b·3-c)은 닫혔고 D8-6(영구 삭제)·D8-7(인프라 노출면, G1=C)도 닫혔다. **남은 구현은 D8-5 관리자 API/화면뿐**이며 그것은 오너 결정 C-1~C-6에 막혀 있다. D8-7은 HTTP 인가와 섞지 않는 별도 축이었고, **자격증명(G2~G6)은 원격 배포 시점으로 유예**됐다.
