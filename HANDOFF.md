...
- **Phase 8 Slice 8.2b = G1=C·G2~G6=A 확정·구현(SoT v1.7.86)·독립 검증 불합격 — 8.3 진행 차단**(검증 [`slice_8_2b_duplicate_request_lock.md`](docs/verifications/2026-08-03/slice_8_2b_duplicate_request_lock.md)). `DuplicateKeyError` 뒤 보조 read 사이에 release/TTL이 끼면 **만료 잠금을 1초 거짓 차단**하거나, 더 심하게는 **DB에 잠금을 쓰지 않은 `LockGranted` 뒤 다음 요청도 grant**한다. 다음 작업은 이 race의 양방향 회귀를 먼저 세우고 closure한 뒤 재검증하는 것이다. 그 외 확정 계약(진행 중+최소 창, 세 축 키, force claim+holder fencing, TTL=청소, 실패 두 값, 5/180 분리)은 브리프 [`plans/08-2b-duplicate-request-lock-decisions.md`](docs/plans/08-2b-duplicate-request-lock-decisions.md)에 있다.
...
1. **Phase 8 Slice 8.2b 검증 B1/B2 closure + 독립 재검증**([`slice_8_2b_duplicate_request_lock.md`](docs/verifications/2026-08-03/slice_8_2b_duplicate_request_lock.md)). 충돌 후 release/TTL 교차에서 `grant ⇒ DB holder 소유`를 보장하고 거짓 차단·저장 없는 성공을 양방향 회귀로 막는다. **그다음** 8.3 시행(차감·한도 검사 + 잠금 조립, `429` 선언·tier 전수 가드)으로 간다.
