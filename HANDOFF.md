...
>
> 마지막 자가 검수: 2026-08-01 · 224줄 (**D8-6c-1 memory vector/lexical 파기 purge_project 완료** — drain 연결 없이 메서드만, 회귀 12 케이스 + 양방향 뮤테이션 검증(A purge 반전 / B composite 예외 삼킴), 1803 passed/회귀 0. 진행표 6 행과 회귀 기준선을 6c-1 완료로 갱신했다 — 완료 서술은 `daily_logs/2026-08-01/`에 있다. 6c 재진입 출발점은 같은 로그 + `2026-07-31/` 말미 인수인계(탐색 결과 4). 다음 검수 트리거는 300줄)
...
- **진행 중 페이즈 = 다중 사용자 인증(D8).** 오너 지시로 **슬라이스를 잘게 쪼개 진행 중**이다(한 번에 큰 덩어리 금지). 진행표:
  ...
  | **6 (D8-6a·6b·6c-1)** | **6a·6b·6c-1 완료, 6c-1b/6c-2 대기** | 영구 삭제 — **6a**(core_sot 8) + **6b**(derived 10) 파기 인터페이스 + 전수 가드(`test_purge_project_coverage.py`) + **6c-1**(memory vector/lexical 백엔드 purge, drain 연결 없음) 완료. **6c-1b**(candidate 동일 패턴) → **6c-2**(worker `_drain_purge` + `run_once` 분기 + source_block archive purge + **깨진 guard `_archive_where` PURGED `ValueError` 교체**) → **6d**(endpoint). endpoint 는 d 에서만(부분 삭제 고아 위험 D5). **6c 재진입 출발점 = `daily_logs/2026-08-01/`(6c-1 완료·탐색 결과 검증) + `2026-07-31/` 말미 인수인계(탐색 결과 4·분할)** |
