...
- 정본은 `docs/system-contract-sot.md` **v1.7.60**(Approved). 미확정 항목은 추측 구현하지 않는다.
...
- **★ 컨텍스트 예산의 다음 설계 단계는 결정 대기지만, 1b 재감사 B1~B5 closure는 결정 없이 먼저 할 수 있다.** 그 뒤 K-1(C-2 선행) · K-3·**K-6**(C-3 선행) · K-4(C-4 선행) · K-5(C-5 선행)는 브리프 `plans/context-budget-korean-tokens-decisions.md` §3의 오너 결정이 필요하다. **K-1·K-3·K-6은 판단 근거가 실측으로 갖춰져 있다**(§3-1·§1·§2-2).
...
2. **★ 컨텍스트 예산 트랙 — 관측 1b 독립 재감사 FAIL, 먼저 B1~B5를 닫는다**([재감사](docs/verifications/2026-07-29/slice1b_context_window_output_cap_reaudit.md)). 이 트랙은 "정확도 개선"이 아니라 **오늘 깨져 있던 것의 수리**다. 다음 순서 = **1b 비차단/회귀 closure → R-e(K-6) → 가드(K-3) → 밀도(K-1)**. R-a(report 전용 예산)는 폐기가 아니라 보류다.
   - **1b closure가 다음이다.** 느린 `/props`를 생성 성공 반환과 같은 deadline에서 순차 대기해, 생성이 끝났어도 상위 120초 timeout으로 결과가 사라질 수 있다(B1). 실제 HTTP GET, gateway non-null 봉투, application non-null 파싱, Mongo non-null 왕복, 실패 호출의 `max_output_tokens`도 각각 삭제해도 테스트가 green이었다(B2~B5). 구현 수정 + 다섯 경계의 mutation-biting 회귀가 모두 필요하다.
   - **그 다음 R-e(K-6)** — 포인터를 **번호 참조 + 서버측 매핑**으로 바꾼다(프롬프트에 대응표를 싣는 변형은 절감이 0이라 그 안이 아니다). 프롬프트 본문과 `parse_report`의 allowlist를 함께 바꾸며, **report 템플릿은 Mongo 영속이 아니라 새 버전이 필요 없다**(핀은 `analysis_extract` 전용). 착수 전 짧은 스파이크 권장: **모델이 번호 인용을 정확히 하는가**, 그리고 그것이 report repair 비율을 낮추는가(현재 관측 repair는 4회 중 1회).
   - **★ 초과 데이터 자체는 독립 재확인됐다.** 배포 레코드의 `writing_report`는 입력 **11,905** + 상한 6,144 = **18,049 > 16,384**, 헤드룸 **−1,665**, 실제 출력 1,371, 입력 비중 73%다. 다만 이 실측 성공은 위 전달/저장 회귀 공백이나 느린 probe의 비차단 계약 위반을 닫지 않는다.
   ...
   - **K-3 창 관측의 현재 상태**: gateway provider가 `/props`의 `n_ctx`를 한 번 읽어 `context_window`로 올리는 코드는 들어왔고 상수는 쓰지 않는다. 그러나 위 B1~B5 때문에 아직 완료로 취급하지 않는다. 특히 첫 probe를 요청 경로에서 동기 대기하는 현 구조는 “관측이 기능을 깨뜨리지 않는다”는 SoT v1.7.59와 충돌한다.
