...
>
> 마지막 자가 검수: **2026-09-13 · 341줄** — 완료 서술 대삭제: 검증 닫힘 절 셋 · Next Tasks 완료 표·서사 · 미수리 닫힌 행 둘 · 완료 항목 압축(계약 주의만 잔존). 계약 절 정합: 활동 로그 D2=ⓑ 네 표면 반영(전에는 결정 전 상태로 남아 있었다) · Slice 5 ⓒ D5 시행 반영 · SoT v1.8.67. 기준선 줄 역사 사슬 압축(현재값+귀속+함정만). 세션 81 승격 재검 반영.
...
정본은 [`docs/system-contract-sot.md`](docs/system-contract-sot.md) **v1.8.68**(Approved). **미확정 항목은 추측 구현하지 않는다.** 아래는 코드를 만지기 전에 알아야 하는 요약이다.
...
- **회귀 기준선**: backend **test-mongo ON 3075 passed / 1 skipped / 4239 subtests** — **세션 85 재시도 500 수선**(`3757693`)이 백엔드 소스를 바꿔 전수로 재측정 — **3075/1/`4239` · EXIT=0 · 351초**. 귀속은 둘로 갈린다 — **+6 passed** = 신규 `tests/test_retry_cooldown_naive_datetime.py` 6셀(이 세션 몫) · **+1 subtest** = **세션 84 몫의 뒤늦은 반영**이다(스크린샷·GIF 등재가 문서 가드 둘을 **961 → 962** 로 올렸는데, 그 세션이 *"코드 무변"* 으로 전수를 안 돌려 이 줄에 안 들어왔다 — 실측: 문서 가드 둘 단독 **29 passed / 962 subtests**, 이 세션의 셀 6은 `subTest` 를 쓰지 않아 subtest 를 0 낸다). **★ 이것이 "유도 금지" 의 실례다** — 문서만 바꾼 세션도 가드 회계를 움직이고, 다음 전수가 그 몫을 대신 짊어진다. 종전 값 **3069/1/4238** 의 확정 경위는 아래 그대로다. **세션 80 두 리비전 통제 대조로 확정**: 최종 트리(`542e087`) 전수 **3066/1/`4236` · EXIT=0 · 357초**(skip 1 = live Chroma) · 탈치 클론 `d9a1bd2`(세션 78 끝) 전수 **3065/1/`4234` · 294초** — **+2 passed** = `463a042`(특성 셀 분리 +1 · HTTP 경계 셀 +1 — `267ba22` 은 셀 개명 2쌍이라 수 무변) · **+1 passed** = `867ba15` under 셀 · **+2 subtests** = `449784d` 검증 기록 문서(문서 가드 둘 **957 → 959** 양단 실측). **세션 81 승격 재검** 이 문서 가드 둘을 **959 → 961** 양단 실측으로 올렸다(승격 기록 문서 하나 = `test_repo_hygiene` 문서당 1 + 검증 인덱스 행 1 = `test_docs_indexes` 1 — 백엔드 소스·테스트 무변 · passed 무변) → **4238**. **세션 83 오너 즉닃 셋**(다섯째 표면·login_failures 청소·약관 v1.1)이 백엔드 소스를 바꿔 전수로 재측정 — **3069/1/`4238` · EXIT=0 · 354초**(+3 passed = H4 셀 둘 + login_failures 셀 하나 · subtests 무변). 종전 유도 사슬(4215→4232, 세션 70~77)의 귀속 서사는 각 세션 work_log·검증 기록에 있다 — 그중 유도 오류 둘(4213·4154)이 아래 함정들의 출처다. **★ 유도값은 시점의 함수다** — 유도 직후 커밋(자기 슬라이스의 검증 기록·work_log 포함)이 문서를 늘리면 그 값은 그 자리에서 낡고, verifications 인덱스 행도 subtest 를 낸다. **유도하는 슬라이스는 자기 기록이 낼 +N 을 함께 세야 한다.** **유도가 정당한 조건은 '백엔드 소스 무변'이다 — 한 줄이라도 바뀌면 전수를 돌린다.** 프런트는 **489 passed / 41 files**(2026-09-13 실측 · **EXIT=0** — 세션 83 의 탈퇴 패널 고지 셀 +1 · 버전 핀 1.1 동반). **★ 프런트 값에는 백엔드와 달리 상호 가드가 없어 조용히 샜다**(2026-09-10 실측 사례) — 프런트 수를 움직인 슬라이스는 같은 날 이 줄까지 와야 한다. **갱신은 이 수를 움직인 슬라이스의 몫이고, 이제 [`README.md`](README.md) 절차 표 ②행이 이 줄을 정본으로 가드에 물려 있다**(`test_docs_indexes::test_the_readme_repeats_the_regression_baseline` — 둘이 다른 수를 말하면 전수가 빨개진다). 어긋났을 때 유도하는 법은 `git worktree add --detach <경로> <리비전>` 뒤 양쪽에서 `python3 -m pytest --collect-only -q`(수집 수 = passed + skipped)다. **★ 전수 판정은 `passed` 가 아니라 `skip` 수를 먼저 본다** — `skip 1 = live Chroma` 하나뿐이고 2 이상이면 호스트에 빠진 패키지가 있다는 뜻이다(함정 절 "호스트 도구 공백"). **전수 1실패를 만나면 위 미수리 표의 *알려진 플레이크* 부터 볼 것**(프런트 둘 · 백엔드 S13 — 단독 재실행으로 가른다). **★ subtest 수는 커버리지 대리지표가 아니다** — 위생 가드는 위반이 있을 때만 subtest 를 내고 파일 회계는 조용히 돈다. **★ 문서 하나가 몇을 늘리는지는 문서 *종류* 에 달렸다 — 예측하지 말고 잰다**(실측 선례: plans 브리프 +1 · 검증 기록 +2 · 일자별 work_log 신규 파일 +1). 재는 법: `git worktree add --detach <경로> <기준 리비전>` 뒤 양쪽에서 문서·프런트를 읽는 가드만 돌린다 (`test_docs_indexes`·`test_repo_hygiene`·`test_design_token_provenance`·`test_activity_ui_labels`·`test_typecheck` + 문서 내용을 읽는 `test_service_policy_contract`·`test_product_name`). **★ 소요는 머신·파일시스템 로컬 값이다** — 같은 날 같은 머신에서 `/mnt`(메인 트리) 357초 vs `/tmp`(탈치 클론) 294초가 갈렸다. 소요로 회귀를 판정하지 말 것.
...
- **★ 2026-08-23 이전 기록의 커밋 해시는 `git show` 로 안 잡힌다 — 결함이 아니다.** 그날 `git filter-repo` 로 이력을 재작성해 **해시가 전부 바뀌었다**(퍼블릭 전환 전 보안 점검 — `daily_logs/2026-08-23/work_log.md` 세션 16). 코퍼스 전수로 경계를 쟀다: **2026-07-06~08-23 구간은 371개 중 1개만** 해소되고 08-24 이후는 **239개 중 229개**가 해소된다(06-24~07-05 는 69/70 · 2026-09-13 재측정 — 오너 결정 ⓐ 교체). 종전 발행분의 분모(`1/264`·`269/272`)는 코퍼스 증가 전 측정이라 재현되지 않는다(독립 검증 2026-09-13 H2). **분자 `1` 과 경계는 정확히 재현되므로 결론은 그대로이고, 수치를 인용할 땐 [`docs/verifications/README.md`](docs/verifications/README.md) 머리의 유도 명령으로 직접 유도한다.** **그 구간 기록의 해시를 따라가 실패하면 근거를 의심하지 말고 이 줄을 떠올릴 것** — 실제로 한 검증 세션이 그 이유로 시간을 썼다. **옛 해시는 고쳐 쓰지 않는다**(그때의 사실이고 이 저장소는 이력 문서를 고치지 않는다).
...
- **pymongo는 BSON 날짜를 naive로 돌려준다** — aware `datetime.now(UTC)` 와 비교하면 `TypeError` 다. **★ 이 함정은 세 번 났다**(2026-07-27 세션 어댑터 · 2026-09-13 analysis·writing 재시도 둘). 처방은 어댑터 경계의 `_aware()` 한 줄인데 **번번이 빠지는 이유는 in-memory/fake 저장소가 넣은 aware 를 그대로 돌려주기 때문**이다 — 전수는 초록인데 배포만 깨진다. **새 `*_mongo.py` 에 시각 필드를 실으면 `_aware` 를 같이 넣고, 그 자리를 무는 셀은 fake 문서의 tzinfo 를 떼고 넣는다**(선례 `test_auth_sessions_mongo.py::NaiveBsonDatetimeTest` · `test_retry_cooldown_naive_datetime.py`). 재발 시 스윕 한 줄: `grep -L "tzinfo=UTC\|_to_utc_datetime" $(grep -rl "MongoClient" services/application/app --include=*_mongo.py --include=mongo_repository.py)`.
...
> **★ 새 대기 항목이 생기면 브리프를 먼저 쓰고 여기 한 줄로 올린다.** 그리고 **브리프에 추천이 있고 그것이 기존 결정의 뜻을 잇는 것이면 묻지 말고 그대로 진행한다** — 2026-09-13 에 오너가 그 셋을 *"당연한 것들"* 로 지적했다. 진짜로 물어야 하는 것은 **오너만 가진 정보**가 필요한 때다: 제품 방향 · 돈 쓰는 선택 · 외부 서비스 승인 여부 · 트리거가 실제로 왔는지. **법률 문언은 이제 제외다**(오너 2026-09-13: *"대충 써놔, 이거 포트폴리오"*). **이미 결정된 것의 자명한 귀결도 묻지 않는다**(같은 날: 30일 뒤 취소 거부를 되물어 짜증을 샀다).
...
**★ 세션 79 가 만든 대기 둘은 같은 날 저녁 오너가 답해 시행됐다(세션 83)** — **H4 ⓐ 넓힌다**(`76568ba` · 다섯 표면이 한 규칙) · **옛 해시 분모 재측정값 교체**(`5be61b3` · `1/371`·`229/239`·`69/70`). 같은 날 오너 즉닃 셋이 더 시행됐다: `login_failures` 파기 청소(`377cbe3` — 사용자명 재사용이 옛 잠금을 물려받지 않는다) · **약관·방침 v1.1**(`5be61b3` — 유예 중 저장·유료 제한 고지 + 파기 뒤 취소 불가 경계 + 신청 패널 고지).
...
| ~~**`PUT /projects/{id}/brief` 가 replay 에 활동 행을 남긴다 — D2=ⓑ 가 못 덮은 다섯째 표면**(2026-09-13 독립 검증 H4)~~ | **닫혔다(2026-09-13 H4 ⓐ 시행, `76568ba`)** — 오너 승인으로 넷과 같은 규칙으로 넓혔다(셀 한 쌍 · 변이 검증 완료). 이제 다섯 표면이 한 답이다 | [`services/application/app/routers/projects.py`](services/application/app/routers/projects.py) |
...
| **응답의 시각 넷이 배포에서 오프셋 없이 나간다**(2026-09-13 등재 — 재시도 500 수선의 이웃) | 같은 `naive datetime` 뿌리인데 **이쪽은 계약면**이라 같이 안 고쳤다. `created_at`(생성 잡 · loop audit · scratch)·`terminal_at`(gate findings)은 어댑터에서 정규화 없이 나와 `.isoformat()` 이 **`+00:00` 없는 문자열**을 만든다 — 프런트 픽스처는 `…Z` 를 가정하므로 **셀과 배포가 다른 모양을 본다**. 지금은 프런트가 이 값들을 날짜로 파싱하지 않아 **보이는 증상이 없다**(렌더 경로 0건). 파싱하는 화면이 생기는 순간 KST 브라우저가 **9시간 과거로 읽는다** — 그때가 트리거다. 닫으려면 어댑터 넷에 `_aware` 를 넣고 **응답 문자열 모양 변경**을 계약으로 받아들인다(프런트 픽스처 동반) | [`writing/generation_job_mongo.py`](services/application/app/writing/generation_job_mongo.py) · [`writing/loop_audit_mongo.py`](services/application/app/writing/loop_audit_mongo.py) · [`writing/scratch_mongo.py`](services/application/app/writing/scratch_mongo.py) · [`context_search/gate_findings_mongo.py`](services/application/app/context_search/gate_findings_mongo.py) |
...
>
> **★ 2026-09-13 현재**: 구현 백로그 없음 — 계정 탈퇴 전 축(0~5·4b)·랜딩 ①~④·가입 동의 게이트·Phase W 전부 닫혔다. **검증 승격 백로그도 비어 있다** — 마지막 대기 건(오너 결정 셋 검증의 조건 C1)을 세션 81 승격 재검 [`owner_decision_set_promotion.md`](docs/verifications/2026-09-13/owner_decision_set_promotion.md)이 변이 다섯 종 전건 일치로 닫았다(2026-09-13). **착수 가능**: 6번 육안 확인(랜딩 실기기 포함) · 7번 도그푸드 관찰 · 8번 계정 정리. 그 밖는 위 "열린 것"의 ⚠️ 오너 결정 표(2건)·🔧 미수리 표·⏸ 유예 표가 정본이다. 종전 백로그 서사(승격 재검 넷·작은 후속 둘·AC1 감사 계열)는 각 세션 work_log·검증 기록에 있다.
>
> **★ 착수 순서는 이제 간단하다**: 6번 육안 확인(프론트 재빌드 선행) → 7번 도그푸드 관찰 → 8번 계정 정리. 배포(5번)는 오너 몫이고 그 선행 조건이다.
...
1. **~~Slice 6(grouped Inbox UI)~~ 완료(2026-09-06, SoT v1.8.34·v1.8.35)** — 검증 하드닝 H1~H4(그룹 승인 유료 402/429 화면 처리가 `WritingPanel` 표준과 안 맞음 · 409 뒤 재동기화 없음 · `idempotent_replay` 표시 무셀 · outcome 단일 슬롯)는 **비차단**으로 [`identity_group_slice_6.md`](docs/verifications/2026-09-06/identity_group_slice_6.md)에 남아 있다 — 완료 기준 #6("실패를 사람이 이해할 수 있다")과의 관계는 오너 판단 몫.
2. **~~최종 저장·분석 연동 재검증 1~6차~~ 완료·승격(2026-09-12 세션 73 · [`final_save_b1_promotion.md`](docs/verifications/2026-09-12/final_save_b1_promotion.md))** — 남는 것: 하드닝 H1(finalize 키 상수 무물림)·H2(함수 층 가드)는 비차단·오너 값 인정 대기. ~~N3 은 오너 결정 대기~~ → **N3(finalize 같은 키 재전송의 활동 행 중복)은 D1=ⓑ(v1.8.66, 2026-09-12)가 이미 닫았다** — 그 결정이 정확히 그 병을 시행한 것이다(낡은 대기 문장을 세션 83 이 정정).
3. **~~장면 메모 Slice 3~4(화면 둘)~~ 완료(2026-09-06, SoT v1.8.36·v1.8.37)** — 남은 하드닝 H2 는 이 슬라이스 밖 선결함이다(드로어 읽기 전용이 보관 3축 중 2축만 — `DraftPayload` 에 chapter 축이 없다). 남는 계약 주의: 목록은 미리보기(200자)만 싣고 전문은 단건 GET 이 준다 · 검색은 서버가 하고 화면은 다시 거르지 않는다 · **저장 버튼을 "변경 없음"으로 잠그지 말 것**(오너 결정 "같은 값 재저장도 활동 행을 남긴다"가 화면에서 도달 불가가 된다 — 요청 중 잠금만 둔다) · 드로어의 편집 대상은 현재 Scene 하나다. 확정값 [`scene-note-decisions.md`](docs/plans/scene-note-decisions.md).
4. **~~장면 메모 후속(수정 시각 · "더 보기")~~ 완료(2026-09-07, SoT v1.8.38·v1.8.39)** — 남는 계약 주의: **"더 보기"는 단건 GET 한 번이지 목록의 전문이 아니다** · 펼침은 **두 화면에 똑같이** 있고 드로어에서 **편집 대상을 옮기지 않는다** · **늦게 도착한 펼침 응답은 버린다**(행마다 따로 나가는 요청이라 주인을 확인해야 한다 — 상태가 아니라 ref 로) · 목록이 새로 오면 접는다 · 새 CSS 규칙이 `var(--type-*)` 를 쓰면 **`typeScale.test.ts` 이관 목록에 등재**해야 한다(이번에 실제로 걸렸다).
...
9. **~~서비스 정책 계약 문서~~ 완료(2026-09-07)** — [`service-policy-contract.md`](docs/service-policy-contract.md) · 결정 [`plans/service-policy-decisions.md`](docs/plans/service-policy-decisions.md). **문서를 고칠 때 주의**: §1~7 값은 파생본이라 `tests/test_service_policy_contract.py` 가 상수와 대조한다 — 값을 고치려면 **상수를** 고치고, 새 행에는 `모듈::상수` 포인터를 단다(포인터 없는 행은 잠기지 않는다). **§8 은 지위가 다르다**(정해졌으나 미시행) — 시행되면 §1~7 로 올리고 포인터를 단다. 보존 기간은 무기한(삭제 요청 전까지)으로 닫혔다.
11. **★ 셀프 계정 탈퇴 + 30일 유예 파기(오너 결정 2026-09-07)** — **버튼 하나가 아니다.** 구현 순서는 [`plans/account-withdrawal-implementation-phases.md`](docs/plans/account-withdrawal-implementation-phases.md) 에 있고 **~~D4~~·~~Slice 0~~·~~1~~·~~2~~·~~3~~ 이 닫혔다**(SoT v1.8.43·v1.8.45·v1.8.46·v1.8.48·**v1.8.51**). **~~Slice 4(화면)~~ 도 닫혔다**(2026-09-10, SoT v1.8.54 — 브리프 [`slice4-…-decisions.md`](docs/plans/slice4-withdrawal-screen-decisions.md), D1=ⓐ 전역 배너 · D2=ⓐ 관리자 축 분리). **~~Slice 5(문서 승격)~~ 도 닫혔다**(2026-09-12, SoT v1.8.60). **~~Slice 4b(관리자 잔여 정리)~~ 도 닫혔다**(2026-09-12, SoT v1.8.62) — 이 계획서의 구현 축은 전부 완료.
    - **★ Slice 3 이 남긴 계약 주의 여섯**(2026-09-09, 브리프 [`slice3-…-decisions.md`](docs/plans/slice3-withdrawal-purge-daemon-decisions.md) ⓑ·ⓔ):
      ...
      ⓔ **`login_failures`(`_id` = username)는 어느 규칙도 못 찾는다** — 사용자명이 재사용되면 새 계정이 옛 잠금을 물려받는다. **2026-09-13 시행 완료(오너: *"그걸 이어받으면 당연히 안 되지"*)** — 파기가 sweep 뒤에 이 행을 명시적으로 지운다(`377cbe3`, 실패 라벨 `login_failures`).
      ⓕ **파기 본체는 여전히 한 벌**이다. 워커는 `_project_purge_boundary` 로 `HTTPException` 만 옮긴다 — 두 벌째를 만들지 말 것.
    - **★ 배포 전에 `python scripts/migrate_ledger_user_axis.py` 를 돌려야 한다** — 안 돌리면 옛 행이 새 질의에 안 잡혀 **사용량이 0 으로 보인다**. 그리고 **`withdrawal_worker` 서비스가 새로 생겼다**(compose) — 배포에 `up -d` 로 함께 올라온다. **개발 스택도 재생성이 필요하다**(2026-09-10 관측): 현 컨테이너들이 Slice 3 이전 상태라 `withdrawal_worker` 가 아직 안 떠 있다.
...
│   ├── deletion/             # 계정 파기·reconcile 본체(라우터·스크립트 공유)
