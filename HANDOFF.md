...
- 정본은 `docs/system-contract-sot.md` **v1.7.59**(Approved). 미확정 항목은 추측 구현하지 않는다.
...
2. **★ 컨텍스트 예산 트랙 — 슬라이스 1(회계) 완료, 다음은 R-e**(브리프 `plans/context-budget-korean-tokens-decisions.md`). 이 트랙은 "정확도 개선"이 아니라 **오늘 깨져 있던 것의 수리**다. **오너 확정 순서 = 회계 → R-e(K-6) → 가드(K-3) → 밀도(K-1)**이고 R-a(report 전용 예산)는 폐기가 아니라 **보류**(앞 둘 뒤에 수치 보고 판단).
   ...
   - **★ 초과가 이제 데이터로 보인다 — 그리고 실제로 초과하고 있다.** K-3 관측(1a·1b)이 들어가 감사 레코드가 `prompt_tokens`·`completion_tokens`·`max_output_tokens`·`context_window`를 남기므로 **헤드룸(`창 − 입력 − 출력상한`)이 사후에 판정된다**(파생값이라 저장하지 않는다). **2026-07-29 실 배포 관통 실측**: `writing_report`가 입력 **11,905** + 상한 6,144 = **18,049 > 16,384**로 **헤드룸 −1,665**인데 **job은 성공했다**(실제 출력 1,371이라 상한을 안 썼다). 즉 **초록불이면서 계약이 깨진 상태**이고, 그것이 이제 산술이 아니라 **레코드로** 보인다. report는 **창의 73%를 입력에 쓴다**. **관측은 아무것도 거부하지 않는다**(오너 결정) — 남은 일은 이 초과를 **없애는 것**이고 그게 R-e다.
