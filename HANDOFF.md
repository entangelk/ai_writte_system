...
- **★ Phase 8 Slice 8.2b — 중복 요청 잠금 G1~G5**(브리프 [`plans/08-2b-duplicate-request-lock-decisions.md`](docs/plans/08-2b-duplicate-request-lock-decisions.md), 2026-08-03). **★ 실측이 요구의 형태를 바꿨다 — G1이 핵심이다**: 이 제품의 동기 생성은 **약 23초**(45 tok/s × short 1024. medium 45초·long 91초는 그래서 비동기로 뺐다)라 **고정 5초 창은 "결과가 나오기 전 재클릭"을 거의 못 막는다** — 10초 뒤 재클릭이면 창은 지났는데 첫 요청은 진행 중이라 두 번 과금된다. 추천은 **C(진행 중 유지 + 최소 5초)**. 나머지 추천: G2 키 축 `(user, action, target_project_id)`(동작을 빼면 정상 연쇄 generate→gate가 서로를 막는다) · G3 `find_one_and_update` upsert · G4 강제 재차지 · G5 남은 시간+진행 중 여부. **★ TTL 함정**: Mongo TTL 삭제는 약 60초 주기라 **존재 여부로 판정하면 5초가 최대 1분이 된다** — TTL은 청소용, 판정은 `expires_at` 비교. **fake에는 TTL 주기가 없어 테스트로는 안 보이고 운영에서만 보인다**. **G6(에러 통로) 추가**: 이 저장소에 **에러 넘버링은 없다** — 앱 전체에 `error_code`류 필드가 0곳이고 H3가 `detail` 문자열 분기를 금지하므로 **기계 판독 축은 상태코드 하나**다. 통로는 있다(`ApiError{status, detail}`, 화면이 실제로 status로 분기). 유료 9경로 실측상 **`409`는 accept·analysis run에서 이미 다른 뜻이라 충돌**하고 **`429`는 앱 전체 미사용**이라 추천은 429 + `detail` 문구다(H3 개정 불필요). **TTL 지연 자체는 생성을 안 막는다**(판정이 `expires_at` 비교라서) — 다만 **크래시 뒤에는 lease 만료까지 최대 2분가량 막히고**, 그 탈출구가 G4의 강제 재차지다. **결정 전에는 잠금 저장소를 만들지 않는다.**
...
1. **Phase 8 Slice 8.2b — 오너 결정 대기 중이다**(브리프 제출 완료, 2026-08-03. 위 Owner Decisions Needed 참조). G1~G5가 닫히면 ① 계약을 **양방향 회귀로 먼저** 잠근다 — **동시 두 요청 중 하나만 차지**(이 슬라이스의 존재 이유) · **만료된 잠금은 TTL 주기와 무관하게 다시 차지된다**(fake에 TTL이 없으므로 판정이 `expires_at` 비교임을 셀이 직접 단정) · G1=C면 **진행 중 잠김 + 완료 뒤 최소 5초**(두 구간을 각각, 경계 직전·직후) · **다른 `action`·다른 프로젝트는 서로를 막지 않는다** · 확인된 요청의 **강제 재차지 뒤 곧바로 온 요청은 다시 막힌다** · **lease > 최소 창**(두 값을 한 상수로 합치는 회귀 방지) ② 도메인 + Mongo 어댑터(`_id`가 곧 키라 추가 인덱스 불필요 + **청소용 TTL**) ③ `mongo_collections.md` §43E ④ 8.3 시행 브리프. **HTTP 응답은 8.3이다.**
