...
- 회귀 기준선: backend **test-mongo ON 2046 passed / 4 skipped / 1723 subtests**(베타 2026-08-03, Slice 8.2b까지. 직전 1988/1/1719 대비 +58 passed·+3 skipped·+4 subtests이고 **합계 +61 = 8.2b 신규 셀 그대로**다 — **skip 3건 증가는 코드와 무관하다**: `elasticsearch` 파이썬 패키지가 그 셸 인터프리터에 없어 `test_context_search_memory_lexical_retrieval.py` 3 cells가 자기 skip 가드로 빠졌다. 숫자가 안 맞으면 `-rs`로 사유부터 본다. 그 이전 기록은 Slice 8.2 + 검증 보강까지. 직전 알파 기준선은 1911/4/1625였고 차이는 전부 설명된다 — Slice 8.0 신규 가드 8+75, 검증 보강 4+15, 그리고 알파에서 skip되던 3건이 베타에서는 실행됐다. 남은 skip 1건은 호스트에서 구조적으로 항상 skip되는 live Chroma 셀), frontend **236 passed / 17 files**, build **698 modules · 진입 410.29 kB · AdminConsole lazy 8.39 kB · 관측 lazy 386.70 kB**. skip 수는 머신·인프라 기동 여부마다 달라 같은 환경에서 비교한다. backend는 `argon2-cffi`, frontend는 `npm install`이 선행돼야 한다.
...
- **[★ 최신 베타 관측, 2026-08-03 — 지금 작업 중인 머신]** `nvidia-smi` **GTX 1060 3GB**로 베타 확정. HEAD `05286a6` clean. `docker compose ps`에는 **`frontend`(healthy)·`worker`·`generation_worker`만** 있고 application·mongo·gateway·embedding·chroma·ES와 test-mongo는 **떠 있지 않다**. **하루 종일 이 상태로 작업했다** — Phase 8의 8.0~8.2b는 저장 계약·회귀라 스택이 필요 없다. backend 회귀를 돌릴 때만 `docker compose -f docker-compose.test.yml up -d`로 test-mongo를 올렸다가 끝나고 내렸다(그 절차가 표준이다). 이미지는 코드보다 뒤처져 있다 — gateway 07-31, **application/generation_worker 07-29**(= D8-5~D8-7 인증·관리자 작업 이전), frontend·worker 07-27. **화면 확인이나 라이브 관통을 하려면 `docker compose build application frontend && docker compose up -d --no-deps application frontend`가 선행돼야 하고, 스택을 올렸으면 `curl :8520/projects`가 401인지 먼저 본다**(아래 ★★ 이미지 부채 — 낡은 application은 죽지 않고 인증 없는 제품으로 뜬다). 외부 12B(`192.168.1.22:9080`)는 살아 있고 `/props` 실측 **`n_ctx=16384`·`total_slots=1`**이며, 모델 스냅샷은 **`29d0977…`**(알파 `-hf` 리비전 부채가 가리키던 새 리비전을 외부 서버는 이미 쓴다).
...
- **Phase 8 Slice 8.2b = G1=C·G2~G6=A 확정·구현 완료(SoT v1.7.86)·독립 검증 대기**(오너 2026-08-03, 브리프 [`plans/08-2b-duplicate-request-lock-decisions.md`](docs/plans/08-2b-duplicate-request-lock-decisions.md)). 잠금은 **"5초 타이머"가 아니라 진행 중 보호 + 최소 냉각 창**이다(G1=C) — 동기 생성이 **약 23초**라 고정 5초 창으로는 "결과가 나오기 전 재클릭"을 못 막기 때문이다. 문서는 `_id="{user}:{action}:{project}"`(키가 곧 id라 **추가 인덱스 불필요**) + `claimed_at`·`expires_at`(**판정의 유일한 축**)·`released_at`(`None`이면 진행 중). 연산 셋: 차지(`now + lease`) · 해제(`max(now, claimed_at + 최소 창)`) · **강제 재차지**(확인 — UX이자 **크래시 복구 경로**). **막힌 이유는 `released_at`에서 파생**된다. **★ 두 상수를 합치지 말 것**: 최소 창 5초는 제품 정책, lease(잠정 180초)는 기술 한계(gateway timeout 120초보다 커야 한다). **★ TTL은 청소용이고 판정은 `expires_at` 비교다** — 존재 여부로 판정하면 Mongo TTL 주기(~60초) 때문에 5초가 최대 1분이 되고, **fake에는 TTL 주기가 없어 테스트로는 안 보인다**. 실패 응답은 **`429`**(앱 전체 미사용이라 충돌 없음) + `detail` 문구이며 **H3 균일 본문 개정은 하지 않는다** — 429 선언·전수 가드 등재는 8.3이 한다. **★ 의도적 중복은 금지가 아니라 확인이다** — 여러 안을 받고 싶은 사용자는 확인으로 통과하고 사용량 1회를 더 쓴다(8.0 B1=A). 그래서 **확인 문구가 사용자를 꾸짖으면 안 된다**(○"다른 안을 하나 더 만들까요? 사용량 1회가 추가됩니다" / ✗"중복 요청입니다", 문구는 8.4). **그 결정이 `해제`에 소유권 검사를 필수로 만든다** — 강제 재차지 뒤 **먼저 시작한 요청이 완료되며 남의 잠금을 푸는** fencing 문제라, 잠금이 `holder` 토큰을 들고 해제는 일치할 때만 동작한다(브리프 §0.4). 회귀 필수. **알려진 한계**: lease 만료 시점에 원래 요청이 살아 있으면 중복이 샌다(`WRITING_LOOP_MAX_WALL_CLOCK_MS`가 기본 무제한이라 동기 상한을 증명할 수 없다) — **최선 노력 통제이지 절대 보장이 아니다**. **구현 결과**: [`quota/lock.py`](services/application/app/quota/lock.py)(도메인·in-memory 저장소)와 [`quota/lock_mongo.py`](services/application/app/quota/lock_mongo.py)(`request_locks`, 인덱스는 **청소용 TTL 하나뿐**)이며 회귀 61 cells·뮤테이션 12종 재실패. **아직 아무도 조립하지 않는다** — 소비자는 8.3이다. **8.3이 알아야 하는 것 둘**: ① 실패는 `retry_after_seconds`(올림·최소 1)와 `in_flight`를 들고 오므로 두 문구를 그것으로 가른다 ② **잠금은 차지 → 처리 → 해제가 한 요청 안에서 닫혀야 한다** — 비동기 생성(202)도 **워커가 아니라 요청 경로가** 주인이다(워커까지 끌고 가면 lease와 job 수명이 서로 다른 두 시계가 된다). 구현이 더한 계약 하나: **재차지는 `released_at`을 지운다**(안 지우면 지금 생성 중인 요청이 "방금 요청함"으로 보고돼 화면이 거짓말을 한다).
...
1. **Phase 8 Slice 8.2b 독립 검증**(구현은 끝났다 — SoT v1.7.86, `docs/guides/verification.md` 절차). 그다음이 **8.3 시행**이다: 차감 시점·한도 검사와 함께 잠금을 조립하고, 차지 실패를 **`429` + `detail`**로 옮기며(코드는 8.2b에서 이미 골랐다), 유료 9경로의 `responses=`에 429를 **선언**하고 tier 전수 가드에 등재한다. 조립 주의 둘은 위 8.2b 항목 끝에 있다(요청 경로가 잠금의 주인 · 실패가 들고 오는 두 값).
