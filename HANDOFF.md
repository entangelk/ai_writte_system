...
정본은 [`docs/system-contract-sot.md`](docs/system-contract-sot.md) **v1.8.54**(Approved). **미확정 항목은 추측 구현하지 않는다.** 아래는 코드를 만지기 전에 알아야 하는 요약이다.
...
| 항목 | 상태 | 위치 |
|---|---|---|
| **프런트 전수의 알려진 플레이크** | `me/PersonalHubPage.test.tsx > gives the remaining request budget a home`(2026-09-07 실측 5739ms) · `admin/AdminUserDetail.test.tsx > shows only this user's projects and filters them by name`(2026-09-08 실측 **11938ms** 타임아웃, 단독 **497ms** 초록) — 둘 다 **전수에서만**, **부하에서 느려지는 타이밍**이다. **★ 셋째 계열(2026-09-11 등재): 시계 뒤점프** — `me/withdrawal.test.tsx` 의 픽스처 `Date.now()+3일` 이 **모듈 스코프**라 렌더 시각이 모듈 로드보다 과거로 점프하면 `ceil` 이 4일을 내 "남은 기간 3일" 셀이 깨진다(실측 2실패·재실행 초록). **WSL2 부팅 직후 창이 트리거**다(journal `Clock change detected` 반복 — 실패 창에 11회). **같은 창에서 재실행으로 가른다.** 전수 1실패를 만나면 **먼저 이 표의 셀인지 보고**, 맞으면 **단독 재실행으로 가른 뒤** 회귀가 아님을 확인한다 | `daily_logs/2026-09-07/work_log.md` 세션 38 · `2026-09-08/work_log.md` 세션 46 · `verifications/2026-09-11/withdrawal_slice4_screen.md` H1 |
...
## 계정 탈퇴 Slice 4(화면) — 독립 검증 조건부 (2026-09-11)

기록 [`withdrawal_slice4_screen.md`](docs/verifications/2026-09-11/withdrawal_slice4_screen.md) **조건부 합격**. 구현 자체는 계약 전 축과 일치하고 변이 재적용 6종 중 5종이 재현됐으나, **신규 반증 변이 둘이 이 슬라이스의 핵심 계약 문장 두 개가 무셀임을 실증**했다:

- **B1 — 배너 장착 지점(AuthGate) 무셀**: `AuthGate.tsx` 에서 `<WithdrawalBanner />` 를 **통째로 지워도** 관련 3파일 52셀 전건 초록이다. 배너 셀들은 `AuthGate` 없이 배너를 직접 마운트하는 **복제 셸**을 쓴다. SoT v1.8.54 의 *"셀이 `/me` 아닌 경로에서 배너를 단정해 잠근다"* 는 장착 지점에 대해 거짓 — **처방: 진짜 `AuthGate` 를 렌더하는 셀 하나**(`App.test.tsx` 선례 + 시드 인프라 있음).
- **B2 — 취소 409 재조회 무셀**: SoT 가 명시한 *"서버의 지금 상태를 다시 읽어"* 를 로컬 `null` 합성으로 치환해도 11/11 초록. **처방: 409 셀에 fetch 호출 단정 한 줄**(DELETE 뒤 GET 이 실제로 나가는지).
...
- 폐쇄 순서는 Slice 3 선례 그대로: **조건 폐쇄(셀 둘, 구현 세션) → 승격 재검(변이 재적용, 다음 검증 세션)**. 조건을 닫은 세션은 자기 판정을 못 올린다.
...
>
> **★★ 계정 탈퇴 Slice 4(화면)는 2026-09-11 독립 검증에서 조건부 합격을 받았다(위 절).** 구현은 계약과 일치하지만 **셀 둘이 없어서** 합격이 아니다 — 조건 폐쇄(셀 둘, 구현 세션) → 승격 재검(다음 검증 세션)이 Slice 3 과 같은 순서다. 남은 것은 그 뒤 **Slice 5(문서 승격)** 와 **Slice 4b(관리자 잔여 정리 — 브리프 선행)** 다.
>
> | 순서 | 무엇 | 왜 이 순서인가 |
> |---|---|---|
> | **1** | **Slice 4 검증 조건 B1·B2 폐쇄(셀 둘) → 승격 재검** | 검증 기록 [`withdrawal_slice4_screen.md`](docs/verifications/2026-09-11/withdrawal_slice4_screen.md) — 배너 장착(AuthGate) 셀 · 취소 409 재조회 fetch 단정. **각각 한 셀**이라 랜딩보다 싸다 |
> | 2 | **2번 랜딩 + 10번 동의 게이트** | **한 창에서 본다** — 아래 3번 행이 그 순서다 |
> | 3 | 11번 **Slice 5**(정책 §8 → §6 승격) | 화면까지 섰으므로 *"정해졌으나 미시행"* 이 더는 참이 아니다. 한 편집이고 포인터를 단다 |
> | 4 | **12번 랜딩 + 10번 동의 게이트** | **한 창에서 본다.** 순서는 ~~① 약관 대괄호 넷 치환 + 시행 버전 문자열~~ **완료(2026-09-09, 버전 `1.0`)** ② `/terms`·`/privacy` + 푸터 ③ 랜딩 본문 ④ 동의 게이트. **Phase S-2(nginx 헤더)·AdSense 화면별 제어도 같은 창** — 광고가 랜딩·로그인에도 뜬다 |
