...
6. **육안 확인(누적)** — 프론트 재빌드 선행. 스타일 개선은 [Phase W](docs/plans/frontend-writing-studio-phase.md)를 따른다. **W2 한글 타이포(시스템 서체 축)는 닫았다(2026-09-12)** — 원고 본문 한글은 OS별 명조 폴백, UI 한글은 system-ui 뒤 고딕 고정, 라틴·숫자 불변. **웹폰트 도입(기기 간 일관성·두자 해소)은 오너 결정 갈림길로 열려 있다**, 제목 등 나머지 serif 표면의 한글 서체도 원고 명조가 자리 잡은 뒤 별도 비교다. **다음은 W3: 서재**, 이후 W4 마감이다. 모바일 도구는 48rem 이하에서 원고 앞의 일반 흐름(42rem 이하 2열)에 있고, 데스크톱은 고정 우측 띠다. 모바일을 스티키로 바꾸면 원고 겹침을 다시 검증해야 한다. 기존 블루 팔레트·공통 외곽 폭은 유지한다. Phase 10 마감 다섯(첫 화면 콘텐츠 · 오른쪽 끝 정렬 · 목록 행 간격 · 차트 막대 테두리 · 버튼 hover) · 비활성 버튼 색 · 편집기 드로어와 설정 탭이 좁은 화면에서 겹치는지 · 관측 화면과 비동기 패드 렌더 · **최종 저장·분석 연동의 첫 실사용**(운영 이력 0건) · **장면 메모 행 꼬리줄**(좁은 드로어에서 수정 시각과 "더 보기"가 한 줄에 남는지 · 긴 전문을 펼쳤을 때 목록 스크롤).
