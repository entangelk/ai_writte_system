...
>
> **착수 가능한 순서**: **13·14번(오너가 실사용에서 본 결함 — 배포된 화면이 지금 그렇다)** → 11번(계정 탈퇴 Slice 0~5, 가장 큰 덩어리·결정 전부 확정) → 12번(랜딩 기획) → 10번(가입 동의 게이트, 오너 검토 선행) → S-2(nginx 헤더). 그 밖은 오너 병목이거나 운영 데이터 대기다. **13·14 를 앞에 둔 이유**: 다른 항목은 *아직 없는 것*을 만드는 일이지만 이 둘은 **이미 나가 있는 화면이 사용자에게 잘못 동작하는 것**이고, 둘 다 원인 후보가 코드 수준에서 이미 잡혀 있어 싸다.
...
5. **★ 배포 — 2026-09-07 완료, 다만 커밋 둘이 아직 origin 밖이다.** 오늘 배포로 원장 소유 축 개명(v1.8.43·v1.8.44)까지 나갔다(마이그레이션 `renamed: 14` · 옛 인덱스 3개 제거 · 전 컨테이너 healthy). **남은 미푸시는 진입점 부트스트랩 둘**(`5759d9e`·`a207a7c`) — 그래서 **지금 서버의 스크립트로는 `-e PYTHONPATH=/app` 가 필요하고, push 뒤 배포부터는 필요 없다.** 다음 배포 절차: `git pull --ff-only` → `build` → **마이그레이션이 필요한 슬라이스면 `run --rm --no-deps <서비스> python scripts/…`** → `up -d`. **★ `exec` 가 아니라 `run` 이다** — `scripts/` 는 이미지에 COPY 되므로 도는 컨테이너(옛 이미지)에는 새 스크립트가 없다.
...
13. **★ 메모 읽기면 결함 둘(오너 실사용 관측 2026-09-07, 배포 빌드)** — ① *"메모 작성 중인 페이지에서 저장된 메모를 눌렀을 때 메모가 안 보였다"* ② *"메모가 보이는 곳이 흰 바탕에 텍스트라 보기 힘들다"*. **①의 확인된 원인 후보(코드 실측)**: 드로어 패널의 읽기면이 `body: null`(메모 없음)과 `""`(빈 메모)를 **둘 다 빈 `<p>` 로 그린다** — `SceneNotePanel.tsx` 의 `setOtherBody(note.body ?? "")` 뒤 `<p className="note-body">{otherBody}</p>` 라 **아무 문구도 안 뜬다**. 편집면은 그 둘을 구분하는데(`missing ? "아직 메모가 없습니다."`) 읽기면만 안 한다 — **SoT v1.8.11 이 `null`/`""` 구분을 계약으로 박아 둔 자리라 계약 위반이다.** 또 하나: **현재 장면의 행을 누르면** `selected` 가 `null` 이 돼 편집 상자로 돌아가는데, 이미 거기 있었으면 **아무 변화가 없어 보인다**(의도된 동작이지만 피드백이 없다). **②는 `.note-body` 가 면·테두리 없이 맨 텍스트**여서다 — 같은 패널의 편집 상자(`.rail-note-editor`)는 면이 있어 **읽기면만 어긋난다**. 고칠 때 양방향 셀을 함께: `null`/`""`/내용 셋이 각각 다른 것을 그리는지.
14. **★ 이어쓰기 "채택" 이 아무 동작도 안 한다(오너 실사용 관측 2026-09-07)** — *"애초에 채택을 눌렀을 때 어떤 동작이 되는지도 모르겠다"*. **코드 실측으로 원인이 잡힌다**: `WritingPanel.tsx` 의 버튼 `disabled` 는 `canAccept`(= 후보 있음 · gate pass · `nextUnitReady`)인데, `accept()` 핸들러는 **조건 하나를 더 본다 — `contextRef.current === null` 이면 조용히 `return`**. 둘이 어긋나는 순간 **버튼은 활성인데 눌러도 아무 일도 안 나고 메시지조차 없다.** `contextRef` 는 생성 경로에서만 채워지고(`WritingPanel.tsx:376`) **자동 개선(revise) 경로는 후보·gate 만 갈아 끼우고 채우지 않는다**(`:475` 부근) · 채택 성공 뒤에도 비워진다(`:575`). **처방 방향**: ⓐ `disabled` 가 핸들러의 **모든** 조건을 덮게 하고 ⓑ **조용한 return 을 없앤다**(왜 못 하는지 화면이 말해야 한다 — 이 화면의 다른 차단은 이미 그렇게 한다). **오너의 두 번째 문장이 더 큰 지적이다** — 버튼 라벨이 *"채택하고 저장"* 인데 **무엇이 저장되는지**가 화면에 없다: 채택은 **생성 시점에 고정된 base + 후보**를 저장하고 **그 뒤 편집기에 친 글은 버린다**(핸들러 주석). 그 사실이 확인 대화상자에만 있고 버튼 옆엔 없다. **어느 쪽이 진짜 결함인지 먼저 재현할 것** — 스크래치 패드의 `채택`(`ScratchRecovery.tsx:313`)은 **다른 버튼**이라 오너가 본 것이 어느 쪽인지 확인이 먼저다.
