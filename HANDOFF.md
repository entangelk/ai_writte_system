...
9. **~~서비스 정책 계약 문서~~ 만들었다(2026-09-07, SoT v1.8.40)** — [`service-policy-contract.md`](docs/service-policy-contract.md). **남은 것은 오너 결정 셋**이고 브리프에 선택지·추천이 다 있다([`plans/service-policy-decisions.md`](docs/plans/service-policy-decisions.md)): ① **추론 경계 밖 전송 고지** — 실측상 **가정이 아니다**(지금 배포가 `EMBEDDING_SERVICE_URL` 을 필수로 만들고 임베딩이 본문을 그대로 받는다. 제3자인지 오너 LAN 인지는 저장소 밖 `.env` 가 정한다) ② **데이터 보존 기간** — 지금 답이 **"영원히"** 다(TTL 인덱스는 운영용 넷뿐, 원고·활동 로그·원장엔 0건) ③ **계정 삭제** — **셀프 삭제 경로가 없다**(관리자 비활성화뿐). **문서를 고칠 때 주의**: 값은 파생본이라 `tests/test_service_policy_contract.py` 가 상수와 대조한다 — 값을 고치려면 **상수를** 고친다. 새 행을 더하면 `모듈::상수` 포인터를 함께 단다(포인터 없는 행은 잠기지 않는다).
