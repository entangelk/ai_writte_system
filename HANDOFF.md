...
- **[해소됨 2026-08-01] 앱 HTTP API를 치는 스크립트 8종에 세션 로그인이 붙었다.** `--username <계정>` + **`APPLICATION_PASSWORD` env**(비밀번호는 argv 금지 — `create_user.py` 선례, shell history·`ps` 노출)로 로그인하고, 얻은 토큰을 **명시 `Cookie` 헤더**로 싣는다. 헤더인 이유가 핵심이다 — 세션 쿠키는 `Secure`라 httpx 쿠키 저장소가 **plain http에서 조용히 버리는데** 이 스크립트들은 전부 `http://application:8000`이다(뮤테이션 실측: 저장소에 맡기면 `cookie: None`). 공용 코드는 [`scripts/script_auth.py`](scripts/script_auth.py), 배선 전수 가드는 `tests/test_script_login.py::ScriptLoginWiringCoverageTest`(**앱 HTTP를 치는 스크립트를 새로 만들면 이 목록에 넣는다** — 안 넣으면 다시 조용히 401난다). **자격증명을 안 주면 종전대로 익명 진행**이라 로그인 없이 되는 호출은 그대로 쓴다. 라이브 확인: 실 앱 관통으로 무자격 401 → 로그인 후 통과(phase3a exit 0, phase2a 인증 write 6건 200). **워커는 여전히 무관**(HTTP를 안 쓰고 Mongo 직접 — D8-7).
  - **대상 8종**: HANDOFF가 지목했던 6종(운영 smoke `phase2a`·`phase3a`·`phase4`·`phase6` + 진단 `diagnose_writing_report`·`diagnose_writing_gate`)에 **패턴 스윕이 찾은 2종**(`benchmark_writing_loop`·`measure_writing_stages` — 둘 다 `/projects`를 친다)이 더해졌다. 진단 2종은 **`--current-position`을 주면 시드를 건너뛰므로 로그인도 필요 없다**(로그인이 필요한 것은 시드 write뿐).
