...
- 구현된 계층(모두 회귀로 잠김, 아래는 현재 동작하는 표면):
  ...
  - **Writing AI(Phase 5.1~5.3, v1.6.68~70)**: continue_scene 평문 `WritingCandidate` 생성 → 별도 LLM Writing Gate(다섯 decision+구조화 findings) → user accept 시 latest base 검증·결정적 paragraph append·immutable save·pending Analysis job 생성. `POST /writing/generate|gate|accept`; accept non-pass는 `200 accepted=false`, job write 실패는 saved artifact 포함 502+same-key retry 수렴. 구조적 self-report·부분 revise/retrieve·background analysis run은 후속.
...
- **테스트**: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **861 passed / 48 skipped / 142 subtests**(2026-07-12, v1.6.70 Writing accept 독립 검증 hardening 포함). skip은 대부분 live Mongo/Chroma/embedding 미가용 통합 테스트.
