...
| # | 항목 | 왜 이 순서인가 | 자리 |
|---|---|---|---|
| **S-0** | **문서 위생 — 사설 LAN 주소가 49개 파일**에 있다. (확인용 계정 `timeline_demo` 축은 **닫혔다** — 오너 확인 2026-09-05: **알파 머신 로컬 DB 에만 있었고 배포 서비스에는 존재한 적이 없다.** 즉 저장소에 남은 문자열에 대응하는 **살아 있는 계정이 없다** — 감사가 이 사실을 몰라 MEDIUM 으로 올렸다.) | **이 문서 머리의 "공개 저장소 보안 규칙" 을 저장소가 스스로 어기고 있다** — 남은 것은 침해가 아니라 **규칙 위반**이다. ★ 파일에서 지워도 **git 이력에는 남는다**(이력 재작성은 2026-08-23 에 한 번 했고 비싸다) — 스윕의 목적은 소급 삭제가 아니라 **재유입 방지**다 | 감사 §B.2~B.4 |
| **S-1** | **공개(다중 사용자) 전 필수 — quota 우회 체인** ① **[HIGH]** 클라이언트가 보내는 `body.request_id` 가 원장 dedupe 키다 — 같은 값을 반복 전송하면 LLM 이 실제로 돌면서 **과금은 1행으로 접힌다**(`X-Confirm-Duplicate` 로 5초 잠금도 덮인다) ② 실패 job **retry 에 횟수·쿨다운이 없다**(analysis·generation 둘 다) ③ identity judging 이 페어당 판정을 **무제한 팬아웃** | **지금 막혀 있는 이유는 코드가 아니라 승인제 가입이다** — 유료 경로는 세션을 요구하고 pending 계정에는 세션이 발급되지 않는다(`request_signup` 은 `pending` 행만 만든다). **오너 확인 2026-09-05: 승인된 계정이 오너 본인뿐이다** → 외부 행위자가 도달할 수 없다. **그 방벽은 가입을 여는 순간 사라진다** — 랜딩·약관과 **같은 트리거를 공유한다.** (오너 계정은 `limit=None` 정책 행이라 이 결함이 오너에게는 애초에 무의미하다 — 순수하게 공개 전 항목이다.) | 감사 §A.1~A.4 |
...
| **S-3** | **미인증 표면 넷** — LLM 게이트웨이·in-stack llama(`:9080`)·embedding `/embed` 에 **호출자 인증이 없고**, 공개 `/auth/signup` 에 레이트리밋이 없다(Argon2 홍수 축은 username 잠금이라 아이디를 돌리면 무력) | 지금은 **loopback 바인드가 유일한 방어**이고 그것이 성립하는 동안은 LAN 밖에서 못 닿는다(llama `:9080` 만 의도적 LAN 공개). **D8-7 G2~G6(자격증명) 유예와 같은 트리거** — 원격·다중 호스트 배포 | 감사 §A.5·8~11 |
| **S-3** | **미인증 표면 — ★ 승인과 무관하게 지금 열려 있는 것이 하나 있다.** 공개 `POST /auth/signup` 은 **승인 전에 Argon2 해싱을 수행하고**(`auth/users.py::request_signup` — `self._hasher.hash(password)`) **레이트리밋이 없다**. 즉 **계정을 하나도 승인하지 않아도** 미인증 요청자가 CPU 를 태우고 pending 행을 무제한 쌓을 수 있다. 나머지 셋(LLM 게이트웨이·in-stack llama `:9080`·embedding `/embed`)은 **호출자 인증이 없지만 loopback 바인드 뒤**다 | **signup 축만 트리거가 "이미 왔다"** — 배포가 도메인으로 공개돼 있으면 오늘 도달 가능하다. 나머지 셋은 **D8-7 G2~G6(자격증명) 유예와 같은 트리거**(원격·다중 호스트 배포)이며 llama `:9080` 만 의도적 LAN 공개다 | 감사 §A.5·8~11 |
...
| **S-6** | **배포 호스트 노출면 — 2026-09-05 오너 승인 하에 원격 점검(읽기 전용).** **★ 인바운드 포트는 열려 있지 않다** — **Cloudflare Tunnel**(`cloudflared tunnel run`) 로 서비스하므로 공유기 포워딩이 아니라 **서버발 아웃바운드 터널**이 유일한 진입로다(오너 확인 + 프로세스 실측). 따라서 아래 노출은 전부 **LAN 한정**이다: ① `application`·`frontend` 가 `0.0.0.0` 게시라 **nginx 를 우회하는 경로가 LAN 에 열려 있다** ② **이 서버는 다른 프로젝트와 공유한다** — 타 프로젝트의 Mongo 2대·Redis 1대·공용 nginx 가 전부 `0.0.0.0` 게시 ③ **`ufw` 는 active 지만 Docker 게시 포트는 ufw 를 우회한다**(DOCKER 체인이 먼저 평가된다 — 다른 머신에서 실제로 도달 확인). **★ 이 프로젝트의 저장소는 설계대로 loopback 바인드였다**(mongo·ES·chroma·gateway) — **D8-7 G1=C 가 배포 서버에서도 성립한다.** | **★ 내일 S-3 을 하기 전에 반드시 확인할 것 — 터널이 nginx 로 가는가, 앱으로 바로 가는가.** 토큰 방식 터널이라 ingress 규칙이 **Cloudflare 대시보드에 있고 서버에서는 볼 수 없다**. **터널이 앱(:8000/:8520)을 직접 가리키면 nginx `limit_req` 는 실트래픽에 아무 효과가 없다.** ②는 이 프로젝트 범위 밖이지만 **같은 호스트라 함께 무너진다** — HANDOFF 가 오래 경고한 *"그 포트는 다른 프로젝트 것"* 함정이 배포 서버 규모로 재현된 것이다 | 원격 점검 |
| **S-7** | **[LOW · 위생] Cloudflare 터널 토큰이 프로세스 인자에 있다.** `/proc/<pid>/cmdline` 은 **0444(누구나 읽기)** 이고 `/proc/<pid>/environ` 은 **0400(소유자만)** 이라 인자로 준 비밀값은 파일·env 보다 넓게 읽힌다(2026-09-05 실측). **★ 다만 심각도는 낮다 — 어느 쪽이든 서버 접근이 선행한다.** 이 호스트의 로그인 계정은 `uid 1000` **하나뿐**이고 cloudflared 는 root 로 돈다. 타 프로젝트는 컨테이너라 호스트 프로세스가 root 이므로 **탈출 시나리오에서는 인자든 env 든 차이가 없다.** 실질 차이가 남는 곳은 **uid 1000 으로 도는 무언가**(개발 도구·스크립트·SSH 로 붙은 에이전트)와 **`ps` 출력이 밖으로 새는 경로**(로그·모니터링 수집·지원 덤프에 붙여 넣기) 둘이다 | **처방은 둘이고 서로 다른 문제를 푼다.** ① **저장 방식** — `cloudflared` 는 `--token-file`(`$TUNNEL_TOKEN_FILE`)과 `$TUNNEL_TOKEN` 을 **둘 다 네이티브 지원**한다(`--help` 실측). `0600` 파일 + `--token-file`, 또는 systemd `EnvironmentFile=`(0600)로 옮기면 인자에서 빠진다. **★ 함정 — systemd `Environment=` 에 값을 직접 적으면 `systemctl show` 로 드러나므로 `EnvironmentFile` 이어야 한다.** ② **회전** — 저장 방식과 무관하다. **이 값이 서버 밖으로 나갔는지**의 문제이며, 2026-09-05 점검에서 세션 출력에 남았다. **그 기록을 신뢰하면 회전은 선택**이고, 아니면 한 번 돌린다. **회전만 하고 인자를 그대로 두면 새 값이 같은 자리에 놓인다** | 원격 점검 |
...
> **★ 2026-09-05 보안 감사 후속(Phase S)이 들어왔다.** 감사 판단 **critical 0건** + 오너 확인 둘(확인용 계정은 배포에 없었다 · 승인된 계정이 오너뿐이다)을 반영한 실제 우선순위는 이렇다 — **오늘 도달 가능한 것은 S-3 의 signup 축 하나뿐**이고(승인과 무관하게 열려 있다), **S-1(HIGH)은 가입을 여는 순간 위험해지므로 랜딩·약관과 같은 트리거**다. **S-0 은 침해가 아니라 이 문서 머리의 공개 저장소 보안 규칙 위반**이다. Slice 5 검증은 **이미 병합된 코드를 다시 뜯는 일이라 미뤄도 외부 위험이 늘지 않는다** — 그래서 S-1 을 앞에 뒀다.

0. **★ Phase S — 보안 감사 후속**(기록 [`security_audit_dual_workflow.md`](docs/verifications/2026-09-05/security_audit_dual_workflow.md), 확정 39건 · critical 0 · HIGH 1). 아래 "🛡 보안 — Phase S" 표가 항목별 정본이다. **S-3 signup 레이트리밋(★ 착수 전에 터널이 nginx 로 가는지 확인) → S-1 quota dedupe(가입 열기 전 필수) → S-0 문서 스윕 → S-7 토큰 저장 방식(위생, 설정 한 줄) → 나머지는 트리거.**
