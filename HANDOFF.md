...
| **수동 저장 replay 특성 셀이 행의 *정체* 를 안 본다**(2026-09-12 세션 74 승격 재검 HP-1) | `test_resending_the_same_save_key_still_records_a_second_row` 가 **행 수만** 세어, replay 가 엉뚱한 action 을 남겨도 238셀 전건 초록이다(실측). **형제 셀인 accept 쪽은 문다** — 같은 슬라이스가 낳은 두 셀의 비대칭이고 **첫 저장 축은 이미 두 겹으로 잠겨 있어** 열린 것은 replay 행 하나뿐이다. 처방 한 줄: `assertEqual(self.repo.events[-1].action, "draft_version_saved")`. **오너 결정 D2 와 같은 자리라 D2=ⓑ 면 셀 자체가 뒤집힌다** | [`tests/test_activity_api.py`](tests/test_activity_api.py) · [`verifications/2026-09-12/activity_replay_c1_promotion.md`](docs/verifications/2026-09-12/activity_replay_c1_promotion.md) §Hardening |
