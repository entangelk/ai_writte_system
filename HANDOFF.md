...
- 정본은 `docs/system-contract-sot.md` **v1.7.45**(Approved). 미확정 항목은 추측 구현하지 않는다.
...
- **`parse_error` 재분류는 호출부가 명시할 때만 일어난다**(데코레이터는 도메인 거부를 모른다). gate는 재분류하고, **extractor는 일부러 안 한다** — repair를 유발한 첫 응답은 실패가 아니라 **회수된** 호출이고, 오너가 보려는 repair 빈도는 `correlation_id`당 레코드 2건으로 이미 드러나기 때문이다. 재분류하면 성공률이 실제보다 낮아진다(SoT v1.7.45).
