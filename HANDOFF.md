...
>
> 마지막 자가 검수: 2026-08-02 · 231줄 (**D8-7 G1=C + SoT v1.7.75**. 오너가 노출면 축소를 골라 저장소·내부 5종 + test-mongo가 `127.0.0.1` 바인드가 됐고, **"외부 노출 금지" 해제 조건이 *인증*→*노출 없음*으로 개정**됐다. 코드 0줄·compose 3파일. 뮤테이션 3방향 실증. 1836 passed/회귀 0. **오너 결정 대기 페이즈 트랙은 이제 D8-5 하나뿐**. 다음 검수 트리거는 300줄)
...
- **[문서 부채, 오너 지시 2026-08-02 · 미착수] 기획 축 "제품 한 장 요약"이 없다.** README를 기획·개발·서비스 3축으로 재구성하면서 드러난 공백이다 — **제품 컨셉·MVP 범위·핵심 위험이 전부 [`docs/abstract.md`](docs/abstract.md)(48KB 초안) 안에** 있어서, 평가자든 새 작업자든 **§1·§2·§15·§16을 직접 뽑아 읽어야** 한다. 지금은 README 기획 절이 그 역할을 임시로 대신하고 있어 급하지는 않다. 만들 때 **원본을 요약하는 게 아니라 *현재 상태 기준으로* 다시 쓴다** — abstract는 2026-06월 초안이고 그 뒤 D0(다중 사용자 전환)·MVP 범위 조정이 있었으므로, 초안을 그대로 압축하면 낡은 그림이 정문에 걸린다. 참조할 현재 사실: `plans/00-foundations.md`(제품 목표·불변 원칙) · `plans/README.md`의 Phase↔MVP 표 · SoT §"제품과 프로젝트 경계" · `observability-kpi-rationale.md`(운영 기획 톤의 선례).
