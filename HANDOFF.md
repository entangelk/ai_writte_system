...
- 회귀 기준선: backend는 **test-mongo ON 전량 실행 1848 passed / 4 skipped / 1559 subtests**가 기준이다(2026-08-02 D8-7 G1=C + 검증 hardening + 문서 인덱스 가드 + purge reconciler 후 실측, 104s — 08-01 기준선 1831 대비 **passed +17 = `tests/test_compose_exposure.py` 6셀 + `tests/test_docs_indexes.py` 5셀 + `tests/test_purge_reconciler.py` 6셀, subtests +7 = loopback 5종 + 공개 표면 2종, 회귀 0건**; skip 4는 알파 호스트 환경). 시간은 부하마다 다르니 기준이 아니다. test-mongo OFF 보조 실행은 **1612 passed / 89 skipped / 1468 subtests**(2026-07-28 D8-5c 후 실측, 182s)이며, 이 **89 skip을 정상 기준선으로 읽으면 안 된다**(Mongo 통합 계약 + live Chroma). **OFF 수치는 07-28 것이라 그 뒤 추가분(+49)을 포함하지 않는다** — 두 실행을 직접 빼서 비교하지 말고 각각 같은 날 다시 잰다. frontend는 **227 passed / 15 files**. build JS는 **진입 404.87 kB + 관측 화면 청크 385.71 kB**(차트는 `React.lazy`로 분리). **skip 수는 머신·인프라 기동 여부마다 다르다** — 숫자가 안 맞으면 `-rs`로 skip 사유부터 볼 것. **백엔드는 `argon2-cffi`가 설치돼 있어야 한다** — 없으면 auth 관련 26개 모듈이 수집 단계에서 실패해 회귀처럼 보인다. 핀은 루트가 아니라 [`services/application/requirements.txt:1`](services/application/requirements.txt#L1)에 있다(`argon2-cffi>=23,<24`). **프론트는 `npm install`이 선행**돼야 한다.
