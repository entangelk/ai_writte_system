...
4. **~~장면 메모 후속(수정 시각 · "더 보기")~~ 완료(2026-09-07) — 독립 검증 합격**([`verifications/2026-09-07/scene_note_follow_up.md`](docs/verifications/2026-09-07/scene_note_follow_up.md)**)** — 구현 SoT **v1.8.38**(`f46a462`·`9262f27`). 남는 계약 주의: **"더 보기"는 단건 GET 한 번이지 목록의 전문이 아니다**(목록이 전문을 기대하게 만들면 12000자 × 장면 수가 응답에 들어온다) · 펼침은 **두 화면에 똑같이** 있고 드로어에서 **편집 대상을 옮기지 않는다**(행 제목의 선택과 다른 일) · 목록이 새로 오면 접는다(옛 전문이 새 스니펫을 이기면 안 된다) · 새 CSS 규칙이 `var(--type-*)` 를 쓰면 **`typeScale.test.ts` 이관 목록에 등재**해야 한다(이번에 실제로 걸렸다).
