...
정본은 [`docs/system-contract-sot.md`](docs/system-contract-sot.md) **v1.8.60**(Approved). **미확정 항목은 추측 구현하지 않는다.** 아래는 코드를 만지기 전에 알아야 하는 요약이다.
...
>
> **★★ 랜딩 축 ③(본문)과 ④(가입 동의 게이트)가 2026-09-12 에 구현됐다(SoT v1.8.59 · 아래 10·12번).** **다음 구현 축은 Slice 4b(관리자 잔여 정리 — 브리프 선행)** 와 5번(최종 저장 6차 재검증)이다. **Slice 5(정책 §8 → §6 승격)는 다른 작업 AI가 2026-09-12 동시 진행 중이다** — 같은 저장소를 공유하므로 커밋·문서 편집 전 `git log`·`git status`를 먼저 본다.
>
> | 순서 | 무엇 | 왜 이 순서인가 |
> |---|---|---|
...
> | 3 | ~~11번 **Slice 5**(정책 §8 → §6 승격)~~ **완료(2026-09-12, SoT v1.8.60)** — §6 에 30일 행 + 정본 포인터 · 두 문서 머리말의 **거짓이 된 미제공 고지** 정리 · 약관 제8조 3항·방침 제5조 3항의 `30일` ↔ 상수 신규 가드. **독립 검증 조건부 합격 → 조건 셋(B1 취소 경계 · B2 방침 머리말 · B3 §8 자기모순) 같은 세션에 폐쇄 + 하드닝 다섯**(SoT v1.8.61). **판정 승격은 다음 검증 세션 몫이다.** 계정 탈퇴 축에 남은 구현은 **Slice 4b 하나**(브리프 선행·트리거 없음) |
