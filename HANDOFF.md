...
- 정본은 `docs/system-contract-sot.md` **v1.7.46**(Approved). 미확정 항목은 추측 구현하지 않는다.
- **진행 중 페이즈 — LLM 파이프라인 관측(KPI)**(브리프 `plans/observability-kpi-decisions.md` 승인 D1=B·D2=C·D3=A). **계측 seam은 provider 데코레이터로 확정**(오너 결정 2026-07-25, 브리프 `plans/observability-instrumentation-seam-decisions.md` C안) — `ObservedProvider`가 `generate()` 1회를 레코드 1건으로 만들고, 워크플로 귀속은 `llm_call_scope` contextvar가 나른다. **계측된 호출부: `analysis_extractor` · `writing_gate`(둘 다 seam C)**. 운영기획 포트폴리오는 `docs/observability-kpi-rationale.md`. QUAL-1(제품 품질·수기·dogfood)과 별개 트랙.
- **새 호출부를 계측하는 법**: ① `main.py` 조립 지점에서 provider를 `ObservedProvider(inner, call_site=…)`로 감싼다(도메인 코드는 건드리지 않는다). ② 그 호출이 일어나는 요청 경로에서 `llm_call_scope(llm_call_audit, project_id=…, correlation_id=…)`를 연다 — **이걸 빠뜨리면 레코드가 조용히 0건이 되므로 배선 회귀를 함께 넣는다**(scope 유닛 테스트만으로는 안 물린다). ③ **조립 가드도 함께 넣는다** — 테스트 하네스는 `ObservedProvider`를 직접 만들기 때문에 `_default_*`가 감싸기를 빠뜨려도 스위트는 green이고 **배포에서만** 계측이 사라진다(실측: gate 조립에서 wrapper를 벗겨도 56 passed). ④ 도메인만 아는 판정(gate decision·파생점수, `parse_error` 재분류)은 `scope.annotate_last(...)`로 flush 전에 얹는다 — **얹을 수 있다는 것과 얹어야 한다는 것은 다르다**(extractor 사례).
...
- **`parse_error` 재분류는 호출부가 명시할 때만 일어난다**(데코레이터는 도메인 거부를 모른다). gate는 재분류하고, **extractor는 일부러 안 한다** — repair를 유발한 첫 응답은 실패가 아니라 **회수된** 호출이고, 오너가 보려는 repair 빈도는 `correlation_id`당 레코드 2건으로 이미 드러나기 때문이다. 재분류하면 성공률이 실제보다 낮아진다. 그래서 **site마다 낼 수 있는 `outcome`이 다르다** — gate는 셋 다(`success`·`provider_error`·`parse_error`), extractor는 둘뿐(`success`·`provider_error`). 집계에서 extractor의 `parse_error`=0은 **구조적 사실이지 데이터 부족이 아니다**. 새 site를 계측할 때 그 범위를 SoT 본문에 **함께** 적는다(SoT v1.7.46).
...
- 회귀 기준선: backend **1499 passed / 593 subtests**(test-mongo 기동), frontend **194 passed / 13 files**, build JS 399.03 kB. **skip 수는 머신마다 다르다** — live Chroma 1건은 항상 skip이고, `elasticsearch` 파이썬 패키지가 없는 머신에서는 lexical retrieval 3건이 추가로 skip된다(2026-07-25 이 머신 = 4 skipped). 숫자가 안 맞으면 회귀를 의심하기 전에 skip 사유부터 `-rs`로 확인할 것.
...
1. **관측 KPI 잔여 증분**: (C) `compare_judge`(candidate당 1회 → N레코드) · `query_planner`(revise loop 내부) · `writing_generation`(reporter 붙으면 2회, `generate_metered` 없음) 계측 — 각각 **조립 가드를 함께** 넣는다. 이어 **증분 5** = `GET …/observability/kpi` 집계 API + H3 에러 선언. 집계 규칙 2개는 이미 계약에 고정돼 있다 — 토큰은 `success`+`parse_error`만, repair 빈도는 `correlation_id`당 레코드 수.
