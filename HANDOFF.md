...
- 정본 SoT는 `docs/system-contract-sot.md`이며 현재 **v1.6.80**(Approved). SoT가 정본 우선순위이고, 미확정 항목은 추측 구현하지 않는다.
...
- 구현된 계층(모두 회귀로 잠김, 아래는 현재 동작하는 표면):
  ...
  - **Writing AI(Phase 5.1~5.10, v1.6.68~80)**: 평문 candidate 생성→typed report→Writing Gate→accept/save→pending Analysis job. inline report/revise와 `/writing/revise-and-gate` bounded 합성까지 구현. 합성은 단일 continuity finding auto-revise와 targeted retrieval을 설정 가능한 구조적 상한(기본 revision 총2·retrieval1·Gate3) 안에서 순서와 무관하게 이어간다. **aggregate budget(v1.6.80)**: revise/report(+repair)/Gate/retrieval planner(+repair)의 provider usage를 내부 채널로 정확히 한 번씩 합산하고, optional `max_total_tokens|max_wall_clock_ms`를 post-accounting/deadline 시맨틱으로 강제한다. env/Compose `WRITING_LOOP_MAX_TOTAL_TOKENS|WRITING_LOOP_MAX_WALL_CLOCK_MS`; 미설정은 off이며 production 숫자는 B2b live 계측 후 확정한다. 초과 결과는 채택하지 않고 마지막 완전 candidate/최근 Gate를 보존하며, 첫 Gate 전 소진은 `gate=null` 가능. ephemeral `loop`/`stages`는 무변이고 aggregate는 opt-in persisted audit summary/detail에만 노출된다. **persisted loop audit(v1.6.78→79 opt-in)**: append-only immutable bodyless trail+final candidate text, 요청 `persist_audit`/env 기본 off, persist 실패 격리+`audit_error`; retention은 운영과제 후속. side effect는 감사 쓰기 1건뿐(Core SOT/Analysis/memory 0).
...
- **테스트**: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **972 passed / 48 skipped / 215 subtests**(2026-07-13, v1.6.80 aggregate budget B1 closure; 현재 환경은 ES Python dependency 미설치라 관련 3개 skip). skip은 대부분 live Mongo/Chroma/embedding 미가용 통합 테스트.
...
- **현재 구현을 막는 오너 결정 없음.** Phase 5.10 M1~M6은 승인·구현 완료(v1.6.80). 다음 수치 결정은 B2b full-stack loop benchmark가 aggregate token/wall-clock p95를 만든 뒤 production 기본값을 off→on 승격할지와 함께 확정한다.
...
1. **Phase 5.2 Writing Gate 구현 완료(v1.6.69)** — 별도 LLM Gate service/prompt/models + `POST /projects/{id}/writing/gate`, 구조화 findings와 다섯 decision/우선순위, project isolation·strict JSON/provider error mapping을 회귀로 잠갔다. 다음 핵심 후보:
   ...
   - **bounded revise/retrieve loop + persisted audit + aggregate budget 완료(v1.6.71~80)** — 다음 Writing 후보는 **B2b full-stack loop-level benchmark**(production token/time 숫자 확정). 이후 multi-finding, stable pointer, persisted 감사 retention(TTL/archive 운영과제)·전체 중간 artifact(P1=C).
...
├── system-contract-sot.md       # 정본 계약 SoT(Approved, v1.6.80)
