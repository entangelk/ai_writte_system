...
- 정본은 `docs/system-contract-sot.md` **v1.7.66**(Approved). 미확정 항목은 추측 구현하지 않는다.
...
- **컨텍스트 예산 트랙 = K-1(c) 보류 확정 · R-a 결정·구현 완료**(오너 2026-07-30/31). **K-1(c)(색인 시점 정확 계수)는 만들지 않는다** ((c)의 하중 근거가 K-3 가드로 소멸했고, 회계 비율 실측 1.07, (c)는 색인에 LLM 의존을 새로 만든다). **R-a 형태 = (ii) 창에서 유도 + (iii) 후보 길이에서 유도**(상수 (i) 기각)로 결정됐고, 적용 지점 5곳(generate · 생성 워커 · report · **revise-and-gate 루프** · **accept**, v1.7.66) 전부 구현됐다. **남은 것은 알파 R-c 관측 1회뿐** — 창을 32768로 올리면 유도가 자동으로 넓어진다(자세한 건 Next Tasks #2).
...
2. **R-a 적용 지점은 전부 채워졌다(SoT v1.7.66) — 남은 것은 알파 R-c 관측 1회뿐.** 오너 결정(2026-07-31)은 **(ii) 창에서 유도 + (iii) 후보 길이에서 유도**, **(i) 상수는 기각**.
   ...
   - **★ 남은 것 = 알파 R-c 관측 1회** — 창을 32768로 올리면 유도가 **자동으로** 넓어진다(루프도 포함). 착수 체크리스트(이미지 빌드 · `-hf` 재다운로드 · 오래된 볼륨의 `PromptTemplateConflict`)는 아래 함정 절과 같다.
