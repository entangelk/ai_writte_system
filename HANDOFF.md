...
2. **★ 컨텍스트 예산 — 한글 토큰 보정 · 창 가드 · 입력 상한 8192**(브리프 `plans/context-budget-korean-tokens-decisions.md`). **아직 코드는 한 줄도 안 바뀌었다** — C-1은 실측 슬라이스였다. 요지: 토큰 추정식이 `len/4`(영어용)인데 **한글 실측은 1.68~1.70자/token**이라 2.4배 과소평가하고, 그 결과 `max_tokens=4096` 예산이 **16,384자 = 실측 10,094 토큰**을 통과시켜 창을 그것만으로 넘긴다. 창을 지키는 코드는 없고 프론트에는 글자수 제한·표시가 아예 없다.
   - **★ 우선순위가 바뀌었다 — 이 트랙은 이제 "정확도 개선"이 아니라 "깨진 것 수리"다.** 2026-07-29 베타 실측(브리프 **§2-3**, 착수 전 필독): `long` 프리셋이 실제 원고 분량 입력에서 **4/4 실패**한다. **가장 큰 오차는 한글 밀도(2.4배)가 아니라 포인터 렌더링이 예산 회계에서 통째로 빠진 것(12.7배)**이며, 그래서 **C-2(밀도 보정)를 먼저 해도 이 실패는 그대로 남는다.** 착수 순서 판단이 K-1·K-6 결정과 함께 필요하다.
   - **다음은 C-2**(토큰 계측 보정: 색인 시 토큰수 저장 + 상수 fallback). **알파 머신이 필요 없다.** 단 위 항목대로 **C-2가 오늘의 실패를 고치지는 않는다.**
   - **C-3 착수 전 브리프 §2-1과 §2-3을 함께 읽을 것.** §2-1(알파·산술)이 "창 16384로도 report는 넘친다"를 말하고, §2-3(베타·실측)이 그것을 확인하면서 **원인과 실패 방식**을 바꾼다: 실패는 조용한 잘림이 아니라 **400 하드 거부(job 실패)**이고, **창을 16384로 올려도 고쳐지지 않고 조용해질 것으로 보인다**(400 → 200; 잘림 부분은 추론). §2-1의 임계치 표(short 8,754 / medium 7,755 / long 5,646)는 **포인터를 뺀 낙관적 상한**이라 실제 임계는 더 낮다.
