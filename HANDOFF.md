...
## 랜딩 축 ② (약관·방침 페이지 + 공개 푸터) — 독립 검증 조건부 (2026-09-11)
...
- **LB1 — 푸터의 세 장착 지점 중 둘이 무셀**: SoT v1.8.56 이 *"정문 세 얼굴"* 로 열거한 중 **AuthStatus(세션 확인·오류)와 가입 접수 얼굴**은 각각만 제거해도 전건 초록(MO-6a·6b). 실패 셀 둘 다 로그인 폼 얼굴만 본다 — **세션 56 B1(배너 장착)과 같은 형태의 재발**. **처방: 셀 하나**(두 얼굴에서 `법적 고지` nav 존재 단정). 폐쇄 → 승격 재검은 Slice 4와 같은 절차.
...
>
> **★★ 랜딩 축 ②(`/terms`·`/privacy` + 푸터)는 2026-09-11 독립 검증에서 조건부 합격을 받았다(위 절)** — 구현은 계약과 일치하나 **푸터 두 얼굴 장착 셀(LB1)** 이 없다. 셀 하나짜리 폐쇄가 1번이다. **구현 세션이 그다음에 집는 것은 ③ 랜딩 본문과 ④ 가입 동의 게이트**고, 그 뒤가 **Slice 5(문서 승격)** 와 **Slice 4b(관리자 잔여 정리 — 브리프 선행)** 다.
>
> | 순서 | 무엇 | 왜 이 순서인가 |
> |---|---|---|
> | **1** | **랜딩 ② 검증 조건 LB1 폐쇄(셀 하나) → 승격 재검** | 세션 확인·가입 접수 두 얼굴의 `법적 고지` 존재 단정 — 검증 기록 [`landing_l2_terms_privacy.md`](docs/verifications/2026-09-11/landing_l2_terms_privacy.md). ~~Slice 4 승격 재검~~ **완료(2026-09-11, 세션 59)** |
