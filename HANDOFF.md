...
2. Phase 2A job 상태 전이(slice 1/2/3) 완료. 다음 Phase 2A 후보: (a) analysis job/candidate를 노출하는 HTTP API(Core SOT API 패턴), (b) runner→gateway `/v1/generate` runtime wiring(단 model tool-call wire format/payload 확정 의존으로 현재 막힘). 진행 전 어느 쪽을 계약화할지 결정 필요.
...
- Phase 2A job 상태 전이 runner 통합(slice 2) 자체 회귀(2026-06-29): `python3 -m unittest tests.test_analysis_runner` 13개 통과(succeeded 전이, replay 비재실행=extractor 1회만 호출, 5개 실패 지점→`failure_reason` 매핑+candidate 미저장). `InvalidCandidateSource` 분기 제거 변이 시 source_invalid test FAIL로 source/schema 구분 load-bearing 증명(백업 복원). 전체 discovery 296개 통과(35 skip). throwaway replica set(port 27026)에서 live runner 합성 smoke(succeeded 영속 + snapshot_not_found `failed` 영속) 통과. 임시 컨테이너 정리.
