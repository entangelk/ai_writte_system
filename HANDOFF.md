...
| **S-7** | **[LOW→MEDIUM · 위생] Cloudflare 터널 토큰이 세계 읽기 가능한 자리 *둘*에 있다.** **★ 2026-09-06 원격 실측이 감사를 한 칸 넓혔다** — 감사는 `/proc/<pid>/cmdline`(**0444**) 하나만 짚었는데, **systemd 유닛 파일 자체가 `0644`** 이고 `ExecStart=` 에 토큰이 그대로 있다. 즉 **프로세스를 안 봐도 파일로 읽힌다.** 감사의 심각도 근거(*"로그인 계정이 uid 1000 하나뿐"*)도 절반만 참이다 — **로그인 셸을 가진 계정**은 하나지만 서비스 계정 여럿이 비-root uid 로 돌고 있어(2026-09-06 `ps` 실측) 0444 를 읽을 주체가 root·오너 말고도 있다. | **처방 ① 저장 방식 — `--token-file` 로 옮긴다**(`--token-file`·`$TUNNEL_TOKEN_FILE` 네이티브 지원 재확인, 2026-09-06). `EnvironmentFile=`(0600)도 되지만 **`--token-file` 이 낫다** — 값이 프로세스 `environ` 에조차 안 들어가 자식 프로세스·코어덤프 경로가 하나 더 닫힌다. **★ 함정 — systemd `Environment=` 에 값을 직접 적으면 `systemctl show` 로 드러나므로 `EnvironmentFile` 이어야 한다.** **★ 작업 AI 는 이 변경을 완결할 수 없다 — 배포 호스트에서 `sudo` 가 비밀번호를 요구한다**(2026-09-06 확인). 오너가 직접 돌린다. **처방 ② 회전 — 저장 방식과 무관한 별개 문제**다(값이 서버 밖으로 나갔는지). **회전만 하고 인자를 그대로 두면 새 값이 같은 자리에 놓인다 — ①이 먼저다.** | 원격 점검 |
