...
- 정본 SoT는 `docs/system-contract-sot.md`이며 현재 **v1.6.68**(Approved). SoT가 정본 우선순위이고, 미확정 항목은 추측 구현하지 않는다.
...
- 구현된 계층(모두 회귀로 잠김, 아래는 현재 동작하는 표면):
  ...
  - **Writing AI(Phase 5.1~5.4, v1.6.68~71)**: 평문 candidate 생성→별도 typed report extractor→Writing Gate→accept/save→pending Analysis job. report는 Gate JSON 입력과 AnalysisJob advisory copy로 소비되며 snapshot extraction 정본과 direct-mint 금지를 유지한다. full pointer/report API/separate report entity·부분 revise는 후속.
...
- **테스트**: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **873 passed / 48 skipped / 146 subtests**(2026-07-12, v1.6.71 structured candidate report + 검증 후속 보강 B1/B2/H3). skip은 대부분 live Mongo/Chroma/embedding 미가용 통합 테스트.
...
- **Phase 5.4 structured report 완료(v1.6.71)**: D1~D6 구현. D2 B(full pointer)·D3 B(별도 report API)·D6 C(별도 report entity)는 확정 후속. agent-loop 종료 `self_report`와 별도 데이터 계약.
...
1. **Phase 5.2 Writing Gate 구현 완료(v1.6.69)** — 별도 LLM Gate service/prompt/models + `POST /projects/{id}/writing/gate`, 구조화 findings와 다섯 decision/우선순위, project isolation·strict JSON/provider error mapping을 회귀로 잠갔다. 다음 핵심 후보:
   ...
   - **구조적 candidate report 완료(v1.6.71)** — 후속 확정: full pointer schema, 별도 report 재평가 API, candidate persistence 후 report entity 승격.
...
- 현재 시스템 전체 스위트: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **824 passed / 48 skipped / 101 subtests**. `git diff --check` clean.
