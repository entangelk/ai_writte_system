...
정본은 [`docs/system-contract-sot.md`](docs/system-contract-sot.md) **v1.8.56**(Approved). **미확정 항목은 추측 구현하지 않는다.** 아래는 코드를 만지기 전에 알아야 하는 요약이다.
...
## 계정 탈퇴 Slice 4(화면) — 조건 폐쇄됨, 승격 재검 대기 (2026-09-11)

기록 [`withdrawal_slice4_screen.md`](docs/verifications/2026-09-11/withdrawal_slice4_screen.md) **조건부 합격 — 조건 둘은 같은 날 세션 57 이 닫았다**(`b52633d` · SoT **v1.8.55** · 프로덕션 코드 무변, 셀 11→13 · 기록 §폐쇄 보고). **판정은 아직 조건부 합격이다** — 조건을 닫은 세션은 자기 판정을 못 올리므로 **승격 재검(변이 재적용)이 다음 검증 세션 몫**으로 남아 있다. 무엇이 왜 열려 있었는지:
...
- 폐쇄 순서는 Slice 3 선례 그대로: ~~조건 폐쇄(셀 둘, 구현 세션)~~ **완료(2026-09-11)** → **승격 재검(변이 재적용, 다음 검증 세션)** 이 남았다. 재적용 목록은 **MV-2·2b·7 + 신규 2d·11·12**(기록 §폐쇄 보고의 표).
...
>
> **★★ 계정 탈퇴 Slice 4(화면)의 검증 조건 둘은 2026-09-11 에 닫혔다(위 절, `b52633d`).** 남은 것은 **승격 재검**이고 그것은 **검증 세션 몫**이다.
>
> **★★ 랜딩 축 ②(`/terms`·`/privacy` + 푸터)는 2026-09-11 에 닫혔다**(SoT v1.8.56 · `d8bae4a`·`95f2592`). **구현 세션이 다음에 집는 것은 ③ 랜딩 본문과 ④ 가입 동의 게이트**다 — 둘은 아래 표 2번의 남은 절반이고, 그 뒤가 **Slice 5(문서 승격)** 와 **Slice 4b(관리자 잔여 정리 — 브리프 선행)** 다.
>
> | 순서 | 무엇 | 왜 이 순서인가 |
> |---|---|---|
> | **1** | **Slice 4 승격 재검**(검증 세션) | ~~조건 B1·B2 폐쇄~~ **완료(2026-09-11, `b52633d` · SoT v1.8.55)**. 남은 것은 변이 재적용(**MV-2·2b·7 + 신규 2d·11·12**)으로 폐쇄를 재현하고 판정을 올리는 일 — [`withdrawal_slice4_screen.md`](docs/verifications/2026-09-11/withdrawal_slice4_screen.md) §폐쇄 보고 |
...
> | 4 | **12번 랜딩 + 10번 동의 게이트** | **한 창에서 본다.** 순서는 ~~① 약관 대괄호 넷 치환 + 시행 버전 문자열~~ **완료(2026-09-09, 버전 `1.0`)** ~~② `/terms`·`/privacy` + 푸터~~ **완료(2026-09-11, SoT v1.8.56)** → **③ 랜딩 본문 ④ 동의 게이트**가 남았다. **Phase S-2(nginx 헤더)·AdSense 화면별 제어도 같은 창** — 광고가 랜딩·로그인에 더해 **약관·방침 페이지에도** 뜬다 |
