...
4. **★ 장면 메모 후속(오너 결정 2026-09-06)** — 두 가지를 함께 연다. ① **목록 행에 수정 시각**(`updated_at` 은 payload 에 이미 있다 — 세션 21 이 계획 범위를 좁게 읽어 뺐다) ② **절단 표시("더 보기")** — `truncated` 를 화면이 읽기 시작하는 것이고, SoT v1.8.37 이 그 자리에 조건을 박아 뒀다: **true/false 양방향 셀과 함께 연다**. 전문은 단건 GET 이 주므로 "더 보기"는 그 호출이다(목록이 전문을 기대하게 만들지 말 것).
5. **★ 배포 반영 대기 — `1a0b94a`(compose restart 정책)**. 오늘 배포는 `c18a993` 까지 나갔고 그 뒤 커밋 하나가 아직 origin 밖이다. **오너가 push 하면 서버에서 `git pull --ff-only` → `docker compose -f docker-compose.yml -f docker-compose.external-embedding.yml up -d` 한 번**이면 끝난다(재빌드 불필요 — compose 변경은 컨테이너 **재생성**에만 적용된다).
6. **육안 확인(누적)** — 프론트 재빌드 선행. Phase 10 마감 다섯(첫 화면 콘텐츠 · 오른쪽 끝 정렬 · 목록 행 간격 · 차트 막대 테두리 · 버튼 hover) · 비활성 버튼 색 · 편집기 드로어와 설정 탭이 좁은 화면에서 겹치는지 · 관측 화면과 비동기 패드 렌더 · **최종 저장·분석 연동의 첫 실사용**(운영 이력 0건).
