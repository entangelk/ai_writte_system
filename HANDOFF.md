...
2. **★ 다음 슬라이스 = R-a/R-c 측정·결정**(오너 2026-07-30 결정 완료, 결정 대기 아님). 구현 순서는 **측정 → 선택 → 구현**이다. 지금 상태: 회계 비율 **1.07** · 입력 예산 **8192** · 창 초과는 가드가 400 · 빠듯한 여유는 KPI 경고.
   ...
   - **① 먼저 R-c를 측정한다 — 오너 지시대로 알파에서 한다**(창이 이 repo의 compose에 있으므로 오너 손을 빌리지 않아도 된다). `docker-compose.llama.yml`의 `--ctx-size ${LLAMA_CTX_SIZE:-8192}`이므로 **`.env`에 `LLAMA_CTX_SIZE=32768` 한 줄**이면 창이 바뀐다(`.env.example`에 값 선택 근거와 함께 주석으로 있다). **착수 전 체크리스트 — 순서대로 하지 않으면 시간을 잃는다**: **ⓐ 이미지를 먼저 올린다**(`docker compose build application gateway frontend`) — 알파 이미지는 2026-07-28 전후라 **오늘 것(R-e·K-3·K-1)이 전혀 없다**. **ⓑ `-hf` 재다운로드 함정**(추적 부채): 모델이 캐시에 온전히 있어도 upstream `refs/main`이 움직여 **6.5 GB를 다시 받는다**. 회피는 함정 절의 `-m` 스냅샷 경로 직접 지정이다 — 기동이 멈춘 것처럼 보이면 이것부터 의심한다. **ⓒ 오래된 볼륨 함정**: 알파 볼륨은 2026-07-28에 내린 그대로다. 구 `analysis_extract` 본문이 남아 있으면 `PromptTemplateConflict`로 app이 뜨지 않는다(베타에서 실제로 3일 잃었다). **ⓓ 재현 데이터**: 지금 어느 머신에도 **예산을 꽉 채우는 프로젝트가 없다**(베타 프로브는 회계 2,876). 초과를 보려면 큰 원고를 넣거나 report 예산을 임시로 낮춰 경계를 앞으로 당긴다. 창 32768은 **알파에서 이미 뜬다**(2026-07-28 실측 `n_ctx_slot=32768`, VRAM 9,774/12,288). 확인할 것은 그 창에서 **만재 예산 + `long`이 통과하는지**이고 산술상 여유는 **+21,976 tok**이다. **알파에서는** `docker-compose.llama.yml`의 `LLAMA_CTX_SIZE`만 바꾸면 되고, **베타에서는 외부 서버 창을 오너가 올려야 한다**(그 서버는 오너 관리 — 어제 8192→16384도 오너가 했다). 측정은 `scripts/gateway_generate_live_smoke.py`가 창을 응답에서 읽어 자기 교정하므로 그대로 쓸 수 있다.
