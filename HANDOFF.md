...
- 정본 SoT는 `docs/system-contract-sot.md`이며 현재 **v1.6.77**(Approved). SoT가 정본 우선순위이고, 미확정 항목은 추측 구현하지 않는다.
...
- 구현된 계층(모두 회귀로 잠김, 아래는 현재 동작하는 표면):
  ...
  - **Writing AI(Phase 5.1~5.9, v1.6.68~77)**: 평문 candidate 생성→typed report→Writing Gate→accept/save→pending Analysis job. inline report/revise와 `/writing/revise-and-gate` bounded 합성까지 구현. 합성은 단일 continuity finding auto-revise와 targeted retrieval을 설정 가능한 구조적 상한(기본 revision 총2·retrieval1·Gate3) 안에서 순서와 무관하게 이어간다. revise 뒤 report→Gate, retrieve 뒤 merge→Gate만 재계산하며 사람 decision/multi-finding은 자동화하지 않는다. 응답은 최소 `loop`/`stages`, partial 실패는 마지막 candidate/Gate와 failed stage를 보존한다. aggregate token/time·전체 artifact·persisted loop 감사는 후속.
...
- **테스트**: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **928 passed / 45 skipped / 209 subtests**(2026-07-13, v1.6.77 bounded Writing loop 독립 PASS 후 H1/H2 hardening; 이 머신은 ES Python dependency가 있어 종전 skip 3개가 실행됨). skip은 대부분 live Mongo/Chroma/embedding 미가용 통합 테스트.
...
- **Phase 5.9 G8 bounded loop 완료(v1.6.77)**: L1~L9 승인·구현. 상한은 `WritingLoopPolicy`와 `WRITING_LOOP_MAX_REVISION_ROUNDS|MAX_RETRIEVAL_ROUNDS|MAX_GATE_EVALUATIONS`(Compose 노출)로 조정 가능하다. L6=A first→C·L9=A first→B에 따라 전체 중간 artifact와 persisted loop 감사는 additive 후속, aggregate token/time은 usage 계측 B2 후속이다.
...
1. **Phase 5.2 Writing Gate 구현 완료(v1.6.69)** — 별도 LLM Gate service/prompt/models + `POST /projects/{id}/writing/gate`, 구조화 findings와 다섯 decision/우선순위, project isolation·strict JSON/provider error mapping을 회귀로 잠갔다. 다음 핵심 후보:
   ...
   - **bounded revise/retrieve loop 완료(v1.6.71~77)** — 다음 Writing 후보는 B2 provider usage/latency/search 계측→aggregate token/time 기본값 브리프 또는 persisted loop 감사 API. 이후 multi-finding, stable pointer.
