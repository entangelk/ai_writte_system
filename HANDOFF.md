...
>
> 마지막 자가 검수: 2026-07-31 · 216줄 (R-a 측정 슬라이스를 **교체로** 반영했다: Next Tasks 2번의 낡은 다섯 항목(외삽 −1,914 · 산식 후보 6,000 · "재현 데이터가 없다" · 두 갈래 (i)/(ii))을 지우고 실측값·세 갈래·알파 1회 절차로 다시 썼고, 베타 관측치도 오늘 상태로 갈아치웠다(재부팅·복구·시드 프로젝트 id·docker 첫 응답 지연). 줄 수는 216 → 216(교체 위주 — Next Tasks 2번은 5줄 → 4줄로 줄었고, 대신 오너 결정 대기 항목이 한 줄 늘었다)(`wc -l` 실측 — 07-30 기록의 218은 그때 값이다) — 완료 서술은 `daily_logs/2026-07-31/`와 브리프 §2-5에 있다. 직전 검수(2026-07-30, 218줄)도 같은 방식이었고 그 기록은 그날 work_log에 있다. 다음 검수 트리거는 300줄)
...
- **★ R-a의 형태 — 측정은 끝났고 이것만 남았다**(2026-07-31). 브리프 [`context-budget-korean-tokens-decisions.md`](docs/plans/context-budget-korean-tokens-decisions.md) **§2-5-1**에 선택지 표와 근거가 있다: **(i) 상수 ≈ 5,000** · **(ii) 창에서 유도** · **(iii) 후보 길이에서 유도**. **구현자 추천은 (ii)+(iii) 한 슬라이스**다 — 창이 다른 세 머신을 옮겨 다니는 프로젝트에서 (i)은 머신-로컬 값을 코드에 박는 것이고, (ii)가 들어가면 **R-c가 같은 결정에 흡수된다**(창을 키우면 예산이 자동으로 넓어진다). (iii) 단독은 실측으로 탈락했다(medium 후보로도 8192는 −784로 거부). **급한 임시안이 필요 없는 이유**: 가드가 이미 400으로 막고 있어 조용히 깨지지 않는다.
...
- **컨텍스트 예산 트랙 = K-1(c) 보류 확정 · R-a 채택까지 결정됨**(오너 2026-07-30). **K-1(c)(색인 시점 정확 계수)는 만들지 않는다** ((c)의 하중 근거가 K-3 가드로 소멸했고, 회계 비율 실측 1.07, (c)는 색인에 LLM 의존을 새로 만든다). **R-a는 채택됐고 R-c도 측정 후 비교**하기로 했다 — 그 측정은 2026-07-31에 끝났고(브리프 §2-5), **남은 것은 위 "Owner Decisions Needed"의 R-a 형태 결정 하나와 알파 R-c 1회**다.
...
2. **★ R-a는 측정이 끝났다 — 남은 것은 오너 결정 1건과 알파 R-c 1회**(2026-07-31 베타 실측, 브리프 §2-5 · `daily_logs/2026-07-31/`). **재현 데이터 없음(종전 ⓓ)은 해소됐다** — [`scripts/report_budget_measure.py`](scripts/report_budget_measure.py) `--seed`가 예산을 포화시키는 프로젝트를 그 자리에서 만든다.
   - **실측(창 16384, 후보 = `long` 상한)**: 고정 오버헤드 = system 465 + 후보 산문 4,159 + 래퍼 94 = **4,718**. 현행 예산 **8192 → 입력 13,076 + 상한 6,144 = 19,220(−2,836) REJECT**, 통과 최대는 **5120(+386)**. R-a 산식 실측 = 실제 5,522 → **회계 약 5,407**(어제의 후보 6,000보다 작다). **가드와 delta 0**으로 관통 확인했고(8192는 400 거부, 5120은 실제 통과) — **이 경계를 실제로 본 것은 이번이 처음**이다(종전 −1,914는 외삽). **독립 검증 합격**(`verifications/2026-07-31/r_a_budget_measure_league.md`, 차단 0) 뒤 보강 3건이 들어갔다 — 그중 하나는 **권장치가 `--budgets` 목록에 따라 흔들리던 것**(5,330 vs 5,381)이며, 이제 만재에 가장 가까운 행에서 뽑아 **5,407 하나**로 나온다. **스크립트를 돌려 다른 숫자가 나오면 그것이 신호다**(문서 숫자를 고치기 전에 왜 갈렸는지부터 본다).
   - **오너 결정 대기 = R-a의 형태와 숫자.** 실측이 갈래를 셋으로 늘렸다: **(i) 상수 ≈ 5,000**(즉시 가능, 창이 다른 배포에서 틀림) · **(ii) 창에서 유도**(정확, 게이트웨이의 창을 앱까지 끌어오는 배선 하나) · **(iii) 출력 프리셋별**(신설 — 고정 오버헤드의 **88%가 후보 산문**이라 초과는 `long`에서만 생긴다. 앱은 창은 모르지만 **프리셋은 안다**. 상수 예산은 short/medium을 불필요하게 굶긴다). (ii)+(iii) 결합 가능.
   - **알파 R-c는 명령 하나다**: `.env`에 `LLAMA_CTX_SIZE=32768` → 같은 스크립트를 돌리면 표가 그 창으로 다시 그려진다(창을 `/props`에서 읽는다). 산술상 같은 입력이면 **+13,548**이라 현행 8192가 그대로 통과할 것으로 보이지만 **확인은 알파에서 한다**. **착수 전 체크리스트**: **ⓐ 이미지를 먼저 올린다**(알파 이미지는 07-28 전후라 R-e·K-3·K-1이 전혀 없다) · **ⓑ `-hf` 재다운로드 함정**(캐시가 온전해도 6.5 GB를 다시 받는다 — 기동이 멈춘 것처럼 보이면 이것부터 의심, 회피는 함정 절의 `-m` 스냅샷 경로) · **ⓒ 오래된 볼륨 — 알파에서는 증상이 다를 수 있다**: 충돌은 **같은 `(task_type, version)`인데 본문이 다를 때만** 난다([`prompt_templates.py:118-121`](services/application/app/analysis/prompt_templates.py#L118-L121)). 베타에서 본 `analysis_extract_v3`(sha `fb4e272…`)는 **베타 사실이지 알파 사실이 아니다**. 증상은 조용하지 않다 — app·worker가 `PromptTemplateConflict`로 **부팅을 실패**한다. 먼저 본다: `docker compose exec mongo mongosh --quiet ai_writing_system --eval 'db.prompt_templates.find({},{task_type:1,version:1}).forEach(t=>print(t.task_type,t.version))'` → 정본 sha는 [`tests/test_prompt_templates.py`](tests/test_prompt_templates.py)의 `_IMMUTABLE_TEMPLATE_DIGESTS`. **★ 베타의 해결책(볼륨 비우기)을 그대로 옮기지 말 것** — 알파 볼륨에 살릴 원고가 있으면 파괴적 선택이므로 오너 확인이 먼저다.
   - **한계 하나는 알고 쓴다**: 시드 코퍼스 밀도 **1.63 자/tok**은 실제 원고 **1.71**보다 5% 촘촘해 권장 예산이 실제보다 약간 작게 나온다(스크립트가 밀도를 함께 출력한다).
