...
- project/draft rename API가 추가됐다(2026-06-28). `PATCH /projects/{id}`·`PATCH /projects/{id}/drafts/{draft_id}`. 없음/cross-project 404, archived rename은 409(쓰기차단 계약). 이로써 create·list·get·rename은 API로, archive는 service-only로 제공된다(archive API endpoint는 후속). repo/SoT 계약 변경 없음.
...
3. plan 01 최소 산출물 #7(후속 Phase 재사용 fixture)은 아직 미구현(실제 소비자 Phase 2 생길 때 그 shape로 추가). project/draft CRUD(create·list·get·rename·archive)·version read·source_ref는 완료.
...
- project/draft rename API 자체 회귀(2026-06-28): 전체 199개(Mongo 미연결 27 skip), replica set 연결 시 전부 통과. 잠근 범위: rename round-trip(get 반영), 없음/cross-project draft→404, archived project/draft rename→409(쓰기차단), Mongo persisted rename(양 경로).
...
└── application/
    ...
    └── app/
        ├── main.py             # FastAPI shell: health/project·draft create·list·get·rename/version save·list·read (archive는 service-only) (+Mongo wiring)
        ├── core_sot/
