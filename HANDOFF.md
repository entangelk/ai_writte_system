...
- 회귀 기준선: backend **test-mongo ON 1955 passed / 1 skipped / 1715 subtests**(베타 2026-08-03, Slice 8.1 + 검증 보강까지. 직전 알파 기준선은 1911/4/1625였고 차이는 전부 설명된다 — Slice 8.0 신규 가드 8+75, 검증 보강 4+15, 그리고 알파에서 skip되던 3건이 베타에서는 실행됐다. 남은 skip 1건은 호스트에서 구조적으로 항상 skip되는 live Chroma 셀), frontend **236 passed / 17 files**, build **698 modules · 진입 410.29 kB · AdminConsole lazy 8.39 kB · 관측 lazy 386.70 kB**. skip 수는 머신·인프라 기동 여부마다 달라 같은 환경에서 비교한다. backend는 `argon2-cffi`, frontend는 `npm install`이 선행돼야 한다.
...
- **★ Phase 8 Slice 8.2 잔여 — L6 · L7 · 범위 선택**(브리프 [`plans/08-2-usage-ledger-decisions.md`](docs/plans/08-2-usage-ledger-decisions.md) §0.5·L6·L7, 2026-08-03). **L6 프로젝트 이름 이력을 어디에**(추천: 전용 컬렉션 — 자체 보존 정책을 붙일 자리가 생긴다) · **L7 5초 가드의 DB 잠금**(추천: 만료를 가진 잠금 문서 + 원자적 차지. **TTL은 판정이 아니라 청소에만** — Mongo TTL은 약 60초 주기라 판정을 맡기면 5초가 아니라 최대 1분 잠긴다) · **범위**(가: 8.2에 전부[추천] / 나: 8.2b 분리 / 다: L7만 8.3으로). **L6은 D8-6 계약 개정 + purge UI 문구 수정을 부른다** — 지금 문구 "원고·기억·감사·색인 전체가 삭제되며 복구할 수 없습니다"가 이름이 남는 순간 부분적으로 거짓이 되고, 나로 분리하면 그 거짓 문구가 배포된 중간 상태가 생긴다.
...
- **Phase 8 Slice 8.2 = L1~L5 확정 / L6·L7 결정 대기**(오너 2026-08-03, 브리프 [`plans/08-2-usage-ledger-decisions.md`](docs/plans/08-2-usage-ledger-decisions.md)). 원장은 **`(user_id, action, target_project_id, 창 키 2개, at, dedupe_key)`** 행이고 관리자 조정은 **같은 컬렉션의 다른 종류**(`kind`, 필드 구성이 겹치지 않는다)다. 집계 정본은 **행 count**(카운터 캐시 없음), **TTL 없음**. **★ 필드명은 반드시 `target_project_id`다** — `project_id`로 적으면 purge reconciler가 그 필드를 가진 컬렉션을 DB에서 발견해 **과금 기록을 지운다**. 오너 결정이 "프로젝트가 삭제돼도 사용 기록은 남는다"이므로 이름 하나가 결정의 성패를 가른다. **삭제된 프로젝트는 id 수준으로만 답해진다** — purge가 이름을 안 남기므로 원장도 이름을 스냅샷하지 않는다(그러면 삭제 계약을 우회한다). **중복 방지 키에 `action`이 들어간다** — 프론트가 한 흐름에서 uuid를 공유하므로 그것 없이는 유료 동작 4개가 1개로 접힌다. **5초 오·중복 가드**(프로젝트별·확인 필수·locking)는 오너 요구다. **★ 주의 — 초판 브리프가 "유니크 인덱스가 곧 locking"이라 적은 것은 오류이며 정정됐다**: 그 인덱스는 dedupe 키가 같을 때만 물고 프론트는 **클릭마다 새 uuid**를 만들므로 실수 중복은 그대로 통과한다. **별도 잠금이 필요하다**(L7). 그리고 오너가 **프로젝트 이름·제목을 히스토리로 보존**하기로 해(삭제 요청이 지우지 않는다) **L6**이 신설됐다 — 이것은 **D8-6 삭제 계약 개정**과 **purge UI 문구 수정**을 부른다(현재 "전체가 삭제되며 복구할 수 없습니다"가 부분적으로 거짓이 된다). **L6·L7과 범위(8.2에 전부 / 8.2b 분리 / L7은 8.3으로)가 오너 결정 대기**다.
...
1. **Phase 8 Slice 8.2 — L6·L7·범위 결정 대기**(위 Owner Decisions Needed). L1~L5는 닫혔다. 셋이 닫히면 브리프 "결정 뒤 구현 슬라이스"대로: ① 계약을 **양방향 회귀로 먼저** 잠근다 — 같은 dedupe 키 재삽입 · **다른 `action`은 같은 `request_id`라도 각각 센다** · 회원/창 격리 · **`target_project_id`라는 이름 자체를 단정**(`project_id`로 적으면 purge reconciler가 과금 기록을 지운다) · 조정 행의 부호와 필드 구성 분리 · 창 키를 8.1에서 가져오기 · **잠금의 원자성**(동시 두 요청 중 하나만 차지) · **만료된 잠금은 다시 차지된다**(TTL 주기와 무관) ② 도메인 + Mongo 어댑터(인덱스: 유니크 `(user_id, action, dedupe_key)` · 집계 `(user_id, daily_key)`·`(user_id, weekly_key)` · 최근성 `(user_id, action, target_project_id, at)`, fake-collection naive/aware 셀 필수) ③ `mongo_collections.md` 등재 ④ **L6이면 D8-6 계약 개정 + purge UI 문구 수정을 같은 슬라이스에서** ⑤ 8.3 시행 브리프.
