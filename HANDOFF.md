...
2. Phase 2A run endpoint API contract는 완료됐다. 실제 provider/Gateway runner factory wiring은 Gateway JSON output/prompt 계약과 source_ref 생성 boundary가 확정된 뒤 구현한다.
3. Gateway `/v1/generate` runtime wiring은 model tool-call wire format/payload 확정 의존으로 현재 구현하지 않는다.
