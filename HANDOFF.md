...
2. **★ 컨텍스트 예산 트랙 — 다음은 밀도(K-1)다.** 오너가 정한 순서(회계 → R-e → 가드 → 밀도)에서 **가드까지 들어갔다**(SoT v1.7.62). 남은 것은 K-1이고, R-a(report 전용 예산)는 R-e·가드로 필요가 사라진 쪽이라 폐기 판단만 남았다.
   ...
   - **경고는 저장되지 않는다 — KPI가 읽기 시점에 파생한다**(v1.7.59 준수). `thin_headroom_calls`(여유 < 창의 10%)와 **분모 `headroom_considered`**가 per-site·전역 payload에 있고 관측 화면이 그린다. **분모가 0이면 숫자가 아니라 "측정되지 않음"으로 읽어야 한다** — 창을 아는 호출이 없었다는 뜻이며, 경고 0건과 다르다.
