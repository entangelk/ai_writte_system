...
- Gemma Q4 benchmark harness가 추가됐다(2026-06-28). `scripts/benchmark_llm_provider.py`가 외부 llama.cpp-compatible endpoint를 대상으로 short smoke, JSON extraction, scene continuation case를 반복 실행하고 latency/token/error summary JSON을 출력한다. 실제 endpoint benchmark와 budget/retry production 기본값 확정은 아직 남아 있다.
...
- Gemma Q4 benchmark harness 자체 회귀(2026-06-28): `python3 -m unittest tests.test_llm_benchmark_script -v` 5개 통과, `python3 -m py_compile scripts/benchmark_llm_provider.py tests/test_llm_benchmark_script.py` 통과. 잠근 범위: success latency/token/tokens-per-second 기록, provider error literal/retryable 기록, invalid repeats/warmups 거절, per-case summary/report shape.
