...
- **Phase 8 Slice 8.2 = L1=B · L2~L5=A 확정, 구현 대기**(오너 2026-08-03, 브리프 [`plans/08-2-usage-ledger-decisions.md`](docs/plans/08-2-usage-ledger-decisions.md)). 원장은 **`(user_id, action, target_project_id, 창 키 2개, at, dedupe_key)`** 행이고 관리자 조정은 **같은 컬렉션의 다른 종류**(`kind`, 필드 구성이 겹치지 않는다)다. 집계 정본은 **행 count**(카운터 캐시 없음), **TTL 없음**. **★ 필드명은 반드시 `target_project_id`다** — `project_id`로 적으면 purge reconciler가 그 필드를 가진 컬렉션을 DB에서 발견해 **과금 기록을 지운다**. 오너 결정이 "프로젝트가 삭제돼도 사용 기록은 남는다"이므로 이름 하나가 결정의 성패를 가른다. **삭제된 프로젝트는 id 수준으로만 답해진다** — purge가 이름을 안 남기므로 원장도 이름을 스냅샷하지 않는다(그러면 삭제 계약을 우회한다). **중복 방지 키에 `action`이 들어간다** — 프론트가 한 흐름에서 uuid를 공유하므로 그것 없이는 유료 동작 4개가 1개로 접힌다. **5초 오·중복 가드**(프로젝트별·확인 필수·locking)는 오너 요구이며 **8.2는 최근성 질의만 지원**하고 응답 계약은 8.3, 묻는 UX는 8.4다. 유니크 인덱스가 곧 locking이라 애플리케이션 락은 안 들인다.
...
1. **Phase 8 Slice 8.2 구현**(결정 완료, 위 항목 참조). 순서는 브리프 "결정 뒤 구현 슬라이스"대로: ① 계약을 **양방향 회귀로 먼저** 잠근다 — 같은 dedupe 키 재삽입 · **다른 `action`은 같은 `request_id`라도 각각 센다** · 회원/창 격리 · **`target_project_id`라는 이름 자체를 단정**(D8-6 tombstone 셀과 같은 성격) · 조정 행의 부호와 필드 구성 분리 · 창 키를 8.1에서 가져오기 · 프로젝트별 최근성 질의 ② 도메인 + Mongo 어댑터(인덱스 3종: 유니크 `(user_id, action, dedupe_key)` · 집계 `(user_id, daily_key)`·`(user_id, weekly_key)` · 최근성 `(user_id, action, target_project_id, at)`, fake-collection naive/aware 셀 필수) ③ [`docs/mongo_collections.md`](docs/mongo_collections.md) §43D 등재 ④ 8.3 시행 브리프. **차감·차단은 8.3이다.**
