...
| # | 항목 | 왜 이 순서인가 | 자리 |
|---|---|---|---|
| **S-0** | **즉시 — 공개 저장소에 서 있는 노출** ① 확인용 계정 `timeline_demo` 의 **평문 비밀번호가 기록 3곳에 남아 있다**(값은 여기 옮기지 않는다) ② **사설 LAN 주소가 49개 파일**에 있다 | **이 문서 머리의 "공개 저장소 보안 규칙" 을 저장소가 스스로 어기고 있다.** ★ 파일에서 지워도 **git 이력에는 남는다**(이력 재작성은 2026-08-23 에 이미 한 번 했고 비싸다) — **실효 처방은 계정 비활성화**이고, 문서 스윕은 재유입 방지다 | 감사 §B.1~B.4 |
| **S-1** | **공개(다중 사용자) 전 필수 — quota 우회 체인** ① **[HIGH]** 클라이언트가 보내는 `body.request_id` 가 원장 dedupe 키다 — 같은 값을 반복 전송하면 LLM 이 실제로 돌면서 **과금은 1행으로 접힌다**(`X-Confirm-Duplicate` 로 5초 잠금도 덮인다) ② 실패 job **retry 에 횟수·쿨다운이 없다**(analysis·generation 둘 다) ③ identity judging 이 페어당 판정을 **무제한 팬아웃** | **지금 막혀 있는 이유는 코드가 아니라 승인제 가입**이다 — 공격에 오너 승인이 선행한다. **랜딩·약관이 끝나 가입을 열면 그 방벽이 사라지고, 한 회원이 LLM 예산을 무제한 소진할 수 있다.** 그래서 "공개 직결" 항목과 **같은 트리거**를 공유한다 | 감사 §A.1~A.4 |
...
| **S-4** | **재현 가능 빌드 비대칭**(사후 비평). 프론트는 `package-lock.json` + `npm ci` 인데 **파이썬은 범위 핀뿐이고 lockfile·constraints 가 없다**(실측 부재). 베이스 이미지도 `python:3.12-slim`·`nginx:1.27-alpine` 등 **플로팅 태그** | 재빌드가 **조용히 다른 패키지를 실어 나를 수 있고**, 지금 실제로 설치된 버전을 원천에서 증명할 방법이 없다. 공급망 사고 때 *"우리가 무엇을 깔았나"* 에 답할 수 없다는 것이 실질 비용 | 감사 §C |
...
> **★ 순서가 2026-09-05 에 바뀌었다 — 보안 감사(아래 Phase S)가 앞으로 왔다.** 감사 판단: **critical 0건**이라 지금 당장 무너지는 것은 없지만, **S-0 은 공개 저장소에 서 있는 노출이라 즉시**이고 **S-1 은 공개(다중 사용자) 전에 반드시 닫아야 한다** — 지금은 승인제 가입이라 공격에 오너 승인이 선행하므로 막혀 있을 뿐이고, 랜딩·약관이 끝나 가입을 열면 그 방벽이 사라진다. Slice 5 검증은 **이미 병합된 코드를 다시 뜯는 일이라 미뤄도 외부 위험이 늘지 않는다.**

0. **★ Phase S — 보안 감사 후속**(기록 [`security_audit_dual_workflow.md`](docs/verifications/2026-09-05/security_audit_dual_workflow.md), 확정 39건 · critical 0 · HIGH 1). 아래 "열린 것 — 보안" 표가 항목별 정본이다. **S-0(즉시) → S-1(공개 전 필수) → 나머지는 트리거.**
