...
>
> 마지막 자가 검수: 2026-07-30 · 218줄 (컨텍스트 예산 트랙 4슬라이스(R-e·K-3 가드·K-3 경고·K-1)를 반영하면서 **추가가 아니라 교체**로 처리했다: 수리된 ★ 부채 2건(`long` report 초과 · 렌더링 중복)을 다시 썼고, Next Tasks 2번과 Owner Decisions의 예산 항목을 슬라이스마다 갈아치웠다. 새로 들어온 것은 **다음 작업자의 오늘 행동을 바꾸는 것들**뿐이다 — 가드의 의도된 공백(오너 결정 대기) · 뮤테이션 원복에 `git checkout --`을 쓰면 미커밋 작업이 날아간다는 함정 · 예산을 바꿀 때 프론트가 값을 명시 전송한다는 함정 · 이미지가 코드보다 뒤처진 상태. 오너 결정 2건(K-1(c) 보류 확정 · R-a 기본+R-c 측정)을 받아 Owner Decisions에서 **덜어내고** Next Tasks 2번을 착수 절차로 다시 썼다 — 결정 대기 항목이 하나 줄었다. 다음 검수 트리거는 300줄)
...
- 정본은 `docs/system-contract-sot.md` **v1.7.62**(Approved). 미확정 항목은 추측 구현하지 않는다.
...
- **[베타 머신 관측치, 2026-07-30 작업 종료 시점]** 전체 스택이 **떠 있다**(healthy 7 + 워커 2). **★ 이미지가 코드보다 뒤처져 있다 — 화면으로 확인하기 전에 반드시 본다**: `gateway`는 14:52 빌드로 **K-3 가드까지 들어 있고**(그 뒤 게이트웨이 코드는 안 바뀌었다), `application`은 **2026-07-29 15:48 빌드**라 **K-3 앱 절반(400 매핑·job 사유)·KPI 경고·K-1(환산·예산 8192)이 이미지에 없다**, `frontend`는 **2026-07-27 빌드**라 **KPI 경고 타일과 `MAX_TOKENS=8192`가 없다**. 즉 지금 브라우저로 보는 것은 **오늘 오전 이전의 제품**이다. 확인하려면 `docker compose build application frontend && docker compose up -d --no-deps application frontend`. 오늘의 라이브 검증은 전부 **작업 트리를 마운트**해 돌렸다(`docker compose run --rm --no-deps -v "$PWD/services:/app/services" …`) — 재빌드 없이 실 파이프라인을 관통하는 그 방법이 표준이다. 외부 12B(`.env`의 `192.168.1.22:9080`)는 `/props` 실측 **`n_ctx=16384` · `total_slots=1`**. GPU는 **GTX 1060 3GB**. DB에는 프로브가 만든 **project 9개**와 `llm_call_audits`가 있다(일부러 남겼다 — 육안 확인용). 계정 `probe`(admin). Mongo `prompt_templates`는 **`analysis_extract` v1~v4 4행뿐**이다(report·gate·생성 템플릿은 in-memory seed).
...
- **컨텍스트 예산 트랙 = 전부 결정됨**(오너 2026-07-30). **K-1(c)(색인 시점 정확 계수)는 보류 확정** — 만들지 않는다((c)의 하중 근거가 K-3 가드로 소멸했고, 회계 비율 실측 1.07, (c)는 색인에 LLM 의존을 새로 만든다). **R-a(report 전용 예산)는 기본으로 채택**하되 **R-c(창 확대)도 측정한 뒤 최종 결정**한다 — 오너 문장: "리포트는 전용 예산이 기본이되 창을 늘리는 방안도 테스트해보고 결정한다." 착수 절차와 산식 후보는 **Next Tasks 2번**에 있다.
...
2. **★ 다음 슬라이스 = R-a/R-c 측정·결정**(오너 2026-07-30 결정 완료, 결정 대기 아님). 구현 순서는 **측정 → 선택 → 구현**이다. 지금 상태: 회계 비율 **1.07** · 입력 예산 **8192** · 창 초과는 가드가 400 · 빠듯한 여유는 KPI 경고.
   - **고칠 대상은 이 초과 하나다**: 예산을 꽉 채운 프로젝트의 `long` report 호출이 `항목 7,656 + system 465 + 후보 산문 4,033 + 출력 상한 6,144 = 18,298 > 16,384`(**−1,914**). **장애가 아니라 품질/여유 문제다** — 가드가 400으로 거부하므로 조용히 깨지지 않는다. 지금 베타 프로젝트는 회계 2,876이라 닿지 않으니 **재현에는 예산을 꽉 채우는 큰 프로젝트가 필요하다**(또는 report 예산을 임시로 낮춰 경계를 앞으로 당긴다).
   - **① 먼저 R-c를 측정한다 — 오너 지시대로 알파에서 한다**(창이 이 repo의 compose에 있으므로 오너 손을 빌리지 않아도 된다). `docker-compose.llama.yml`의 `--ctx-size ${LLAMA_CTX_SIZE:-8192}`이므로 **`.env`에 `LLAMA_CTX_SIZE=32768` 한 줄**이면 창이 바뀐다(`.env.example`에 값 선택 근거와 함께 주석으로 있다). **착수 전 체크리스트 — 순서대로 하지 않으면 시간을 잃는다**: **ⓐ 이미지를 먼저 올린다**(`docker compose build application gateway frontend`) — 알파 이미지는 2026-07-28 전후라 **오늘 것(R-e·K-3·K-1)이 전혀 없다**. **ⓑ `-hf` 재다운로드 함정**(추적 부채): 모델이 캐시에 온전히 있어도 upstream `refs/main`이 움직여 **6.5 GB를 다시 받는다**. 회피는 함정 절의 `-m` 스냅샷 경로 직접 지정이다 — 기동이 멈춘 것처럼 보이면 이것부터 의심한다. **ⓒ 오래된 볼륨 함정 — 알파에서는 증상이 다를 수 있다**: 알파 볼륨은 2026-07-28에 내린 그대로이고 **그 안에 어느 버전이 남아 있는지는 아무도 모른다**. **베타에서 관측된 것(`analysis_extract_v3`, sha `fb4e272…`)은 베타 사실이지 알파 사실이 아니다** — 충돌은 **같은 `(task_type, version)`인데 본문이 다를 때만** 나므로([`prompt_templates.py:118-121`](services/application/app/analysis/prompt_templates.py#L118-L121)), 알파에서는 ⑴ 다른 버전에서 나거나 ⑵ 아예 안 날 수도 있다(없는 버전은 그냥 새로 seed된다). **증상은 조용하지 않다** — app·worker·generation_worker가 `PromptTemplateConflict`로 **부팅 자체를 실패**한다. **어느 (task, version)이 다른지 먼저 본다**: `docker compose exec mongo mongosh --quiet ai_writing_system --eval 'db.prompt_templates.find({},{task_type:1,version:1}).forEach(t=>print(t.task_type,t.version))'` → repo의 정본 sha는 [`tests/test_prompt_templates.py`](tests/test_prompt_templates.py)의 `_IMMUTABLE_TEMPLATE_DIGESTS`에 있다(v1~v4 전부). **★ 베타의 해결책을 그대로 옮기지 말 것**: 베타는 **오너 판단으로 데이터 볼륨을 비웠다**(그 머신 데이터가 프로브용이라 버려도 됐다). **알파 볼륨에 살릴 원고가 있으면 비우는 것은 파괴적 선택**이므로 먼저 오너에게 확인한다. **ⓓ 재현 데이터**: 지금 어느 머신에도 **예산을 꽉 채우는 프로젝트가 없다**(베타 프로브는 회계 2,876). 초과를 보려면 큰 원고를 넣거나 report 예산을 임시로 낮춰 경계를 앞으로 당긴다. 창 32768은 **알파에서 이미 뜬다**(2026-07-28 실측 `n_ctx_slot=32768`, VRAM 9,774/12,288). 확인할 것은 그 창에서 **만재 예산 + `long`이 통과하는지**이고 산술상 여유는 **+21,976 tok**이다. **알파에서는** `docker-compose.llama.yml`의 `LLAMA_CTX_SIZE`만 바꾸면 되고, **베타에서는 외부 서버 창을 오너가 올려야 한다**(그 서버는 오너 관리 — 어제 8192→16384도 오너가 했다). 측정은 `scripts/gateway_generate_live_smoke.py`가 창을 응답에서 읽어 자기 교정하므로 그대로 쓸 수 있다.
   - **② R-a 산식 후보**: `창 − 출력상한 − system − 후보 산문 − 래퍼` = `16,384 − 6,144 − 465 − 4,033 − 약 150` ≈ **5,592 tok(실제)** → 회계 단위 **약 6,000**(현행 8192에서 약 2,200 축소). **결정이 아니라 출발점이다.**
   - **★ ③ 설계상 걸리는 것 — 앱은 창 크기를 모른다**(전수 확인). 창을 아는 곳은 게이트웨이 provider의 `/props` 캐시뿐이고 **패키지를 조립하는 앱 경로에는 없다**. 그래서 R-a는 두 갈래다: **(i) 상수 report 예산**(즉시 가능, 창이 다른 배포에서는 틀린 값) · **(ii) 창에서 유도**(정확하지만 게이트웨이의 창 값을 앱까지 끌어오는 배선이 하나 필요 — K-3이 이미 그 값을 감사에 남기므로 "아는 값을 옮기는" 일이다). **어느 쪽이 맞는지가 ①의 결과에 달려 있다** — 창을 32768로 올릴 수 있으면 (i)조차 필요 없을 수 있다.
   - **R-a를 고르면 대가가 있다**: report가 보는 근거가 줄어든다(브리프 §2-1의 R-a 단점). R-c는 근거를 줄이지 않지만 **창을 통제할 수 있는 배포에서만** 성립한다. 그래서 오너가 둘을 같은 실측 위에서 비교하라고 했다.
