...
>
> **착수 가능한 순서**: **11번(계정 탈퇴 Slice 0~5 — 가장 큰 덩어리, 결정 전부 확정)**(계정 탈퇴 Slice 0~5, 가장 큰 덩어리·결정 전부 확정) → 12번(랜딩 기획) → 10번(가입 동의 게이트, 오너 검토 선행) → S-2(nginx 헤더). 그 밖은 오너 병목이거나 운영 데이터 대기다. **13·14 는 같은 날 고쳤다** — 남은 것은 14번의 재현 하나이고, 그것은 **오너가 다시 겪을 때 화면이 말하는 문구**를 받아야 진행된다.
...
14. **★ 이어쓰기 채택 — 어포던스는 고쳤고, "아무 동작 안 함"은 재현이 남았다(2026-09-07)** — 고친 것 셋: ① 버튼 `disabled` 가 `accept()` 의 **모든** 조건을 덮게 했다(종전엔 `contextRef.current === null` 을 안 봐서 **활성인데 조용히 return 하는 죽은 버튼**이 원리적으로 가능했다) ② **조용한 return 을 없앴다** — 그 자리에 닿으면 이유를 말한다 ③ **무엇이 저장되는지를 버튼 옆 상시 문구로** 올렸다(오너: *"어떤 동작이 되는지도 모르겠네"* — 종전엔 **확인 대화상자에만** 있어 누르기 전에는 어디에도 없었다). **★ 남은 것**: 오너가 본 *"아무 동작도 하지 않는다"* 의 **재현 경로를 코드에서 못 찾았다**(`contextRef` 는 성공 분기에서만 비고 그때 후보도 함께 사라진다). 위 ②로 이제 **무반응이면 화면이 이유를 말하므로**, 다시 겪으면 **그 문구를 그대로 적어 두면 된다** — 그것이 다음 단서다. 스크래치 패드의 `채택`(`ScratchRecovery.tsx`)은 **다른 버튼이고 가드가 일치한다**(결함 아님) — 오너가 본 것이 어느 쪽인지도 함께 확인할 것. **미배포**.
