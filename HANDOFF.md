...
- 정본 SoT는 `docs/system-contract-sot.md`이며 현재 **v1.6.79**(Approved). SoT가 정본 우선순위이고, 미확정 항목은 추측 구현하지 않는다.
...
- 구현된 계층(모두 회귀로 잠김, 아래는 현재 동작하는 표면):
  ...
  - **Writing AI(Phase 5.1~5.9, v1.6.68~79)**: 평문 candidate 생성→typed report→Writing Gate→accept/save→pending Analysis job. inline report/revise와 `/writing/revise-and-gate` bounded 합성까지 구현. 합성은 단일 continuity finding auto-revise와 targeted retrieval을 설정 가능한 구조적 상한(기본 revision 총2·retrieval1·Gate3) 안에서 순서와 무관하게 이어간다. revise 뒤 report→Gate, retrieve 뒤 merge→Gate만 재계산하며 사람 decision/multi-finding은 자동화하지 않는다. 응답은 최소 `loop`/`stages`+`audit_id`, partial 실패는 마지막 candidate/Gate와 failed stage를 보존한다. **persisted loop audit(v1.6.78→79 opt-in)**: loop 종료를 durable **append-only immutable** 레코드로 영속화(P1=B bodyless per-stage hash/fingerprint/pointer + final candidate text, P3=A 재시도=새 uuid, P4=A `GET .../writing/loop-audits`[요약]+`GET .../{audit_id}`[전체 trail], P5=A 상태전이·자동삭제 없음—retention은 운영과제 후속). **P2=B opt-in(v1.6.79)**: 요청 `persist_audit` 플래그(기본 null→env `WRITING_LOOP_AUDIT_DEFAULT` 기본 off)가 true일 때만 persist, loop는 trail을 항상 서버 권위로 계산하되 persist만 gate. persist는 loop critical path 밖 try/except-격리 → 쓰기 실패해도 loop 결과 반환+`audit_id=null`+`audit_error`(H3 구조적 해소). pre-loop 거부는 on이어도 미기록. side effect는 감사 쓰기 1건뿐(Core SOT/Analysis/memory 0). aggregate token/time 계측은 B2 후속.
...
- **테스트**: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **947 passed / 45 skipped / 215 subtests**(2026-07-13, v1.6.78 persisted loop audit +12 → B1/H1/H2 hardening +4 → v1.6.79 감사 opt-in +3; 이 머신은 ES Python dependency가 있어 종전 skip 3개가 실행됨). skip은 대부분 live Mongo/Chroma/embedding 미가용 통합 테스트.
...
- **Phase 5.9 L9 B persisted loop audit 완료(v1.6.78→79)**: P1=B/P3=A/P4=A/P5=A + **P2=B opt-in**(v1.6.79 재개정, `plans/05-writing-persisted-loop-audit-decisions.md`). loop 종료를 append-only immutable 레코드로 영속화하고 `GET .../writing/loop-audits`(요약)+`/{audit_id}`(전체 trail)로 읽는다. **감사는 opt-in**(`persist_audit` 플래그·env 기본 off), persist는 loop 밖 격리라 실패해도 loop 결과 보존(H3 해소). **retention(TTL/archive)은 명시된 운영과제 후속** — 오래된 run 보존, 자동 삭제 없음. 전체 중간 artifact 본문(P1=C)·aggregate token/time(B2)은 additive 후속.
...
0. **⚠️ 검증 필요 — v1.6.79 감사 opt-in delta 독립 재검증**: v1.6.78 persisted loop audit는 오너 독립 검증 조건부 합격 → B1 closure(합격)까지 완료됐으나, 그 위에 얹은 **v1.6.79 P2=A→B opt-in 재개정(`persist_audit` 플래그·env 기본 off·persist 실패 격리+`audit_error`)은 작업자 self-check만 있고 독립 검증이 없다**. 다음 작업자는 착수 전 이 delta를 `docs/verifications/2026-07-13/writing_persisted_loop_audit.md`의 boundary 재-scope("모든 종료 감사"→"persist on일 때만")·신규 회귀 3종(default-off·env default·persist-failure isolation)·`audit_error` taxonomy·B1 detail lock 유지를 기준으로 독립 검증할 것. 검증 완료 시 이 항목 제거하고 검증 기록을 재-promote.
...
1. **Phase 5.2 Writing Gate 구현 완료(v1.6.69)** — 별도 LLM Gate service/prompt/models + `POST /projects/{id}/writing/gate`, 구조화 findings와 다섯 decision/우선순위, project isolation·strict JSON/provider error mapping을 회귀로 잠갔다. 다음 핵심 후보:
   ...
   - **bounded revise/retrieve loop + persisted loop audit 완료(v1.6.71~78)** — 다음 Writing 후보는 B2 provider usage/latency/search 계측→aggregate token/time 기본값 브리프. 이후 multi-finding, stable pointer, persisted 감사 retention(TTL/archive 운영과제)·전체 중간 artifact(P1=C).
...
- 현재 시스템 전체 스위트: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **928 passed / 45 skipped / 209 subtests**. `git diff --check` clean.
...
├── system-contract-sot.md       # 정본 계약 SoT(Approved, v1.6.77)
...
└── application/app/
    ...
    └── writing/                # Phase 5.1 글 생성: models(WritingRequest/Brief/Candidate) · prompt(master+ContextPackage 포맷) · service(WritingService 1-turn Gateway 평문)
    │                            #   + gate/report/revise/retrieval/accept · revise_gate(bounded loop) · loop_audit(+mongo, v1.6.78 persisted 감사) · audit_hash(공유 fingerprint)
tests/                           # 77개 모듈(도메인별 회귀 + live/smoke skip-aware) + fixtures/core_sot.py
scripts/
