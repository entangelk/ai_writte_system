...
- **테스트**: `python3 -m pytest -q --ignore=tests/test_memory_mongo.py` → **717 passed / 48 skipped**(2026-07-10, v1.6.59 review queue +13). 종전 3 환경-의존 failed(`ConnectElasticsearchTest`, b-5 도입 — sandbox에 `elasticsearch` 패키지 부재 시 hard-fail)는 **skip guard(`@unittest.skipUnless(find_spec("elasticsearch"))`)로 해소**(2026-07-10, 테스트 전용, 계약·프로덕션 코드 무변) → 패키지 없으면 skip(45→48), 있으면 종전대로 실행. skip은 대부분 live Mongo/Chroma/embedding 미가용 통합 테스트.
...
- 현재 시스템 전체 스위트: `python3 -m pytest -q --ignore=tests/test_memory_mongo.py` → **717 passed / 48 skipped**(v1.6.59 review queue +13). `git diff --check` clean.
