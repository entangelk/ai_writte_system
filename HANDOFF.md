...
2. **★ R-a는 측정이 끝났다 — 남은 것은 오너 결정 1건과 알파 R-c 1회**(2026-07-31 베타 실측, 브리프 §2-5 · `daily_logs/2026-07-31/`). **재현 데이터 없음(종전 ⓓ)은 해소됐다** — [`scripts/report_budget_measure.py`](scripts/report_budget_measure.py) `--seed`가 예산을 포화시키는 프로젝트를 그 자리에서 만든다.
   - **실측(창 16384, 후보 = `long` 상한)**: 고정 오버헤드 = system 465 + 후보 산문 4,159 + 래퍼 94 = **4,718**. 현행 예산 **8192 → 입력 13,076 + 상한 6,144 = 19,220(−2,836) REJECT**, 통과 최대는 **5120(+386)**. R-a 산식 실측 = 실제 5,522 → **회계 약 5,330**(어제의 후보 6,000보다 작다). **가드와 delta 0**으로 관통 확인했고(8192는 400 거부, 5120은 실제 통과) — **이 경계를 실제로 본 것은 이번이 처음**이다(종전 −1,914는 외삽).
