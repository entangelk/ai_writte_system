...
| **S-7** | **★ Cloudflare 터널 토큰이 프로세스 인자에 노출돼 있다** — `cloudflared --token <값>` 형태라 **그 호스트의 아무 로컬 사용자나 `ps` 로 읽을 수 있다**(2026-09-05 점검에서 실제로 그렇게 보였다. 값은 여기 옮기지 않는다). 이 서버는 **다른 프로젝트와 공유**하므로 로컬 사용자 축이 이론적이지 않다 | 터널 토큰은 **그 터널을 실행할 권한 자체**다 — 유출되면 제3자가 같은 이름으로 터널을 띄워 트래픽을 가로챌 수 있다. **처방 둘**: ① `--token-file` 또는 systemd `EnvironmentFile`(0600)로 옮겨 인자에서 뺀다 ② **점검 과정에서 세션 로그에 남았으므로 토큰을 한 번 회전한다.** 감사 범위 밖이었고 이번 점검에서 나왔다 | 원격 점검 |
...
0. **★ Phase S — 보안 감사 후속**(기록 [`security_audit_dual_workflow.md`](docs/verifications/2026-09-05/security_audit_dual_workflow.md), 확정 39건 · critical 0 · HIGH 1). 아래 "🛡 보안 — Phase S" 표가 항목별 정본이다. **S-7 터널 토큰 회전·인자 제거(가장 싸고 가장 즉시) → S-3 signup 레이트리밋(★ 착수 전에 터널이 nginx 로 가는지 확인) → S-1 quota dedupe(가입 열기 전 필수) → S-0 문서 스윕 → 나머지는 트리거.**
