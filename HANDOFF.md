...
- 회귀 기준선: backend **test-mongo ON 1922 passed / 1 skipped / 1700 subtests**(베타 2026-08-03. 직전 알파 기준선은 1911/4/1625였고 차이는 전부 설명된다 — 신규 `test_billable_actions.py` 8+75, 그리고 알파에서 skip되던 3건이 베타에서는 실행됐다. 남은 skip 1건은 호스트에서 구조적으로 항상 skip되는 live Chroma 셀), frontend **236 passed / 17 files**, build **698 modules · 진입 410.29 kB · AdminConsole lazy 8.39 kB · 관측 lazy 386.70 kB**. skip 수는 머신·인프라 기동 여부마다 달라 같은 환경에서 비교한다. backend는 `argon2-cffi`, frontend는 `npm install`이 선행돼야 한다.
