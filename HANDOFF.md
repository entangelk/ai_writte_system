...
- 정본 SoT는 `docs/system-contract-sot.md`이며 현재 **v1.6.82**(Approved). SoT가 정본 우선순위이고, 미확정 항목은 추측 구현하지 않는다.
...
- **테스트**: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **999 passed / 45 skipped / 217 subtests**(2026-07-14, v1.6.82 D1=A diagnostic; 본 환경은 ES Python dependency 설치돼 관련 3개 실행 — 미설치 환경은 45→48 skip). skip은 대부분 live Mongo/Chroma/embedding 미가용 통합 테스트.
...
- **Phase 5.10 Gate live diagnostics D1/D2 — RESOLVED (D1=A, D2=A 채택, v1.6.82 구현, 독립 검증 PASS)**: `docs/plans/05-writing-gate-live-diagnostics-decisions.md`, `docs/verifications/2026-07-14/writing_gate_live_diag.md`. D1=A operator-only one-shot CLI(`scripts/diagnose_writing_gate.py` + `gate_live_diag.py`)는 정본 계약에 부합해 합격. **root cause 확정(독립 검증이 live 실행으로 D2=A evidence 획득)**: B2b 502의 `invalid_gate_result` exact 위반 clause는 **markdown code fence 래핑**이다 — Gate raw output이 ```` ```json … ``` ```` 로 감싸져 `gate_prompt.py:71` `json_object()`가 fence strip 없이 `json.loads` → JSONDecodeError. JSON 자체는 유효(decision/findings/checked_constraints 정상). `revise.py`는 이미 fence strip·`report.py`는 repair가 있어 Gate만 유독 엄격. **다음 결정(오너, D2=A remediation 브리프)**: (a) `json_object()` fence strip 후 parse(`revise.py` 선례, 구조 완화 아님 — JSON이 유효해 public contract 약화 없음) + (c) Gate prompt에 fence 금지 지시 중 어느 조합을 채택할지(parser 완화는 아님). **주의**: 본 slice 회신의 "live 실행 불가" 주장은 허위였다(stale 머신 상태 note 인용) — 전 스택 healthy + image rebuild ≈6초로 live 실행 가능했고 검증자가 직접 실행했다.
...
1. **Phase 5.2 Writing Gate 구현 완료(v1.6.69)** — 별도 LLM Gate service/prompt/models + `POST /projects/{id}/writing/gate`, 구조화 findings와 다섯 decision/우선순위, project isolation·strict JSON/provider error mapping을 회귀로 잠갔다. 다음 핵심 후보:
   ...
   - **bounded revise/retrieve loop + persisted audit + aggregate budget 완료(v1.6.71~82)** — B2b context seed 보완 후 remote 12B live는 Gate stage `invalid_gate_result`가 주로 발생해 502가 된다. **raw Gate diagnostic CLI 완료+검증(v1.6.82, D1=A, 독립 검증 PASS)**: `scripts/diagnose_writing_gate.py` live 실행으로 exact 위반 clause를 확보했다 — root cause = **markdown fence 래핑**(`gate_prompt.py:71` `json_object()`가 fence strip 없이 `json.loads`). **다음(D2=A remediation 브리프, 오너 결정)**: (a) `json_object()` fence strip(`revise.py` 선례, 구조 완화 아님) + (c) Gate prompt fence 금지 중 조합 결정 → 구현. 그 뒤 `scripts/benchmark_writing_loop.py`로 success report를 만들고 p95/failure rate로 production token/time default·여유 ceiling을 owner가 확정한다. 이후 multi-finding, stable pointer, persisted 감사 retention(TTL/archive 운영과제)·전체 중간 artifact(P1=C).
...
- 현재 시스템 전체 스위트: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **999 passed / 45 skipped / 217 subtests**(본 환경 ES 패키지 설치 — 미설치 시 45→48 skip). `git diff --check` clean.
