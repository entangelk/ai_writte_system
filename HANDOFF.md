...
| **`PUT /projects/{id}/brief` 가 replay 에 활동 행을 남긴다 — D2=ⓑ 가 못 덮은 다섯째 표면**(2026-09-13 독립 검증 H4) | 응답은 `idempotent_replay` 를 싣는데([`projects.py:218`](services/application/app/routers/projects.py#L218)) `activity.record` 는 **무조건** 돈다([`projects.py:211`](services/application/app/routers/projects.py#L211)) — D2=ⓑ 가 고친 수동 저장 경로와 **같은 모양**이다. 실측: 같은 키 2회 PUT → `idempotent_replay` `false`·**`true`** · `project_brief_saved` **2건**. `git blame` → `d422bc4`(2026-08-09 Phase 9 Slice 9.0) — **의도적 이탈이 아니라 D1·D2 이전의 기본값**이다. **★ 오너 결정 감** — 어느 브리프도 이 표면을 열거한 적이 없어 검증자가 임의로 넓히지 않았다. 갈래 둘: **ⓐ 넓힌다**(한 줄 + 행위 셀 한 쌍 · 규칙이 진짜로 하나가 된다) · **ⓑ 열거된 넷으로 둔다**(비용 0 · 같은 개념에 답이 다시 둘). **구현자 추천 ⓐ.** ※ `analysis.py` 의 나머지 기록 자리는 멱등 키가 없거나 이미 분기가 있다(`:497`·`:994`) — 스윕이 낸 실제 구멍은 이 하나다 | [`services/application/app/routers/projects.py`](services/application/app/routers/projects.py) · [`verifications/2026-09-13/owner_decision_set_d2_and_purge_cancel.md`](docs/verifications/2026-09-13/owner_decision_set_d2_and_purge_cancel.md) H4 |
...
2. **~~최종 저장·분석 연동 재검증 1~6차~~ 완료·승격(2026-09-12 세션 73 · [`final_save_b1_promotion.md`](docs/verifications/2026-09-12/final_save_b1_promotion.md))** — 남는 것: 하드닝 H1(finalize 키 상수 무물림)·H2(함수 층 가드)는 비차단·오너 값 인정 대기, N3 은 오너 결정 대기(검증 기록 Outstanding).
