...
- project/draft list/get API 자체 회귀(2026-06-28): 전체 182개(Mongo 미연결 23 skip), replica set 연결 시 전부 통과. 잠근 범위: project list+get round-trip, 없는 project→404, draft list+get+project 격리(B는 A draft 미노출, should-NOT-fire), cross-project/없는 project→404, Mongo persisted round-trip(양 경로).
