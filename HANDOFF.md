...
- **[★ 오늘 배포에서 일어나고 있을 수 있음, 미확인] `long` 프리셋의 self-report가 창 부족으로 잘릴 수 있다.** 2026-07-28 실측(브리프 §2-2): 관측 report 출력은 **3,089~4,341 tok**(n=6)인데, 창 8192 · `long`(출력 4096 → 후보 산문 ~4,033 tok) · **컨텍스트 0**에서도 실제 확보되는 출력 여유는 `8192 − 4,594 = 3,598`뿐이라 **관측 분포의 4/6이 그 선을 넘는다.** llama.cpp는 이때 400이 아니라 **200을 주고 출력만 자르므로**(`truncated: true`) 잘린 JSON이 `invalid_report`가 되고 **아무 에러도 안 뜬다**. **직접 관측한 것은 아니다** — 확인 경로는 스택을 올려 `llm_call_audits`에서 `writing_report`의 `parse_error` 비율을 보는 것이다. **수리는 K-6 결정 뒤 C-3**이며, 창을 16384로 올리는 것만으로는 안 고쳐진다(입력 예산도 함께 커지기 때문).
