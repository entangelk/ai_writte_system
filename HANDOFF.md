...
5. **★ 배포 반영 대기 — `1a0b94a`(compose restart 정책) + 2026-09-07 장면 메모 후속**. 마지막 배포는 `c18a993` 까지 나갔고 그 뒤 커밋들이 아직 origin 밖이다. **`1a0b94a` 는 재빌드가 필요 없지만 장면 메모 후속은 프론트 소스를 바꿨으므로 `frontend` 이미지 재빌드가 필요하다**(2026-09-06 선례: 프론트를 건드린 슬라이스는 재빌드 없이는 안 실린다). **★ 그리고 이번 배포는 마이그레이션이 하나 붙는다** — `python scripts/migrate_ledger_user_axis.py`(원장 소유 축 개명, 멱등·`--dry-run` 있음). **앱 기동 전에 돌린다 — 권고가 아니라 필요조건이다**: 안 돌리면 어댑터의 같은-이름-다른-키 `create_index` 를 MongoDB 가 **code=86 으로 거부해 앱이 기동하지 못한다**(2026-09-07 독립 검증 + 폐쇄 세션 실 mongo 실측, 재현 [`repro_migration_crash_window.py`](docs/verifications/2026-09-07/repro_migration_crash_window.py)). 중간에 죽어도 **그냥 다시 돌리면 된다** — 지울 인덱스를 실제 키로 판정하므로 크래시 창이 없다(v1.8.44). **오너가 push 하면 서버에서 `git pull --ff-only` → `docker compose -f docker-compose.yml -f docker-compose.external-embedding.yml up -d` 한 번**이면 끝난다(재빌드 불필요 — compose 변경은 컨테이너 **재생성**에만 적용된다).
