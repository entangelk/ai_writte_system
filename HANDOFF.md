...
- 정본 SoT는 `docs/system-contract-sot.md`이며 현재 **v1.6.75**(Approved). SoT가 정본 우선순위이고, 미확정 항목은 추측 구현하지 않는다.
...
- 구현된 계층(모두 회귀로 잠김, 아래는 현재 동작하는 표면):
  ...
  - **Writing AI(Phase 5.1~5.7, v1.6.68~75)**: 평문 candidate 생성→typed report→Writing Gate→accept/save→pending Analysis job. inline report/revise와 별도 `/writing/revise-and-gate` 1회 합성까지 구현. 동일 package로 revise→report 최신화→Gate를 실행하며 report/Gate 실패 시 단계별 partial candidate를 보존한다. 모델은 호출/응답, Application은 anchor·validation·splice·합성·loop/budget 제어를 소유한다. retrieve_more/multi-finding/내부 loop·persisted 감사 이력은 후속.
...
- **테스트**: `python3 -m pytest --ignore=tests/test_memory_mongo.py -q -p no:cacheprovider` → **906 passed / 45 skipped / 185 subtests**(2026-07-13, v1.6.75 revise→report→Gate 합성; 이 머신은 ES Python dependency가 있어 종전 skip 3개가 실행됨). skip은 대부분 live Mongo/Chroma/embedding 미가용 통합 테스트.
...
1. **Phase 5.2 Writing Gate 구현 완료(v1.6.69)** — 별도 LLM Gate service/prompt/models + `POST /projects/{id}/writing/gate`, 구조화 findings와 다섯 decision/우선순위, project isolation·strict JSON/provider error mapping을 회귀로 잠갔다. 다음 핵심 후보:
   ...
   - **report + 부분 revise + report 최신화 + Gate 1회 합성 완료(v1.6.71~75)** — 다음 후보는 retrieve_more lifecycle 또는 G8 B 내부 loop 브리프. 이후 multi-finding, stable pointer, persisted 감사 API.
