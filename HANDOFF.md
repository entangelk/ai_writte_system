...
정본은 [`docs/system-contract-sot.md`](docs/system-contract-sot.md) **v1.8.55**(Approved). **미확정 항목은 추측 구현하지 않는다.** 아래는 코드를 만지기 전에 알아야 하는 요약이다.
...
>
> **★★ 계정 탈퇴 Slice 4(화면)의 검증 조건 둘은 2026-09-11 에 닫혔다(위 절, `b52633d`).** 남은 것은 **승격 재검**이고 그것은 **검증 세션 몫**이다 — 구현 세션이 다음에 집는 것은 아래 표의 2번(랜딩 + 동의 게이트)이다. 그 뒤가 **Slice 5(문서 승격)** 와 **Slice 4b(관리자 잔여 정리 — 브리프 선행)** 다.
>
> | 순서 | 무엇 | 왜 이 순서인가 |
> |---|---|---|
