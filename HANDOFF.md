...
>
> 마지막 자가 검수: 2026-07-29 · 207줄 (1b 재감사 FAIL 반영. 완료 서술을 추가하지 않고 stale 관측 공백·Next Tasks·K-3 현재 상태를 교체했다. 다음 검수 트리거는 300줄)
...
- 정본은 `docs/system-contract-sot.md` **v1.7.61**(Approved). 미확정 항목은 추측 구현하지 않는다.
...
- **[수리됨 2026-07-30 — 다만 창 가드는 아직 없다] `long` report의 창 초과는 R-e로 없어졌다.** 2026-07-29에는 시드 3,586자 입력에서 `writing_report`가 4/4 `provider_error`(400 하드 거부)로 죽었고 원인은 항목마다 붙는 포인터 JSON이었다(report 컨텍스트의 **79%**). K-6=R-e(SoT v1.7.61)가 그것을 번호 인용으로 바꿔 **같은 project에서 프롬프트가 11,841 → 3,216 tok(−72.8%)**, 헤드룸이 **−1,665 → +7,024**가 됐다(베타 실측, 실 12B 관통 3/3 성공·repair 0). **그러나 초과 자체가 구조적으로 불가능해진 것은 아니다** — 창 8192(알파 기본)에서는 `3,216 + 출력 상한 6,144 = 9,360 > 8,192`이고 그 초과는 **400이 아니라 조용한 잘림**이다. 그래서 **다음 작업이 K-3 가드(`입력 + 출력 ≤ 창`)**이며, 그 전까지 알파에서는 `LLAMA_CTX_SIZE=16384`를 계속 넣는다(위 ★ 함정).
...
- **★ 컨텍스트 예산 트랙 — R-e까지 들어갔고, 다음 슬라이스(K-3 가드)는 "넘을 때 무엇을 할지"가 오너 결정이다.** 가드가 필요하다는 것과 식(`입력 + 출력 ≤ 창`)은 실측으로 굳었지만, **초과를 만났을 때의 동작**은 브리프 `plans/context-budget-korean-tokens-decisions.md` §3 K-3의 세 후보(**지금처럼 조용히 제외** · **400 거부** · **경고를 응답에 실어 통과**) 중 결정이 필요하다. 검색 항목 초과의 현행(조용히 제외 + `budget_excluded` 노출)은 유지 전제다.
  - **K-1**(토큰을 어떻게 셀지, 가드 뒤) — 추천 **(c)+(a)**. 근거(§3-1): 밀도가 종류에 따라 **1.30~2.46**으로 갈리고, 상수 1.70은 **동종 코퍼스에서 −23.4% 과소평가**한다. 섞이면 −1.3%로 상쇄되지만 **검색은 정의상 동종을 모으므로** 상쇄에 기대면 안 된다. R-e 뒤 실측 간극은 **2.17배**(회계 1,232 vs 실제 2,678).
  - **K-4**(프론트 표시/제한) · **K-5**(splitter 블록 크기 상한)는 그 뒤이며 각각 §3에 후보와 구현자 의견이 있다. **K-6은 결정·구현 모두 끝났다**(R-e, SoT v1.7.61).
...
2. **★ 컨텍스트 예산 트랙 — 다음은 창 가드(K-3)다.** 이 트랙은 "정확도 개선"이 아니라 **2026-07-29에 깨져 있던 것의 수리**이며, 오너가 정한 순서 중 **회계 → 관측(1a·1b) → R-e까지 들어갔다**(SoT v1.7.61). 남은 것은 **가드(K-3) → 밀도(K-1)**이고, R-a(report 전용 예산)는 폐기가 아니라 보류다(R-e로 헤드룸이 벌어져 필요 없어질 쪽으로 기울었다 — 판단은 가드 뒤 수치를 보고).
   - **가드(K-3)가 다음이고, 왜 R-e 뒤에도 필요한지가 수치에 있다.** R-e 뒤 report 프롬프트는 **3,216 tok**(2026-07-30 베타 실측, 항목 69개)이라 창 16384에서는 헤드룸 **+7,024**로 넉넉하다. 그러나 **창 8192(알파 `docker-compose.llama.yml` 기본)에서는 `3,216 + 출력 상한 6,144 = 9,360 > 8,192`**다. 가드 식은 **`입력 + 출력 ≤ 창`**이어야 한다 — 프롬프트 단독 초과는 400으로 거부되지만 **`프롬프트+출력` 초과는 200인데 출력만 조용히 잘려** 아무도 모른다. 창은 이미 `/props`에서 읽어 감사에 남으므로(1b) 남은 일은 **고정 오버헤드**(system 프롬프트 실측 **465 tok** · 후보 산문 · 구조적 래퍼)를 더해 판정하는 자리와 넘을 때의 동작이다.
   - **어느 머신에서 해도 되나 — 베타가 오히려 낫다.** 가드 회귀는 **창을 스텁**한 유닛 테스트라 LLM이 필요 없고(감마·CI 포함 어디서든), 창 초과 시 서버 거동은 **이미 알파·베타 양쪽에서 측정**됐다(브리프 §1 ②). 실제 토큰 수는 베타 외부 서버 `/tokenize`로 충분하다. **베타에서 만들면 "창을 상수로 박는" 틀린 설계가 애초에 작동하지 않는다** — repo가 그 값을 모르기 때문이다.
   - **밀도(K-1)는 마지막이고, 회계와 다른 오차원이다.** 회계는 자기 단위(`len/4`) 안에서 정확하고, 실제 토큰과의 차이가 밀도다 — R-e 뒤 실측으로 **회계 1,232 vs 실제 2,678(2.17배)**. **유닛 회귀는 양변이 같은 단위라 이 간극을 보지 않는다 — 의도한 분리다**(두 오차를 한 테스트에 섞으면 어느 쪽이 회귀했는지 모른다). 추천은 여전히 (c)+(a).
   - **R-e를 건드릴 사람이 알아야 하는 것**: report extractor는 항목을 **번호**로 인용하고 매핑은 서버가 한다. **프롬프트의 번호 순서와 `package_pointers`의 allowlist 순서(macro→micro)가 갈라지면 실패가 아니라 근거 오귀속**이다 — `test_writing_context_pointer.py`의 세 셀이 그 순서를 잠근다(뮤테이션 실증): `test_rendered_number_resolves_to_the_item_it_labels`(**렌더링에서 번호를 읽어 되돌리므로 provider 없이 단독으로 양쪽 발산을 잡는다** — 여기부터 보면 된다) · service 경유 e2e 셀 · 섹션 연속성 셀. 번호는 **1-based**이고 `0`·음수·`bool`·구 포인터 객체는 거부한다.
