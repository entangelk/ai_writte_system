...
>
> 마지막 자가 검수: 2026-07-31 · 217줄 (하루에 세 슬라이스가 들어와 **매번 교체로** 처리했다: 측정 리그 → 독립 검증 보강 → **R-a 구현**. Next Tasks 2번은 그때마다 통째로 다시 썼고(측정 대기 → 결정 대기 → 구현 완료+남은 것), 결정이 내려진 **Owner Decisions Needed의 R-a 항목은 삭제**했다 — 결정 이력은 `daily_logs/2026-07-31/`와 SoT v1.7.65에 있다. 베타 관측치·회귀 기준선도 오늘 값이다. 완료 서술은 여기 쌓지 않았다. 다음 검수 트리거는 300줄)
...
- 정본은 `docs/system-contract-sot.md` **v1.7.65**(Approved). 미확정 항목은 추측 구현하지 않는다.
...
2. **★ R-a는 구현됐다(SoT v1.7.65) — 남은 것은 루프 슬라이스와 알파 R-c 1회.** 오너 결정(2026-07-31)은 **(ii) 창에서 유도 + (iii) 후보 길이에서 유도**이고 **(i) 상수는 기각**됐다.
   - **지금 도는 식**: `예산(회계) = (창 − report 출력상한 − system 템플릿 − 후보 상한 − 포장 150) × 0.96`. 창·토큰 계수는 게이트웨이가 답하고(`GET /v1/capabilities` · `POST /v1/tokenize`) 앱이 **프로세스당 한 번** 캐시한다. **모르면 요청값 그대로**(종전 동작)이며 **유도는 줄이기만 한다**.
   - **적용된 곳**: `/writing/generate` · **생성 워커** · `/writing/report`. 제품의 주 경로는 워커다(생성 직후 **같은 패키지로** self-report가 돈다) — report만 고치면 제품은 안 바뀐다는 것이 착수 시 확인됐다.
   - **★ 아직 안 된 곳 = revise-and-gate 루프**([`revise_gate.py`](services/application/app/writing/revise_gate.py)). 패키지를 병합하고 `retrieve_more`로 **키우기까지** 하므로 "무엇을 기준으로 언제 줄일지"가 별도 판단이다. **다음 슬라이스가 이것이다.**
   - **관통 실측(베타, 창 16384, 포화 프로젝트)**: 종전 8192 → 가드가 400 거부 · **유도 5307 → 실제 report 호출 통과**. 게이트웨이가 앱에 준 값도 리그가 llama에 직접 물은 값과 같다(창 16384 · system 465).
   - **알파 R-c는 이제 구현이 아니라 관측이다** — 창을 32768로 올리면 유도가 **자동으로** 넓어진다. 착수 체크리스트(이미지 빌드 · `-hf` 재다운로드 · 오래된 볼륨의 `PromptTemplateConflict`)는 아래 함정 절과 같다.
   - **함정**: 요청의 `max_tokens`는 이제 **상한**이지 그대로 쓰는 예산이 아니다(공개 스키마는 무변). 예산이 요청보다 작게 나오는 것은 **정상**이며, 그 값이 궁금하면 `scripts/report_budget_measure.py`로 같은 산식을 재현할 수 있다.
