...
- **★ 컨텍스트 예산 트랙 — 구현은 다 끝났고 결정 2건이 남았다.** 회계·R-e·가드·밀도가 전부 들어갔다(SoT v1.7.60~63). 남은 둘은 **더 할지 말지**의 결정이다:
  - **ⓐ K-1(c)(색인 시점 정확 계수)를 보류로 확정할지** — 추천 **보류(사실상 불필요)**. 브리프가 (c)를 추천한 **유일한 하중 근거**는 "가드를 상수 위에 세우면 가드가 −23% 틀린 값을 검사한다"였는데 **K-3 가드는 상수를 쓰지 않고 서버 계수를 쓴다**(delta 0). 회계 비율도 실측 **1.07**이다. (c)는 **색인 경로에 LLM 의존을 새로 만든다**(지금 색인은 임베딩만 필요). 근거가 되살아나는 조건: 영문·숫자 비중이 큰 프로젝트에서 항목별 오차가 실제 문제로 관측될 때.
  - **ⓑ R-a(report 전용 예산)를 만들지** — 만재 예산 + `long`에서 report 입력이 창을 **1,914 넘는다**(위 Next Tasks 2). 선택지: R-a를 만들어 report에 더 작은 예산을 주거나(근거가 줄어든다), 지금처럼 **가드의 400으로 두고 사용자가 분량을 줄이게** 하거나(실패가 보이지만 요청은 실패한다), 창을 32768로 올린다(알파에서 뜨는 것은 확인됐고 베타 외부 서버는 오너 설정 사안).
  - **K-4(프론트 글자수 표시·경고)는 이제 착수 가능하다** — 선행이던 환산 확정(C-2)이 끝났다. 이것은 결정 대기가 아니라 순서 대기였다.
...
2. **★ 컨텍스트 예산 트랙 — 오너가 정한 순서(회계 → R-e → 가드 → 밀도)를 전부 끝냈다.** 남은 것은 **오너 결정 2건**이고(아래 Owner Decisions Needed) 그 결정 없이 더 진행할 것은 없다. 지금 상태: 회계 비율 **1.07**(종전 0.46) · 입력 예산 기본값 **8192** · 창 초과는 가드가 400으로 거부 · 빠듯한 여유는 KPI 경고.
   - **환산은 `len/1.7`이고 실측 근거가 있다**(배포 원고 429블록·21,774자 → 밀도 **1.708 자/tok**). **LLM이 없는 곳(감마·유닛 테스트·색인)의 fallback**이라는 것이 이 상수의 자리다 — 정확한 수가 필요한 유일한 지점(창 가드)은 서버에 직접 센다. **영문·숫자 비중이 큰 프로젝트에서는 다시 재야 한다**(영문 혼합은 2.46까지 = 과대평가 쪽).
   - **★ 예산을 바꿀 때 함정 — 서버만 바꾸면 제품에 반영되지 않는다.** 프론트가 `max_tokens`를 **명시적으로** 보낸다([`WritingPanel.tsx`](frontend/src/writing/WritingPanel.tsx)의 `MAX_TOKENS`). 서버 기본값(`main.py::DEFAULT_CONTEXT_BUDGET_TOKENS`)은 **생략한 클라이언트에게만** 적용된다. 양쪽을 같이 올려야 하며 두 회귀(공개 default 전수 셀 · 프론트 전송 payload 단정)가 그 대칭을 잠근다.
   - **8192는 확장이 아니라 회계 수정의 짝이다**: 종전 `4096 회계 ≈ 실제 8,904 tok`, 지금 `8192 회계 ≈ 실제 7,656 tok`. 숫자는 두 배지만 **실제 분량은 줄었다** — 회계만 고치고 예산을 그대로 뒀다면 실효 컨텍스트가 절반이 됐을 것이다.
   - **아직 남은 한 가지 초과**: 예산을 꽉 채운 프로젝트에서 `long` 후보의 report 호출은 `7,656 + 465 + 4,033 + 6,144 = 18,298 > 16,384`다(종전 19,546보다 개선). **조용한 잘림이 아니라 가드의 400**으로 나오며 근본 해결은 **R-a**(아래 결정 대기). 지금 베타 프로젝트는 회계 2,876이라 닿지 않는다.
