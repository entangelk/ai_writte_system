...
- 구현된 계층(모두 회귀로 잠김, 아래는 현재 동작하는 표면):
  ...
  - **Memory(Phase 2B.1~2B.7)**: canonical `MemoryEntry` store + candidate 승격(수동/threshold), compare→ActionProposal→versioned upsert(append-only), compare judge(Gateway 1-turn), memory→vector 재색인(async outbox→worker), event/open_question semantic identity(off 기본) + **character 별칭 semantic 보강(v1.6.62, off 기본)**: name-key 매칭 0인 character가 다른 이름 canonical과 의미적으로 가까우면(별칭 false-negative) `conflict`(review)로 surface — 자동 병합 없음(judge 미호출, D2=A 존중), `AnalysisCompareService.alias_matcher` seam(env `ANALYSIS_CHARACTER_ALIAS_MATCH_THRESHOLD`). 동명이인 false-positive 반증·threshold 실값은 후속. **conflict review queue 영속화(v1.6.59)**: 2B.4 apply가 review-only(`conflict`) proposal을 durable `review_queue` store(in-memory+Mongo)에 결정적 id upsert(멱등) — status=`open`(+v1.6.61 `resolved`/`dismissed`), `GET /projects/{id}/analysis/review-queue`로 조회. **resolve/dismiss 전이는 v1.6.61로 실경로화**(candidate confirm→resolve·reject→dismiss); merge/split 산출은 Phase 6 UI 잔여.
