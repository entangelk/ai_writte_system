...
- **★ Phase 8 Slice 8.2 — 사용량 원장 L1~L5**(브리프 [`plans/08-2-usage-ledger-decisions.md`](docs/plans/08-2-usage-ledger-decisions.md), 2026-08-03). L1 행 필드 · L2 중복 방지 키 · L3 집계 정본 · L4 보존 기간 · L5 관리자 조정 표현. **추천은 전부 A**. **실측이 뒤집은 가정 둘을 먼저 본다**: ① **클라이언트 `request_id`는 멱등키가 아니다** — 프론트가 클릭마다 새 uuid를 만들고 **그 하나를 generate·gate·revise-and-gate·accept가 공유**한다. `(user, request_id)`로 중복을 지우면 **한 흐름의 유료 동작 4개가 1개로 접혀** 8.0의 "요청 1건 = 1회"가 조용히 깨진다(그래서 추천은 키에 `action`을 넣는다). ② **원장에 `project_id`를 넣으면 project 영구 삭제가 과금 기록을 지운다** — purge reconciler가 그 필드를 가진 컬렉션을 DB에서 발견해 고아를 지우기 때문이며, D8-6이 `target_project_id`를 쓴 이유가 그것이다. **결정 전에는 원장 저장 모델을 만들지 않는다.**
...
1. **Phase 8 Slice 8.2 — 오너 결정 대기 중이다**(브리프 제출 완료, 2026-08-03. 위 Owner Decisions Needed 참조). L1~L5가 닫히면: ① 계약을 **양방향 회귀로 먼저** 잠근다 — 같은 dedupe 키 재삽입이 두 행이 되지 않는다 · **다른 `action`은 같은 `request_id`라도 각각 센다**(프론트가 한 흐름에서 uuid를 공유하므로 이 셀이 8.0 계약을 지킨다) · 회원/창 격리 · 조정 행이 부호대로 집계된다 · **창 키를 8.1의 `daily_key`/`weekly_key`에서 가져온다**(KST 경계는 `quota/policy.py`가 유일한 지역 시간대 지점이다) ② 도메인 + Mongo 어댑터(유니크 `(user_id, action, dedupe_key)` + 집계 `(user_id, daily_key)`·`(user_id, weekly_key)`, fake-collection naive/aware 셀 필수) ③ [`docs/mongo_collections.md`](docs/mongo_collections.md) §43D 등재 ④ 8.3 시행 브리프. **차감·차단은 8.3이다.**
