...
- Verdict — **the first line must start with exactly one of these three Korean tokens** (records are written in Korean; an English gloss in parentheses is optional):
  - `**합격**` — no blocking finding.
  - `**조건부 합격** — <the condition that must be closed>` — the condition is named on the same line.
  - `**불합격** — <the contract clause violated>` — the violation is named on the same line.
  These three are the *only* verdict values. There is no fourth ("서술형", "승인", "PASS (조건 없음)" …).
  **Why this is a rule and not a preference** (measured 2026-08-06): 222 records had produced **111 distinct
  verdict phrasings** because this line used to read "pass / conditional pass / fail" while the blocking
  rule below used "합격 / 조건부 합격 / 불합격" — both vocabularies were sanctioned. The cost was real:
  two attempts to classify them by regex misread **5 and 4 records**, one **불합격 was filed as "—" and
  went uncounted** for a week, and the top-level `README.md` published a wrong verdict distribution.
  The index's 판정 column and this first line must say the same thing, so both must have one vocabulary.
  Then list the load-bearing reasons.
