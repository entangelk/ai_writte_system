...
| 문서 | 역할 | 지위 |
|---|---|---|
| 이 문서 | 서비스/계약 SoT 인덱스와 우선순위 | Approved SoT v1.6.33 |
...
- 착수 브리프가 v1.6.30으로 승인됐고 Slice 4.1(domain 계약 + orchestration + Context Gate, planner 주입)이 v1.6.31로, Slice 4.2(터미널 JSON LLM planner adapter)가 v1.6.33으로 구현됐다. 첫 slice literal: purpose `writing_context`, need `current_scene`/`recent_scenes`/`event_context`/`source_quote`.
