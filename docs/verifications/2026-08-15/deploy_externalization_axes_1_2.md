...
- **대상 커밋**: `b6b1269` · `3ff94a3` · `8e57369`(코드/compose) + `86e193b` · `6a51bbb` · `e13a482` · `9e2f1ef`(기록·문서). **★ 추기로 `6352121` · `3b71eac` 를 더해 총 9커밋** — 초판은 대상을 08-14 의 7커밋으로 잡았으나 **그 앵커가 세션 경계였다**(§추기 A1). 진짜 시작점은 마지막 검증 기록 커밋 **`c08b0c2`**(08-13 11:40)이다.
...
- **어느 쪽으로 닫을지는 결정 사안이라 검증자가 고르지 않는다**: (a) external override 에서 gateway `LLAMA_BASE_URL` 을 `:?` 로 필수화해 문서와 맞춘다. (b) `.env.example` 이 `EXTERNAL_CHROMA_PORT` 처럼 예외를 명시한다 — 문서가 실제를 반영하되 fail-fast 는 셋에만 남는다.
- **★ (a)의 비용은 초판이 적은 것보다 작다 — 아래 §추기 A2 에서 정정했다.** 초판은 (a)가 *"배포 서버가 호스트 llama 를 쓰는 선택지를 배제한다"* 고 적었으나 **그것은 과대 서술이다.**
...
## 추기 (2026-08-15, 같은 세션 · 두 번째 감사 지적 반영)

이 기록을 대상으로 **또 다른 세션이 독립 감사**를 돌렸다([`alpha_day_slice_audit.md`](alpha_day_slice_audit.md), 판정 `조건부 합격 — F1 정정 조건`). B1·B2 는 그 세션이 **뮤테이션까지 재현해 수치까지 동일**하게 확인했다. 아래는 그 감사가 잡은 결함에 대한 처리이며, **지적은 전부 사실로 재확인한 뒤 반영했다**(다른 세션의 보고를 그대로 옮기지 않는다).

### A1. [F1 대응 — 정정 경로 (a) 실행] 검증 범위를 두 커밋 넓혀 미검증 구간을 실제로 0 으로 만든다

**감사 지적은 사실이다.** 이 기록의 초판은 대상을 *"2026-08-14 의 7커밋"* 으로 잡았는데, **그 앵커는 검증 커버리지가 아니라 세션 경계였다.** 마지막 검증 기록 커밋은 `c08b0c2`(08-13 11:40)이고, 그 뒤 검증 세션 자기 반영(`d7e52c8`)을 빼면 **미검증은 9커밋**이었다 — 08-13 의 **`6352121`·`3b71eac` 둘이 어느 검증에도 걸리지 않았다.** 초판이 *"git 에서 다시 유도했다"* 고 적은 것은 절반만 참이다: 커밋 목록은 git 에서 유도했지만 **시작 지점을 디렉터리 이름(`docs/verifications/2026-08-13/` 이 있으니 그날은 검증됐다)으로 추정**했다. 날짜 디렉터리는 **그날 쓰인 기록**을 뜻하지 그날 커밋 전부가 검증됐다는 뜻이 아니다.

**그러므로 여기서 두 커밋을 추기로 검증한다.** 둘 다 docs-only 다(`git show --stat`: `HANDOFF.md` + `docs/daily_logs/2026-08-13/work_log.md`).

**`6352121`(전수 회귀 기준선 `2273/1/2466`) — 합격.** 주장을 **역산으로** 검산했다: 오늘 알파 실측 `2281/4/2515`(베타 보정 `2284/1/2515`)에서 오늘의 증분을 빼면 **2284 − 11 = 2273** · **2515 − 49 = 2466** 으로 **세 자리가 전부 맞는다.** 그 커밋이 예상 근거로 든 두 자리도 지금 상태에서 재현된다 — `test_design_token_provenance.py` 는 **5 passed / 90 subtests**(주장 "3셀/62 → 5셀/90" 의 도착점) · 검증 기록은 현재 **244건**으로 주장 시점 242 + 이 기록 1 + 감사 기록 1 이다. **오늘의 예측 체인 자체가 이 기준선을 입력으로 삼아 맞았다는 것이 가장 강한 교차 검증이다** — 이 값이 틀렸다면 오늘 예상이 맞을 수 없었다.

**`3b71eac`(배포 외부화 지형 등재) — 합격.** 이 커밋은 **아직 아무도 코드로 확인하지 않은 위험 서술**을 담고 있어 추기 대상 중 실질이 크다. 검증 가능한 주장을 직접 확인했다:

- *"`/props`·`/tokenize`·`/apply-template` 는 llama.cpp 전용이고 **셋 다 예외를 삼켜 `None` 을 반환**한다"* → **사실.** [`client.py:75`](../../../services/llm_gateway/app/client.py#L75)(`except Exception:  # 관측이 기능을 깨뜨리지 않는 경계`) · [`:224`](../../../services/llm_gateway/app/client.py#L224)(`# 셀 수 없으면 "모른다"`) · [`:262`](../../../services/llm_gateway/app/client.py#L262). **★ 262 줄의 주석이 위험을 그 자리에서 말한다 — `# 셀 수 없으면 판정하지 않는다(통과)`.** 가드가 판정을 포기하고 **통과**시키는 구조이며, 이것이 *"에러가 아니라 조용한 품질 저하"* 의 실체다.
- *"코드 주석이 그 추정은 **과소평가 방향**(= 가드가 늦게 걸린다)이라고 명시한다"* → **사실.** [`client.py:237-238`](../../../services/llm_gateway/app/client.py#L237) — *"메시지 내용만 세면 템플릿 몫이 빠져 **과소평가**된다(실측 −16 ~ −80). 과소평가는 가드가 늦게 걸리는 방향이라 특히 나쁘다"*.
- *"생성 경로는 `POST /v1/chat/completions` 하나"* → **사실**(파일 내 1건).
- 당시 compose 하드코딩 서술(`:99·101·106` 등)은 **`b6b1269` 가 이미 해소한 과거 상태**이고, 그 해소가 이 기록 본문의 검증 대상이었다.

**결론: 두 커밋 모두 Blocking 0.** 이로써 `c08b0c2` 이후 미검증 구간은 **실제로 0** 이다(이 기록의 초판·추기 + 감사 기록이 전 구간을 덮는다).

**★ 다음 사람에게 남기는 규칙**: **미검증 구간의 시작점은 `docs/verifications/<날짜>/` 디렉터리가 아니라 마지막 검증 기록 *커밋*이다.** 날짜 디렉터리로 앵커를 잡으면 그날 오후의 커밋이 통째로 사각지대가 된다 — 이 저장소에서 같은 계열의 오류가 이것으로 **세 번째**다(08-13 "미검증 다섯"→셋 · 08-14 "2커밋"→7 · 오늘 "7커밋"→9).

### A2. [F2 정정 — B2 결정에 직접 영향] 옵션 (a)의 단점 서술이 과대했다

초판은 (a)(external override 에서 `LLAMA_BASE_URL` 을 `:?` 로 필수화)의 단점을 *"배포 서버가 호스트 llama 를 쓰는 선택지를 배제한다"* 고 적었다. **틀렸다.** `${VAR:?msg}` 는 **값이 있으면 그 값을 그대로 쓴다** — 필수화 뒤에도 `.env` 에 `LLAMA_BASE_URL=http://host.docker.internal:9080` 을 명시하면 호스트 llama 는 그대로 쓸 수 있다.

**사라지는 것은 선택지가 아니라 *암묵적 폴백*이고, (a)의 실제 비용은 `.env` 에 한 줄이다.** 즉 (a)와 (b)의 무게는 초판이 그린 것보다 훨씬 가깝다 — (a)는 *"배포 서버에서 LLM 주소를 명시하게 강제한다"* 이지 *"호스트 llama 금지"* 가 아니다. **오너가 B2 를 고르기 전에 이 정정된 프레이밍으로 봐야 한다**(초판 프레이밍으로 보면 (a)의 비용을 과대평가한 채 고르게 된다).

### A3. 사소한 정정 셋 (결론 불변)

- **"compose 를 읽는 가드 전부"** — 정확히는 **base `docker-compose.yml` 을 읽는 가드 전부(3파일)** 다. `tests/` 에서 `docker-compose` 를 언급하는 파일은 4개이고, 넷째 [`test_core_sot_mongo.py:7-8`](../../../tests/test_core_sot_mongo.py#L7)은 `docker-compose.test.yml` 만 말한다. **M-A 의 결론(0셀)은 불변**이다 — 그 파일은 base 를 읽지 않으므로 대상이 아니다.
- **"subtest +49 는 전부 신규 가드"** — **+48 이 신규 가드이고 +1 은 검증 기록 자리**(`test_docs_indexes.py` 의 판정 열 전수 셀)다. 이 기록의 Outstanding items 와 work_log 표는 처음부터 그 1 을 별도 행으로 분리해 적었고, 요약 문장만 뭉뚱그렸다.
- **"260커밋 뒤처져 있었다"** — 앵커를 안 밝힌 값이다. `--since="14 days ago"`(= 08-01) 기준이며, 실제 이미지 시각(08-02) 기준으로는 **215커밋**이다. **수백 커밋 규모라는 성질은 불변**이나 숫자를 인용할 때는 앵커를 함께 적는다.

### 판정 재확인

**조건부 합격 유지** — 조건은 여전히 **B1·B2** 다. F1 은 이 추기(A1)로 닫혔고, F2·F3~F5 는 이 절에서 정정했다. **B1·B2 는 감사 세션이 독립적으로 재현했으므로 두 세션이 같은 결론에 도달한 상태**이며, 처리는 오너 결정과 후속 구현에 남는다.
