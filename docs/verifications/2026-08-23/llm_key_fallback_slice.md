...
**최종 판정은 합격이다** — 조건 **B1** 은 오너 갈래 ⓑ(코드 정렬)에 따른 폐쇄 커밋 `d71294a`(B1)·`b2be4ad`(H1·H2)가 닫았고, 원 검증자의 사후 재검(아래 §사후 재검 — B1·H1 뮤테이션 양축 재실패 · 전수 2493/4/2720 재현)이 판정을 올렸다. 아래 본문은 발행 시점 그대로다.
...
**합격**(승격 — 2026-08-23 · 오너 갈래 ⓑ 폐쇄 뒤 원 검증자의 사후 재검으로 확정). 발행 시점 판정 원문:

> **조건부 합격** — 브리프 §1의 "timeout/5xx는 쿨다운 없이" 조항과 동기 축(임베딩·리랭커)
> 구현의 모순(B1)이 오너 결정으로 화해될 때까지.

**[→ 승격 2026-08-23 · 판정 합격]**
오너가 ⓑ(코드 정렬)를 골라 폐쇄 커밋 `b2be4ad`(H1·H2)·`d71294a`(B1)가 나왔고, 원
검증자(다른 세션에서 폐쇄 재검)가 아래 "사후 재검"으로 확정했다 — B1 회귀 뮤테이션
양축 재실패·H1 리터럴 뮤테이션 양축 재실패·전수 2493/4/2720 재현.
...
미추적 + 정본 내부 불일치" 규칙에 걸리는 결함이었다 — 폐쇄 후 그 예외도 사라졌다.

## 사후 재검 — B1·H1·H2 폐쇄 확인 (2026-08-23, 검증 세션 2차)

폐쇄 커밋 `b2be4ad`(H1 리터럴 핀 양축 + H2 브리프 §1 파싱 규칙 한 줄)·`d71294a`
(B1 오너 ⓑ — 동기 축 쿨다운을 3분류로 정렬·docstring 정정·no-cool 가드 셀 2).
재검 환경은 본 검증과 동일(test-mongo 기동·mypy 설치·트리 클린).

- **H1 폐쇄 실증(뮤테이션)**: R1 = 검증 M1의 재현(게이트웨이 `600.0`→`60.0`) —
  종전 **0셀** → 이제 1셀 `CooldownLiteralTests`(게이트웨이) 재실패. R2 = 동기 축
  `RATE_LIMIT_COOLDOWN_SECONDS` `60.0`→`30.0`(관계 셀이 원리적으로 못 보는 방향) —
  1셀 `CooldownLiteralTests`(동기) 재실패.
- **B1 폐쇄 실증(뮤테이션)**: R3/R4 = 임베딩·리랭커 각각 `elif exc.status_code == 429:`
  → `else:` (5xx/네트워크 60s 쿨다운 재삽입 = B1 이탈 행동의 원복) — 각 1셀
  `test_a_5xx_failure_does_not_cool_the_slot` 재실패. 가드 셀이 정확히 요청 *간*
  블라인드 스팟(M9c가 드러낸 것)을 겨냥한다. 3분류(401/403=600s·429=60s·그 외
  없음)는 게이트웨이 형제·브리프 §1과 글자 일치.
- **H2**: 브리프 §1 신설 행("항목별 trim·빈 항목 무시·중복 제거·첫 등장 순서 유지",
  잠금 셀 `test_splits_strips_and_dedups` 2곳)이 코드 계약과 일치.
- **전수 재실측**: **2493 passed / 4 skipped / 2720 subtests (227.8s, exit 0)** — 폐쇄
  세션 주장과 일치. 셀 검산 2489 + H1 2 + no-cool 2 = 2493, subtest 2720 = 본 검증
  예고(기록 2건 판정 열 +2).
- **문서**: work_log 세션 7·HANDOFF(B1 해소 1행·기준선 2493·예고 중첩 정리·헤더
  자가 검수 454줄)·CHANGELOG 확인. **니트 1(비차단)**: ⓑ 결정이 work_log
  **Decisions 절에 D-항목으로 없다**(그날 오너 결정 6건은 전부 D-원문과 함께 등재).
  세션 헤더·HANDOFF에는 실재하므로 결정 자체는 문제없다 — 다음 문서 슬라이스에서
  D-2026-08-23-g 한 줄로 보충하면 된다.

## Outstanding items

- ~~B1 화소 방향(브리프 개정 vs 코드 정렬) — 오너 결정 대기~~ **[해소 2026-08-23 —
  오너 ⓑ 코드 정렬, `d71294a`, 위 사후 재검으로 확인]**
...
- `e952506`(임베딩 구글 확장 — 경로 정규화·dimensions)의 세부 축은 본 검증의 명명된
  ...
  기록으로만 확인(재실측 안 함).
- 배포 서버에는 B1 폐쇄분(`d71294a`)이 미반영 — 서버가 쓰는 임베딩 축 변경이므로
  다음 서버 세션에서 동기화 필요(오너 승인 대기).
