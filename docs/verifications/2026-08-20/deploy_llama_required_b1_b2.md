...
- **H1 — 실측 표에 환경 통제 절차가 없다**(Findings 4). 기계-로컬 `.env` 유무에 따라 같은 명령의 rc 가 뒤집히는 자리에서 "어떻게 통제했는가"가 재현성의 전부다. 원 work_log 를 고치지는 않는다(사실 오류가 아니라 절차 누락) — 이 기록 §Methodology 와 [repro 스크립트](repro_deploy_llama_required.sh)가 표준 절차로 대신한다. **교훈으로 남기는 규칙: compose 실측을 기록할 때 `.env` 상태를 함께 적는다.**
