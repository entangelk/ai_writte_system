...
## 권고 반영 — **H4·H5 둘 다 적용 (2026-08-20, 구현 세션 `6268519`)**

> 이 절은 **재검자가 아니라 구현 세션이 나중에 추가한 것**이다. 위 Findings·Verdict 는
> 재검 시점 그대로 두었다.

**둘 다 문언만 바뀌고 동작은 그대로다**(`8 passed / 3 subtests` 무변). 그래서 뮤테이션을
다시 돌리지 않았다 — 단정식이 한 자도 안 바뀌었고 바뀐 것은 `msg` 인자와 주석뿐이다.

**H4 — 왜 "비차단 문언" 을 그냥 두지 않았나.** 지적대로 종전 메시지는 원인을 **거짓으로**
말했다. 결정 1-b②(*"실패가 원인을 안 말하면 다음 사람이 셀을 지운다"*)의 **더 나쁜 형태**다 —
원인을 **안** 말하는 실패는 사람을 멈춰 세우지만, **틀리게** 말하는 실패는 **엉뚱한 방향으로
멀리 보낸다.** `warn_unused_ignores` 를 넣으려던 사람이 *"이 키는 조용해지는 길이다"* 를 읽으면
**가드를 강화하려다 포기한다.**

바뀐 문언의 요지: **이 검사는 트립와이어이지 억제 키 목록이 아니다.** 허용 목록 밖이면
무해한 키도 실패하며, 요구하는 것은 *"조용해지지 말 것"* 이 아니라 **"의식적으로 추가할 것"**
이다. `_ALLOWED_KEYS` 주석에 **정당한 키의 예 넷**(`warn_unused_ignores` · `python_version` ·
`plugins` · `explicit_package_bases`)과 **추가가 정상 경로라는 것**을 명시했다.

**H5** — `files` 등가 단정이 **확대도 거부한다**는 사실과, **확대가 정당해지는 날(예: `tests/`
편입 트리거) 이 줄을 함께 고친다**는 것을 메시지에 넣었다.

**남긴 것**: 재검 §2 가 관찰한 O2 의 부수 사실(`frontend` 를 넣으면 mypy 자신이
`There are no .py[i] files` 로 별도 실패한다)은 **셀과 무관**하므로 문언에 넣지 않았다.

## Outstanding items

- **미검증 = 1커밋**(`cd1d82d`, 2026-08-16) — 이 재검으로 `5182cad`(과 docs `0741a45`·`d3cc557`)는 커버됐다.
- ~~**H4·H5 권고 대기**~~ — **반영됨(2026-08-20 `6268519`)**. 위 §권고 반영 참조.
...
#   ※ 그 메시지 문언은 H4 로 바뀌었다(6268519). 지금 재현하면 같은 셀이 같은 이유로
#      실패하되 "허용 목록 밖의 키는 …일단 실패한다(트립와이어)" 라고 말한다.
