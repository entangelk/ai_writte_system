...
**최종 판정은 합격이다** — 조건 **C1** 은 폐쇄 커밋 `92b9b24`(2026-08-20 구현 세션 · H1·H2 동반)가 닫았고, 독립 재검([`reranker_c1_h1_h2_closure.md`](../2026-08-21/reranker_c1_h1_h2_closure.md) — repro Part 1 뒤집힘 · C1-M1~M3·RV-B1/B2 재유도 일치)이 판정을 올렸다. 아래 본문은 발행 시점 그대로다.
...
- ~~**C1 — fail-open 이 투영(`text_of`) 예외를 안 덮는다**~~ **[닫힘 2026-08-20 `92b9b24`]**(Findings 1). 셀 문언 *"모든 실패"* · 브리프 값 *"검색이 죽지 않는다"* 와 행동이 어긋난다. 폐쇄: (i) 단계 전체 fail-open(권장) 또는 (ii) 문언·브리프 스코프 명시. 확인: 재현 스크립트 Part 1 출력 뒤집힘.

### Hardening recommendations (비차단)

- ~~**H1 — 유일-생성자 셀의 속성·별칭 우회**~~ **[닫힘 2026-08-20 `92b9b24`]**(Findings 2). B1 폐쇄 학습(`_constructor_names`)을 이 셀에도 적용하거나, 셀 문언을 "원이름 직접 생성"으로 좁힌다.
- ~~**H2 — 동률 문언 정밀도**~~ **[닫힘 2026-08-20 `92b9b24`]**: [`rerank.py:190`](../../../services/application/app/context_search/rerank.py) 주석 *"점수 동률에서는 요청 순서를 유지한다"* 는 **응답이 동률 항목을 요청 순서로 보낼 때만** 참이다(안정 정렬은 응답 순서를 보존한다). 셀(`test_ties_keep_the_request_order`)도 그 전제의 응답만 시험한다. R6b 가 물긴 하지만, 문언을 "응답의 동률 순서를 유지(정합 서버에서 요청 순서)"로 정확히 하거나 인덱스 tie-break 를 명시 계약으로 만드는 편이 정직하다.
...
**합격**(승격 — 2026-08-21 · 폐쇄 커밋 `92b9b24` 의 독립 재검 [`reranker_c1_h1_h2_closure.md`](../2026-08-21/reranker_c1_h1_h2_closure.md) 으로 확정). 발행 시점 판정 원문:

> **조건부 합격** — C1(fail-open의 text_of 예외 미달 폐쇄)을 닫을 것.

**[→ 승격 2026-08-21 · 판정 합격]** 폐쇄 커밋 `92b9b24`(구현 세션)를 검증자가 독립 재검했다 — repro Part 1 뒤집힘·C1-M1~M3·RV-B1/B2 전부 같은 diff 로 재유도 일치. 근거: [`reranker_c1_h1_h2_closure.md`](../2026-08-21/reranker_c1_h1_h2_closure.md). 나머지는 전부 성립: R1~R7 같은 diff 재현 · 산출물 4건이 확정값 넷과 일치(no-op 기본·데코레이터 도메인 0줄·하네스 무정답+경계셀 실증) · D2=A 글자-성질 계약 수정이 사실에 맞고 브리프에 기록됐으며 · 임베딩 조건 B1 폐쇄는 독립 재검으로 **승격 확정**됐다.

## 조건 폐쇄 — **C1 닫힘 · H1·H2 도 함께 (2026-08-20, 구현 세션 `92b9b24`)**

> 이 절은 **검증자가 아니라 구현 세션이 나중에 추가한 것**이다. 위 Findings·Verdict 는
> 검증 시점 그대로 두었다.

**폐쇄안 (i) 단계 전체 fail-open 을 골랐다.** (ii) 문언 스코프 축소는 *"모든 실패"* 를
*"프로바이더 실패"* 로 줄여 기록을 참으로 만들지만 **계약을 약해진 채 합의**시킨다 —
**오늘 같은 갈래가 네 번째였고 네 번 다 "검사를 넓힌다" 를 골랐다**(정본 산출물 문언 ·
셀 실패 메시지 · 임베딩 조립 가드 · 이것).

**★ 그리고 이 조건은 구현자가 스스로 넘긴 축이었다** — 인계 시 *"fail-open 이 정말 모든
실패를 덮는가(지금 `text_of` 가 던지면 데코레이터가 안 잡는다)"* 를 **볼 만한 축 ②** 로
적었고, 검증이 그것을 **런타임으로 실증**했다. 의심을 넘기는 것이 실제로 작동한 사례다.
...
**리랭킹이 영원히 no-op 인 채로 아무도 모르고**, 그것이 이 저장소가 오늘 네 번 만난 실패
모양이다. 같은 형태의 선례는 [`activity/log.py`](../../../services/application/app/activity/log.py)
의 A4=A 격리 경계(`except Exception` + `noqa: BLE001` + 경고 로그)다.

**재검(구현 세션, 검증자 스크립트 그대로)**

| 블록 | 검증 시점 | 폐쇄 후 |
|---|---|---|
| **Part 1** 투영이 `ValueError` | 예외가 새어나감 | ✅ **`결과: 정상 반환(fail-open 작동)`** + WARNING 로그 |
| **Part 2 RV-B1** 모듈 속성 생성 | 18 passed(침묵) | ✅ **유일-생성자 셀 실패** |
| **Part 2 RV-B2** 별칭 생성 | 침묵 | ✅ 같은 셀 실패 |
| Part 3 R1·R2·R4·R5·R6b·R7 | 물음 | 물음(보존) |
| **Part 3 R3b** | 물음 | **리터럴 불일치(`count=0`)** — 아래 |
| Part 4 하네스 경계 | 물음 | 물음 |

**★ R3b 는 "안 문 것" 이 아니라 "뮤테이션이 낡은 것" 이다.** 스크립트가
`except RerankProviderError:` 리터럴을 찾는데 폐쇄가 그 줄을 바꿨다. **같은 의도의
뮤테이션을 새 형태로 직접 돌려 확인했다**(아래 C1-M1). 다음 검증자는 스크립트의 그
블록을 새 리터럴로 갱신해야 한다.
...
| # | 뮤테이션 | 재실패한 셀 |
|---|---|---|
| C1-M1 | 경계를 다시 `except RerankProviderError` 로(조건 재도입) | `FailOpenScopeTest` 3셀 + subtest |
| C1-M2 | 경고 로그 제거(**조용한** fail-open) | `test_the_fallback_is_logged_rather_than_swallowed` |
| C1-M3 | 정상 경로에서도 경고(로그가 소음이 되는 방향) | `test_a_healthy_reranking_logs_nothing` |

**H1** — 유일-생성자 셀에 **별칭(`asname`)·모듈 속성** 인식을 넣었다. **같은 날 임베딩
B1 폐쇄에서 배운 것이 이 가드에 적용되지 않았던 것**이고, 그 자체가 교훈이다:
**한 곳에서 배운 것은 같은 형태의 다른 가드에도 옮긴다.** 잔여(할당 별칭)는 임베딩
가드와 같게 문언에 명시했다.

**H2** — 동률 주석의 *"요청 순서 유지"* 를 **"응답에 담긴 순서 유지"** 로 정정했다.
지적대로 안정 정렬이 보존하는 것은 응답 순서이고, 잠그는 성질은 **"같은 응답이 언제나
같은 순서를 낸다"** 이다.

## Outstanding items

- ~~**조건 C1 폐쇄 대기**~~ — **닫혔다(2026-08-20 `92b9b24`)**. 위 §조건 폐쇄 참조. **H1·H2 도 함께** 닫았고, 폐쇄 자체의 양방향 뮤테이션 셋(C1-M1~M3)까지 돌렸다. **★ 재현 스크립트 Part 3 의 R3b 블록은 리터럴이 낡았다** — 폐쇄가 `except RerankProviderError:` 줄을 바꿨으므로 다음 검증자가 새 리터럴로 갱신한다.
