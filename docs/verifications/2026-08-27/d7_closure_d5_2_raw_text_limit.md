...
## 조건·보강 폐쇄 (2026-08-27, 구현 세션 `873ff84`)

> 이 절은 독립 검증자가 아니라 후속 구현 세션의 폐쇄 보고다. 발행 시점 판정은 위에 보존하며,
> `합격` 승격은 다음 독립 재검이 담당한다.

- **블로킹 조건 닫힘**: CHANGELOG에 세션 2·3 행이 생겼고 세션 1의 반증된 근거에는
  `40c4524` 정정 주석이 붙었다.
- **H1 닫힘**: env 상한을 성공 accept 뒤 3000으로 내린 같은 멱등키 재시도가 replay보다
  상한 오류를 먼저 내고 provider 호출을 추가하지 않는 셀을 추가했다.
- **H3 닫힘**: 저장 축(strip 전)·accept 축(strip 후)의 측정 시점을 양쪽 코드에 “각 축이
  자기가 저장하는 값을 잰다”로 명시했다.
- **H4 닫힘(오너 선택: 카운터 추가)**: ProjectBrief 스칼라 4필드에 1000자 코드포인트 카운터,
  90% warn, 초과 danger·`aria-invalid`·저장 차단을 추가했다. 정확히 1000자는 허용하고
  `maxLength`는 없다. 네 필드 각각의 under/over 방향 셀이 잠근다.
- **H2는 비차단 상태 유지**: 프론트 본문 4000 상수를 서버 env와 런타임 동기화하려면 public
  설정 계약이 새로 필요하다. 이번 구현은 그 계약을 임의 확장하지 않았고 서버 최종 방어는 무변이다.
- 자기 검증: 프론트 **34 files / 377 tests** + `tsc` clean, accept 집중 **5/5**, 브리프 모델
  1000/1001 경계 직접 확인, `py_compile`·`git diff --check` clean. 호스트 앱 기동 셀은 기존
  `test_an_invalid_limit_fails_app_creation`에서 5분 이상 무출력 대기해 중단했으며 전수 성공으로
  기록하지 않았다.

## 미해결 항목 (운영 상태 — 결함 아님)

- 위 조건(CHANGELOG)은 구현 세션 `873ff84`에서 닫힘 — 다음 독립 재검 뒤 승격 가능.
