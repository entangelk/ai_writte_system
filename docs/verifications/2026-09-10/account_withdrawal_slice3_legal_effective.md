...
**이 절이 하지 않는 것 — 판정 승격.** 조건을 닫은 세션이 자기 판정을 올리면 독립성이 사라진다. 위 Verdict 는 **조건부 합격 그대로**이며, 승격은 다음 검증 세션이 변이를 재적용해 판단한다(Outstanding 3번의 뜻대로). **[정정 2026-09-13 — 그 승격은 [`slice3_closure_promotion.md`](slice3_closure_promotion.md)에서 났다. 위 Verdict 는 더 이상 조건부 합격이 아니라 **합격**이고, 이 문단은 발행 시점의 상태를 적은 것이다.]**
