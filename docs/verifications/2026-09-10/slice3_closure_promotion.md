...
- **이 기록 자신**이 인덱스 건수를 295 → **296**(70일치 유지)으로 움직인다. 셀은 안 늘었으므로 **passed 3023 · skip 1 은 그대로**이고, **subtest 는 4117 → 4119** 다 — 예고가 아니라 **워크트리 양단 실측**이다: `git worktree add --detach … c1cb282` 에서 `test_docs_indexes` **305** · `test_repo_hygiene` **606**, 현행 트리에서 **306** · **607**(각 +1 = 이 기록 파일 하나가 판정 열 전수 셀과 파일 회계에 들어간 몫). **다음 기준선 갱신은 3023 / 1 / 4119** 이며, 셀을 더한 슬라이스가 아니라 이 기록이 그 +2 의 주인이다.
