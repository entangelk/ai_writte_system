...
> ([`daily_logs/2026-08-23/work_log.md`](../daily_logs/2026-08-23/work_log.md) 세션 16). 그래서 그 전에 쓰인
> 기록들이 인용한 해시는 **재작성 전 공간의 값**이고 현행 저장소에는 없다. 2026-09-13 코퍼스 전수에서
> 경계가 칼같이 갈렸다 — **2026-07-06~08-23 구간은 371개 중 1개만** 해소되고, 2026-08-24 이후는 239개 중 229개가
> 해소된다. **해시가 안 잡힌다고 그 기록의 근거를 의심하지 말 것**(실제로 한 검증 세션이 이 이유로 시간을
> 썼다). 옛 해시는 **고쳐 쓰지 않는다** — 그때의 사실이고, 이 저장소는 이력 문서를 고치지 않는다.
>
> **★ 종전 발행분의 분모(1/264·269/272)는 유도 명령 없이 발행돼 어떤 추출 변형으로도 재현되지 않는다**
> (독립 검증 2026-09-13 H2). 오너 결정 ⓐ(2026-09-13)로 위 수치를 같은 방법
> (*`docs/verifications/**/*.md` 의 `` `<7~40 hex>` `` 토큰을 날짜 디렉터리로 묶어 `git cat-file -e <h>^{commit}`*)
> 의 재측정값(**06-24~07-05 69/70 · 07-06~08-23 1/371 · 08-24 이후 229/239** · 미해소 총계 376종)으로 교체했다.
> **분자 `1` 과 경계의 날카로움은 정확히 재현**된다 — 수치를 인용할 때는 아래 명령으로 직접 유도할 것(수치가
> 움직이면 환경이 아니라 분류 규칙이 달라진 것이다).
>
> ```bash
> python3 - <<'PY'
...
>     d=f.relative_to('docs/verifications').parts
>     if len(d)<2 or not re.fullmatch(r'\d{4}-\d{2}-\d{2}',d[0]): continue
>     b='a' if d[0]<='2026-07-05' else ('b' if d[0]<='2026-08-23' else 'c')
...
| 기록 | 요약 | 판정 |
|---|---|---|
| [`owner_decision_set_promotion.md`](2026-09-13/owner_decision_set_promotion.md) | **오너 결정 셋 검증 조건 C1 승격 재검 — 폐쇄(`867ba15`)를 변이 재적용으로 확증.** 조건을 닫은 세션(79)이 자기 판정을 올리면 독립성이 사라지므로(v1.8.52·v1.8.63·v1.8.65·세션 77 선례) 독립 세션(81)이 승격 몫을 맡았다. **핵심 확증 재료 M14·M15(502 partial 폐쇄의 양방향)를 대상 기록 변이 표의 diff 원문 그대로 재적용해 건수·기명 셀·subtests까지 전건 일치**(under: `test_a_replayed_partial_accept_leaves_no_second_row` · over: `test_a_partial_accept_still_records_the_saved_version` — 각 1 failed / 87 passed / 113 subtests). 표본 셋도 독립 확인: **M9**(경계 벗김 3 failed) · **M10**(항상 거부 6 failed, 기명 셀까지 동일) · **M1**(수동 저장 under 1 failed — passed 76→77 은 `867ba15` under 셀 순증가로 정확히 설명되는 트리 드리프트). 다섯 회 전부 「앵커 count==1 단정 → copy2 백업 → 치환 → 초점 실행 → 복원 → 바이트 동일성 단정 → 워크트리 clean」 절차. 폐쇄 배선(`writing.py:1355`)과 under 셀이 최종 트리(`1715bae`)에 실재하는 것도 1차 소스로 확인(그 사이 커밋은 문서만). **대상 기록 판정 조건부 합격 → 합격 승격**(머리 한 줄 + §Verdict 선두 토큰, 발행 시점 원문은 인용 보존). 기준선은 문서 가드 양단 실측으로 유도(백엔드 소스 무변): 959→**961**(+2 = 이 기록 문서·인덱스 행) · passed 무변 → **3066/1/4238**(HANDOFF 기준선 줄 · 루트 README ②행 동반). SoT 무변 — 승격-only 커밋(`2e025dc`·`432f790`·`1f4f95a`)이 SoT 를 안 올린 선례를 따랐다. | **합격** |
| [`owner_decision_set_d2_and_purge_cancel.md`](2026-09-13/owner_decision_set_d2_and_purge_cancel.md) | **오너 결정 셋 시행(`267ba22`·`463a042`·`d9a1bd2`, SoT v1.8.66) 독립 검증.** 활동 로그 D2=ⓑ 가 **열거한 세 경로**는 흠이 없다 — 변이 여섯이 전부 **정확히 1셀씩** 기명 재실패해 finalize·accept·수동 저장이 **분리 소유**됨을 실증(등가·흡수 0). HP-1 하드닝이 걷힌 뒤에도 `action`·`target_type` 축이 살아 있다는 구현자 주장은 **변이 M7·M8 로 반증 실패**(둘 다 over 짝 `test_saving_a_draft_version_records_who_and_when` 1 failed). 파기 청구 뒤 취소 거부는 양방향으로 단단하다 — under 3셀, **over(유예 중 취소가 막히는 더 나쁜 사고) 6셀**, 스탬프 되돌림 과잉 교정 2셀, 라우터 매핑 제거 1셀. 경계가 `purge_started_at` 하나로 충분함을 파기 그래프 **1단계 = 청구** 에서 유도했고(표식 전에는 아무것도 파괴되지 않는다), 선언(`_ERRORS_WITHDRAWAL` 에 409 존재)·tier 예외 집합·파기 완료 계정(404 축 무변)도 확인. **★ 차단 C1 — 계약이 열거한 넷째 표면이 무셀이었다**: `writing/accept`(502 partial)을 *"D3=ⓐ 로 별개 축"* 이라 한 것은 **오귀속**(D3 은 `auto-promote` 의 **503** partial · 증상은 침묵이지 중복이 아니다)이고, 그 자리는 **브리프 §D2 의 실측표가 세 표면 중 하나로 명시 열거한** 곳이다. 프로베 실측 **같은 키 2회 → 502·502 · 저장 version 1개 · 활동 행 2건** — v1.8.66 이 새로 적은 *"replay 는 행을 안 남긴다"* 가 코드에서 거짓이었다. **이 검증이 `867ba15` 로 닫았다**(배선 + under 셀 신설 · 변이 M14·M15 양방향 확인 · 브리프 표 네 행으로 정정). 하드닝 셋: SoT v1.8.66 근거 링크의 세션 번호 오기(76→78, 정정) · `_ERRORS_WITHDRAWAL` 주석 *"409 는 두 생산자"* → 셋(정정) · **옛 해시 주석의 구간별 분모가 재현되지 않는다**(원 측정이 밝힌 방법으로 재측정하면 `1/264`→**`1/371`**, `269/272`→**`229/239`** — 분자와 경계는 정확히 재현되고 총계도 1 차이라 질적 주장은 성립하나 발행된 두 자리에 **유도 명령이 없다**). **판정 승격 완료** — 같은 날 승격 재검([`owner_decision_set_promotion.md`](2026-09-13/owner_decision_set_promotion.md))이 변이 재적용 다섯 종 전건 일치로 폐쇄를 확증해 **합격**으로 올렸다. | **합격** |
| [`promotion_alignment_and_ha1_hp1_closure.md`](2026-09-13/promotion_alignment_and_ha1_hp1_closure.md) | **선재 5건 판정 줄 정합(`a1d71fc`) + 미수리 둘 폐쇄(`5a8e8c7`) 독립 검증.** **승격 귀속 다섯 건을 1차 자료에서 재유도 — 근거 없이 올라간 것 0건**(승격 기록 다섯이 실재하고 각각 명시적으로 승격을 선언한다: `landing_l2_promotion`·`mypy_guard_closure`·`reranker_slice` §7·`reranker_c1_h1_h2_closure`·`llm_key` §사후 재검). 발행 시점 판정 원문은 **재조립 바이트 대조로 손실 0**(두 줄짜리 `llm_key` 포함) · mypy 의 *"발행 시점 문구는 그대로 둔다"* 선언과 **양립**(고쳐 쓴 것이 아니라 덧붙였다) · 부채 서술 정정 *"두 등급"* 도 `a1d71fc^` 본문에서 사실 확인. **★ 해시 축을 코퍼스 전수로 갈랐다** — 폐쇄 해시 다섯 중 넷이 미해소인데 원인은 오기가 아니라 2026-08-23 `git filter-repo` 이력 재작성이다(기록 날짜별 해소율 **2026-07-06~08-23 1/264** vs **08-24 이후 269/272**, 선재 377종). **변이 열둘 — 대상이 잰 넷 전건 재현 + 자체 공격 여덟**: MA 폐쇄 전 20 passed→폐쇄 후 1 failed · MA-over 20 passed · MB 2 failed · MB2 폐쇄 전 **50 passed 조용**→폐쇄 후 1 failed(분리 증명) · 신규 **MB3**(`target_type` 만 replay 경로 변조) 폐쇄 전 조용→폐쇄 후 1 failed = **단정 실효** · 신규 MC-1·MS-1·MS-2·MS-3·MS-5·MS-6 전부 기명 셀 재실패. **★ 신규 사각 MS-4** — 대상이 지목한 "불릿 모양 의존"은 구멍이 아니었고(`assertIsNotNone` 이 큰 소리로 문다), 실제 사각은 `next()` 가 *첫* 행을 고른다는 것이었다: 미끼 `- ` 행을 앞에 끼우면 지시받은 문장을 통째로 걷어도 **20 passed 조용**. 보강 `d3ef8cd` 로 셋을 닫았다 — 후보 **정확히 하나** 단정(MS-4 폐쇄, MA·MA-over 무회귀 재측) · HP-1 짝 셀 이름 정정(finalize 축이 아니라 같은 수동 저장 축의 `test_saving_a_draft_version_records_who_and_when`) · 승격 뒤 거짓이 된 *"위 Verdict 는 조건부 합격 그대로"* 문장 셋 정정(`a1d71fc` 가 만든 landing_l2 + 선재 slice3·slice4 화면 — 발행 시점 문장 보존하고 `**[정정 …]**` 이어 붙임). **D2 선점 아님** — 리터럴은 택소노미 등재값이고 같은 축 첫 저장 셀이 이미 핀하며 D2=ⓑ 면 행 자체가 사라져 잠글 것이 남지 않는다(다만 재감사가 D2 와 묶어 둔 배치를 단독으로 푼 사실은 오너 통지 감). 기준선: 문서 가드 둘 **952→953→955 양단 실측**이 HANDOFF **4229→4232(+3)** 와 일치 · 프런트 무변 · `passed` 무변(셀 증가 0). | **합격** |
| [`activity_replay_ac1_promotion.md`](2026-09-13/activity_replay_ac1_promotion.md) | **재감사 조건 AC1 승격 재검 — 폐쇄(`88a16fe` · 선재 5건 정합 `a1d71fc`)를 변이 재적용으로 확증.** AC1 폐쇄를 세 층으로 확인: diff 1차 소스(머리 요약 줄·§Verdict 줄 **둘 다** 승격 + 발행 시점 판정 원문은 두 자리 모두 인용 보존 — 선례 모양 그대로) · **인덱스 311행 전수 대조 병 0건**(선두 토큰 있는 91건 기준 — 옛 영어 병기 220건은 폐쇄 보고가 "형식을 먼저 정해야 한다"고 적은 범위 밖 집합) · 폐쇄 보고가 부채로 등재한 선재 5건은 `a1d71fc` 가 정합(백엔드 소스·테스트 0줄). **변이 재적용 열 회 전건 일치** — 감사가 새로 찍은 **MA-2~MA-5**(휴면 절 가드 경계 넷: 사각 초록·꼬리 제거 초록·접미 추가 초록·접두 공백 1failed) + MP 표본 여섯(MP-1·MP-2·MP-3·MP-7·MP-8b·MP-11b): under 셋은 C1 폐쇄 특성 셀의 **기명 셀까지 같은 재실패**, over(MP-7 정상 다듬기)·경계 초록 넷도 감사 실측과 한 치 없음. 기준선: 문서 가드 20/324(감사 기록 등재분 +1) · 두 파일 77/19 · 넓은 셋 237+1skip/694(이 호스트 Mongo 미도달 — 감사의 "238"은 passed+skip 합산 · 셀·서브테스트 무변) · quota 54/2. **감사 기록 판정 조건부 합격 → 합격 승격(감사 자신의 HA-2 처방대로 머리 판정 줄을 `**합격**` 토큰으로 시작).** | **합격** |

### 2026-09-12

| 기록 | 요약 | 판정 |
|---|---|---|
| [`activity_replay_c1_promotion_audit.md`](2026-09-12/activity_replay_c1_promotion_audit.md) | **조건 C1 승격 재검(`432f790`) 재감사 — 승격의 자격과 변이 표를 독립 세션이 다시 쟀다.** **MP-1 ~ MP-11b 열한 종을 diff 원문으로 재적용해 건수·기명 셀까지 전건 일치**(과장·누락 0) · C1 처방 세 항목이 `file:line` 으로 이행 확인(행 수 · `idempotent_replay` 단정 · 지시받은 docstring 문장 두 셀 모두 실재) · 세 갈래 분리 소유 재현. 대상이 안 잰 점 다섯을 더 찍어 **경계를 좁혔다**: **MA-2 — H3 가드의 두 앵커 중 `` `verifications/` `` 쪽이 무가드**(미끼가 절 *안* 43·45·47행에 이미 있어 실효 앵커는 `전신` 한 낱말, 전건 초록) · MA-3·MA-4·MA-5 로 절 제목 경계를 문자열 `\n## 휴면 디렉터리` 보존 여부 하나로 확정(꼬리 제거·접미 추가는 조용, 접두 내부 공백만 문다). **HP-1 비차단 분류는 결론이 옳고 근거가 틀렸다** — 계약(HANDOFF §지금의 계약 · SoT v1.8.64)이 요구한 것은 *행을 남기는가* + `idempotent_replay` 단정 둘뿐이고 replay 행의 action 리터럴은 계약 침묵이다(형제 셀이 처방을 초과했다는 것은 근거가 못 된다). 수치 전건 재측정 일치(기준선 여섯 · 셀 수 20→21·55→56·2→3 · `--collect-only` 3064 · 문서 가드 948→950 양단 · 건수 네 자리 310 · 분포 211/94/5) · 범위 밖 0. **★ 차단 AC1: 인덱스 판정 열 `합격` ↔ 선행 기록 본문 `조건부 합격` — 디스크 310건 전수 대조에서 유일한 불일치이고, 바로 앞 커밋이 같은 상황을 4/4 로 반대로 처리했다.** **조건 AC1 은 폐쇄 커밋 `88a16fe`(같은 세션의 폐쇄 보고 — 패턴 스윕으로 찾은 선재 5건은 `a1d71fc` 가 정합)가 닫았고, 승격 재검([`activity_replay_ac1_promotion.md`](2026-09-13/activity_replay_ac1_promotion.md))이 변이 재적용 열 회로 확증해 판정을 합격으로 올렸다.** | **합격** |
| [`activity_replay_c1_promotion.md`](2026-09-12/activity_replay_c1_promotion.md) | **활동 로그 D1=ⓑ · 문서 D1=ⓐ 승격 재검 — 조건 C1 폐쇄(`70efc23` · 기록 `d6e381e`, SoT v1.8.65)를 변이 재적용 열세 회로 확증.** **MP-1(=MB-8)·MP-2(=MB-9) 각 1 재실패 = 특성 셀 그 자체** — 발행 검증에서 236셀 전건 초록이던 두 자리다. **세 갈래 분리 소유 확인**: finalize 변이(MV-1·MV-2 재적용)는 자기 셀만 물고 신규 둘을 안 건드린다 → *"D2 를 다음 슬라이스가 코드 두 줄로 조용히 풀 수 있다"* 는 차단 사유 해소. **H3 가드의 세 주장 전부 성립**(삭제는 문다 · 절 밖 미끼 안 통한다 · 정상 문장 다듬기는 안 문다). 깨려는 프로브 넷: 처분표 변이는 **다른 층**(`KeyConsumptionTest` 2셀)이 흡수 · **덤으로 MB-7 의 절반이 닫혔다**(절 제목 접두가 사라지면 문다 — 꼬리만 바꾸면 여전히 조용해 H1 은 *좁혀진* 것이지 닫힌 것이 아니다). 셀 수 셋·기준선 산술(`--collect-only` 3064)·건수·문서 가드(**변이 전 트리 `2e025dc` 기준 948**) 전건 일치 · 범위 밖 0. **신규 하드닝 HP-1**: 수동 저장 특성 셀이 행 *수* 만 세어 **replay 가 다른 action 을 남겨도 238 전건 초록**(형제 accept 셀은 문다 — 같은 슬라이스가 낳은 두 셀의 비대칭, 처방 한 줄). **선행 기록 판정 조건부 합격 → 합격 승격.** | **합격** |
