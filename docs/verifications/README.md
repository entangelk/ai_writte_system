...
> 경계가 칼같이 갈렸다 — **2026-07-06~08-23 구간은 264개 중 1개만** 해소되고, 2026-08-24 이후는 272개 중 269개가
> 해소된다. **해시가 안 잡힌다고 그 기록의 근거를 의심하지 말 것**(실제로 한 검증 세션이 이 이유로 시간을
> 썼다). 옛 해시는 **고쳐 쓰지 않는다** — 그때의 사실이고, 이 저장소는 이력 문서를 고치지 않는다.
>
> **★ 위 수치는 유도 명령 없이 발행됐고 독립 재현되지 않는다**(독립 검증 2026-09-13 H2). 원 측정이 밝힌 방법
> (*`docs/verifications/**/*.md` 의 `` `<7~40 hex>` `` 토큰을 날짜 디렉터리로 묶어 `git cat-file -e <h>^{commit}`*)
> 으로 재측정하면 **06-24~07-05 69/70 · 07-06~08-23 1/371 · 08-24 이후 229/239**(미해소 총계 376종)다.
> **분자 `1` 과 경계의 날카로움은 정확히 재현**되고 총계도 1 차이라 위 문단의 주장은 성립하지만, **분모는 어떤
> 추출 변형으로도 맞지 않는다** — 수치를 인용할 때는 아래 명령으로 직접 유도할 것(수치가 움직이면 환경이 아니라
> 분류 규칙이 달라진 것이다).
>
> ```bash
> python3 - <<'PY'
