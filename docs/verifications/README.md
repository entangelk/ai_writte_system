...
| 판정 | 건수 | 뜻 |
|---|---|---|
| 합격 | 211 | blocking 결함 없음 |
| **조건부 합격** | **94** | 합격이되 닫아야 할 조건이 있었다 |
...
| 기록 | 요약 | 판정 |
|---|---|---|
| [`activity_replay_c1_promotion.md`](2026-09-12/activity_replay_c1_promotion.md) | **활동 로그 D1=ⓑ · 문서 D1=ⓐ 승격 재검 — 조건 C1 폐쇄(`70efc23` · 기록 `d6e381e`, SoT v1.8.65)를 변이 재적용 열세 회로 확증.** **MP-1(=MB-8)·MP-2(=MB-9) 각 1 재실패 = 특성 셀 그 자체** — 발행 검증에서 236셀 전건 초록이던 두 자리다. **세 갈래 분리 소유 확인**: finalize 변이(MV-1·MV-2 재적용)는 자기 셀만 물고 신규 둘을 안 건드린다 → *"D2 를 다음 슬라이스가 코드 두 줄로 조용히 풀 수 있다"* 는 차단 사유 해소. **H3 가드의 세 주장 전부 성립**(삭제는 문다 · 절 밖 미끼 안 통한다 · 정상 문장 다듬기는 안 문다). 깨려는 프로브 넷: 처분표 변이는 **다른 층**(`KeyConsumptionTest` 2셀)이 흡수 · **덤으로 MB-7 의 절반이 닫혔다**(절 제목 접두가 사라지면 문다 — 꼬리만 바꾸면 여전히 조용해 H1 은 *좁혀진* 것이지 닫힌 것이 아니다). 셀 수 셋·기준선 산술(`--collect-only` 3064)·건수·문서 가드 948 subtests 전건 일치 · 범위 밖 0. **신규 하드닝 HP-1**: 수동 저장 특성 셀이 행 *수* 만 세어 **replay 가 다른 action 을 남겨도 238 전건 초록**(형제 accept 셀은 문다 — 같은 슬라이스가 낳은 두 셀의 비대칭, 처방 한 줄). **선행 기록 판정 조건부 합격 → 합격 승격.** | **합격** |
| [`activity_replay_and_dormant_docs.md`](2026-09-12/activity_replay_and_dormant_docs.md) | **오너 결정이 남긴 작은 후속 둘 — 활동 로그 D1=ⓑ(`7979ff2`) · 문서 D1=ⓐ(`68aedf3`), SoT v1.8.64 독립 검증.** 두 결정의 *뜻* 은 **실행으로** 성립한다 — 검증자 자작 프로브가 세 경로를 같은 자로 재 finalize **1행** · accept **2행** · 수동 저장 **2행**(구현자 셀·하네스 미재사용). **구현자의 반증 주장(*"accept 도 replay 에 행을 남긴다"*)은 참**이고 세 갈래로 확인했다(행위 · `KEY_REPLAY_ACTIONS` 둘 다 `"handler"` · 인용된 2026-08-09 기록 §6-② 원문이 처음부터 반대). **보고 변이 5종 diff 재적용 전건 일치**(1·1·1·2·39초록). 지시가 깨라 한 가설은 **MB-1 이 반증** — `X-Confirm-Duplicate` 를 빼면 입장이 **429** 로 시끄럽게 실패하므로 셀은 실제로 handler 분기에 도달한다. 범위 밖 변경 0(훅 단위 확인) · 기준선 `--collect-only 3061` 산술 일치 · 구현 세션이 두 리비전 통제 대조로 **앞 슬라이스의 유도 오류(4213→4215)를 잡아 고친 것**도 확인. **차단 C1**: SoT v1.8.64·HANDOFF·브리프가 새로 계약으로 적은 replay **세 갈래 중 둘**(accept 유행 · 수동 저장 유행)이 **무셀** — MB-8·MB-9 가 각각 **236 passed / 694 subtests 전건 초록**이라 **오너가 유예로 명시한 D2 를 다음 슬라이스가 조용히 풀 수 있다**(이 브리프의 원래 근거가 거짓이 된 것과 같은 병). 처방은 특성 셀 둘(v1.8.61 `WithdrawalCancelAfterPurgeClaimTest` 선례). 하드닝 다섯(`.md` 아닌 링크·앵커 무가드 · `_DORMANT_DIRECTORIES` 수동 목록과 `glob("*/*.md")` 깊이 · 지시 문장 무셀 · 파일 이동 무셀 · `drafts.py` 동일 앵커 2곳). 변이 **15종**(재적용 5 + 검증자 10) 전부 탈치 워크트리 — 공유 트리에 `git checkout` 을 쓰지 않았다. **조건 C1 은 같은 날 `70efc23` 이 특성 셀 둘로 닫았고, 승격 재검 [`activity_replay_c1_promotion.md`](2026-09-12/activity_replay_c1_promotion.md) 가 변이 재적용 열세 회로 확증해 판정을 합격으로 올렸다.** | **합격** |
| [`landing_l3_closure_promotion.md`](2026-09-12/landing_l3_closure_promotion.md) | **랜딩 ③·④ · S-2 승격 재검 — 폐쇄(`cbd88f7`)를 변이 재적용으로 확증.** 셋 전부 폐쇄 보고와 일치: **MV-A** 1실패(쓰기면 왕복 셀 — 발행 검증 277셀 전건 초록이던 자리) · **MV-B 5실패**(타축 legacy 넷 + **동의 legacy 셀 기명 포함** — 발행 검증이 실측한 "동의 이름 셀 0"의 갭이 메워짐) · **C2 변이** 1실패(확인란 문구·새 탭 셀). 기준선 `test_auth_users_mongo` **27 passed**(폐쇄 보고 25→27 일치) · `App.test.tsx` 34셀. **선행 기록 판정 조건부 합격 → 합격 승격 — 이로써 승격 재검 백로그 넷(L3·Slice 5·Slice 4b·최종 저장 6차)이 전부 닫혔다.** | **합격** |
| [`slice5_closure_promotion.md`](2026-09-12/slice5_closure_promotion.md) | **계정 탈퇴 Slice 5 승격 재검 — 폐쇄(`dfc455a`, SoT v1.8.61)를 변이 재적용으로 확증.** 넷 전부 기명 셀 일치: MX-D 1실패(`test_the_document_exists_and_carries_pinned_rows` — 발행 검증 10/70 초록이던 자리를 H3 하한이 물었다) · MX-I 1실패(`test_the_undisclosed_restriction_keeps_its_row_while_code_enforces_it` — H2 양방향) · **B1-ⓐ 방향** 1실패(특성 셀 `WithdrawalCancelAfterPurgeClaimTest` — ⓐ 선택 시 셀·§6 문장이 함께 뒤집힌다는 폐쇄 설계의 실증) · MU-A 7실패(발행 기록의 셀 짝 전건 일치: §6 축 1 + 문서 2 + 핀 1 + 경계 3). B2(grep 0건 · `cmp` ×2 · legalSource 4 passed)·B3(§8 "열린 시행 전제 0" 자기모순 해소)는 상태 확인. 정책 가드 기준선 **11셀/70 subtests**(폐쇄 보고와 일치). **선행 기록 판정 조건부 합격 → 합격 승격.** | **합격** |
| [`slice4b_closure_promotion.md`](2026-09-12/slice4b_closure_promotion.md) | **계정 탈퇴 Slice 4b 승격 재검 — 폐쇄(`a460237`, SoT v1.8.63)를 변이 재적용으로 확증.** 다섯 전부 기명 셀 일치: M3 2실패(C1 셀 둘 — 발행 검증에서 184통과/1322서브 전건 초록이던 자리) · M6 1실패(C2) · **M5b 2실패**(계정 2단계 셀 + C3 로 추가된 프로젝트 purge 무변 셀 — 과교정 방향이 부활했고, 폐쇄 보고의 "1셀"은 새 셀만 세어 적은 것) · M1·M4 각 2실패(원본 잠금 생존 확인). 하드닝 H3 인용부호 원복 유지(곡은 2건 실측). 기준선: reconcile 16통과 · `test_auth_api` 153/1222(발행 검증과 동일 수치) · 관리 프런트 18통과. **선행 기록 판정 조건부 합격 → 합격 승격.** | **합격** |
| [`final_save_b1_promotion.md`](2026-09-12/final_save_b1_promotion.md) | **최종 저장·분석 연동 6차(승격) 재검증 — B1 폐쇄(`0a2fee4`)를 변이 재적용 양방향으로 확증.** 기준선 `DraftEditor.test.tsx` **65 passed**(B1 셀 포함 — 5차 68 과의 차는 이후 슬라이스 증분). **MV-D(over) 1실패 = B1 폐쇄 셀 그 자체**(5차에서 프런트 전수 418 전건 초록이던 변이) · **under(배지 3상태 파괴) 2실패**(5차 M1 셀 + B1 셀) — 폐쇄 셀 하나가 계약 분기의 양면(저장은 나간다·배지는 전이한다)을 함께 잠근다는 처방의 실증. **5차 판정 조건부 합격 → 합격 승격 — 재검증 축(1~6차) 종결.** | **합격** |
| [`admin_residual_purge_slice4b.md`](2026-09-12/admin_residual_purge_slice4b.md) | **계정 탈퇴 Slice 4b — 관리자 잔여 정리(`73ae7f7`…`9806b7a`, SoT v1.8.62) 독립 검증.** 구현은 계약 전 축 성립 — 본체 한 벌(스크립트 질의·조건·출력 키 무변 이관 — diff 줄 대조 + 모듈 잔류 0)·라우터 404/409/400 + 감아 2단계 fail-closed·outcome best-effort·등재 전부(tier 107/admin 19·선언 지도 19·EXCLUDED 32 POST 만·gen:api 무차분 실측)·프런트 ②ⓐ 흐름(stalled 게이트 → 조사 → 사유+사용자명 확인 → 실행 → 결과, CSS 신규 0). 뮤테이션 10종 중 7종 기명 셀 정확히 재실패(M1·M2 조건 파괴 2씩 · M4 감아 요청행·M5 상속·M8 사용자명 확인·M9 stalled 게이트·M10 본체 우회 각 1-2). **그러나 셋이 무셀이었다(전부 변이 실증)**: **C1** POST 측 스탬프 재확인 제거 → 184통과/1322서브 전건 초록(SoT "*양쪽이* 호출 직전에 재확인"의 실행 쪽 절반 무셀) · **C2** `list_destructive_events` 에 member_quota 누수 → 171통과 전건 초록("계속 이 화면 밖" 무셀) · **C3** outcome 상속 과교정(프로젝트 purge 축 오염) → test_auth_api 만으로 초록("프로젝트 purge 는 None 이라 무변" 무셀). 하드닝 여섯(503 사실상 죽은 분기 · dry-run survey 이중 호출 · 곡은→직은 인용부호 범위 밖 변경 두 줄 · payload target_user_id 부재(스펙 침묳) · 스크립트 main 무셀(이전과 동일) · InMemory 저장소 분리(구현 인지)). 스크립트 dry-run 스모크는 이 환경 Mongo 접속 불가로 대체 확인. 초점 실측: 백엔드 199통과/1419서브 · 프런트 20통과(전수는 지시대로 미실행). **조건 셋은 폐쇄 커밋 `a460237`이 닫았고(기록 §폐쇄 보고), 승격 재검(위 첫 행)이 변이 재적용 5종으로 판정을 합격으로 올렸다.** | **합격** |
| [`landing_l3_gate_and_landing.md`](2026-09-12/landing_l3_gate_and_landing.md) | **랜딩 ③ 본문 · ④ 가입 동의 게이트 · S-2 nginx 헤더(`58a8ac9`·`86a4de7`·`f36c7bd`·`b6fd464`, SoT v1.8.59) — 세션 68 산출 독립 검증.** 핵심 축 전부 성립: 400 단일 얼굴(HTTP 셀 `400_not_422`)·서버 전용 스탬프(시계+상수, 재요청 새 스탬프·관리자 None 서비스 셀 5)·**버전 핀 두 곳이 import 로 상수를 가리킴**(양쪽 확인)·프런트 미체크 이중 잠금(강제 클릭 미발송까지)·랜딩 네 얼굴 LB1(**진짜 App/AuthGate 를 지남**)·만료 건너뜀·딥링크 제외·nginx server 레벨·`always`. **변이 6종 신규 설계**(구현자 8종과 각이 다름) — MV-C 상수 "1.1" → 문서 핀+바디 리터럴 3실패 · MV-D 한계 항목 제거 → 1 · MV-F `!sessionExpired` 제거(over) → 만료 셋 3 · MV-E 헤더 location 이동 → 2 SUBFAILED. **★ MV-A 가 조건을 열었다**: `users_mongo.py` 쓰기면(`_doc`)에서 동의 필드 두 줄을 지워도 **277셀 전건 초록** — 방침 제3조 "기록" 의무의 Mongo 영속이 무셀이다(읽기면도 동의 이름 셀 없음 — MV-B 의 4실패는 타축 legacy 셀의 우발 포착). 같은 파일의 C-6(2026-08-02)·탈퇴축 H1(2026-09-08) 선례만 있으면 되는 자리. **조건 ②**: 확인란 문구의 버전 표기·약관·방침 링크 새 탭이 HANDOFF 10 완료 문언의 계약인데 무셀. 하드닝 셋(로그인 폼 h1 무핀 · 랜딩 벤더 금지 부정단정 무셀 · gen:api 드리프트). 전수 수치는 트리 이동(Slice 5 커밋)으로 재실측 불가 — 귀속 산술(+4 it/+6 def/+2 nginx 셀)로 기록과 대치해 모순 없음. 검증 중 다른 AI 트리 간섭 관측·회피(워크트리+cmp 원복) 기록. **조건 C1·C2는 폐쇄 커밋 `cbd88f7`이 닫았고(기록 머리 폐쇄 보고), 승격 재검(위 첫 행)이 변이 재적용 3종으로 판정을 합격으로 올렸다.** | **합격** |
| [`account_withdrawal_slice5_promotion.md`](2026-09-12/account_withdrawal_slice5_promotion.md) | **계정 탈퇴 Slice 5 — 정책 문서 §8 → §6 승격(`e169e96`, SoT v1.8.60) 독립 검증.** 승격의 근거는 재확인됐다 — §6 새 문장 다섯 중 넷(유예 중 권한·재요청 멱등·판정 경계 `>=`·파기 뒤 잔존)이 코드에서 성립하고 유료 경로 **11 POST 전건**이 유예 가드를 quota 앞에 둔다. 신규 조항↔상수 셀은 물었다(**MU-A** 7실패 = §6 행 1 SUBFAIL + 신규 셀 2 SUBFAIL + 핀 1 FAILED + 경계 3 · **MO-B** 1 SUBFAIL · **MO-C** 10/70 초록). **핀 존치 논증은 검증자 변이 MU-C2 가 실증**했다 — 문서와 상수를 *함께* 60일로 옮기면 정책 가드 10/70 전건 초록이고 값을 드는 것은 핀뿐이다(선례 `MIN_PASSWORD_LENGTH`·`SCENE_NOTE_MAX_CHARS` 도 행+핀 둘을 든다). `미기재` 행의 사실성도 독립 확인(약관 제4조 4항은 *정지* 축이라 인용 불가 — 문언이 새로 필요하다). **차단 셋**: **B1** §6 *"취소는 파기가 시작되기 전까지"* 가 무시행·무셀(`purge_started_at` 찍힌 계정의 취소가 200 으로 통과 — 실측) · **B2** 방침 머리말의 *"제3조 안내 상자는 아직 기능이 제공되지 않습니다"* 가 거짓인 채 회원 화면에 있고 HANDOFF 는 그 정리를 Slice 5 몫이라 적은 채 Slice 5 를 닫았다 · **B3** `legal/README.md` §8 이 자기 표와 모순(*남은 전제 한 줄* vs 전제 *없음*). 하드닝 다섯(재로그인 무셀 · `미기재` 행·§6 산문 무셀 실측 · 행 하한 `>=10` vs 실제 14 · 잔존 축 셋 중 둘만 열거 · 앵커/등가표현 두 진단 등재). 수 전건 일치: 9셀/66 → **10셀/70** · 초점 **110/1008** · 바이트 가드 4/4. **조건 셋은 폐쇄 커밋 `dfc455a`가 닫았고(기록 §폐쇄 보고), 승격 재검(위 첫 행)이 변이 재적용 4종으로 판정을 합격으로 올렸다.** | **합격** |
| [`landing_l2_promotion.md`](2026-09-12/landing_l2_promotion.md) | **랜딩 축 ② 승격 재검 — 세션 60 폐쇄(`7be433e`, SoT v1.8.57)를 변이 재적용으로 확증.** 여섯 종 전부 기록과 일치: MO-6a 1실패(LB1 셀 — 종전 전건 초록) · MO-6b 1실패(같은 셀) · **ML-3b 2실패 = 기존 두 셀**(세 얼굴 분할 점유의 재현) · MH-3a 1실패(H3 셀) · MH-3b over 2실패(H3+링크 셀) · **MO-8 등가 45/45 초록**(경계 유도 방식 비고정). **선행 기록 판정 조건부 합격 → 합격 승격.** 같은 세션이 백엔드 전수로 회귀 기준선을 확인: **3026/1/4156 EXIT=0** — 선행 유도 4154 의 +2 는 Phase W 새 문서 둘(`test_repo_hygiene` 문서당 1)로 귀속, 문서 가드 7종 59/1138 교차 확인. **유도값이 낡은 것이지 회귀가 아니므로 기준선은 4156.** | **합격** |

### 2026-09-11

| 기록 | 요약 | 판정 |
|---|---|---|
| [`landing_l2_terms_privacy.md`](2026-09-11/landing_l2_terms_privacy.md) | **랜딩 축 ②(약관·방침 페이지 + 공개 푸터, `d8bae4a`·`95f2592`·`83e335e`, SoT v1.8.56) — 세션 58 산출 독립 검증.** D2=ⓐ의 핵심인 **사본↔정본 바이트 가드가 양방향임을 신규 실증**(사본만 ML-1 1실패 · **정본만 ML-9 1실패** · 감시 축소 ML-7 1실패)하고 라우팅·렌더러 종료·링크·메타·번들(457.22/139.18 · 청크 17.40/6.37 SoT 와 소수점 일치)·전수 **476/41**·문서 가드 **58/1110**·세션 58 변이 재적용 6종 전건 일치. **그러나 신규 반증 MO-6a·6b 로 갭 하나**: SoT 가 *"정문 세 얼굴"* 로 열거한 푸터 장착 지점 중 **AuthStatus(세션 확인·오류)와 가입 접수 둘이 무셀** — 각각만 제거해도 legal 14 + App 29 전건 초록(세션 56 B1 같은 형태의 재발). ML-3 의 "2실패"가 그 신호였다(실패 셀 둘 다 로그인 폼 얼굴). 하드닝: 편집 메모 재유입(양쪽 일관) 무가드(ML-10 초록) · `근거:` 필터가 문서 전체에 적용 · **H1 시계 플레이크 2회째 관측**(25분 창 journal Clock change 50회). **조건 LB1 은 같은 날 세션 60(`7be433e`, SoT v1.8.57)이 닫았고(하드닝 H3 동반 — 기록 §폐쇄 보고), 2026-09-12 승격 재검(위 행)이 변이 재적용으로 재현해 판정을 합격으로 올렸다.** | **합격** |
| [`withdrawal_slice4_promotion.md`](2026-09-11/withdrawal_slice4_promotion.md) | **계정 탈퇴 Slice 4 승격 재검 — 세션 57 폐쇄(`b52633d`, SoT v1.8.55)를 변이 재적용으로 확증.** 여섯 종 전부 기록과 일치: MV-2b 1실패(B1 셀) · MV-2 1실패(첫 측정 3 = 진짜 1 + H1 플레이크 2, 재측 1) · **MV-2d 등가 13/13 초록**(줄 위치가 아니라 *셸이 거는가*) · MV-7 2실패(수단 단정 + 실패 경로 셀) · MV-11·MV-12 각 1실패. 폐쇄↔처방 대조: B1 은 처방 그대로(실측 전제부터 세우고 남은 일수를 안 재 H1 을 안 끌어들임), B2 는 처방을 넘어 재조회 실패 경로까지. **선행 기록 판정 조건부 합격 → 합격 승격.** | **합격** |
| [`withdrawal_slice4_screen.md`](2026-09-11/withdrawal_slice4_screen.md) | **계정 탈퇴 Slice 4(화면, `b68d923`, SoT v1.8.54) — 세션 55 산출 독립 검증.** 구현 자체는 계약 전 축 일치(409 두 뜻 분할의 전제를 백엔드에서 재확증 — POST 409=`LastActiveAdmin`·DELETE 409=`WithdrawalNotRequested` 각각 하나뿐 · 남은 일수 산술 · 상태 한 벌 · 확인 가드 · CSS 토큰 · `AuthUserContext` 소비자·`schema.d.ts` 무변·`gen:api` 무차분 전부 실측). 변이 재적용 6종 중 **5종 일치(MV-1·3·4·5·6)**, 그러나 **MV-2(배너 `/me` 한정)는 프로덕션 장착 지점(AuthGate)에서 0실패 — 세션 55 기록 "5실패" 재현 불가**(컴포넌트 내부 변이로는 2실패). **신규 반증으로 갭 둘**: MV-2b(**AuthGate 에서 배너 통째 제거 → 3파일 52셀 전건 초록** — SoT "셸에 있다"의 장착 지점 무셀) · MV-7(**취소 409 재조회를 로컬 합성으로 치환 → 11/11 초록** — SoT "서버의 지금 상태를 다시 읽어" 무셀). 산술 리터럴은 잘 잠깐(ceil→floor 4실패 · Math.max 제거 1실패) · CSS 원색 리터럴은 프런트 `designTokens` 가 물음(백엔드 provenance 가드는 `:root`만 봐 초록 — 가드 분업 실측). 전수 재현: 프런트 **460/39 EXIT=0** · tsc 0 · build 성공 · 문서 가드 36/1018. **환경 플레이크 신규 등재**: 새 셀 픽스처가 모듈 스코프 `Date.now()+3일` 이라 WSL2 부팅 창 시계 뒤점프에 "3일→4일" 로 깨짐(실패 창 journal "Clock change detected" 11회 · 재실행 초록) — 미수리 표. **조건 둘은 같은 날 세션 57 이 닫았다**(`b52633d` · SoT v1.8.55 · 프로덕션 코드 무변, 셀 11→13 — 기록 §폐쇄 보고): 진짜 `AuthGate` 를 세우는 셀 하나가 MV-2b·MV-2 를 물고(등가 재배치 MV-2d 는 초록), 409 셀이 재조회 **호출 자체**를 센다(+재조회 실패 경로 셀). **승격 재검은 다음 검증 세션 몫**이라 판정은 아직 조건부 합격이었다. **같은 날 승격 재검(위 둘째 행)이 판정을 합격으로 올렸다.** | **합격** |

### 2026-09-10

| 기록 | 무엇을 반증했나 | 판정 |
|---|---|---|
| [`slice3_promotion_recheck.md`](2026-09-10/slice3_promotion_recheck.md) | **승격 재검 2차 — 세션 52 산출 셋(`4f451c4`·`3644147`·`7f30027`)을 오너 지시("의심하고 또 의심")로 다시 뜯었다.** **① 변이 재표집** — MU-20(정지 배선 인자 삭제 → 43셀 전건 초록)·MU-8(정지 확인 두 줄 삭제 → 기명 셀 정확히 1 실패)이 주장 그대로 재현; 선례 인용(`test_index_sync_worker_script.py:294·341`)·대역 버림(`test_account_withdrawal_worker.py:110`)·production 배선(`account_withdrawal_worker.py:257-259`) 전부 1차 소스와 일치. **② 양단 산술 독립 재현** — `c1cb282` 워크트리 305/606 ↔ 현행 306/607(+2=4119). **③ 전수 재실행에서 EXIT=0 은 재현 못함** — 1 failed(`final_save S13 unknown draft status` expected=404 observed=503)·3022 passed·1 skip·4119 subtests(1712초). 셀 총계·skip·subtest 는 일치하고 단독 재실행 초록 + 코드 동일성(`7e07b37` 이후 전부 문서 커밋)으로 **전수 한정 백엔드 플레이크**로 판정, 미수리 표에 등재(503 은 SoT v1.7.38 전역 저장-오류 얼굴). **④ 잔류 둘을 고쳤다** — 루트 README 절차 표 ②행 subtest 4,117(낡음)·**이 인덱스 자신의 분포 문장 33%**(승격 커밋이 루트 백분율만 고침 — 실측 32%). 가드가 지키는 표 바깥의 산문 숫자는 등재 슬라이스가 센다. 승격 자체는 전건 확증. | **합격** |
| [`slice3_closure_promotion.md`](2026-09-10/slice3_closure_promotion.md) | **승격 재검 — 세션 51 폐쇄 커밋 둘(`b23f689`·`7e07b37`, SoT v1.8.52).** 조건을 닫은 세션이 자기 판정을 못 올리므로 남아 있던 미검증 구간을 독립 세션이 다시 뜯었다. **① 처방 대조** — B1 은 처방(토큰 부재)보다 넓게(머리말 버전 **줄 전체** 대조까지), B2·B3 은 처방 그대로 닫혔다. **② 지정 변이 재적용(MU-7·8·12~16 + over MU-7b·8b) 아홉 종이 전부 세션 51 이 적은 기명 셀·개수 그대로 재실패** — 한 셀이 여럿을 흡수한 자리 없음. **③ 반증 시도 신규 여섯** — **반사실 MU-15r(가드만 옛 형태로 되돌리면 초록)** 로 조임이 하중을 받음을 실증하고, MU-17·18·19·22 로 새 셀들이 주장보다 넓게 묾을 확인(H1 셀은 정지 확인의 **위치**까지 잠근다). **★ MU-20 이 새 갭을 열었다 — `run_loop` 이 `stop_check` 를 `run_once` 로 넘기는 배선이 무셀**(대역이 인자를 받고 버린다). 형제 워커 `index_sync_worker` 는 같은 축을 `assertIsNotNone(last_stop_check)` 로 잠근다 — 복사할 선례가 저장소에 있다(비차단 H1'). 전수 **3023/1/4117** 셀·skip·subtest 전건 재현(1618초 — 소요는 머신-로컬). 법률 축 변이 넷은 `SUBFAILED` 로 나온다(`FAILED` 만 걸면 오독). | **합격** |
| [`account_withdrawal_slice3_legal_effective.md`](2026-09-10/account_withdrawal_slice3_legal_effective.md) | **세션 49 산출물 둘 — 계정 탈퇴 Slice 3(`0919f95`·`8f182bf`·`e9110a9`, SoT v1.8.51) · 약관·방침 시행 표기(`559c310`·`b6c5aa5`·`c8f106e`, v1.8.50).** 구현자 신고 약점 다섯을 전건 조사 — **① 실 mongod 파기 경로를 워커 진입점부터 재현(21/21 단정 · 분리 DB · compose 최소 env만으로 조립 성립)**: 청구→묘비→본체→스윕→계정 행·reconciler 수습 3단계·naive→UTC 재라벨링 전부 무결. **② 실 개발 DB `_id` 충돌 0건**(사용자 4·컬렉션 35) — `user:<hex>` 안전 근거 실데이터 성립. 스윕 도달 범위 완전 열거(제3축 `request_locks`·`quota_replay_responses` 는 TTL 자정리, 무-TTL 잔류는 `login_failures` 뿐). **③ 수습 통로는 현 단계 수용**(Slice 4 가 화면을 맡는다). **④ 변이 신규 5종** — 새 셀 교합 재확인 3종은 각 1실패로 물고, **MU-7(스윕↔계정 행 순서 교환)·MU-8(stop_check 제거)이 초록 통과해 갭 실증**(구현자가 예고한 "MU-1·MU-3 계열" 잔존). **⑤ 약관 축에서 차단 2** — **부칙에 `draft-0 (미시행)` 잔류**(양 문서 — 머리말 `시행·1.0`과 자기모순, 결합 가드는 옛 상태줄 정확 문구만 봐 새 간다) · **제5조 1항 4,000자**(시행값 6,000자, 상향 `97bc149` 미반영 — 다른 모든 숫자는 일치). **차단 셋째 = B3 파기 순서 4→5단계 미잠금**(MU-7 — 순서가 바뀌면 스윕 실패 시 표식이 사라져 reconciler 가 잔류를 영영 못 찾는다). 전수 **3017/1/4114**(주장과 셀·skip 일치 · subtest +1은 워크트리 대조로 재현 스크립트 회계로 귀속). 하드닝 6(stop_check 무셀 · one-shot apply 무셀 · work_log auth 묶음 262/1207 재현 불가(실측 241/1205) · HANDOFF 기준선 줄·health 노트 낡음 — 검증자 갱신). **조건 셋은 세션 51 이 닫았고, 2026-09-10 승격 재검(아래 행)이 변이 재적용으로 재현해 판정을 승격했다.** | **합격** |

### 2026-09-09

| 기록 | 무엇을 반증했나 | 판정 |
|---|---|---|
| [`slice2_withdrawal_grace_closure.md`](2026-09-09/slice2_withdrawal_grace_closure.md) | **Slice 2 종결(세션 45 수령) — 커밋 4건(`7e73233`·`19e422b`·`b4b1d7c`·`5288b7c`) + 오너 정정 `79c065a`.** 주장 전량 재측정 — 전수 **2963/1/4086 EXIT=0**(1932초)·초점 **281/1773**·tsc rc=0·문서가드 **24/899** 전부 일치. 변이 표본 3종(방향 under·over·순서) 재적용: MS2-7 **1실패(`403 != 404` — 핀 셀, 오너 재현과 동일)**, MS2-4 **11 SUBFAILED**, MS2-6 **17실패** — 세션 45 표 정직. 증분 산술 worktree 양단 실측으로 성립: 수집 2955→2964(+9 = 추가 10 defs − 개명 1, 개명은 몸무변 — 오너 코드 커밋 +0 증명), 초점 272/1600→281/1773(+173). S9 6001 경계·재현 스크립트 임포트(`test_final_save_analysis.py:18`) 실증. 하드닝 2(비차단): `core_sot/service.py:102` 주석이 "= 4000의 3배"로 낡음(97bc149 후폭 — 값 12000 자체는 `test_scene_notes.py` 독스트링이 올바르게 문서화) · 검증 인덱스 단면 헤더가 2026-09-08 기록 둘을 "### 2026-09-07" 아래에 둔 채였음(가드는 도달성만 검사 — 이 기록 등재 시 새 단면으로 정리). | **합격** |
| [`session48_unranked_warning_and_shortlist_wiring.md`](2026-09-09/session48_unranked_warning_and_shortlist_wiring.md) | **세션 48(커밋 6건 — 구현 `e82c9ed`·`7a34e21` + 브리프 2건 + 기록).** 전수 **2975/1/4091 EXIT=0**(2313초) 재현 — 보고 수치와 완전 일치. 변이 **6/6 기명 재실패**(구현자 5종 재유도 + 검증자 자체 축: 상한 break 삭제 → `test_limit_caps…` 적중). 실측 재측정 전부 일치(memories 0·candidates 0·jobs 0·source_blocks 640·drafts 12 · 컨테이너 env `CHROMA_HOST`·`EMBEDDING_SERVICE_URL` 있음/`ANALYSIS_SEMANTIC_MATCH_THRESHOLD` 없음). ★ fake-index 테스트가 못 보는 면을 형 검사로 닫음 — 실제 Chroma `query_similar` 시그니처가 구조적 Protocol과 정확히 일치·읽기 투영=색인 쓰기 투영(`candidate_index.py:145`)·조립 지점 유일(`main.py:904`). **차단 2: dedup 브리프 상태가 `Proposed — 오너 결정 대기`로 낡음**(ⓒ 채택·구현 완료 — plans 인덱스 행도 같이 낡음, 문서↔인덱스 일치 가드의 맹점) · 루트 `README.md:104` 전수 기준선이 세션 47 값(2964/4089)에 멈춤(종결마다 갱신하는 관례 — 가드 밖). 하드닝 3(비차단): env K 파싱 엣지 · work_log 하드캡 인용 줄좌표가 e82c9ed 이전 기준 · shortlist 실데이터 미실행(dev 스택 env 이미 충족 — 재빌드 즉시 활성화). | **조건부 합격** |

### 2026-09-08

| 기록 | 무엇을 반증했나 | 판정 |
|---|---|---|
| [`account_withdrawal_slice1_api.md`](2026-09-08/account_withdrawal_slice1_api.md) | **계정 탈퇴 Slice 1 — 셀프 요청·취소 API(`5ce06f3`·`374d93f`·`23235d6`·`40c1a77`, SoT v1.8.46).** 구현 주장 전부 재현 — 초점(검증자 실행) **283/1244**·전수 **2954/1/3900 EXIT=0**(2937+17 정확)·tsc rc=0. **403 부재가 계약**(행위+선언 두 축 — MW-3 의 6 subtests 구성까지 일치)·활동 비등재 사유(project 축·admin_audit 축) 원문 확인 — 계획서 두 줄 정정 정당, 오너 재판단 불요 동의. **등재 체크리스트 6축 전부 갱신**(tier 102→104 등, 낡은 표기 스윕 0건). **이전 검증 H1·H2 폐쇄 실증**(X1 재실행 1실패·MW-4 1실패 — 구현자가 자기 실토한 MW-4 무가드의 폐쇄 확인). 변이 9종(구현자 6+대항 3) 전부 기명 셀 재실패. 비차단 기록 정확성 3: 초점 "277"·MW-1 "2"는 H2 셀 커밋 **이전 트리 측정치**(최종 트리 283·3) · **CHANGELOG 행 부재 2슬라이스 연속**(D4 선례 있음 — 묶음 행 1개 권고). | **합격** |
| [`account_withdrawal_slice0_state_axis.md`](2026-09-08/account_withdrawal_slice0_state_axis.md) | **계정 탈퇴 Slice 0 — 상태 축(`9e2bb94`·`10845f9`, SoT v1.8.45).** 구현 주장 전부 재현 — 초점 **229/1054**·전수 **2936/1/3876 EXIT=0**(산술 2903+7(중간 커밋 `git diff` 실측)+26 ✓). 경계 행렬 **26/26 빈 칸 0**(전이 양방향·29/30/31일·1μs·30일 핀·Mongo 5). **구현자 변이 7종 독립 재유도 — 전부 기명 셀 재실패, 셀 수까지 일치**(V5 "3실패"를 의심했으나 레거시 행이 세 키를 전부 결핍해 C-6·status 선례 셀까지 함께 물리는 것이 정답 — 표 정직). **검증자 대항 X1(`_doc` 탈퇴 필드 등재 제거)만 0실패** → 하드닝 H1(쓰기면 왕복 셀, Slice 1)·H2(첫 시각 원자화 — 읽기-쓰기 경쟁). ★ 계획서 §6 승격 Slice 5 이관의 심판 근거(정책 문서 머리말+§8 머리의 §1~7/§8 지위 규칙) 원문 확인 — 오너 재판단 불요에 동의, 핀 셀 대체는 `MIN_PASSWORD_LENGTH` 선례 정합. 검증 도중 **개발 스택 8일 다운 발견·복구**(인프라 재시작 정책 `no`가 원인 — 슬라이스와 무관, Outstanding). | **합격** |
### 2026-09-07

| 기록 | 무엇을 반증했나 | 판정 |
|---|---|---|
| [`account_withdrawal_d4_ledger_axis.md`](2026-09-07/account_withdrawal_d4_ledger_axis.md) | **계정 탈퇴 D1~D6 확정 + D4 원장 소유 축 개명(`3cb07ce`·`badc01a`·`6eeedf5`·`db19e30`, SoT v1.8.43).** 구현 주장 전부 재현 — 백엔드 전수 **2903 passed / 1 skipped / 3839 subtests**(test-mongo ON, 캡처 EXIT=0, 2250.95s)·focused 74/9·산술 2892+5(정책분)+6=2903 ✓. **개명 범위를 전 줄 감독** — 저장 필드·인덱스 키 3종·질의만 움직였고 메서드 인자(`has_usage(user_id,…)`)·`admin_user_id`·다른 컬렉션 무변, `enforcement.py` 의 `user_id=` 는 전부 kwargs, 컬렉션 직접 질의자는 어댑터·마이그레이션뿐. **필드-이름-스위치 전제를 원문 확인**(`purge_reconciler.py:56` `find_one({field:{$exists}})` 표본 한 건). **마이그레이션을 실 mongo(스크래치 DB)에서 왕복 실측** — dry-run 무변경 → 적용(renamed 1·옛 인덱스 drop) → 재실행 멱등. **★같은-이름-다른-키 인덱스 생성이 code=86 으로 거부됨을 실측** — "배포 시 마이그레이션 선행"이 필요조건임이 서버 동작 수준에서 확정. **구현자 변이 MD-1~MD-4 를 4종 전부 재유도해 셀 수까지 일치**(7·1·4·1 — MD-1 의 7엔 SUBFAILED 포함, 요약 라인 판독). D1 바로잡은 계약(유예 중 vs 취소 후)·D3 재시도≠재호출 문언이 계획·work_log·HANDOFF 에 일치. 하드닝 2: **H1 마이그레이션 부분 실패 창**(update_many↔drop_index 사이 크래시 → 재실행이 조기 반환이라 옛 인덱스 잔존 → 어댑터 기동 code=86 — 실측 근거, 처방은 실제 키로 판정해 drop) · **H2 멱등 셀이 `dropped_indexes==[]` 다리 무셀**(가짜가 예외를 삼켜 통과). | **합격** |
| [`scene_note_follow_up.md`](2026-09-07/scene_note_follow_up.md) | **장면 메모 후속 — 목록 행 수정 시각 · "더 보기"(`f46a462`·`9262f27`, SoT v1.8.38, 오너 결정 2026-09-06).** 구현 주장 전부 재현 — 프런트 전수 **457 passed / 38 files**(캡처, EXIT=0)·tsc rc=0·`test_docs_indexes` 15/298(문서만 바꾼 슬라이스의 백엔드 미실행 갭 보완 실시). **무변 주장을 소스까지 추적해 전부 성립 확인** — 4커밋이 `services/`·`tests/`·`schemas/` 0건, `client.ts` 는 주석 2줄뿐, `schema.d.ts` 의 `updated_at`·`truncated` 는 이전부터 선언, `updated_at` 은 읽기 경계 `_aware()` 로 UTC 보장(`mongo_repository.py:913`)이라 naive 함정 없음. **v1.8.37 ② 의 양방향 셀 조건 이행** — 한 셀에 참 행 존재+거짓 행 부재. 검증자 변이 3종 직접 재유도 전부 기명 셀 물림(접힘 제거 1 · `truncated` 무시 2 · 단건 GET → 미리보기 4 — 구현자 표의 MN-5=3 은 셀 커밋 **이전 트리** 측정치로 지금은 초집합, 표는 측정 시점 기준 정직). **★미확인 "1 failed / 457" 조사** — 문서-간섭 배제가 전체 스위트에 성립함을 확인(파일 읽는 테스트는 10개뿐, 전부 `frontend/` 내부; `ReviewInbox` 의 `docs/` 는 주석). 다만 **"머신 부하가 극단적" 해석은 반증** — 실패 실행 environment 1778s vs 본 검증 초록 실행 1712s(≈4% 차). 동일 트리 전수 초록 3연속, 정체 미상 유지(낮은 재현율 타이밍 플레이크가 잔여 가설). 하드닝 3: `expand()` 만료 응답 경쟁(A 로딩 중 B 클릭 → A 본문이 B 행에 일시 렌더 · A 늦은 실패가 B 펼침 접음) · "불러오는 중…" 잠금 무셀 · 드로어의 수정 시각 존재 무셀(페이지 셀만 — v1.8.37 B1 과 같은 종, 한 줄로 닫힘). | **합격** |

### 2026-09-06

| 기록 | 무엇을 반증했나 | 판정 |
|---|---|---|
| [`scene_note_slices_3_4.md`](2026-09-06/scene_note_slices_3_4.md) | **장면 메모 Slice 3·4(화면 둘 — `fe358d0`·`aa9fcdf`·`4d73f01`·셀 보강 3커밋·기록 `58d9315`, SoT v1.8.36, D1=C+A·D4=A).** 구현 주장 전부 재현 — 프런트 전수 **446 passed / 38 files**·집중 `src/notes` **25**·`tsc` rc=0·`gen:api` 후 `schema.d.ts` **무변**(UI 슬라이스가 API 계약을 안 건드린다는 주장 확인). **구현자 변이 표 MN-1~MN-10 은 정직** — diff 를 직접 다시 써서 10종 전부 **재실패 셀 이름·개수까지 일치**했고, 자기 보고한 두 무잠금(MN-8 장면 전환 초기화 · MN-10 미저장 글 보존)의 보강(`4e52339`·`e3dc8a6`)도 지금 실제로 문다(유일한 흠은 MN-2 라벨 "초기 403"이 실제로는 404 픽스처). **★검증자 변이 9종 중 4종이 무물림 = 차단 5건**: **B1** 이 페이즈의 **완료 기준 1 "두 화면이 같은 검색 결과를 읽는다"가 무잠금** — MV-A(드로어 모드에서만 제목 브라우저 필터 = 본문 매치 행이 드로어에서만 소실, ① 이 금지한 현상)가 **25 passed 전건 초록**; 컴포넌트 공유는 구현 사실이지 잠금이 아니고 드로어 셀은 URL 만 본다. **B2** SoT ⑨ "403 **만**" 의 over-strict 무셀 — MV-B(모든 ApiError 를 읽기 전용 승격)가 초록, 보관 장의 409 가 "grant 읽기 전용"이라는 틀린 이유로 저장 버튼을 영구 제거해도 아무도 실패하지 않는다(계획 Slice 4 "저장 실패" 빈 칸과 같은 자리). **B3** SoT ⑧ "경고 + 저장 차단" 의 **경고 절반 무셀**(MV-C 초록). **B4** SoT ② 가 코드에 없는 리터럴 `truncated` 를 "목록이 읽는다"고 말한다 — 픽스처·주석 외 참조 0(문언 정정 vs 구현은 오너 결정). **B5** 드로어 링크의 이탈 가드 무셀 — MV-F(가드 무력화)가 **94 passed**, 같은 모양의 선례 둘(`WorkspaceReviewPanel.test.tsx:132`·`AnalysisTrigger.test.tsx:262`)은 이미 잠겨 있다. 하드닝 6(`""`↔`null` 이 상한 셀에 우연 잠김 · 드로어 readOnly 가 보관 3축 중 2축 · `:299` 셀이 재현하는 "드로어 닫힘"이 실제 배선에서 안 일어남 · `aria-pressed` 무셀 · SoT 회귀 산술 27 vs 28 귀속 · MN-2 라벨). CSS 가드 4종은 스타일시트/마크업에서 유도하므로 신규 규칙을 실제로 덮는다. 변이 원복은 병렬 세션 때문에 `git checkout` 없이 cp 백업 + 역방향 Edit + 바이트 대조로만 수행(9회 전부 무출력). | **조건부 합격** |
| [`final_save_n1_promotion.md`](2026-09-06/final_save_n1_promotion.md) | **최종 저장·분석 연동 5차(승격) 재검증 — 4차 조건 N1 폐쇄(`f248d8c`·`a875fc0`)의 판정.** 09-01 스냅샷이 아니라 **현행 HEAD**(그 뒤 `d02837a`·`61cd7a1`이 같은 축을 다시 건드렸다)에서 재유도 — 집중 **68 passed**·프런트 전수 **418/35**·tsc rc=0. 구현자 변이 표 11종을 드리프트한 줄 번호에서 **diff를 직접 다시 써** 재유도해 **재실패 셀 짝까지 일치**(M6만 1→3인데 09-02 신설 in-flight 2셀로 증분이 설명된다 — 표는 정직). 4차가 열거한 요구 분기(배지 3상태·final 버튼과 사유·성공/부분 성공 안내·미실행/진행 중)는 전건 기명 셀 확인, 09-02 in-flight 축도 MV-A로 잠금 실측. **★차단 B1: 확정 계약 "final marker 뒤의 일반 저장은 허용한다"의 과잉교정 방향이 무가드** — 일반 저장 버튼에 `isFinalized`를 더한 MV-D가 **프런트 전수 418 전부 초록**으로 통과한다(백엔드는 프로브 S4가 잠근다 = 한쪽만 잠긴 계약). 이 분기는 배지 세 번째 상태에 사용자가 도달하는 **유일한 경로**이고, `:1980` 셀은 그 상태를 픽스처로 마운트할 뿐 저장으로 전이시키지 않는다. 하드닝 3: **MV-B**(finalize 키를 상수로 바꿔도 68 passed — 4차가 N3를 "도달 불가"로 분류한 전제가 무잠금이고, 실패 뒤 재시도가 S-1 계약상 409로 굳는다)·**MV-C**(finalize() 함수 층 가드 제거도 무물림 — 흡수층은 비활성 버튼)·**N2는 이미 닫혔는데 HANDOFF만 낡음**(브리프 §2026-09-02가 "배지 구현"으로 닫았고 `DraftList.tsx:10-25`+셀 2개 실재). 변이 원복은 남의 미커밋 트리가 살아 있어 `git checkout` 없이 cp 백업+역방향 편집+바이트 대조로만 수행. **B1은 폐쇄 커밋 `0a2fee4`가 닫았고, 6차 재검증(2026-09-12 · 위 첫 줄 승격 재검)이 변이 재적용 양방향으로 판정을 합격으로 올렸다.** | **합격** |
| [`identity_group_slice_6.md`](2026-09-06/identity_group_slice_6.md) | **identity group Slice 6(grouped Inbox UI, `c8ab9cf`·`07a7428`·`0083f34`·기록 `149263a`·기준선 `c260e49`, SoT v1.8.34, D1=A·D2=B).** 구현 주장 전부 재현 — 전수 **2891/1/3792(333s)**·프론트 **417 passed**(+14 산술 일치)·tsc 무오류·OpenAPI **md5 `d60b94d3…` 무변**(worktree 대조)·`gen:api` 후 트리 공백·"revision은 `set_group_status`에서만 상승" 코드 확인. HANDOFF 지정 검증자 자리 셋 전부 확정 — ① 결과 상자 페이지 수준(M6f 재조회 폐기 → **4 재실패**) ② 픽스처 revision≠0(M2r 상수 0 → **1**·M1r 멤버 수 → **6**·M4f body 상수 0 → **1**) ③ 좁은 폭 규칙성(M9f → **1**). 구현자 뮤테이션 8종 재유도 전부 수치 일치. **★차단 2건**: **B1** SoT가 열거한 잔여 step 리터럴 `pending` 무셀 — 검증자 변이 M10f(잔여 판정을 failed/conflict 열거로)가 **22 passed로 통과**, D4=A 실전 부분 패스 모양(applied·failed·pending…)의 대부분이 정확히 이 step. **B2** SoT v1.8.34 변경이력의 *"실크기 1.3rem→1rem 변화 · 육안 확인 대상"* 서술이 **실측 반증** — `var()` 실패는 cascade에서 진 `.resource-link` 1.3rem을 되살리지 않고 상속값으로 떨어지며, headless Chrome 실측 pre/post **모두 16px·system-ui(가시 변화 0)**. 가시 변화가 있었다면 그 시점은 `d02837a`(09-02)이고 `c8ab9cf`는 같은 계산값의 유효 선언화(옳은 정리 — 거짓은 "가시 변화" 서술). 전파: SoT:36·페이즈 370-371·CHANGELOG:5·work_log 세션 17·typeScale.test 주석. 하드닝 5(유료 402/429 UX가 WritingPanel과 불일치 + 재클릭 안내 vs 5s 냉각 429 · 409 후 재조회 없음 · idempotent_replay 무셀 · outcome 단일 슬롯 · plans 인덱스 꼬리 낡음). | **조건부 합격** |
| [`phase_s0_repo_hygiene.md`](2026-09-06/phase_s0_repo_hygiene.md) | **Phase S-0 문서 위생(커밋 3개: `0b64a73` 브리프 · `5aa7687` 시행 · `13ad728` 기록).** 스윕 완결성 독립 재측정 — HEAD 잔여는 감사기록 1파일(주소 29발생)뿐이고 치환 산술이 폐쇄된다(`0b64a73` 57파일/165발생 − 감사기록 29 = **56파일/136발생 제거**, diff ±측정). D4 exit 2 실측(rc=2)·전수 **2879/4/7886·324s 재현**(subtest +5는 기록 커밋 이전 측정). 구현자 뮤테이션 10종 표는 정직(물지 않음 2종은 NOT-fire 확인으로 정상 기록). **★검증자 변이 V-M3b(가드 자기예외 추가+금지값 통째로 심기)가 초록 통과 — `test_the_guard_covers_its_own_file` 은 목록 멤버십만 단정해 "가드 파일이 예외로 확장되지 않는다"는 계약 분기를 잠그지 않는다(서브테스트 4개가 소실돼도 green).** 하드닝: 광의 규칙 후행 마침표 우회(잠재·HEAD 피해 0)·134(라인)/136(발생) 단위 혼란 정정·예외집합 구조 노출. | **조건부 합격** |
| [`identity_group_slice_5.md`](2026-09-06/identity_group_slice_5.md) | **identity group Slice 5(그룹 승인 오케스트레이션, `ffc9525`·`871b634`·`ea55474`·기록 `fad29c5`, SoT v1.8.29).** 구현·등재·생성물 주장 전부 재현 — 집중 **33 passed**·인접 **146**·전수 **2883/4/3789**(개시 HEAD `7e7df39`)·OpenAPI **102 operation**·유료 얼굴 402/409/429·`schema.d.ts` 재생성 **바이트 동일**·분류표 29·`BILLABLE_ACTIONS` 11행·파기 그래프 approvals 합류·프론트 라벨. 뮤테이션 — VM-1(=구현자 M2, 409 제거) 셀 짝 일치·**VM-2(채택 전면 제거) 3 failed로 "채택은 셀 2종이 잠근다" 주장 확인**. **★차단 2건**: **B1** 리터럴 ⑦ 위반 **행동** — 채택된 canonical의 원천 멤버(개별 promote로 needs_review 유지)가 유일 runnable이면 판정이 필요 없는데도 judge 미구성 503으로 승인 불가(probe 실측·대조 시나리오의 503은 정당); 처방 4줄(VM-3) 선적용에도 **33 passed로 무셀**. **B2** 리터럴 ⑥ D3=A 멤버행 축 무셀 — 행동은 계약대로(probe: open·revision 무변·멤버행 유지)지만 승인 관측면이 진행 문서 기반이라 멤버행 삭제가 어디에도 안 보임(삭제 후 replay 200·steps 동일) — Slice 4의 우연 잠금은 승인 구조에서 불성립. 그룹행 축은 replay 200 우연 잠금 존재. 하드닝 4(파기 docstring 낡은 22컬렉션 산문·steps 정렬 축·구현자 변이 표 diff 문언 권고·B1 과잉 방지 짝 셀). | **조건부 합격** |

### 2026-09-05

| 기록 | 무엇을 반증했나 | 판정 |
|---|---|---|
| [`security_audit_dual_workflow.md`](2026-09-05/security_audit_dual_workflow.md) | **보안 감사(슬라이스 검증이 아니다)** — 코드 보안 12렌즈·공개 레포 정보노출 5렌즈를 두 workflow 로 병렬 감사하고 각 발견을 독립 검증자의 반증으로 확정(critical/high 2표). 확정 39건(코드 24 · 정보노출 15) · **critical 0건**. 위험은 **quota 우회 체인**(클라이언트 제어 `request_id` 가 원장 dedupe 키 = HIGH · 무제한 retry 루프)과 **문서 정보노출**(평문 비밀번호 · 사설 IP 49파일)에 집중. 사후 비평이 두 축 누락을 지적: 파이썬 lockfile 부재 · 인증 SPA 의 CSP·보안헤더 전무(AdSense 서드파티 스크립트). **★ 판정 열 주의** — 이 기록에는 `## Verdict` 절이 없다(감사이지 슬라이스 검증이 아니라서). 인덱스 어휘가 셋뿐이라 *조건부 합격* 으로 등재했다 = **확정 발견을 닫아야 할 조건으로 본다**. 감사용 판정 어휘 신설은 오너 결정 사안. | **조건부 합격** |
| [`phase_s1_quota_bypass_chain.md`](2026-09-05/phase_s1_quota_bypass_chain.md) | **Phase S-1 quota 우회 체인(커밋 7개, SoT v1.8.32) + S-3 검증 조건 폐쇄 확인.** 구현자 뮤테이션 11종(M-A~M-K)은 셀 수까지 전부 재현됐고, "실행 전 409"를 HTTP 셀이 `provider.calls`로 행동 잠금(S-3 B1급 공백 없음)·등재 목록 양방향(N-a)·STANDING 1:1 배선(N-b)·409 얼굴 6경로 선언 행렬까지 실증. **S-3 조건 B1~B4는 전건 실질 폐쇄 확인** — 폐쇄 셀에 검증자 변이(M5·M6·TTL 24→2)를 재적용해 전부 재실패. 검증자 신규 변이 5종 중 2종 무물림: **B1** 재생 TTL 24h literal 하방(86400→3600에 0 실패 — "경과 후 409" 셀만 있고 "경계 안 재생" 셀 없음)·**B2** `MongoReplayResponseRepository`·`has_usage` 테스트 0건(S-3 폐쇄가 같은 날 세운 가짜-몽고 규범 미준수). 산수(순증 42=15+27)·전수 2720/151/3206·tsc 0·프론트 사전존재 2·민감정보 0건 전부 독립 확인. | **조건부 합격** |
| [`phase_s3_signup_throttle.md`](2026-09-05/phase_s3_signup_throttle.md) | **Phase S-3 공개 signup 속박(커밋 4개, SoT v1.8.30).** 구현자 뮤테이션 4종은 전부 재현 재실패했고 등재 목록 양방향·"LAN 제외" 기본값도 진짜로 잠겨 있다(신규 M7·M8). 그러나 **검증자 뮤테이션이 무셀 3종을 찾았다** — ① 스로틀을 `request_signup` 뒤로 옮기는 변이(= 감사 §A.5 원결함의 전면 재등장, 429 확정 전 Argon2+행 생성)가 **전수 2678 통과로 무물림** — "거절은 해셔 앞"이 주석·SoT 의 계약임에도 잠금 셀 없음 ② env 기동거부 literal(`AUTH_SIGNUP_MAX_REQUESTS` ≤0 등)도 테스트 참조 0건, 가드 제거 시 조용히 기동 ③ `signup_guard_mongo.py`(TTL 인덱스 = SoT literal)에 테스트 파일 자체가 없음. 더해 ④ SoT v1.8.30 신규 429행이 "**유료 10경로**"라 적었는데 실제 `BILLABLE_OPERATIONS` 는 **11**(09-01 finalize 등재 때 SoT 서술이 안 올라간 선결함을 이 슬라이스가 정본에 반복). 민감정보 0건·트리 공백·프론트 2실패 사전존재(전날 v1.8.27 등재)·schema.d.ts 재생성 무차이는 전부 확인. | **조건부 합격** |
...
| [`identity_group_slice_2.md`](2026-09-03/identity_group_slice_2.md) | identity group Slice 2(분석 runner 배선, `488b867`·`e6a4c87`·`fd02e88`, SoT v1.8.23) 독립 검증. 주장 재현 — 수집 2741=주장 2740+1, 집중 18셀 green, OpenAPI **코드 경계(2d467b5↔HEAD) 바이트 동일** 재덤프(md5 선례 동일), SoT 리터럴 9종·계측·재분류 목록과 코드 대조 무불일치. 뮤테이션 — 검증자 신설 2종(**focal 확대 과잉**·max_tokens 512→511)이 각자 셀에 물리고 구현자 표 M2·M3·M9 재유도가 셀 짝까지 일치, M4(가드 제거) 관측 동등 재확인(흡수층=격리 경계). **★차단 B1: 러너 레벨 judge 미구성 격리 무셀** — 완료 기록·SoT v1.8.23 리터럴 ③이 "ProviderError·parse·judge 미구성 어느 것이 와도 job은 succeeded"를 주장하나 셀은 앞 둘뿐(행동은 probe로 계약대로 실측). 하드닝 3: provider 호출 없는 `InvalidIdentityJudgement`의 재분류가 관계 없는 extractor success 행을 parse_error로 오염(probe 실증 — "마지막 호출=실패한 repair"는 기본 조립에서만 성립하는 가정), HANDOFF KPI 사이트 열거 잔류 8개, 구현자 OpenAPI "HEAD~1 대조"는 커밋 시각 대비상 테스트 커밋 전후였을 개연성 높음(무변 결론은 검증자가 올바른 경계에서 확정). | **조건부 합격** |
| [`identity_group_slice_1.md`](2026-09-03/identity_group_slice_1.md) | identity group Slice 1(판정 서비스, `f5c0ead`·`98c5c13`·`3dfef65`, SoT v1.8.21) 독립 검증. 경계 행렬 전개 후 잠긴 17축은 코드·셀·변이 10종(구현자 표와 셀 짝까지 일치 재현)으로 확인 — 병합 셀 보강 두 커밋의 실효(결정적 id·클록 전진)도 M8 재현. OpenAPI 사전 트리(`7ab3df6`)와 바이트 동일 독립 재덤프, Slice 0 면 무접촉·runner/HTTP 미배선·public service 전용 사용 확인. **검증자 신설 3종이 전부 물지 않아 계약 요구 분기 3곳이 무셀임을 입증** — B1 retriever 미주입 no-op, B2 relation `source` 리터럴, B3 재사용 경로 효과 재적용(자가 치유). 행동은 계약대로(빈 것은 잠금). | **조건부 합격** |
| [`contract_schema_duplication_execution.md`](2026-09-03/contract_schema_duplication_execution.md) | 계약 스키마 중복 전수조사 시행 5커밋(`a63b521`·`226a821`·`6e9d497`·`159157b`·`ed9e19e`, SoT v1.8.19) 독립 검증. 주장 전면 재현 — 전수 **2701/1/3131·EXIT=0**(1788.36s), OpenAPI 덤프 사전 트리(`8655653`)와 **바이트 동일 독립 재덤프**(md5 `10978d55…`·384,414B), **logical_key 이행 핀값을 사전 트리 코드로 재계산해 동일 입증**(retry 멱등), v5/v6 프롬프트 sha 핀 재계산 일치, "프롬프트가 plan_id 를 요구한 적 없다" 사전 템플릿 grep 확인, 정책 bool 소비처 조건분기 0건·프론트 무판독 확인. 변이 **14/14 기명 재실패**(작업자 11종 전량 + 신설 3종: 유도 공식 max→first, unknown-id fails-open 유령 ref 합성, legacy decision 키 거부→조용한 무시). 옛 extractor repair-후 카탈로그 재검증이 죽은 검사였던 것(양 갈래 `return repaired`)과 shape-error repair 출력이 카탈로그 대조를 안 거쳤던 것을 신규 코드가 같이 닫음을 확인(수용 경계 조임 — 방향: 강화). 하드닝 4: repair 수용 조임 SoT 명문화, planner parse_error 단절 병기, 게이트 프롬프트 입력측 상수 bool 렌더 제거(D축 후보), 정책 상수 공개 표면 이중 잠금. | **합격** |

### 2026-09-02

| [`identity_group_slice_0.md`](2026-09-02/identity_group_slice_0.md) | identity group Slice 0 저장 모델·수명(`183af60`·`ea29bb5`) 독립 검증. 주장 전면 재현 — 전수 **2696/1/3124·EXIT=0**(2046.65s), baseline 2677→2697 **순수 +20셀** 검산, OpenAPI/`schema.d.ts` pre-slice↔post-slice dump **바이트 무변**(384,414B), 변이 독립 **7종** 전부 기명 재실패(작업자 4종 재현+**신규 3종**: 과잉 파기·revision +1 제거·self-pair 방어 제거), 경계 행렬·리터럴(status 3값·축 4필드·revision 0/+1·created_at 보존)·파기 그래프 10계약/22컬렉션·양 경로 스파이·오너 결정 기록 전부 확인. **★차단 B1: 실몽고 datetime 경계 정규화 누락** — 프로브 실측 read-back `tzinfo=None`·`get_relation==기록` False, 선례 10/12 어댑터는 `_aware` 재라벨링(인용 선례 scene_notes도 core_sot에서 정규화), fake는 구조적으로 못 잡고 실몽고 셀은 충실도 미단언이라 "in-memory**와** Mongo round-trip" 축에 빈 칸. 하드닝 4: groups `_id`=group_id 단독의 축 문장 긴장, self-pair 거부 SoT 미기재, 변이 표 I1' 관측("unique 인덱스 위반")은 실측 len 단언, "member 참조만" 명시 셀. | **조건부 합격** |

| [`identity_group_plan.md`](2026-09-02/identity_group_plan.md) | identity group 구현 페이즈 계획(`28a6dd3`) 독립 검증. C의 승인/거절 의미론·멱등·보존 경계가 7슬라이스에 완전·단방향 배정(저장→서비스→배선→읽기→거절→승인→UI), SoT v1.8.16과 무모순, additive payload 시 schema.d.ts 재생성 명시 등 직전 라운드 교훈이 제도화됨. **하드닝 4**: 브리프 Follow-up 5건 중 2건 미배정(추이성 모순 상태·uncertain 사람 합치기/분리 — 정본 요구는 아니나 트리거 부착 권고), 그룹 액션 경로가 기존 `/analysis/review-*` 패밀리 밖, 슬라이스별 예상 operation 수 미기재. docs 13/283·diff --check·117개 수 일관. | **합격** |
