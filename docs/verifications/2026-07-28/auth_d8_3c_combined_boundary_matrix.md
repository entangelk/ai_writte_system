...
## 추기 — 검증 대상 트리와 최종 커밋의 차이 1건 (2026-07-28, **구현자 작성**)

> 이 절만 검증자가 아니라 **구현자**가 덧붙였다. 위 본문의 소견·판정은 원문 그대로이며 수정하지
> 않았다. 목적은 "이 기록이 기술하는 트리"와 "실제로 커밋된 코드"가 한 셀에서 다르다는 사실을
> 기록 안에 남겨, 나중에 이 기록만 읽는 사람이 코드와 대조하다 혼란을 겪지 않게 하는 것이다.

- 위 기록은 **커밋 전 작업 트리**를 기술한다. 그 시점의
  `test_the_ownership_dependency_cannot_run_without_authentication`은
  `inspect.signature(require_project_owner)`의 하위 dependency를 단정하는 **구조 결합** 셀이었고,
  본 기록은 그것을 비차단 **H-a**("구현 형태 결합 — 합성/래퍼 dependency로 리팩터링하면 거짓
  경보")로 지적했다.
- 구현자는 그 지적을 받아 **커밋 전에** 같은 셀을 행위 잠금으로 교체했다: 바깥 겹을 일부러 뺀
  일회용 앱에 `require_project_owner`만 마운트해 **세션 없음 401 · 소유자 200 · 타인 403**을
  단정한다. 따라서 커밋 `3b99c80`의 코드는 이 기록의 H-a 서술과 다르다.
- 양방향 실측(구현자): 안쪽 겹을 제거하는 뮤테이션은 **여전히 실패**하고, 인증을 유지한 합성
  dependency(`Depends(_auth_chain)`)로 바꾸는 정당한 리팩터링은 **통과**한다 — H-a가 지적한
  거짓 경보 조건이 사라졌다.
- **판정에는 영향이 없다.** H-a는 애초에 비차단이었고, 교체는 잠금을 약화시키지 않고 강화한다.
  후속 D8-5a 검증(`auth_d8_5a_admin_boundary.md`)이 개선판도 green임을 독립 확인했다.
- 정본 v1.7.55 본문은 **교체된 형태**를 기술한다(격리 구동 셀). 이 기록과 정본이 다르게 읽히면
  **정본과 코드가 맞고 이 기록이 그보다 이른 시점**이다.
