...
5. **관측이 제품 기능으로 들어왔다.** 초안에 없던 축이다. **호출부 9곳**이 표준 감사 레코드를
   남기고, 그것을 집계한 KPI 화면이 프로젝트별·전역으로 있다.
   - **"9"의 기준**은 `LlmCallSite` enum 리터럴 수 = LLM 어댑터 수다(`observability/llm_call_audit.py`).
     ...
     `identity_judge`(2026-09-03, 후보 정체성 판정).
...
| **컨텍스트 창 초과** (초안에 없던, 실사용에서 나온 위험) | 예산을 **창에서 유도**하고, 그래도 넘는 요청은 모델을 부르기 전에 400으로 거부 | 잘린 출력(`finish_reason≠stop`·빈 content)은 산문 두 표면(생성·개정)에서 거부되고 report repair는 length를 예측 실패로 단락한다(2026-09-14) — 1회 repair 파서 5곳의 length 재생성 낭비는 트리거 붙은 유예 |
