...
HUE = 250
# Phase N: 페이지·구분선·본문은 중성에 가깝게. 차트 검산면 slate-50은 유지한다.
NEUTRAL_HUE = 250
...
NEUTRAL = {
    ...
    "slate-100": (0.965, 0.002),
    "slate-200": (0.910, 0.002),
    "slate-300": (0.850, 0.016),
    # ★ 10.1 구현이 푼 값 둘. **"최악 배경"을 두 번 틀렸다** — 브리프 초판은
    # 카드(가장 밝은 면)만 보고 정했고(테두리 2.95 / placeholder 3.97 미달),
    # 1차 수정은 페이지(`blue-50`)를 최악으로 잡았는데 **침강면(`blue-100`)이
    # 더 어둡다**는 것을 `.gate-finding` 매핑에서야 알았다. 지금 값은 **모든
    # 표면 중 가장 어두운 `blue-100`** 기준이며, 그래서 밝은 면에는 여유가 있다.
    # 교훈: 최악 배경은 "주로 쓰는 면"이 아니라 **정의된 표면 전체의 최소**다.
    "slate-400": (0.627, 0.020),   # border-control — 최악(sunken)에서 3.01:1
    "slate-450": (0.528, 0.020),   # placeholder    — 최악(sunken)에서 4.55:1
    ...
    "slate-600": (0.480, 0.005),
    "slate-700": (0.390, 0.022),
    "slate-900": (0.255, 0.005),
}
...
# ---------- 검산 대상: 실제로 화면에 나올 짝 ----------
SURFACES = {
    "page (slate-100)":  P["slate-100"],
    "card (slate-0)":    P["slate-0"],
    "sunken (slate-100)": P["slate-100"],
...
PAIRS = [
    ...
    ("slate-600", "blue-100", 4.5, "보조 문구 / 침강면"),
    ("slate-450", "blue-100", 4.5, "placeholder / 침강면"),
    ("slate-0",  "blue-600", 4.5, "주 버튼 라벨"),
    ("slate-0",  "blue-800", 4.5, "주 버튼 라벨(hover)"),
...
# Phase N: 실제 중성 페이지·침강면 및 랜딩 반전면까지 검산한다.
PAIRS += [
    (fg, bg, need, "Phase N / " + fg + " / " + bg)
    for bg in ("slate-100", "slate-0", "slate-50", "blue-100")
    for fg, need in (("slate-900", 4.5), ("slate-600", 4.5),
                     ("slate-450", 4.5), ("blue-700", 4.5),
                     ("danger-700", 4.5), ("slate-400", 3.0), ("blue-600", 3.0))
]
PAIRS.append(("slate-0", "slate-900", 4.5, "랜딩 반전면 본문"))
