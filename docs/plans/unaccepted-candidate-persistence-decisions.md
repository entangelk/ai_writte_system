...
상태: `결정 확정 (2026-07-20) — D0=B / D1=B / D2=A, 구현 착수`
...
## 잠정 보존/만료 정책 (구현자 재량 — 오너 SoT 승격 대기)

D1=B(이력)가 요구하는 보존/만료 정책을 오너 위임에 따라 구현자가 잠정 결정한다. **아래는 정본 계약이 아니며, 나중에 오너가 SoT로 승격·확정해야 한다.**
...
- **D2 = A** (별도 collection `writing_drafts_scratch`, 정본 무변). 오너가 A/B/C 저장 위치를 직접 지정하지는 않았으나, **D1=B(이력)는 서버측 영속이 필수라 B(loop_audit 재사용, 의미 충돌)·C(localStorage, 이력·기기간 이동 불가)와 양립하지 않는다 → A만 정합적**이므로 A로 확정한다.
  - **보존/만료 정책**: 오너가 **구현자(Claude) 재량으로 잠정 정책을 정하고 테스트**하도록 위임했다. **단, 이는 잠정값이며 나중에 오너가 SoT로 승격·확정해야 한다**(아직 정본 계약 아님). 잠정 정책은 구현 시 이 문서와 work_log에 기록한다.
