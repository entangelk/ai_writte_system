...
**★ 그 가드가 무엇을 잠그는지는 독립 검증에 한 번 반박당하고 다시 쓴 문언이다.** 초판은 *"생성자를 직접 부르는 자리가 **하나라도** 있으면 실패"* 였는데, [검증](../verifications/2026-08-20/embedding_adapter_slice.md)이 `import … as REP` 뒤의 `REP(…)` 로 **그 "하나" 를 통과시켜 보였다.** 폐쇄는 문언을 좁히는 대신 **검사를 넓혀서** 했다(`7af3a87`).
