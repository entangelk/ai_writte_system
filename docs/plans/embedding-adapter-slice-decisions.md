...
## 구현에서 정해진 것 (브리프가 위임한 자리 — 2026-08-20)

§후속 고려가 *"`EMBEDDING_SERVICE_URL` 의 의미가 둘이 된다 — **변수를 하나 더 둘지 형식을 별도 env 로 뺄지**는 결정 1=A 를 고른 뒤 구현에서 정할 문제"* 라고 남긴 자리다. 정한 값과 근거를 적는다.

| env | 값 | 표기 | 왜 |
|---|---|---|---|
| `EMBEDDING_API_FORMAT` | `native`(기본) · `openai` | **콜론** `${…:-native}` | 코드가 `get(name, DEFAULT)` 로 읽는다 |
| `EMBEDDING_API_MODEL` | 없음(openai 면 필수) | **대시** `${…-}` | `get(name)` + falsy 검사 — 빈 값이 빈 채로 가야 "없음" 분기에 닿는다 |
| `EMBEDDING_API_KEY` | 없음 | **대시** | 〃. 키를 안 받는 OpenAI 호환 서버가 있다 |

**★ 형식을 키 유무로 추론하지 않는다.** 추론이 더 짧지만 **키를 잠깐 지우면 형식이 조용히 바뀐다** — 그리고 그 실패는 `404` 로 나타나 **원인에서 가장 먼 자리**에 떨어진다. `EMBEDDING_API_FORMAT` 을 명시로 둔 이유이고, 오타(`open-ai`)도 자체 형식으로 떨어지지 않고 유효값을 이름으로 말한다.

**★ `EMBEDDING_MODEL_NAME` 을 재사용하지 않았다.** 그 변수는 **우리 임베딩 컨테이너가 받는 HuggingFace 모델 id**(`dragonkue/BGE-m3-ko`)이고, 새 변수는 **벤더의 모델 이름**(`text-embedding-3-small`)이다. 같은 이름을 쓰면 컨테이너용으로 설정한 HF id 가 외부 API 로 나간다.

**★ base 주소는 호스트 루트다** — `/v1/embeddings` 는 provider 가 소유한다(gateway 가 `LLAMA_BASE_URL` 로 `/v1/chat/completions` 를 부르는 것과 같은 관례). 벤더 문서는 base 를 `…/v1` 로 인쇄하므로 **접미 `/v1` 하나를 벗긴다** — 안 그러면 문서에서 복사한 주소가 그대로 404 이고 그 원인이 주소에 안 보인다. 경로 **안**의 `v1`(`…/v1/proxy`)은 접미가 아니므로 건드리지 않는다(over-strict 셀이 잠근다).

## 산출물 (2026-08-20)

| # | 산출물 | 자리 |
|---|---|---|
| ① | `OpenAIEmbeddingProvider` | [`indexing/embedding.py`](../../services/application/app/indexing/embedding.py) |
| ② | 조립 헬퍼 + 전수 가드 | 같은 파일 `build_embedding_provider_from_env` · [`tests/test_embedding_assembly.py`](../../tests/test_embedding_assembly.py) |
| ③ | README 외부 API 절 | [README §외부 임베딩 API에 붙이기](../../README.md) (재색인 절 바로 뒤) |
| ④ | external override 문단 | [`docker-compose.external.yml`](../../docker-compose.external.yml) §"아직 안 되는 것" — **[해소됨]** 로 표시하고 새 env 3종 배선 |

**seam 무변**(Protocol 세 곳) · **새 컨테이너 0** · **배치 없음**(결정 2=A). **가드는 등재 목록이 아니라 AST** 다 — 선택지 B 를 기각한 이유(M5 침묵)를 그대로 따랐다.

**★ 그 가드가 무엇을 잠그는지는 독립 검증에 한 번 반박당하고 다시 쓴 문언이다.** 초판은 *"생성자를 직접 부르는 자리가 **하나라도** 있으면 실패"* 였는데, [검증](../verifications/2026-08-20/embedding_adapter_slice.md)이 `import … as REP` 뒤의 `REP(…)` 로 **그 "하나" 를 통과시켜 보였다.** 폐쇄는 문언을 좁히는 대신 **검사를 넓혀서** 했다(`a9bca6d`).

| | 잠금 |
|---|---|
| **잡는다** | 원이름 호출 · 모듈 속성 호출(`emb.RemoteEmbeddingProvider(…)`) · **`import … as REP` 뒤의 `REP(…)`** |
| **잔여(안 잡는다)** | **할당 별칭** `P = RemoteEmbeddingProvider` 뒤의 `P(…)` — 이름 재결합 추적은 타입체커의 일이다 |
| 범위 | 최상위 파이썬 디렉터리는 **스캔하거나 이유와 함께 범위 밖**이어야 한다(새 디렉터리는 셀이 문다) |

**잔여를 적는 것이 이 표의 요점이다** — 이 저장소는 하루에 **세 번** 계약 문언이 잠금보다 넓은 실수를 했고(정본 산출물 · 셀 실패 메시지 · 이 가드), **세 번 다 폐쇄는 "검사를 넓힌다" 였다.**
