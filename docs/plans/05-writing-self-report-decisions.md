...
상태: `Resolved` (D1=A·D4=A·D6=A first→C, D2=A first→B — **B 구현 완료 2026-07-15, SoT v1.6.92**, D3=A then B committed, D5=B)
...
- **D2=A first→B**: 첫 slice는 pointer 없는 최소 typed schema. stable ContextPackage pointer가 extractor 입력에 제공되는 후속에서 full `related_context_pointers` schema로 확장한다. **2026-07-15 오너가 B 확장을 승인**했으며 identity·노출·필수성은 `05-writing-stable-context-pointer-decisions.md`의 D1=A/D2=A/D3=A(+ 필드 불변식 sub-decision P-i)로 잠갔다. 여기서 새 브리프의 D2=A는 report extractor-only 노출 결정으로, 본 문서의 A→B 단계와 별도 축이다. **B는 SoT v1.6.92로 구현 완료** — B의 원래 위험("compact ContextPackage가 pointer id를 모델에 제공하지 않아 hallucinated id")은 extractor 전용 pointer 노출 + exact package allowlist 검증으로 닫혔다.
