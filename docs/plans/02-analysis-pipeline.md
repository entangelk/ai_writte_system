...
- 3종 taxonomy의 최소 payload schema는 `character_observation {name, observation}`, `event_observation {event}`, `open_question_observation {question}`이다. 모든 payload field는 non-empty string이며 누락된 required field나 알 수 없는 추가 field는 malformed payload로 거절한다. `character_observation`은 optional `aspect`(자유 문자열, 예: `voice`/`trait`)를 추가로 담을 수 있다(증분 3 D4=B, 오너=optional; SoT v1.7.23) — 캐릭터 어투 식별용이며 taxonomy 3종은 무변, 기존 `{name, observation}` payload는 그대로 유효하다.
...
- Application API는 `POST /projects/{project_id}/analysis/jobs`, `GET /projects/{project_id}/analysis/jobs/{job_id}`, `GET /projects/{project_id}/analysis/jobs/{job_id}/candidates`, `POST /projects/{project_id}/analysis/jobs/{job_id}/run`, `POST /projects/{project_id}/analysis/jobs/{job_id}/retry`를 제공한다. 생성/조회 surface는 job 생성/idempotent replay와 상태·candidate 조회만 담당하며 runner 또는 Gateway 호출은 시작하지 않는다. `run` surface는 pending job만 runner dependency로 실행하고 요청 안에서 async runner를 await한다. `running`/`succeeded`/`failed` job은 일반 run에서 재실행하지 않고 `idempotent_replay=true`로 현재 job과 저장된 candidate 목록을 반환한다. `retry`는 명시 사용자 intent로 failed job만 같은 ID의 pending으로 되돌리고 failure fields를 비우며, 다른 상태는 409다. pending job에서 runner가 구성되지 않았으면 503이다. 존재하지 않는 project와 cross-project job/candidate/run/retry 접근 및 `snapshot_not_found`는 404다. SourceRef 자동 생성과 Gateway runtime wiring은 run endpoint 범위가 아니다.
