...
- Application API는 `POST /projects/{project_id}/analysis/jobs`, `GET /projects/{project_id}/analysis/jobs/{job_id}`, `GET /projects/{project_id}/analysis/jobs/{job_id}/candidates`, `POST /projects/{project_id}/analysis/jobs/{job_id}/run`을 제공한다. 생성/조회 surface는 job 생성/idempotent replay와 상태·candidate 조회만 담당하며 runner 또는 Gateway 호출은 시작하지 않는다. `run` surface는 pending job만 runner dependency로 실행하고 요청 안에서 async runner를 await한다. `running`/`succeeded`/`failed` job은 재실행하지 않고 `idempotent_replay=true`로 현재 job과 저장된 candidate 목록을 반환한다. pending job에서 runner가 구성되지 않았으면 503이다. 존재하지 않는 project와 cross-project job/candidate/run 접근 및 `snapshot_not_found`는 404다. SourceRef 자동 생성과 Gateway runtime wiring은 run endpoint 범위가 아니다.
