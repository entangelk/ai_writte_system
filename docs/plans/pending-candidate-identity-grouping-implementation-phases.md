...
참조만 한다. candidate purge/project purge는 그룹·멤버·관계 고아를 남기지 않는다.

**검증:** in-memory와 Mongo round-trip, project/type 격리, 중복 member idempotency, relation pair 정규화
(`A,B`와 `B,A` 동일), project purge 정리를 잠근다. 이 Slice는 OpenAPI와 `schema.d.ts`를 바꾸지 않는다.
...
**범위:** `POST /projects/{pid}/review/groups/{group_id}/reject` 계열의 그룹 거절만 추가한다. 그룹 승인은
아직 없다.
...
detail과 edit/conflict 화면은 유지한다.
