...
- [x] sync 실패 시 retry/backoff와 운영 가시성 — `index_sync_outbox` schema는 bounded retry metadata를 수용하고 `max_attempts=3`, `backend_error`/`not_found` 분리를 잠갔다. Claim/backoff 숫자와 retry 실행은 worker slice에서 확정
