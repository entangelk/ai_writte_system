...
2026-07-02 automatic sync/outbox의 첫 구현 범위는 [`03-index-sync-outbox-decisions.md`](03-index-sync-outbox-decisions.md)에 제안 브리프로 정리했다. 추천안은 `project_archived`/`draft_archived`를 첫 automatic event source로 삼고, inline adapter 호출이나 외부 queue 대신 Mongo `index_sync_logs` 기반 pending outbox entry를 먼저 남기는 방향이다. 검증 권고 후 브리프는 승인 전 schema lock 항목으로 `success` status literal 재사용, canonical `targets` shape, required `project_id`, dedup key, §64 stale-hit job과 `content_hash` 기준의 긴장을 명시한다. 이 브리프는 아직 SoT/public contract가 아니며, 승인 전에는 archive API나 persistent sync log schema를 구현하지 않는다.
...
- [ ] event queue, outbox, polling 중 sync 전달 방식 — [`03-index-sync-outbox-decisions.md`](03-index-sync-outbox-decisions.md)에 Mongo outbox/polling 우선 추천안 있음, owner 승인 전
