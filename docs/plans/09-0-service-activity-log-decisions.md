...
상태: `Resolved (2026-08-09) — A1~A8 확정 · 구현·독립 검증 완료 · A2 추가 확정(19 → 20)`
작성: 2026-08-05
확정: 2026-08-09
...
> | 항목 | 확정 | 비고 |
> |---|---|---|
> | **A1** | **A** — 새 컬렉션 `activity_events` | §43 `system_events`는 폐기 포인터 |
> | **★ A2** | **B**(정본 변경 + 검토 결정 9) — **단, C로 넓힐 수 있게 만든다** | 착수 시 **19**, **구현 뒤 `writing/accept` 를 더해 20**(아래 "A2 확정 조건" + 추가 확정) |
> | **A3** | **B** — 고정 코어 + 짧은 값 변화(`before`/`after`), "짧은 값"은 계약 | |
> | **A4** | **A** — 격리(fail-open) | 활동 로그는 보안 하중을 지지 않는다 |
> | **A5** | **B** — 소유자 조회 하나 `GET /projects/{id}/activity`(operation **77**) | |
> | **A6** | **A** — TTL 없음 | 부피가 문제가 되면 그때 C(밀어내기) |
> | **A7** | **A** — endpoint 본문 + 전수 가드 | 라우터 정리가 끝나 `main.py` 파일 파싱 부채 없이 간다 |
> | **A8** | **A** — 중복 기록하지 않는다(소비 시점 조인) | |
>
> **오너 문언(A2)**: *"A2에서 C까지 필요하다면 열어야 할 수 있을 것 같다. 그래서 확장성 있게
> 하되 일단 B로 진행."* 나머지 일곱은 구현자 추천 그대로 채택했다.

> **★ A2 확정 조건 — "일단 B, 그러나 C가 행 추가로 열려야 한다"**
>
> 오너가 B를 고른 것은 범위 판단이지 **C를 각하한 것이 아니다.** 그래서 이 슬라이스는 다음
> 셋을 만족해야 하며, 이것이 A2 결정의 일부다.
>
> 1. **분류표는 전수여야 한다 — "빠진 것"과 "일부러 뺀 것"이 구분돼야 한다.** mutating
>    operation **40개 전부**가 분류표에 `logged` / `excluded(사유)` 중 하나로 등재되고,
>    **미등재 route 는 가드가 실패**시킨다(`quota/billable_actions.py` + `test_billable_actions.py`
>    선례 그대로). **C 로 넓히는 일 = AI 요청 14 행의 값을 `excluded` → `logged` 로 바꾸는 것**이
>    되어야 하고, 코드 구조를 고치는 일이 되어서는 안 된다.
> 2. **문서 형태(A3=B)가 C 를 수용해야 한다.** AI 요청 행도 같은 고정 코어로 적히고, 새 필드가
>    필요해서는 안 된다(`before`/`after` 는 값 변화가 있을 때만 채우는 선택 필드).
> 3. **C 를 열 때 A8 을 다시 본다.** A2=B 아래에서 "중복 없음"(A8=A)이 성립하는 이유가
>    *"AI 요청은 활동 로그 밖"* 이기 때문이다. C 를 열면 같은 사건이
>    `llm_call_audits`·`request_usage_ledger`·활동 로그 셋에 살게 되므로, **A8 은 C 와 같은
>    결정에서 함께 판단한다.** 이 조건을 적어 두지 않으면 다음 사람이 A2 만 넓히고 두 정본을 만든다.
>
> **D(조회 기록)는 여전히 별도 결정**이다 — 축이 다르고 승격 감사(`access_grant_uses`)와 성격이 섞인다.

> **★ A2 추가 확정 (2026-08-09, 구현 뒤) — `writing/accept` 를 더해 19 → 20.**
>
> 구현 중 실측: `POST …/writing/accept` 는 **정본 draft version 을 실제로 저장한다**
> (`WritingAcceptService.accept` → `start_next_unit` → `SaveDraftResult`). §0.2 표는 이 경로를
> **성격**으로 "AI·작업 요청 14" 에 넣었는데, **A2 의 기준은 "사용자가 무엇을 *바꿨는가*"** 이므로
> 두 기준이 이 한 행에서 어긋난다. 그리고 accept 는 **주 저작 흐름의 저장 경로**라, 빼면 부모 계획
> §2 의 *"특정 원고가 마지막으로 저장된 것은 언제인가"* 가 **수동 저장에만** 답해진다.
>
> 구현자가 임의로 넓히지 않고 `excluded` + 주석으로 두어 물었고(오너가 승인한 것은 "B = 19"),
> 독립 검증도 같은 판단을 냈다. **오너 결정: 넓힌다.**
>
> - action 리터럴 **`draft_version_accepted`**, `target_type="draft_version"`.
> - **A8(중복 없음)은 그대로다** — 남기는 것은 *AI 요청*이 아니라 *정본 저장*이고,
>   `llm_call_audits`(호출)·`request_usage_ledger`(과금)가 담는 것과 **다른 사실**이다.
>   **이것은 C 확장이 아니다**(C 는 AI 요청 자체를 담는 것이며 `ai_request` 13 행이 그 대상이다).
> - 기록 조건은 **상태코드가 아니라 정본이 바뀌었는가**다: Gate 가 거부하면(200·`saved=null`)
>   남기지 않고, **502 partial envelope(version 은 저장되고 분석 job 만 실패)** 은 남긴다.
...
| **B. 정본 + 검토 결정 (**19** → 확정 후 **20**)**(추천·채택) | 위 + 승격·거절·편집·확정·apply·reconcile·finding 처리 (+ `writing/accept` = 추가 확정) | **"상태를 바꾼 사용자 행위" 전부**가 한 타임라인에 모인다 · AI 호출은 이미 두 축이 기록하므로 중복이 없다 | 쓰기 지점 19곳 |
...
> **확정 = B(오너 2026-08-09). 단 C 로 넓히는 비용이 "행 값 하나"여야 한다** — 조건 셋은 문서
> 상단 "A2 확정 조건" 에 있다. 요지: **분류표가 40 전수**(`logged`/`excluded`)이고, 문서 형태가
> C 행을 그대로 받으며, **C 를 열 때 A8 을 함께 다시 본다**.
