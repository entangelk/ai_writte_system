...
**상태**: 확정 — **D1=A · D2=B 채택** (2026-09-06, 오너)
...
- `revision` 은 **`set_group_status` 에서만 오른다**(`identity_groups.py:382`). `add_member`
  는 그룹 행을 건드리지 않으므로 판정이 멤버를 붙여도 값은 그대로다. 즉 실전에서 대부분
  `0` 이고 409 는 **상태가 전이된 그룹**(`contradicted` 등)에서만 난다 — 목록을 읽은 뒤
  승인을 누르기까지 값이 낡을 수 있고 **그때 409 로 재동기화하는 것이 D1=A 의 설계**다.
...
**추천: A. → 오너 채택: A**(2026-09-06). 로컬 1인 도그푸드 단계에서 필요한 것은 "승인 버튼이 눌린다" 하나이고, A 는
...
**추천: B → 오너 채택: B**(2026-09-06), 단 **트리거를 고쳐 단다**. 지금의 트리거(*"uncertain 표시가 실제 검토를 막을 때"*)는
