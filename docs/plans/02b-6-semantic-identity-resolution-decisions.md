...
- **D8 = 추천 잠금.** compare endpoint 불변, `_default_compare_service`가 embedding+memory vector+threshold 있을 때 semantic matcher를 붙인다. read-after-write eventual consistency(방금 promote된 memory는 worker drain 전이라 미검색 가능)는 수용 경계(self-exclusion이 대부분 흡수, backfill/재compare로 수렴). **검증 후속(관찰 #1)**: threshold+`CHROMA_HOST`를 켜고 `EMBEDDING_SERVICE_URL`을 빠뜨리면 fake embedding(차원 불일치)으로 첫 query가 실패하는 운영 footgun이 있어, `_build_semantic_matcher`에 wiring guard를 추가했다 — 이 조합에서 `EMBEDDING_SERVICE_URL` 부재 시 startup에서 `RuntimeError`로 fail-fast(회귀 `test_semantic_wiring_without_embedding_url_fails_fast`).
