...
> **정정(2026-07-25, 독립 검증 H2)**: 위 A안 설명의 "ERROR/WARNING 개수 반영"은 **구현되지 않았고 계약도 아니다.** 착수 후 밝혀진 사실 — `writing/gate.py`의 `decision`은 최우선 finding **하나**만 반영하므로, 실제 파생점수는 **decision-category 기반 거친 근사**이고 severity/개수는 의도적으로 제외돼 있다(v1.7.41 독립 검증 H1 정정). **canonical은 SoT 본문 §"LLM 파이프라인 관측(KPI)"**이며, 이 표 셀은 오너가 무엇을 보고 결정했는지를 남기기 위해 **소급 수정하지 않는다**(과거 결정 기록 불변 원칙). severity/개수 보충은 D2-B 후속 슬라이스다.
