...
상태: 확정 — D1~D6 결정됨(오너 2026-07-27, 아래 §2) · 착수는 인증 슬라이스 다음 · 코드 미착수`
...
> **결정(오너 2026-07-27): B + C를 별도 슬라이스로.** 로컬 기본은 self-host `dragonkue/bge-reranker-v2-m3-ko`(옵션 B, 임베딩 서비스와 같은 패턴), 외부 리랭커 API도 붙일 수 있게 `RerankProvider` seam을 함께 뚫는다(옵션 C). §2 참조. 추천은 "지금 제외"였으나 오너가 포함을 택했다.

**원래 추천(A, 지금 제외)의 근거는 기록으로 남긴다**: 리랭커는 삽입 지점·오프라인 평가(리랭킹이 실제로 Gate 품질을 올리는지)까지 필요한 **독립 기능**이라 별도 슬라이스가 맞다 — 이 판단은 결정에도 반영됐다(리랭커 = 독립 슬라이스). 조달만 "제외"에서 "self-host(B) 기본 + 외부(C) seam"으로 바뀌었다. 이 프로젝트가 이미 임베딩을 self-host 서비스로 돌리는 선례가 있어 B가 아키텍처적으로 일관되고, C는 같은 seam 뒤 어댑터로 대칭적으로 얹힌다.
...
## 2. 결정 요약 (오너 2026-07-27)

추천안에서 **D1·D5가 오너 결정으로 바뀌었다** — 세 축 전부 확장하되 슬라이스를 나누고, 리랭커도 이번 계획에 포함(유예 해제).

- **D1 = 세 축 전부 확장 · 슬라이스는 분리** (추천 A에서 확대). LLM·임베딩·리랭커를 모두 외부로 열되, **각각 독립 슬라이스**(LLM 슬라이스 → 임베딩 슬라이스 → 리랭커 슬라이스)로 간다. 오너: "모두 확장이 맞는데 LLM과 임베딩 슬라이스를 별도로." wire 계약·실패 모드·조달이 축마다 달라 한 슬라이스에 묶으면 성격이 섞인다.
...
- **D5 = 리랭커 포함 (유예 해제)** — 뉴럴 cross-encoder 리랭커를 이번 계획에 넣는다. **로컬은 self-host `dragonkue/bge-reranker-v2-m3-ko`**(임베딩 서비스와 같은 패턴의 별도 컨테이너 = 옵션 B), **동시에 외부 리랭커 API도 붙일 수 있게 seam을 뚫는다**(옵션 C). 오너: "로컬에는 이거 쓰고, 외부꺼도 쓸 수 있게 뚫어놓기." 즉 `RerankProvider` seam 뒤에 self-host 어댑터(기본) + 외부 API 어댑터(additive). **리랭커는 별도 슬라이스**(D1). 참고: 이 모델은 2026-07-05에 오너가 처음 임베딩으로 지목했다 유예됐던 바로 그 cross-encoder이며, 이제 제 역할(리랭커)로 복귀한다.
- **D6** — 인증 다음(결정 완료, 기록만).

**리랭커 조달 주의**: 리랭커 API에는 임베딩(`/v1/embeddings`)·LLM(`/v1/chat/completions`)처럼 사실상의 공통 wire 표준이 없다(Cohere·Jina·Voyage가 각자 형식). 따라서 외부 리랭커는 D2의 generic 어댑터로 한 번에 커버되지 않고, `RerankProvider` seam 뒤에 **provider별 어댑터**로 붙는다 — self-host bge-reranker가 기본 어댑터, 외부는 필요 시 추가.
...
- **리랭커의 오프라인 평가·튜닝**(리랭킹이 실제 Gate 품질을 올리는지, top-k·임계값) — 리랭커 슬라이스 착수 시 별도로 다룬다. 리랭커를 *넣는 것*은 이번 계획에 포함(D5)이나, 그 *품질 검증*은 슬라이스 내부 항목.
- Anthropic/Cohere 등 비-OpenAI-호환 **LLM** 전용 어댑터(D2 후속).
- call-site별 provider 오버라이드 구현(D4 설계만).
- 외부 임베딩 전환 시 대규모 재색인 마이그레이션 절차(차원이 바뀌는 경우에 한함 — `expected_dimensions` 가드가 fail-fast로 잡음).
