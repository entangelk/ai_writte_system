...
## Task — 후속 확정: `main.py` 라우터 정리 + 관리자 표면 주소 분리 (메모만)

### User Decisions and Rationale

- 오너: *"main 라우터 정리도 핸드오프에 있지 않았나? 좀 보기좋게 리팩토링 같은 거? **다른 주소로
  두는 방안이 필요할 것 같은데? 이건 기본이잖아.** 일단 후속에 할 거로 메모해두자. 포트폴리오는
  지금으로 괜찮지만 **신경을 안 썼다는 내용은 듣고 싶지 않거든.** 그리고 사실 리팩토링 하는 데
  몇 일씩이나 걸릴 것 같지도 않고."*
- **결정**: 후속으로 **확정**(할지 말지가 아니라 **언제**만 남았다). 착수는 8.4·8.2c로 열린 구멍을
  닫은 뒤. 관리자 주소 분리 방식(선택지 ⓐ~ⓓ)은 **착수 시 결정**한다.
- **오너 기억 정정**: HANDOFF에 있던 것은 `docs/plans/` **디렉터리** 재편(문서 부채)이고, **코드 쪽
  라우터 항목은 없었다.** 그래서 이번에 신설했다.

### Issues found — 실측(메모의 근거)

| 항목 | 실측 |
|---|---|
| `APIRouter` 사용 | **0건**. 75 operation 전부 `main.py` 한 파일의 `@app.*` 데코레이터 77개, **6,074줄** |
| tier 분포 | 제품 62 · 관리자 8 · 인증 3 · 관측 1 · health 1 |
| **분리 비용의 실체** | `create_app` 지역 바인딩 **33개**를 모든 endpoint가 **클로저로** 끌어 쓴다 → 파일만 옮기면 안 되고 라우터별 `register(app, *, …)` 등록 함수가 필요하다 |
| **함께 고쳐야 할 가드** | `test_billable_actions.py`·`test_auth_api.py`가 **`main.py`를 파일로 읽어** `@app.post(` 정규식으로 파싱한다. 분리하면 **실패한다**(조용히 약해지지 않는다 — 파싱 결과와 `app.routes`를 대조하는 셀이 있다). 작업량의 대부분이 이 둘의 갱신이다 |
| 관리자 표면 | `/admin` 8 operation이 제품과 **같은 포트**에 있고 방어는 `require_admin_user` 하나뿐 |

**오너의 "며칠 안 걸린다"는 맞다** — 논리 변경이 없는 기계적 이동이다. 다만 비용은 줄 수가 아니라
**33개 클로저와 가드 2개**에 있으므로, "파일만 쪼갠다"로 접근하면 안전망을 걷어내게 된다.

### Decisions (구현자 판단, 착수 시 오너 확인 대상)

- 관리자 주소 분리는 **ⓐ 같은 이미지·두 번째 ASGI 앱을 `127.0.0.1` 전용 포트로**를 추천한다 —
  D8-7 G1=C가 **이미 쓴 재료 그대로**이고(노출면 축소로 위험을 없앤다), 코드 변경이 가장 작으며,
  LAN에서 `/admin`이 **아예 안 보이게** 된다. ⓑ 별도 compose 서비스는 운영 표면이 하나 늘고,
  ⓒ nginx 계층 방어는 앱이 스스로를 못 지키며, ⓓ 완전 분리는 세션·저장소 공유 재설계라 범위 밖이다.

### Completed work

- [`HANDOFF.md`](../../../HANDOFF.md) — 추적 부채에 항목 신설(실측·클로저 비용·가드 2개·선택지 ⓐ~ⓓ·
  착수 순서). **"의도적으로 미룬 것이지 놓친 것이 아니다"** 를 근거와 함께 명시했다 — 경계를 파일
  배치가 아니라 **dependency tier + 전수 가드**로 세운 것이 선행 결정이었고, 그 가드가 서기 전에
  파일을 쪼개면 이동 중에 경계가 흐려진다. Next Tasks 2번으로도 큐에 올렸다.

---

## Task — Slice 8.4 착수 브리프 작성 (W1~W7, 오너 결정 대기)

### Goals

1. 8.3이 닫힌 뒤 다음 슬라이스를 연다 → **8.4(제품 경로 배선)** 착수 결정 브리프.
   검증: `docs/plans/08-4-product-wiring-decisions.md`가 선택지 표·추천·후속·범위 밖을 갖춘다.
2. 브리프의 근거를 **문서가 아니라 실측**으로 만든다 → 검증: 프론트 호출부·에러 표시 경로를
   코드에서 직접 센다.
3. 인덱스 가드를 통과시킨다 → 검증: `tests/test_docs_indexes.py` green.

**전제(오너 재지정 가능)**: HANDOFF Next Tasks 1번이 "다음은 **8.4**와 8.2c"라 적고 순서를 정하지
않았다. **8.4를 먼저** 잡은 근거는 지금 제품이 **한도가 켜진 채 사용자가 그것을 볼 수 없는 상태**라는
것이다(아래 실측). 8.2c(이름 이력·purge 문구)는 **현재 거짓 서술을 만들고 있지 않다** — L6을 통째로
미룬 덕에 "거짓 UI 문구 구간"이 생기지 않았기 때문이다(8.2 브리프 범위 결정 나). 즉 8.2c는 급하지
않고 8.4는 이미 새고 있다.

### Issues found — 실측이 드러낸 것 셋

| # | 실측 | 왜 중요한가 |
|---|---|---|
| **1** | `frontend/src`에서 `402`·`429`·`X-Confirm-Duplicate`·`quota` 히트가 **생성물 `schema.d.ts` 하나뿐** | 손으로 쓴 프론트에 quota 인지가 **0건**이다. 한도에 걸린 사용자는 `"402: daily quota exhausted…"` **원문**을 본다(`describeWritingError` 최종 분기·`describeApiError`). 서버가 싣는 `Retry-After`는 `ApiError`가 `status`·`detail` 둘만 들고 있어 **프론트에 도달하지 못한다** |
| **2** | 유료 9경로 중 프론트 호출부가 있는 것은 **5개** (`writing_generate`·`gate`·`revise_and_gate`·`accept`·`analysis_extract`). `writing_revise`·`writing_report`·`analysis_compare`·`context_search`는 **호출부 0** | "9경로 전수 배선"이라 쓰면 **거짓**이 된다. 배선 대상은 5개이고 그 사실을 브리프 §0.2에 못박았다 |
| **3** | **★ 한 번의 "생성" 클릭이 유료 요청 2~3건을 연쇄로 부른다** — `generate` → `gate` → (지적 있으면) `revise-and-gate`. `accept`까지 하면 흐름당 최대 **4회** | 8.0 B1=A는 **지켜지고 있다**(요청 1건 = 1회). 어긴 것이 없다 — 다만 **회원이 체감하는 단위(클릭)와 과금 단위(요청)가 1:1이 아니라는 사실이 화면에 처음 드러나는 자리가 8.4**다. 기본 한도 일 20이면 **전체 흐름 기준 하루 5~6회**이며, 이것이 잔여 표시 문구·확인 대화 문구·P7 잠정값 재평가의 공통 입력이 된다 |

### Decisions (구현자 추천 — 오너 결정 대기)

| 항목 | 추천 | 한 줄 근거 |
|---|---|---|
| **W1** 관리자·내부 면제 | **A. 면제 없음 — 정책 행(`limit=None`)으로만 푼다** | 예외를 코드가 아니라 **데이터**로 표현하면 시행에 우회로가 안 생긴다. 게다가 dogfood 직전이라 **관리자 사용량이 곧 실사용 데이터**이고 면제하면 P7 재평가의 입력이 사라진다 |
| **W2** 402/429/403 프론트 분기 | **A. 공용 `describeQuotaError()` 한 곳, 화면은 표시만** | 분기가 한 곳이라 5 호출부가 안 갈라진다 · `status`로만 갈라 H3 준수. **기존 `describeWritingError`에 넣지 않는다** — 그 함수가 이미 `detail` 부분문자열로 분기하는 H3 위반 지점(부채 N-2)이라 위반 표면이 커진다 |
| **W3** 429 확인 UX | **A. 블로킹 확인 → "예"면 헤더 달아 재전송** | 오너 성향(2026-08-03)과 일치. B(안내만)는 "다시 누르면 통과"라 **확인을 무력화**하고, D(자동 재시도)는 사용자가 모르는 사이 사용량을 늘린다 |
| **W4** 확인 헤더의 층 | **A. 유료 호출 함수의 명시 인자** | 확인이 **사용자 행동에서만** 나온다는 것이 타입으로 보인다. 래퍼 자동 재시도·세션 플래그는 G4=A의 방어를 지운다 |
| **W5** 잔여 표시 | **B. `GET /me/quota` 신설(operation 76)** | 지금 회원이 자기 잔여를 볼 통로가 **0개**다. C(응답 헤더)는 **실패 응답에도 실어야 쓸모가 있어** 전역 handler·partial envelope 6곳까지 계약이 번지고 이 저장소에 응답 헤더 계약 선례가 없다 |
| **W6** 프론트 안정 키 | **A. 도입하지 않음(현행)** | 안정 키면 두 번째 요청이 dedupe에 막혀 **확인해도 못 뚫는다** — G4=A(의도적 2안 받기)와 정면 충돌. 두 계약이 서로를 상쇄한다 |
| **W7** 새 유료 화면 전수 가드 | **B. 행동 가드(vitest, fetch mock)** | C(소스 파싱)가 더 촘촘하지만 **정확히 그 형태의 부채가 지금 `main.py` 분리를 비싸게 만들고 있다**(`@app.post(` 정규식 가드 2개). 같은 실수를 프론트에 복제하지 않는다 |

**확인 대화 문구안**(8.2b §0.4가 정한 성격 — 꾸짖지 않고, 의도를 묻고, 대가를 알린다 — 에 §0.3 실측을
얹었다): *"방금 같은 요청을 보냈습니다. 하나 더 만들까요? 새로 만들면 사용량이 1회 더 듭니다.
(이번 창 잔여 N회)"*. **연쇄 중간의 429는 그 단계만 묻는다**.

### Completed work

| 산출물 | 내용 |
|---|---|
| [`docs/plans/08-4-product-wiring-decisions.md`](../../plans/08-4-product-wiring-decisions.md) | 착수 브리프 — §0 실측 5절 + **W1~W7** 각각 선택지 표·추천·근거 + 후속 고려 5건 + 범위 밖 7건 + 결정 뒤 구현 순서(회귀 먼저·뮤테이션 5종·독립 검증) |
| [`docs/plans/README.md`](../../plans/README.md) | Phase 8 표에 8.4 등재. **stale 상태 2칸 정정** — 8.2b "재검증 대기"→**PASS(2026-08-04)**, 8.3 "독립 검증 대기"→**PASS(비차단 5건 처리)** |
| `README.md`·`docs/plans/README.md` | 브리프 수 주장 갱신(79→80, 96→97). `VerificationCountClaimsTest`가 요구한 것이고 **가드가 실제로 물었다** |

### Verification

- `python3 -m pytest -q tests/test_docs_indexes.py` → **9 passed / 10 subtests**.
  중간에 **4 failed**로 정확히 물었다(`docs/plans/README.md가 브리프 79개라 적었지만 실제는 80개`,
  최상위 README 포함 4칸) — 숫자 주장을 디스크에 묶은 2026-08-02 가드가 의도대로 동작한 실증이다.
- 코드 변경 0줄. 회귀 기준선 무변(backend 2150/1/1923 · frontend 236/17).

### Next steps

1. **오너 결정 W1~W7** — 특히 W1(면제)·W3(확인 UX 문구)·W5(`GET /me/quota` 신설 여부)는 제품 정책이다.
...
## Task — Slice 8.2c 착수 브리프 작성 (N1~N6, 오너 결정 대기)

### Goals

1. 8.4가 닫혔으므로 남은 8.2c를 연다 → 검증: `docs/plans/08-2c-project-name-history-decisions.md`가
   선택지 표·추천·후속·범위 밖·구현 순서를 갖춘다.
2. **이 슬라이스가 저장 결정이 아니라 계약 개정임을 먼저 드러낸다** → 검증: 낡게 되는 문서·문구를
   실측으로 열거한다.
3. 인덱스 가드 통과 → 검증: `tests/test_docs_indexes.py` green.

### Issues found — 실측이 드러낸 것 넷

| # | 실측 | 왜 중요한가 |
|---|---|---|
| **1** | L6을 구현하면 **동시에 낡는 자리가 셋**이다 — `mongo_collections.md` §43B("stores no project name…") · purge UI 문구("전체가 삭제되며") · 8.2가 적은 "삭제된 프로젝트는 id로만 답해진다" | 코드만 고치면 정본이 거짓이 되고, 정본만 고치면 관리자가 **거짓 경고**를 읽는다. L6이 "같은 슬라이스에서 함께"라고 못박은 이유가 실측으로 확인된다 |
| **2** | **★ purge endpoint는 파괴 전에 이미 `core_sot.get_project()`로 project를 손에 들고 있다**([`main.py:3546`](../../../services/application/app/main.py#L3546)) | L6의 A안 설명은 "생성·개명 시 갱신"을 전제했는데, 이 실측이 **"파기 시점에만 스냅샷"** 을 실질적 선택지로 만든다(N3). 쓰기 지점이 셋에서 하나로 줄고 살아 있는 이름의 **두 정본 문제**가 사라진다 |
| **3** | **★ 새 컬렉션이 `project_id` 필드를 가지면 reconciler가 이름 이력을 지운다** — [`purge_reconciler.py:49-56`](../../../scripts/purge_reconciler.py#L49)이 컬렉션 목록을 DB에서 발견하고 표본 한 건이면 전체가 sweep 대상이다 | 8.2가 `target_project_id` 개명으로 피한 그 함정이며, 여기서는 **슬라이스의 목적을 정확히 반대로 실행**하게 된다. `_id`를 project id로 쓰면 필드 자체가 안 생겨 구조적으로 안전하다 |
| **4** | **원장을 읽는 API·화면이 아직 0개**다(8.5 미착수) | 이름 이력의 **소비자가 없다**. 8.1·8.2·8.2b와 같은 "저장만" 슬라이스가 되는 것이 자연스럽고, 조인 결과의 응답 형태는 원장 조회를 설계하는 8.5가 정해야 한다(N4) |

**오너 문언의 미해결 부분도 남겼다**: *"프로젝트 이름·**제목**"*의 "제목"이 `Project.name`인지
`Draft.title`까지인지 코드로는 두 갈래다. 원장의 축이 `target_project_id` 하나뿐이라 **draft 제목은
조인할 상대가 없다** — N2에서 선택지로 세웠다.

### Decisions (구현자 추천 — 오너 결정 대기)

| 항목 | 추천 | 한 줄 근거 |
|---|---|---|
| **N1** 저장 위치 | **A. 전용 컬렉션 `project_name_history`** | 오너가 보존 기간을 나중에 정하겠다고 했으므로 **자기 수명을 갖는 개체**여야 한다. 감사(B)·원장(C)에 얹으면 그 문이 닫힌다 |
| **N2** 범위 | **A. 프로젝트 이름 최신 한 값** | 원장 축과 1:1이고 남는 사용자 텍스트가 최소다. **B(개명 이력)는 나중에 넓힐 수 있지만 C(draft 제목)는 되돌릴 수 없다** |
| **N3** 쓰기 시점 | **A. 파기 시점에만 스냅샷**(파괴 전, fail-closed) | 살아 있는 이름의 정본은 `projects`다 — 복제하면 **두 정본 문제**가 생긴다. 대가(reconciler 수습 경로는 이름을 못 남긴다)는 드문 장애 경로에 있고, B의 대가는 매일 쓰는 개명 경로에 있다 |
| **N4** 조회 통로 | **A. 저장 + 계약만, 조회는 8.5** | 조인 결과의 응답 형태는 원장 조회 API가 정해야 한다. 지금 만들면 8.5에서 갈아엎는다 |
| **N5** UI 문구 | **A. 남는 것을 명시** | 프로젝트 이름은 **사용자가 쓴 텍스트**라 "남는다"를 숨기는 쪽이 위험하다. D8-6의 확인 UX와 결이 같다 |
| **N6** 문서화 | **A. §43B 예외 포인터 + 새 절 + SoT 변경이력** | §43B의 그 문장은 **D8-6 검증이 근거로 삼았던 문장**이라, 예외를 안 달면 다음 검증자가 같은 문장으로 이번 구현을 결함이라 판정한다 |

### Completed work

| 산출물 | 내용 |
|---|---|
| [`docs/plans/08-2c-project-name-history-decisions.md`](../../plans/08-2c-project-name-history-decisions.md) | 착수 브리프 — §0 실측 5절 + **N1~N6** 선택지 표·추천 + 후속 고려 4건 + 범위 밖 5건 + 구현 순서(회귀 먼저·뮤테이션 5종·독립 검증) |
| [`docs/plans/README.md`](../../plans/README.md) · `README.md` | 인덱스 등재 + 브리프 수 주장 갱신(80→81, 97→98) |

### Verification

- `python3 -m pytest -q tests/test_docs_indexes.py` → **12 passed / 10 subtests**.
- 코드 변경 0줄. 회귀 기준선 무변(backend 2173/4/1931 · frontend 265/18).
