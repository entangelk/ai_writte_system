...
#### ★ 보강 — 검증자가 괄호로 흘린 지적이 더 컸다 (`ad387a4`)

검증 기록은 *"M22 의 4 라는 수는 실행 머신 시간대에 민감하다"* 고만 적었다. 실측하니
**`TZ=UTC` 에서는 그 뮤테이션이 0 failed** 다 — UTC 머신에서는 `getFullYear` 와
`getUTCFullYear` 가 같은 값을 내므로 **잡을 것이 없다**.

| TZ | 뮤테이션 전 | M22(UTC 그룹) 적용 |
|---|---|---|
| `Asia/Seoul` | 8 passed | **3 failed** |
| `UTC` | 8 passed | **0 failed → 보강 후 1 failed** |
| `America/New_York` | 8 passed | **3 failed** |

**즉 로컬/UTC 불변식이 UTC 머신에서는 전혀 안 잠겼다.** UTC 컨테이너·CI 에서
로컬→UTC 리팩터링이 전부 green 으로 통과하고, 버그는 **시간대가 다른 사용자에게서만**
터진다. 이 저장소는 머신 셋을 옮겨 다니고 컨테이너 기본이 UTC 라 가정이 아니라 실제
위험이다. → 로컬 달력 셀을 **시간대 고정(Asia/Seoul)** 으로 바꾸고 **자정 양쪽 끝**을
둘 다 본다(offset 부호에 관계없이 한쪽은 UTC 경계를 넘는다).

**★ 일반화**: *"뮤테이션이 무는 수"가 환경에 따라 달라지면, 그 환경 중 하나에서는
0일 수 있다.* 시간대·로케일·파일시스템처럼 **머신이 주는 값에 기대는 가드는 그 값을
테스트가 고정해야** 한다. 안 그러면 가드의 세기가 머신마다 다르고, 가장 약한 머신이
CI 일 수 있다.

그리고 비차단(머리글 시각 강조)도 함께 닫았다 — 본문 잉크 + 하단 실선 + 날짜 옆 점.
**10.3~ 로 미루지 않은 이유는 오너가 실제로 기능을 못 알아본 자리**여서다
(*"머릿글이 안 접히는 것 같다"* — 접힘은 정상이었고 강조가 약했다).

#### 회귀·부하

frontend **294/22 → 306 passed / 23 files**(보강 +1 포함). 종전 값 `305/23`.(**+11 cells · +1 file**).
build **702 → 703 modules** · 진입 **420.81 → 421.78 kB**(+0.97) · CSS 30.47 → **30.94 kB**.
...
같은 뿌리를 가진 발견이 반복됐다. **선언과 실제가 갈라져 있는데 아무도 안 보는 자리**다.

1. **주석이 말하는 의도를 설정이 구현하는지 확인한다.** compose 가 *"Shares the
   application image"* 라 적고도 공유하지 않아 **worker 가 15일 뒤처진 채
   `PROJECT_PURGED` drain 없이** 돌고 있었다(D8-6 파기의 절반). 오너 질문이 없었으면
   안 봤을 자리다.
2. **렌더 테스트 사정거리 밖의 사용자 노출면은 DOM 이 아니라 파일을 읽어 잠근다.**
   `<head>` 의 탭 타이틀이 옛 이름으로 남았는데 289셀이 전부 green 이었다 — vitest 가
   `index.html` 을 로드하지 않기 때문이다. `document.title` 로 고쳐도 같은 맹점이 남는다.
3. **"이미 방향이 잡혀 있다"는 미학 판단의 근거가 못 된다** — 그 방향이 *선택된 것*인지
   *기본값*인지 먼저 본다. 현행 크림+테라코타를 "글쓰기 도구다운 방향" 이라고 방어했는데
   생성형 AI 디자인의 대표 기본값 좌표였다.
4. **뮤테이션이 무는 수가 환경에 따라 달라지면 그 환경 중 하나에서는 0일 수 있다.**
   로컬/UTC 불변식이 `TZ=UTC` 에서 전혀 안 잠겼다 — **가장 약한 머신이 CI 일 수 있다.**

그리고 **가드를 만든 값이 같은 날 회수됐다**: 새 CSS 가드가 8.4 의 숨은 결함(2026-08-04
부터 테두리·배경 없이 렌더되던 `.writing-confirm`)을 첫 실행에서 잡았고, 새 prose 가드가
**구현자 자신의 §6 위반**(`git checkout` 으로 커밋 안 한 편집을 날린 것)을 그 자리에서
잡았다.

## Next steps

1. **오너: 13표면 육안 확인** — 10.1 팔레트·10.2 그룹이 적용된 상태를 눈으로 본다.
   회귀가 프로덕션 CSS 를 한 셀도 안 무는 구간이라 **눈이 유일한 검증 수단**이다.
   그 관찰이 곧 10.3~ 의 입력이므로 착수보다 먼저다(구현자·검증자 공통 권고).
2. **Phase 10 Slice 10.3~ 화면별 적용** — 첫 화면은 `DraftEditor`(가장 오래 머무는 화면).
   첫 항목 둘: **타이포**(트리거 = 그 화면을 다시 그릴 때 그 화면의 font-size 부터.
   전역 스케일을 먼저 정하지 않는다) · **관측 화면 차트**(JS 리터럴이라 팔레트가 안 닿아
   혼자 옛 색으로 남아 있다. 계열 팔레트를 따로 세우고 새 배경에 대해 재검증).
3. **오너 결정 잔여**: dogfood 착수(GATE-1) · H2(API 문서 제품명 — **착수 전 오너에게
   자세히 설명할 것**, HANDOFF 에 설명 항목 다섯이 있다).
4. **정리 대상**: 확인용 계정 `timeline_demo` 와 프로젝트 `6a795ab928e4a53aa000a824`.
