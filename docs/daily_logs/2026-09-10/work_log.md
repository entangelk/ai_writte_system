...
- **전수 3025 passed / 1 skipped / 4124 subtests · EXIT=0**(1966초, 알파·호스트, test-mongo ON, 커밋 `12251b3` — 문서 편집 전 고정 트리). 직전 3023/1/4121(`8990fbc`)과의 차는 **셀 +2 = 배선 셀 + 절차 표 기준선 셀** · **subtest +3 = 백분율 두 자리(+2) + 브리프 주장 셋째 자리(+1)** 로 전건 귀속되고, 초점 실측과 정확히 맞는다(예측 3025/1/4124 = 실측). **S13 은 재발하지 않았다** — 같은 날 4회 중 1회(세션 53 §3 의 플레이크 판정과 일관).
- 전수를 **한 번만** 돌렸다: 5% 지점에서 산문 숫자 스윕의 셋째 자리를 발견해 **그 회차를 죽이고** 고친 뒤 다시 시작했다(문서를 고치며 전수를 돌리면 그 회차는 버리는 것이 이 저장소의 선례다 — 09-04·09-09·09-10 셋).
...
## 세션 55 — 계정 탈퇴 Slice 4(화면) (오너 지시 "핸드오프 읽고 다음 작업 진행해줘. 슬라이스4 일꺼야")

Slice 3 검증이 닫혀 1번이 된 **Slice 4(화면)** 를 열었다. 착수 전에 결정 브리프를 올리고 오너 결정 둘을 받았다 — 커밋 `534fb74`(브리프) · `b68d923`(구현) · SoT **v1.8.54**.

### 착수 결정 (브리프 → 오너)

브리프 [`slice4-withdrawal-screen-decisions.md`](../plans/slice4-withdrawal-screen-decisions.md). **왜 브리프였는가** — 둘 다 구현자가 조용히 고르면 안 되는 자리였다.

- **D1(배너 자리)은 Slice 2 가 이 슬라이스로 *명시 유예* 한 질문**이었다. 그 브리프가 *"배너를 전 화면 상단 고정을 원하게 되면 그때 ⓐ(`/auth/me` 확장)를 다시 본다 — **그 트리거가 이 결정을 되돌리는 유일한 자리다**"* 라고 적어 두었으므로, 전역 고정을 고르는 것은 **Slice 2 의 읽기 표면 결정을 번복할지까지 포함**한다.
- **D2(관리자 잔여 정리)는 계획서 두 줄이 서로 다른 크기를 말하고 있었다.** Slice 3 브리프 ⓔ 가 *"관리자 endpoint·화면은 Slice 4"* 로 넘겼는데, 실측하니 `AdminUserPayload` 에 탈퇴 축이 **한 필드도 없어**(`id`·`username`·`is_admin`·`is_active`·`status`) 관리자는 *어느 계정이 멈춰 있는지* 조차 못 본다 — 즉 화면 하나가 아니라 **새 읽기 필드 + 새 operation + 새 화면** 셋이다. ⓔ 를 고른 근거가 *"이 축의 슬라이스 넷이 전부 한 화면 또는 한 계약이었고 그 크기가 검증을 성립시켰다"* 인데, 문언대로면 **Slice 4 만 그 관례를 깬다.**

**오너 결정: D1=ⓐ 전역 배너 · D2=ⓐ Slice 4b 분리.** 둘 다 구현자 추천안 채택.

### Completed work

**범위가 프런트 전용으로 확정됐다** — `GET`·`POST`·`DELETE /me/withdrawal` 셋이 Slice 1·2 에서 이미 서 있어 **operation 105 무변**이고 등재 6곳을 지나지 않는다. `npm run gen:api` 재생성이 **무차분**인 것으로 확인했다.

- **`me/withdrawal.tsx` 신설** — 탈퇴 축의 프런트 전부가 한 파일이다: `WithdrawalProvider`(상태 한 벌) · `WithdrawalBanner`(셸) · `WithdrawalSection`(`/me`) · `remainingGraceDays` · `seedWithdrawal`/`resetWithdrawal`.
  - **★ 상태가 셸에 있는 것이 이 슬라이스의 계약이다.** D1=ⓒ 가 쓰기를 **전 화면에서** 403 으로 막으므로 설명도 전 화면에 있어야 한다. `/me` 안으로 옮기면 편집기의 403 이 **원인에서 가장 먼 자리**에서 정체불명으로 돌아온다 — 13·14번이 기록한 함정과 같은 모양이다.
  - **★ 상태를 두 벌로 두면 취소가 배너를 못 끈다.** `/me` 의 취소와 배너의 취소가 **같은 `apply`** 로 들어간다.
  - **★ 남은 일수는 서버 `purge_due_at` 에서만 유도한다** — 빼기 + **올림**. 30 을 박으면 두 번째 정본이고, 올림인 이유는 서버 판정이 `now >= purge_due_at` 이라 **0.2일이 남아도 아직 취소할 수 있는 하루**이기 때문이다(내림하면 화면이 "0일" 이라 말하는 동안 취소가 여전히 된다).
- **409 를 두 뜻으로 갈랐다.** H3 가 `detail` 분기를 금지하므로 판정은 **상태코드 + 어느 동작이었는가**다 — 요청 409 = 마지막 활성 관리자(D6, 사람 말로) · 취소 409 = **취소할 것이 없음**(오류가 아니다. 다른 탭이 먼저 취소한 것이라 서버의 지금 상태를 다시 읽어 화면을 맞춘다).
- **불가역 확인은 프로젝트 파기 UI 선례 그대로** — `.confirm-panel` + **사용자명** 입력 + `.danger-button`. **새 CSS 규칙을 만들지 않았다**(`.withdrawal-banner` 는 `display:flex`·`gap` 배치만이라 버튼 세 규칙·타입 계단 가드의 대상이 아니다).
- **`AuthUserContext` export.** 화면 셀이 앱 셸 없이 최소 자리를 세우기 위해서다 — `AuthGate` 자체를 세우면 세션 확인 요청이 그 셀의 fetch 시퀀스에 끼어든다. 프로덕션 소비자는 `useAuthenticatedUser` 하나 그대로다.
- **`vitest.setup.ts` 기본 시드**(`seedMemberQuota` 선례). 배너가 셸에 있어 `GET /me/withdrawal` 가 모든 화면 마운트에 끼어드므로, 기본값 *탈퇴 중 아님* 이 없으면 무관한 셀들이 무더기로 깨진다 — 실제로 처음 전수에서 **12셀이 그렇게 빨개졌다**.

### 착수 중에 드러난 것

- **★ 순환 import 를 한 번 만들었다가 끊었다.** `WithdrawalSection` 이 사용자명을 얻으려 `AuthGate` 를 import 했는데, `AuthGate` 가 이 모듈의 provider·배너를 부르므로 **순환**이었다(`tsc` 는 통과한다 — ES 모듈이 함수 선언을 끌어올려 런타임에도 돌아간다). 백엔드의 `routers/* → main` 금지와 같은 이유로 끊었다: **부르는 쪽이 이미 세션을 들고 있으니 값을 prop 으로 내린다.**
- **★ 배너에 `/me` 링크를 넣었다가 뺐다.** `navigationLinks.test.ts` 가 `inline-navigation-link` **사용 파일 집합을 정확 일치**로 잠그고 있어 새 파일이 그 클래스를 쓰면 실패한다. 가드가 이상한 것이 아니라 — 배너의 일은 *설명 + 취소*이고 링크는 없어도 되는 것이었다. **가드가 범위를 되물어 준 자리다.**
- **★ HANDOFF 의 프런트 기준선이 한 슬라이스 뒤처져 있었다.** 적힌 값은 `467 passed / 38 files`(2026-09-07)인데 이번 실측이 `460 / 39` 였다. 원인을 유도했다: **`f66d656`(채택 버튼 제거, 2026-09-08)이 셀을 대량 지웠고**(`WritingPanel` −358 · `ScratchRecovery` −246 · `DraftEditor` −247 줄) 그날 work_log 는 **449 / 38 을 스스로 적었지만**([`2026-09-08/work_log.md:562`](../2026-09-08/work_log.md)) HANDOFF 줄을 안 고쳤다. 이번 값과 정확히 맞는다 — **449 + 11(신규 파일 1) = 460 · 38 + 1 = 39.** 백엔드 기준선은 README 절차 표와 상호 가드가 걸려 있는데 **프런트 값에는 그 가드가 없다** — 그래서 조용히 샜다.

### Verification (세션 55)

- **변이 6종 전부 기명 셀 재실패** — MV-1(남은 일수 30 박기) **6실패** · MV-2(배너를 `/me` 경로로 한정) **5실패** · MV-3(상태 두 벌 = `apply` no-op) **3실패** · MV-4(요청 409 를 일반 오류 문구로) **1실패** · **MV-5 over**(확인 입력이 맞아도 계속 잠금) **3실패** · **MV-6 over**(취소 409 를 오류로) **1실패**. 변이마다 **커밋 → 변형 → 절대경로 원복 → `git status --short` 확인** 순서를 지켰다.
  - **★ `cwd` 함정을 한 번 밟았다** — MV-2 원복을 `frontend/` 안에서 상대경로로 쳐 `pathspec … did not match` 가 났다. HANDOFF 가 경고한 바로 그 자리이고, 다행히 실패한 것이라 트리는 안 다쳤다. 이후 전부 `git -C <절대경로>` 로 쳤다.
- **프런트 전수 460 passed / 39 files · EXIT=0**(635초, 파일 캡처). `tsc --noEmit` 0 · `npm run build` 성공 · `gen:api` 무차분.
- 백엔드는 **문서 축만 건드렸다** — 초점 가드 넷(`test_design_token_provenance`·`test_repo_hygiene`·`test_product_name`·`test_docs_indexes`) 36 passed / 1018 subtests.

### Next steps

- **Slice 4b(관리자 잔여 정리)** — 브리프가 먼저다. 갈래 셋(범위: 계정 하나 vs 전역 스윕 · dry-run 두 단계를 화면에 옮길지 · `admin_audit` 등재 여부)이 [`slice4-…-decisions.md`](../plans/slice4-withdrawal-screen-decisions.md) §후속 고려에 있다. **트리거 없음 — 일정만 안 잡혔다**(S-2 와 같은 분류).
- **Slice 5(정책 문서 §8 → §6 승격)** 는 그대로 열려 있다.
- **육안 확인 목록에 배너가 늘었다** — 좁은 화면에서 배너가 두 줄이 될 때 헤더와 첫 화면이 밀리는지.

### 전수 판단 정정 (세션 55 말미, 오너 지적)

**백엔드 전수를 돌리려다 오너가 물었다 — *"브리프 문서 하나만큼이면 전수를 할 필요는 없지 않아?"*. 맞는 지적이라 중단하고 통제 대조로 바꿨다.**

- **근거는 `git diff --name-only 180d401..HEAD -- services scripts tests` 가 비어 있다는 것**이다. 백엔드 소스도 테스트도 한 줄 안 바뀌었으므로 전수가 답하는 것은 **동작이 아니라 기준선 숫자 하나**뿐이었다.
- 그래서 **기준 리비전을 워크트리로 띄우고 문서·프런트를 읽는 가드만 양쪽에서** 쟀다 — **45 passed / 1054 → 1055 subtests**. `+1` 은 전부 `test_repo_hygiene`(608→609)이고, 그 루프가 **문서당 1 subtest**(`_is_document` = `docs/` + 루트 `*.md`)라 **새 브리프 하나에 정확히 귀속**된다(프런트 `.tsx` 둘은 문서가 아니다). 내가 고친 문서를 읽는 나머지 둘(`test_service_policy_contract`·`test_product_name`)도 초록.
- **★ HANDOFF 의 통칙이 틀렸다는 것도 이때 드러났다** — *"문서 파일 하나가 subtest 를 2 늘린다"* 는 `docs/plans/` 브리프에는 **안 맞는다**(실측 +1 · `test_docs_indexes` 는 310 그대로). 값을 고치는 대신 **재는 법**을 그 자리에 적었다.
- **기준선 줄은 유도값으로 표기했다** — `4124`(커밋 `12251b3` 직접 실측) → `4125`(대조 유도). **유도가 정당한 조건이 "백엔드 소스 무변"이라는 것을 그 줄이 함께 말한다.**

**교훈**: 이 저장소는 *슬라이스를 닫으면 전수* 를 관례로 삼지만, **관례가 아니라 변경 면이 절차를 정한다.** 문서·프런트 전용 슬라이스에서 30분짜리 백엔드 전수는 **검증 가치가 0**이고, 같은 답을 2분짜리 두 리비전 대조가 **더 많은 정보(귀속)와 함께** 준다.
