...
| 문제 | 원인 | 해결 | 결과 |
|---|---|---|---|
| 폐쇄 보고에 **커밋 해시를 지어 썼다** | 기록을 커밋 **전에** 쓰면서 해시 자리를 임시값으로 채우고 그대로 뒀다 | `git log` 로 실제 해시(`85259d6`·`9e2e5cf`)를 넣었다 | **자기 점검에서 잡았다.** 이 저장소가 기록에 거는 요구(측정·식별자는 그 자리에서 잰다)의 정확히 반대 행동이라 남긴다 — **해시는 커밋 뒤에 쓴다** |
...
- 초점: 워커 축 43 → **44 passed** · `test_docs_indexes` 16 → **17 passed / 307 → 309 subtests**.
