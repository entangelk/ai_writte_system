...
> **[세션 5 — 같은 날 추가, 알파] 보안 점검 + 자기 가입(승인제) 슬라이스.**
> 오너 요청으로 시작: *"보안쪽으로 한번 점검해줘봐. 가장 중요한거니까. 인증이나
> 보안쪽말야"* + 계약 해석 정정(*"요청은 할 수 있지만 그 확인은 관리자가 한다,
> 나중에 자동 가입처리를 하더라도"*) → 승인제 가입 구현.
>
> **보안 점검 결과(코드·런타임 실측)** — 잘 선 것: 세션 토큰 256비트 CSPRNG +
> sha256만 저장, 쿠키 HttpOnly/Secure/SameSite=Lax, Argon2id, 열거 타이밍
> 하드닝, 저장소 5종 전부 127.0.0.1(런타임 재생성 완료 확인 — 08-02 함정 해소),
> `.env` gitignore·이력 0건. **발견**: ① 로그인 브루트포스 무방비(→슬라이스
> 1-c로 닫음) ② 활성 admin 시드 계정(visual_demo)·오너 본인 계정 부재(→정리
> 대기, 오너 계정명 필요) ③ `/docs`·`/openapi.json` 공개(8520에서 200 — 오너
> 결정 대기) ④ Secure 쿠키×http 배포 충돌 예고(홈서버 http://IP 접속 시
> 로그인 불가 — HTTPS 또는 `AUTH_COOKIE_SECURE=false` 결정 필요) ⑤ nginx 보안
> 헤더 부재 ⑥ `AUTH_SESSION_TTL_HOURS` 가드 0건(→1-c에서 폐쇄).
>
> **승인제 가입 슬라이스 1-a~1-f**(전부 커밋·뮤테이션 검증, 실서비스 관통 실측):
> - 1-a `4cc16f8` — `User.status`(pending/active/rejected, 기존 행 active
>   기본) + `POST /auth/signup` 공개. rejected 재요청 허용·deactivated는 거부.
> - 1-b `ab12fcd` — 로그인 403+사유(대기/거절 구분). 403 생산자 둘→셋,
>   H3 등재 예외. 틀린 비밀번호는 401 통일 유지(열거 방어).
> - 1-c `99e0b2b` — 브루트포스 잠금(username별 5회→5분, env, 기동 거부) +
>   TTL env 가드 셀 4개(부채 폐쇄).
> - 1-d `241896c` — 관리자 승인 API 3종(ADMIN 8→11, pending 행만 동작).
> - 2b197f8 — 전수 가드 등재(활동 분류표·admin 에러 선언).
> - 1-e `da9d5d5` — 프론트 가입 폼 + 403 안내(프론트 329 passed).
> - 1-f `8eab5c6` — 관리자 승인 UI(프론트 331 passed).
> - 1-g — SoT v1.7.97 + 이 문서 + CHANGELOG + HANDOFF.
>
> **검증**: 백엔드 전수 **2480 passed / 0 failed** (skip 4 = 알파 관례 Chroma 1
> + ES 3) · 뮤테이션 8종 전부 재실패 · 실관통(nginx 5520): 가입 201 →
> 대기 403 → 승인 200 → 로그인 200 / 거절 403 / 재승인 409 / 5회 실패 후
> 정답 401(잠금). 브리프: `plans/auth-signup-approval-decisions.md`.
>
> **함정(실제로 밟음)**: cwd 지속으로 `git checkout -- frontend/...`가
> frontend/ 안에서 pathspec 실패해 **원복이 안 된 채 방치**될 뻔(3회) —
...
**오너에게 남은 것**: ① key[0]의 IP 제한 — 허용 목록에 이 머신의 공인 IP를
