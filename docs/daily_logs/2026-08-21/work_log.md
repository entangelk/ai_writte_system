...
- **환경**: `.env` 없던 알파에 `LLAMA_CTX_SIZE=16384`만 넣어 생성(커밋 금지 분).
  ...
  quota 무제한 정책 행 동반). 오너 요청으로 **일반 계정 `visual_user`(비관리자,
  정책 행 없음 = 코드 기본 일 20/주 100)**도 추가했다. 둘 다 1회용 비밀번호는
  이 세션에서 교체까지 마쳤고 최종 비밀번호는 오너에게 전달. 프로젝트 1건
  시드(**육안 확인용 프로젝트**, `6a8835cdc216173e8e5a906c`) — 비-LLM
  경로(생성·로그인)만 사용했다.
