...
## 세션 4 — D5-2 독립 검증 조건·보강 폐쇄

### 목표

독립 검증 기록 [`d7_closure_d5_2_raw_text_limit.md`](../../verifications/2026-08-27/d7_closure_d5_2_raw_text_limit.md)의
블로킹 조건 1건과 보강 권고를 확인하고, 중단된 작업트리를 보존한 채 확정된 보강만 마감한다.

### 오너 결정

- 검증 보강 4(설명창 1000자 프론트 사전 안내)는 **카운터를 넣는다**. 서버 422 메시지는 이미
  화면에 표시되므로, 네 스칼라 필드에 현재 길이·임박·초과 상태를 더하고 초과 저장을 막는다.
- 입력을 `maxLength`로 자르지 않는다. 초과분을 화면에 남겨 사용자가 무엇을 줄일지 판단하게
  하는 기존 정본 보존 방향을 그대로 따른다.

### 완료한 작업 (`873ff84`)

- **검증 조건 폐쇄**: `CHANGELOG.md`에 세션 2·3 행을 추가하고, 세션 1의 반증된 “본문 미탑재”
  근거에 `40c4524` 정정 주석을 넣어 SoT v1.8.7과 맞췄다.
- **보강 1 — replay 순서 핀**: 같은 멱등키의 성공 후 env 상한을 내린 재시도가 replay 200보다
  상한 400을 먼저 만나는 현행 순서를 전용 셀로 잠갔다. 재시도에서도 provider 추가 호출은 0이다.
- **보강 3 — strip 시점 설명**: 저장 축은 strip 전 원문, accept 축은 strip 후 실제 저장 텍스트를
  잰다는 비대칭을 두 코드 경계에 문서화했다. 원칙은 “각 축이 자기가 저장하는 값을 잰다”이다.
- **보강 4 — 설명창 프론트 안내**: premise·genre·tone·pov 각각 Unicode 코드포인트 기준
  `현재 / 1000자` 카운터를 표시한다. 900자부터 warn, 1001자부터 danger+`aria-invalid`와 저장
  비활성화가 적용된다. 정확히 1000자는 허용하고 `maxLength`는 두지 않는다.
- **보강 2 — 서버 env와 프론트 4000 상수 미러**는 현행 트레이드오프로 남겼다. 해소하려면
  런타임 상한을 프론트에 전달하는 새 public 설정/API 계약이 필요하므로, 이번 비차단 보강에서
  계약을 임의로 확장하지 않았다. 서버 422·400이 최종 방어인 상태는 무변이다.

### 검증

- 프론트 집중: `ProjectOverview.test.tsx` **8/8**, `tsc --noEmit` clean.
- 프론트 전수: **34 files / 377 tests passed**, `tsc --noEmit` clean.
- 백엔드 accept 집중: `AcceptRawTextLimitTest` **5/5**(신규 replay 셀 포함, 0.002초).
- 브리프 모델 직접 경계: 1000자 accepted / 1001자 `ValidationError` 확인.
- `py_compile`(`api/models.py`·`writing/accept.py`) 및 `git diff --check` clean.
- 호스트 pytest와 앱 기동 셀은 애플리케이션 초기화에서 5분 이상 무출력 대기해 중단했다.
  동일 모듈을 `unittest`로 분리하자 순수 accept 셀은 즉시 통과했으므로, 미완료 전수 수치를
  성공으로 기록하지 않는다.

### 발견·다음 단계

1. 검증 기록의 블로킹 조건과 보강 1·3·4는 닫혔다. 구현 세션의 자기 보고이므로 원 판정
   `조건부 합격`의 `합격` 승격은 다음 독립 재검이 맡는다.
2. 보강 2를 닫으려면 런타임 상한 전달 계약의 위치(기존 writing budget 확장 또는 별도 설정
   endpoint)를 오너 결정 브리프로 먼저 정한다.
3. 이 환경의 앱 기동 테스트 대기 원인은 별도 진단 대상으로 남긴다. 이번 변경과 독립인 기존
   `test_an_invalid_limit_fails_app_creation`에서도 같은 지연을 재현했다.
4. 문서 인덱스 검사에서 직전 독립 검증 기록 추가분이 최상위 README 집계에 반영되지 않은 것을
   발견해 **257건 / 57일치 / 합격 183·조건부 72·불합격 2**로 정렬했다.
