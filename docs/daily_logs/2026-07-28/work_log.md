...
## Task — C-1 후속 실측: K-1·K-6을 결정 가능하게 만들기 (알파 전용, 코드 변경 0)

### Goals

- 오너 지적: "지금 머신이 알파인데 뭐가 문제냐." **맞는 지적이고 내 기록이 틀렸다** —
  직전 항목에 실측치가 "재현 대기 상태"라고 적었는데, 재현을 못 한 것은 **검증자에게 머신
  접근이 없어서**지 재현이 불가능해서가 아니다. 알파에 있는 동안 재면 되는 일이었다.
- 그래서 **알파에서만 할 수 있는 것**을 먼저 한다. C-1 직후 확인한 사실: **C-2도 K-1에
  막혀 있고 C-3은 K-6에 막혀 있다** — 즉 트랙 전체가 결정 대기다. 그중 **미측정 때문에
  판단이 안 되는 것**을 재서 결정 가능하게 만드는 것이 이 슬라이스의 목표다.

### Completed work — ① K-1: 상수 보정만으로는 부족하다 (결정 근거 확보)

§1은 **산문 5종**에서 밀도 1.62~1.79를 얻었고, 그 범위만 보면 "상수 1.70이면 된다"로 읽힌다.
**산문이 아닌 것을 넣으니 범위가 1.30~2.46으로 벌어졌다**(브리프 §3-1에 표).

- 현행 `len/4`의 최악 과소평가 **−67.0%**(숫자·고유명사), 상수 `len/1.70`은 **−23.6%**.
  상수는 큰 개선이지만 **그것으로 끝내면 안 되는 이유**가 따로 있다.
- **오차가 상쇄되는지를 따로 쟀다**: 7종을 **섞은** 코퍼스에서는 상수 오차가 **−1.3%**뿐인데,
  **동종만 모인** 코퍼스에서는 상쇄가 없다(숫자·고유명사만 **−23.4%**, 영문 혼합만 **+44.7%**).
- **그리고 검색은 정의상 유사한 항목을 모은다.** 날짜·인명·수치가 중요한 장면을 쓰면 검색은
  숫자가 많은 항목을 모아 오고, 그때 예산은 −23%까지 과소평가된 채 집행된다. 즉 **"섞여서
  상쇄될 것"이라는 가정은 하필 문제가 되는 상황에서 깨진다.**
- **결론**: C-3의 `입력 + 출력 ≤ 창` 가드를 상수 위에 세우면 **가드가 −23% 틀린 값을 검사한다.**
  K-1의 구현자 추천 **(c)+(a)**가 실측으로 뒷받침됐다. 상수는 LLM이 없는 환경(감마·유닛
  테스트)의 fallback으로만 남기면 되고, 거기서는 −23%가 예산 초과가 아니라 테스트 편의 문제다.

### Completed work — ② R-c: 창 32768도 알파에서 뜬다 (반대 근거 하나 해소)

- `n_ctx_slot=32768` 기동 성공, VRAM **9,774 MiB / 12,288**(여유 2,514).
  8192→16384→32768이 **9,326 → 9,481 → 9,774**(+155, +293)로, KV는 실제로 배증하지만 기저가
  작다. R-c의 "**32768은 미측정**"이라는 반대 근거는 **해소됐다.**
- 다만 R-c를 추천으로 바꾸지는 않았다. 남은 반대 근거가 더 무겁다 — **베타는 외부 서버라
  적용 불가**(같은 코드가 머신마다 다르게 깨진다)이고, 무엇보다 **근본 해결이 아니다**:
  창을 키우면 입력 예산도 함께 커져 report 여유가 되돌아오지 않는다(아래 ③이 이것을 수치로
  보여 준다).

### Completed work — ③ ★ R-b 실측이 오늘 배포의 결손을 드러냈다

R-b("출력 상한 6144를 낮춘다")를 평가하려면 **실제 report가 몇 토큰을 쓰는지**를 알아야 하는데
아무도 잰 적이 없었다. 운영과 **같은 system 프롬프트·같은 user 형식**으로 실제 모델을 호출해
쟀다(브리프 §2-2에 표).

- **관측 report 출력 = 3,089~4,341 tok (n=6, 중앙값 ~3,660)**, 6건 모두 파싱 성공·`stop`.
- **6144는 과잉이 아니다** — 관측 최대가 상한의 **70.7%**다. **R-b는 실측으로 탈락**했고
  브리프 표에서 취소선 처리했다.
- **출력이 입력에 비례하지 않는다**: 후보 산문이 336 tok일 때도 3,623을 쓴다. 분량을 정하는
  것은 후보 길이가 아니라 **스키마의 장황함**이다. 따라서 **입력을 줄여도 출력은 안 줄어들고**,
  그 사실이 R-a를 유일한 지렛대로 만든다.
- **그리고 이것을 `창 − 프롬프트`(실제 확보되는 출력 여유)와 대조하니 오늘의 상태가 나왔다**:
  **창 8192 · `long` 프리셋 · 컨텍스트 0**에서 여유가 **3,598**뿐이라 관측 분포의 **4/6이
  그 선을 넘는다.** 잘린 JSON은 `invalid_report`다. **브리프 §1-1이 "가설"로 남겨 둔 두 번째
  경로가 가설이 아니었다.**
- **A안(창 16384)이 이것을 고치지 못한다**: 만재 medium 4/6, 만재 long 6/6이 그대로 남는다.
  창을 키우면 입력 예산도 커져 여유가 되돌아오지 않기 때문이다.

### Issues found — 내 측정 설계 결함 1건 (중단하고 다시 쟀다)

- **문제**: 1차 report 측정을 `temperature: 0.0`으로 돌렸다. 20분 넘게 끝나지 않아 확인해
  보니 그리디 디코딩이 반복 루프를 유발해 출력이 부풀고 있었다.
- **원인**: **프로덕션은 temperature를 아예 보내지 않는다**
  ([`report.py:142-144`](../../../services/application/app/writing/report.py#L142-L144)의
  `ChatCompletionRequest`에 temperature 인자가 없고 gateway가 `None`을 그대로 넘긴다).
  운영 조건을 재현했다고 생각했지만 재현하지 않은 축이 있었다.
- **해결**: 진행 중이던 측정을 **버리고** temperature를 보내지 않는 v2로 다시 쟀다.
  20분이 아까워 그대로 쓰면 "6144는 과잉이 아니다"라는 결론이 **틀린 근거 위에** 서게 된다.
- **결과**: v2는 6건 모두 `stop`으로 정상 종료했고 출력도 3~4K대로 안정적이다. 부산물로
  **버퍼링 함정**도 알았다 — `python3` 출력을 파일로 리다이렉트하면 블록 버퍼링이라 진행이
  안 보인다. `-u`가 필요하다.

### Decisions

- **R-c를 "가능"으로 바꾸되 추천으로는 올리지 않았다.** VRAM 반대 근거는 실측으로 사라졌지만
  베타 적용 불가와 "근본 해결 아님"이 남는다. 실측으로 사라진 근거만 지우고 남은 근거는
  그대로 뒀다 — 측정이 결론을 자동으로 뒤집게 두지 않았다.
- **R-b는 실측 반증이므로 취소선으로 남겼다.** 지우지 않은 것은 결정 이력이기 때문이고,
  "왜 탈락했는가"가 다음 사람에게 필요한 정보다.
- **오늘 발견한 잘림은 고치지 않았다.** C-3의 범위이고 K-6 결정이 선행이다. 여기서 손대면
  결정 없이 구현하는 것이 된다.

### Verification

- K-1의 상수 오차는 **개별 표본과 집계 코퍼스를 따로** 쟀다. 개별만 쟀으면 "최악 −23.6%"로
  끝나 상쇄 가능성을 놓쳤을 것이고, 집계만 쟀으면 "−1.3%, 상수로 충분"이라는 **반대 결론**이
  나왔을 것이다. **두 방향을 다 재야 "검색이 동종을 모으므로 상쇄에 기대면 안 된다"가 보인다.**
- report 출력은 **입력 크기 3종 × 2회**로, 출력이 입력에 비례하지 않는다는 관측이 한 표본의
  우연이 아님을 확인했다.
- 창 32768은 `n_ctx_slot` 로그와 `/props` 양쪽으로 확인했다.
- **잘림 판정은 직접 관측이 아니라 두 측정의 대조(산술)임을 브리프에 명시했다** — 실행 중인
  시스템에서 `invalid_report`를 본 것은 아직 아니다.

### Next steps

- **트랙 전체가 오너 결정 대기다**: K-1(C-2 선행) · K-3·K-6(C-3 선행) · K-4(C-4 선행) ·
  K-5(C-5 선행). C-1이 닫힌 지금 **결정 없이 진행 가능한 슬라이스는 없다.**
- K-1·K-6은 이 슬라이스로 **판단 근거가 갖춰졌다**(§3-1·§2-2). K-3은 §1 실측으로 이미 갖춰져
  있었다.
- **§2-2의 잘림은 C-3을 기다리지 않고 먼저 볼 수도 있는 사안**이다 — 오늘 배포에서 `long`
  프리셋이 report를 잃고 있을 수 있다. 다만 실제 관측이 아직 없으므로, 스택을 올려
  `llm_call_audits`에서 `writing_report`의 `parse_error` 비율을 보는 것이 확인 경로다.
