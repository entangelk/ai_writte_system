...
- 변경 파일: `tests/fixtures/__init__.py`(신규), `tests/fixtures/core_sot.py`(신규), `tests/test_core_sot_fixture.py`(신규), `docs/plans/01-core-sot.md`, `HANDOFF.md`, `CHANGELOG.md`.
- 배경: plan 01 최소 산출물 #7은 "후속 Phase가 재사용할 fixture"였다. Phase 2는 아직 AnalysisJob/Candidate schema가 미확정이므로, 그 schema를 추측하지 않고 Core SOT의 확정 표면(snapshot/block/source_ref/idempotent save)만 reusable fixture로 제공한다.
- 구현: `build_core_sot_fixture()`가 in-memory `CoreSotService`로 project/draft/version save/source_ref 2개를 생성한다. fixture module은 deterministic raw text, expected raw UTF-8 SHA-256, block kind/order/offset/text matrix, source_ref span/quote/block index를 상수로 제공한다.
- 효과: Phase 2 Snapshot Loader, Analysis Gate, anchor validator 테스트가 동일한 snapshot/block/ref 계약을 import해 쓸 수 있다. production service에는 테스트 fixture 데이터가 섞이지 않는다.
