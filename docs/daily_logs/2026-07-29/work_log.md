...
### 독립 검증 후속 보강 (1a, 같은 슬라이스)

독립 검증 `docs/verifications/2026-07-29/slice1a_io_token_breakdown_audit.md`는 **합격(차단 0건)**.
검증자가 뮤테이션 3종을 독립 재현했고(`_doc` 누락 3건 · 0으로 채움 1건 · `ObservedProvider` 캡처
누락 1건), `git blame`으로 캡처 줄이 이 커밋 소산임을 확인했다. 비차단 1건과 운영 지적 1건을
처리했다.

- **H1 — 검증자는 "다음 슬라이스에서 정본에 병기"를 권했지만, 여기서 닫았다.** 근거가 다르다:
  정본 §"통합 per-LLM-call 감사 레코드"는 **필드를 열거**한다("필드 = `project_id`·…·`total_tokens`·
  `latency_ms`·`error_type`·`created_at`"). 즉 미갱신은 "설명이 빠진 것"이 아니라 **정본이 사실과
  달라지는 것**이다. **v1.7.58**로 올리며 ⓐ 필드 목록에 두 필드를 넣고, ⓑ 기존 "실패 레코드의
  `total_tokens` 의미론" 옆에 **분해의 `None` 의미론**을 병기했다 — `None`이 되는 세 경우(provider
  무응답 · 필드 이전 레코드 · 미신고 호출부), **0으로 적으면 효율 지표가 낙관 쪽으로 오염된다**는
  근거, 그리고 **분해를 소비하는 집계는 `None` 행을 분모에서 빼고 그 분모를 응답에 함께 실어야
  한다**는 규칙(이 절의 다른 반직관 수치와 같은 방식). 검증자가 짚은 "같은 개념의 두 표현"
  혼동 위험도 그 문단에서 **왜 둘이 공존하는지**로 설명했다.
- **운영 지적 — 1a가 베타에 배포돼 있지 않았다(맞다).** 검증자가 베타 DB에서 분해 non-null이
  0건임을 확인했다. "1a 완료"는 커밋 완료였고 배포 주장이 아니었지만, **그 상태로 두면 다음
  사람이 KPI를 보고 "기능이 안 도는가"로 오독**한다. 이미지를 재빌드·재배포하고 **실 관통으로
  확인**했다:

  | 호출부 | `total_tokens` | `prompt_tokens` | `completion_tokens` |
  |---|---|---|---|
  | `query_planner` | 524 | 357 | 167 |
  | `writing_generation` | 1,049 | 634 | 415 |
  | `writing_report` | 2,850 | **1,851** | 999 |

  즉 **오너가 원한 "입력에 얼마 쓰는가"가 이제 실제로 답해진다**(위는 얇은 project 기준이며,
  무거운 입력의 report 총량 13,516~14,302에 대해서도 같은 분해가 남는다).
- **검증자의 투명 메모(백그라운드 완료 알림이 실제 종료 전에 세 번 거짓 발생)**는 이 슬라이스의
  결론에 영향이 없다 — 검증자가 `ps`로 직접 확인한 뒤 결과를 확정했다고 기록했다. 다만 **같은
  하네스를 쓰는 다음 작업자에게 유효한 경고**이므로 여기 남긴다.

정본 외 코드 변경은 없다(문서 + 재배포).

---

## Task — K-3 관측 슬라이스 1b: 창 크기와 출력 상한 (관측만)

### Goals

- 1a가 채운 분자(입력 토큰)에 이어 **분모(창)**를 채운다. "입력 1,851"만으로는 많은지 적은지
  말할 수 없다.
- **거부하지 않는다**(오너 결정). 기록만 하고 초과해도 요청은 통과시킨다.
- **창을 상수로 박지 않는다** — repo가 그 값을 통제하지 못하는 배포가 있다(외부 LLM 서버).

### Completed work — 설계 3가지

**① 창을 어떻게 아는가.** 먼저 생성 응답에 창 정보가 있는지 **실측**했다 — 없다(최상위 키는
`choices`·`created`·`id`·`model`·`object`·`system_fingerprint`·`timings`·`usage`뿐). 그래서
게이트웨이 transport에 `get_json`을 더해 llama.cpp `/props`를 **한 번만 조회·캐시**하고, 생성
응답 봉투에 `context_window`로 실어 보낸다. **새 라우트도 앱 쪽 별도 클라이언트도 없다** —
기존 응답 파싱 경로가 그대로 받는다.

**② 관측이 기능을 깨뜨리지 않는다.** `/props` 조회가 실패해도 생성은 성공하고 창만 `None`이
된다. 창은 관측용 부가 정보이고, 그것을 못 읽었다고 멀쩡한 생성을 실패시키면 **관측이 기능을
망가뜨리는 것**이 된다(감사 flush 격리와 같은 원칙, SoT §관측 KPI). 그리고 **실패를 재시도하지
않는다** — "아직 안 물었다"와 "물었는데 실패했다"를 구분해, 죽은 서버에 매 호출 왕복을 쌓지
않는다.

**③ 헤드룸은 저장하지 않는다.** `창 − 입력 − 출력상한`은 **파생값**이고, 저장하면 세 원천값과
조용히 갈라진다. 원천값(`prompt_tokens`·`max_output_tokens`·`context_window`)만 남기고 판정은
사후에 한다. **이 형태가 "거부하지 않는다"를 구조적으로 보장한다** — 기록만 하기 때문이다.

부수: **실패한 호출도 `max_output_tokens`는 남긴다.** 응답이 없어 창은 몰라도 **요청이 얼마를
요구했는지**는 알 수 있고, 그것은 자원 분석에 유효한 정보다.

### Verification

- 전량 회귀(test-mongo **healthy 확인 후**): **`1712 passed / 1 skipped / 1468 subtests`**(647s).
  기준선 `1706` 대비 **+6 = 신규 테스트 6개**(scope 2 · `/props` 조회 4)와 정확히 일치.
  `-rs`로 확인한 유일한 skip은 알려진 live Chroma 1건이다.
- 뮤테이션 2종:

  | 뮤테이션 | 결과 |
  |---|---|
  | 창 조회를 아예 하지 않음 | **3건 실패** |
  | 실패한 조회를 매 호출 재시도 | **1건 실패** |

- 게이트웨이 봉투 exact 비교 테스트가 걸렸고 **의도한 계약 확장**이므로 기대값에 `context_window`를
  더했다(그 fake provider는 창을 신고하지 않으므로 값은 `None` — 기본값을 채우지 않는다).
- 공개 계약 무변(`schema.d.ts`에 없다 — 감사 레코드는 응답 모델이 아니다).

### Issues found — ★ 관측을 켜자마자 계약 위반이 데이터로 드러났다

실 배포에 올려 heavy `long`을 한 번 관통시킨 결과:
...
- **`writing_report`의 헤드룸이 음수인데 job은 성공했다.** 실제 출력이 1,371로 상한(6,144)을
  안 썼기 때문이다. 즉 **초록불이면서 계약이 깨진 상태**이며, 내가 산술로만 말하던 그것이
  이제 **레코드로** 보인다. 관측 슬라이스가 하려던 일이 정확히 이것이다.
- **report는 창의 73%를 입력에 쓴다**(11,905 / 16,384). 오너가 요청한 자원 배분 수치가 이것이다.
- **남은 일은 이 초과를 없애는 것**이고 그것이 K-6=R-e(포인터 제거)다. 관측은 아무것도 거부하지
  않으므로 오늘 되던 것은 계속 된다.

### Next steps

- **R-e(K-6)**: 포인터를 번호 참조 + 서버측 매핑으로 바꿔 report 프롬프트를 줄인다. 위 −1,665가
  양수로 돌아오는지가 그 슬라이스의 성공 판정이며, **이제 그 판정을 데이터로 할 수 있다.**
- 1c(KPI 집계에 효율 노출)는 R-e 뒤에 해도 된다 — 지금도 레코드를 직접 조회하면 위 표가 나온다.

---

## Task — K-3 관측 1b 독립 재감사

### Goals

- 작업 AI의 `3639b77` 완료 보고와 중단된 검증 초안의 PASS 판정을 정본·구현·테스트·라이브
  데이터 순서로 독립 재검증한다.
- 현재 코드가 맞는지와 회귀가 그 사실을 잠그는지를 분리하고, 계약-required 경계의 빈 셀을
  뮤테이션으로 확인한다.

### Completed work

- 독립 검증 기록을
  [`slice1b_context_window_output_cap_reaudit.md`](../../verifications/2026-07-29/slice1b_context_window_output_cap_reaudit.md)에
  작성했다. 중단된 초안에는 최종 판정이 아님을 알리는 배너를 추가했다.
- 배포 Mongo의 non-null 3건을 재조회해 작업자 표의 헤드룸
  `14,877 / 9,655 / -1,665`와 report 입력 비중 73%를 재계산했다.
- 제품 코드의 non-null 전달/Mongo 배선이 현재는 이어져 있음을 확인했다.
- 지연 probe 축소 재현과 5개 독립 뮤테이션으로 구현 위반 1건, 필수 회귀 공백 5개를 확인했다.
  모든 제품 코드 뮤테이션은 즉시 원복했고 제품 코드 diff가 없음을 확인했다.

### Issues found

- **문제**: 생성 성공 뒤 순차 대기하는 `/props`가 느리면 앱과 게이트웨이의 같은 120초 deadline
  때문에 이미 성공한 생성이 timeout으로 사라질 수 있다.
  - **원인**: 즉시 connection/transport 예외만 격리했고 상위 deadline/cancellation 경계를 보지 않았다.
  - **해결 상태**: 이번 감사에서는 수정하지 않았다. 검증 verdict는 **FAIL**.
- **문제**: production GET, 게이트웨이 non-null 봉투, 앱 non-null 파싱, Mongo non-null 왕복,
  실패 호출의 출력 상한에 각각 필수 회귀가 없다.
  - **원인**: 테스트가 fake transport의 첫 경계와 in-memory scope의 마지막 경계를 건너뛰어
    합성 경로를 부분적으로만 확인하고, Mongo fixture는 1b 필드를 기본 None으로 뒀다.
  - **결과**: 각 생산 줄을 삭제/무력화해도 관련 테스트가 green인 것을 독립 재현했다.
- **문제**: HANDOFF에 새 필드 도입 설명과 “분해 없음/창을 아는 코드 0줄”이라는 과거 상태가
  동시에 남아 있었다.
  - **조치**: 다음 작업자가 실패한 1b를 완료로 오인하지 않도록 현재 actionable state로 재작성했다.

### Decisions

- 검증자는 결함을 자동 수정하지 않는다. B1~B5를 닫는 구현 slice를 R-e(K-6)보다 먼저 둔다.
- 라이브 데이터와 산술은 유효한 증거로 보존하되, 그것을 회귀 완전성이나 비차단 계약의 증거로
  확대 해석하지 않는다.
- 중단된 감사 초안의 PASS/차단 0건 판정은 폐기하고 재감사 문서를 최종 판정으로 사용한다.

### Verification

- relevant 집중 테스트: 대상 7개 모듈의 테스트 전부 통과.
- mutation:
  - Mongo 두 필드 저장/복원 제거 → **6 passed**
  - 실패 호출 상한 제거 → scope **14개 통과**
  - 실제 GET→POST → **18 passed / 16 subtests**
  - gateway non-null→항상 None → **7 passed / 5 subtests**
  - application non-null parse→항상 None → 관련 **21개 통과**
- 느린 `/props` 재현: 성공 POST + 0.2초 GET 지연 + 0.05초 상위 deadline → **`TimeoutError`**.
- 테스트 수집: **1713 tests collected**.
- 전체 독립 실행은 3% 뒤 장시간 진행하지 않아 중단했다. 작업자의 기존
  `1712 passed / 1 skipped` 기록과 독립 실행 완료를 혼동하지 않는다.
- 중단된 검증 세션이 남긴 `test-mongo` 컨테이너는 감사 후 stop·remove해 원래 상태로 복구했다.

### Next steps

- B1의 비차단 시간 경계를 수정하고 느린/실패 probe 양방향 회귀를 추가한다.
- B2~B5의 production GET/non-null 전달/Mongo/실패 호출 테스트를 채운 뒤 mutation bite를 재검증한다.
- 그 후에만 R-e(K-6)로 진행한다.

### 독립 재감사 반영 (1b) — **FAIL을 받고 고쳤다**

독립 재감사 `docs/verifications/2026-07-29/slice1b_context_window_output_cap_reaudit.md` 판정은
**FAIL**이었다(차단 5건). **지적 전부를 내가 직접 재현한 뒤** 고쳤다. 라이브 수치(헤드룸 −1,665,
입력 비중 73%)는 검증자도 독립 재확인해 그대로 유효하다.

- **B1 — 실제 결함이었다.** `await probe`가 생성이 **이미 성공한 뒤** 반환 경로에 있었다.
  `/props`가 느리면 게이트웨이 응답이 지연되고, **게이트웨이 transport와 앱 클라이언트가 같은
  120s deadline을 쓰므로 성공한 생성이 timeout 실패로 뒤집힌다**. 내가 "관측이 기능을 깨뜨리지
  않는다"고 적어 놓고 **실패만 삼켰지 지연은 공유한 채로 둔 것**이다.
  **수정**: 조회를 생성 요청과 **동시에 띄우고 기다리지 않는다**. 생성은 초 단위, `/props`는
  밀리초 단위라 실제로는 첫 호출부터 값이 준비되고(실 배포 재확인: 전 호출 `win=16384`),
  안 됐으면 그 호출의 창은 `None`("모른다")이며 **그것이 정직한 답**이다.
  **회귀**: 끝나지 않는 probe를 주고 생성이 그대로 성공하는지 본다 — **차단 구현으로 되돌리면
  이 셀은 영원히 끝나지 않는다.**
- **B2~B5 — 필수 셀 4개가 비어 있었다.** 각각 뮤테이션으로 공백을 재현한 뒤 채웠다:

  | # | 공백 | 재현(보강 전) | 보강 후 |
  |---|---|---|---|
  | B2 | 실 httpx가 GET을 쓰는가 | POST로 퇴행해도 **green** | 1건 실패 |
  | B3 | non-null 창이 게이트웨이·앱 두 경계를 통과하는가 | 양쪽이 항상 `None`이어도 **green** | 각각 1건 실패 |
  | B4 | Mongo 왕복 | 저장·복원에서 두 필드를 함께 지워도 **green** | 3건 실패 |
  | B5 | 실패 호출의 출력 상한 | SoT 명시 분기를 지워도 **green** | 1건 실패 |

- **B4는 같은 함정에 두 번 빠진 것이다.** 오늘 오전 1a에서 **내가 직접 "비어 있는 값으로는
  왕복을 검증할 수 없다"고 문서화**해 놓고, 1b의 새 필드에서 헬퍼에 값을 안 실어 똑같이
  `None == None`으로 지나갔다. 규칙을 적는 것과 다음 필드에 적용하는 것은 별개였다.
- **폐기된 1차 감사 기록은 삭제했다**(오너 승인). 상단에 "판정 폐기" 경고가 있었지만 **본문
  Verdict가 "합격(PASS)"으로 남아 있어**, 판정만 보는 독자를 오도한다. 재감사본이 더 완전하고
  (217줄 vs 206줄) 폐기본에만 있는 정보는 없다. 그 존재 사실은 이 줄로 대신한다.

### Issues found — 실 관통 1회가 `invalid_report`로 실패했다 (1b와 무관)

- B1 수정 후 첫 관통이 `status=failed reason=invalid_report`("report field must be an array")로
  끝났다. 감사에는 `writing_report` 레코드가 **3건**(첫 호출 입력 11,837 + repair 2건 입력
  ~3,119)이라 **repair 2회가 소진**된 것이다.
- **1b 때문이 아니다**: 창 조회는 모델 출력에 영향을 주지 않고, report 입력 크기도 직전
  관측(11,905)과 사실상 같다. **재실행 2/2 성공**으로 간헐성을 확인했다.
- 이것은 HANDOFF의 dogfood 관찰 항목 "`report field must be an array` 실패율"에 해당하는
  **알려진 12B 간헐 이슈**이며, 오늘 처음으로 **repair 소진까지 간 사례**를 관측했다.
- **부수 관측**: repair 호출의 입력은 3,119로 첫 호출(11,837)의 1/4이다 — repair 프롬프트는
  전체 컨텍스트를 다시 싣지 않기 때문이며, 그래서 repair의 헤드룸은 +7,121로 넉넉하다.

### Verification

- 전량 회귀(test-mongo healthy 확인 후): **`1717 passed / 1 skipped / 1468 subtests`**(633s).
  직전 `1712` 대비 **+5 = 신규 셀 5개**와 정확히 일치. skip은 알려진 live Chroma 1건.
- 뮤테이션 6종(B1·B2·B3a·B3b·B4·B5) 전부 해당 셀에서 물었고, 원복 후 `git diff`로 확인했다.
- 실 배포: 게이트웨이 재빌드·재배포 후 관통 3회(실패 1 + 성공 2), 창이 전 호출에 기록됨을 확인.
