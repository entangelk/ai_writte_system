...
- backend 전량 **1749 passed / 1 skipped / 1499 subtests**(766s, test-mongo ON). 직전 기준선
  1738 / 1498 대비 **+11 테스트**(오늘 새 파일) · **+1 subtest**(어제 마지막 커밋 `2480ce2`의
  프롬프트 v4 핀이 기준선 기록 뒤에 들어갔다). **회귀 0건** — `main.py` 변경은 주석뿐이다.
...
- **이 표는 "후보가 `long` 상한을 다 쓴 최악"이다.** 후보를 medium(2,139 tok)으로 낮춰 같은
  ...
  **7,400**. **다만 현행 8192는 medium에서도 −784로 거부된다** — 즉 후보 길이만으로 설명되는
