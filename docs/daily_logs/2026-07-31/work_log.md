...
### Verification — 알파(32768) vs 베타(16384) 동일 시드 대조

시드(`--seed`, 24000자, 밀도 1.63 자/tok)는 알파에서 새로 만들었다 — 베타 시드와 **같은 생성기**라
패키지 구조가 같다. 고정 오버헤드도 **베타와 토큰 단위로 동일**(같은 모델·토크나이저):
`system 465 · 후보 산문 4,159 · 래퍼 94 = 4,718`(베타와 동일).
...
- **★ 성공 기준 ② — 8192가 PASS다.** 베타(16384)에서는 같은 패키지(항목 62, 입력 **13,076** tok)가
  `13,076 + 6,144 = 19,220 > 16,384` → 가드 400 거부(여유 **−2,836**)였다. 알파(32768)에서는 같은
  **13,077** tok가 `19,221 < 32,768` → **PASS(여유 +13,547)**. **같은 요청이 창에 따라 REJECT↔PASS로
  정확히 갈린다** — 여유 차이 `13,547 − (−2,836) = 16,383 ≈ 창 차이 16,384`. R-c의 본질(창을 키우면
  현행 예산이 통과한다)이 관측됐다.
- **★ 성공 기준 ③ — 유도가 자동으로 넓어진다.** R-a 산식(실측)
  `32768 − 6144 − 4718 = 21,906`, 회계 단위 권장 **약 21,487**(만재 비율 0.981, 구간 0.965~0.981).
  베타 권장 5,407(비율 0.979) 대비 **+16,080** 넓어졌고, 이는 창 증분 16,384에 비례한다. **R-c(창 확대)는
