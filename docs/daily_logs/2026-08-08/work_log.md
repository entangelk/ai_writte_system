...
3. **공개 표면 지문**: [`repro_router_split.py`](../../verifications/2026-08-05/repro_router_split.py)
   를 `git worktree` 로 뜬 HEAD 트리와 작업 트리에서 각각 돌려 **바이트 동일**
   (`47d78b68…`). route **76** · order-sensitive pairs **0** · openapi sha
   **`f8b42ef1…`**(Slice 1 내내 고정된 그 값).
...
| `repro_router_split.py` 지문 (HEAD worktree vs 작업 트리) | **IDENTICAL** (`47d78b68…`) · route 76 · order-pairs 0 · openapi sha `f8b42ef1…` |
