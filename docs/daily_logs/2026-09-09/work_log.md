...
- **★ 다음 세션은 새 구현이 아니라 검증부터 시작한다(오너 지시 2026-09-09).** 이 세션이 낸 것 둘(Slice 3 · 약관 시행 표기)이 **독립 검증을 안 받았다** — HANDOFF 에 **"검증자에게"** 절을 새로 두고 착수점·약한 자리 다섯·기준선을 적었다. 검증 없이 Slice 4(화면)로 넘어가면 화면이 아직 안 잡힌 계약 위에 올라서고, **파기 축은 되돌릴 수 없어 그 비용이 특히 크다.**
- **구현자가 스스로 신고한 약한 자리**(그 절에 옮겨 적었다): ① **파기 경로가 실 Mongo 로 한 번도 안 돌았다**(셀이 전부 fake — 이 저장소가 배포에서만 터지는 계열을 여러 번 겪은 조합이다) ② `_id` 스윕의 안전 근거가 **접두 하나**에 걸려 있고 *"user id 와 같은 `_id` 를 가진 문서가 `users` 밖에 있는가"* 를 실 DB 로 안 쟀다 ③ 실패한 계정을 되살릴 통로가 **스크립트뿐**이다(화면은 Slice 4) ④ **새로 넣은 셀 자신**에 변이를 걸어 볼 것(세션 25 선례) ⑤ 약관 축에서 검증할 것은 오너 판단이 아니라 **문서가 실제로 그렇게 적는지**와 **핀 셀이 값을 잡는지**다.
- 그다음이 **Slice 4(화면)** — 요청·취소·남은 일수 배너 + **D3 의 관리자 잔여 정리**(ⓔ 로 여기에 왔다). Slice 5 는 정책 문서 §8 → §6 승격.
