...
### 2. 계정 탈퇴 Slice 3 (SoT v1.8.51, 커밋 `0919f95`·`8f182bf`·`e9110a9`)
...
| **MU-3** 청구 조건 `purge_started_at` 제거 | **통과** | 순차 배수는 `list_withdrawing` 이 이미 거른다. 그러나 그것은 **읽기**이고 읽기와 쓰기 사이는 열려 있다 — 워커 둘이 같은 계정을 두 번 파기할 수 있고 파기는 비가역이다 | `PurgeClaimTest` 5셀(`e9110a9`), 그중 하나는 **Mongo 질의 모양**을 직접 잰다 |
