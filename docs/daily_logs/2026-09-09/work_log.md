...
## 세션 49 이어서 — 오너 결정 셋 · 약관 시행 · **계정 탈퇴 Slice 3 구현**

오너가 브리프 둘과 시행일을 같은 자리에서 답했다.

### 사용자 결정과 근거 (세션 49)

| 무엇 | 답 | 오너 근거 |
|---|---|---|
| Slice 3 계정 축 식별 규칙 | **ⓑ 두 규칙 스윕 + `target_user_id` 보존 표식** | 추천 채택 |
| Slice 3 범위 | **ⓔ 데몬 + reconciler 스크립트**(화면은 Slice 4) | 추천 채택 |
| 약관·방침 시행일 | **지난 날짜 그대로 시행** | *"무시한다. 이거 서비스가 아니라 포트폴리오라니까? 지난 날짜는 신경쓰지마."* |

**★ 세 번째는 내 차단을 뒤집은 결정이다.** 나는 *"시행 전제 둘이 미구현인데 시행일이 지났다 — 채우면 문서가 없는 기능을 시행 중이라 말한다"* 로 막았고, 오너는 **제품 단계 자체가 다르다**는 이유로 진행을 지시했다. 그대로 시행하되 **두 전제가 미구현이라는 사실은 두 문서 머리말과 `legal/README` §8 표에 남겼다** — 실제 회원에게 서비스하기 전에 닫아야 할 목록이라는 지위로. (SoT v1.8.50)

### 1. 약관·방침 시행 (SoT v1.8.50, 커밋 `559c310`)

버전 `draft-0` → **`1.0`** · 시행일 **2026-09-08** · `미시행` 표식 제거 · 제목의 *(초안)* 제거. 파일명 `*-draft.md` 는 그대로 뒀다 — 경로일 뿐이고 참조 넷이 따라 움직인다.

**★ 버전 문자열이 계약 리터럴이다.** 가입 동의 게이트가 회원의 동의 시각과 **함께** 저장하므로 문서에서 조용히 바뀌면 저장된 동의가 어느 판본인지 갈라진다. 상수가 아직 코드에 없어 **핀 셀**이 값을 직접 든다(게이트가 상수를 만드는 날 그 상수를 가리키게 바꾼다).

### 2. 계정 탈퇴 Slice 3 (SoT v1.8.51, 커밋 `0919f95`·`8f182bf`·`3d95c60`)

**착수 실측이 계획서를 두 군데 정정했다** — 그것이 이 슬라이스의 실제 내용이다.

| 발견 | 뜻 |
|---|---|
| `request_quota_policies` 는 `_id` = user id 라 **`user_id` 필드 스윕이 못 본다** | 계획서 D4 절의 *"회원 한도 정책도 `user_id` 를 쓴다"* 가 사실이 아니었다 |
| 규칙을 `_id` 로 넓히면 **사용자명 묘비를 스스로 지운다** | `project_name_history` 의 `_id` 키잉이 *"파기를 살아남는다"* 의 **명시된** 표식이다(그 어댑터 머리말이 그렇게 적는다) |

둘이 **한 갈림길의 양쪽**이라 한 브리프로 물었고, ⓑ 가 규칙을 한 문장으로 만들었다: **`user_id`·`_id` 로 사용자를 가리키면 지운다. `target_user_id` 로 가리키면 남긴다.**

구현: `deletion/account_purge.py`(오케스트레이션) · `deletion/account_axis_sweep.py`(두 규칙) · `deletion/user_name_history.py`(+`_mongo`, `_id` = `user_name:<id>`) · `auth` 의 `purge_started_at` + 청구/삭제/목록 seam · `scripts/account_withdrawal_worker.py` + compose `withdrawal_worker` · `scripts/account_purge_reconciler.py`.

### 3. ★ 변이가 **두 번** 사각을 열었다 — 이 세션의 진짜 소득

| 변이 | 처음 결과 | 무엇이 없었나 | 처방 |
|---|---|---|---|
| **MU-1** 시행일만 채우고 `미시행` 유지 | **통과** | 가드가 대괄호 *문자열의 존재* 를 재는데 내 머리말이 같은 표기를 **언급**해 본문을 다 채워도 초록 | 머리말을 *"시행일 자리"* 로 · 그 규칙이 곧 가드의 전제임을 docstring 에(`c8f106e`) |
| **MU-3** 청구 조건 `purge_started_at` 제거 | **통과** | 순차 배수는 `list_withdrawing` 이 이미 거른다. 그러나 그것은 **읽기**이고 읽기와 쓰기 사이는 열려 있다 — 워커 둘이 같은 계정을 두 번 파기할 수 있고 파기는 비가역이다 | `PurgeClaimTest` 5셀(`3d95c60`), 그중 하나는 **Mongo 질의 모양**을 직접 잰다 |

**공통 뿌리**: 둘 다 *"처방을 넣었다"* 와 *"닫았다"* 를 가른 것이 변이 하나였다. v1.8.48 이 같은 자리에서 배운 것이 이 세션에서 두 번 재현됐다 — **변이를 안 돌렸으면 둘 다 초록인 채 넘어갔다.**

### 변이 (커밋 → 변이 → 원복 → clean 확인, 총 9회)

약관 축 3종(MU-1·MO-1·MU-2)은 앞 절에 있다. Slice 3 축 6종:

| # | 방향 | 변이 | 결과 |
|---|---|---|---|
| **MU-3** | under | 청구 조건 `purge_started_at: None` 제거 | 셀 추가 후 **3실패**(2차 청구 거절 · 첫 시각 보존 · Mongo 질의 모양) |
| **MU-4** | under | 스윕의 `_id` 규칙 삭제 | **2실패**(quota policy 생존 · 어댑터 질의 모양) |
| **MO-2** | over | 묘비를 선례대로 `_id` = user id 로 | **2실패**(저장 모양 · 스윕 생존) |
| **MU-5** | under | 프로젝트 실패해도 다음으로 계속 | **3실패**(멈춤 · 이름 선행 · 다른 계정 무영향) |
| **MO-3** | over | 경계를 `>` 로 조임 | **5실패**(30일째 미도래 — `test_auth_users` 경계 셀 포함) |
| **MU-6** | under | `NEVER_SWEPT` 를 비워 `users` 도 스윕 | **2실패** |

9회 모두 `git checkout` 뒤 `git status --short`·`git diff HEAD --stat` 무출력 확인. **변이 전에 커밋이 먼저였다**(§6 프리플라이트).
...
- **Slice 4(화면)** 가 다음이다 — 요청·취소·남은 일수 배너 + **D3 의 관리자 잔여 정리**(ⓔ 로 여기에 왔다). Slice 5 는 정책 문서 §8 → §6 승격.
- **랜딩 축 ②③④** — `/terms`·`/privacy` + 푸터(D2=ⓐ 는 **본문 대조 가드**를 함께 요구한다) · 랜딩 본문 · 동의 게이트(버전 `1.0` 이 입력).
- **`login_failures`(`_id` = username)** 는 계정 파기의 어느 규칙도 못 찾는다 — 사용자명이 재사용되면 새 계정이 옛 잠금을 물려받는다. 브리프 후속 고려에만 있고 아직 안 정했다.
