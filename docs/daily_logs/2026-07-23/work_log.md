...
- **`CLAUDE.md`·`AGENTS.md`의 `### HANDOFF.md` 절 강화**(양쪽 동일 문안): **"완료 문단을 Current Status에 append하지 말 것"**을 실측 수치(366줄·81마커·4811자)와 함께 명시, **머신-로컬 관측치를 프로젝트 사실로 적지 말 것**을 8001 사고와 함께 명시, 그리고 길이는 **하드 캡이 아니라 자가 검수 트리거**로 다룬다(아래 오너 재지시 반영).
...
### 오너 재지시 반영 — 하드 캡 → 자가 검수 트리거

- **오너 지적**: "줄수·글자수 상한을 제안하는 방식은 안 좋다. 실제로 더 많아질 수도 있고 세세하게 적어야 할 사항도 있다. 대신 200~300줄쯤 지나면 헤드라인이 지켜지고 있는지 재확인하는 자가 검수를 넣자."
- **동의하고 교체했다.** 초안의 하드 캡(150줄 / 한 줄 400자)은 두 방향으로 실패한다: (1) 정당하게 길어야 할 내용을 막고, (2) **줄바꿈을 없애 숫자만 맞추는 회피**를 유도한다 — 내가 방금 지운 **4811자짜리 한 줄**이 정확히 그 회피의 표본이다. 길이는 증상이고 실제 병은 **완료 서술의 중복**인데, 캡은 증상만 재고 병은 못 본다.
- **교체 내용**: 길이 상한 삭제 → **~200줄에서 자가 검수 트리거**(그 뒤 ~100줄마다). 검수는 숫자가 아니라 질문 4개다 — (1) 이 줄이 다음 작업자의 **오늘 할 일**을 바꾸는가 (2) 완료 서술인가(그러면 이미 3곳에 있으니 삭제) (3) 아직 사실인가(날짜 붙은 관측치는 만료된다) (4) **길어야 할 이유가 있으면 유지한다 — 트리거를 넘긴 것 자체는 실패가 아니고, 안 보는 것이 실패다.**
- **구현자 보탬 — "마지막 자가 검수" 마커**: 트리거만 두면 *언제 마지막으로 봤는지* 아무도 모른 채 지나칠 수 있다. `HANDOFF.md` 머리에 `> 마지막 자가 검수: YYYY-MM-DD · N줄` 한 줄을 남기게 해서, 트리거가 실제로 이행되는지를 다음 작업자가 즉시 볼 수 있게 했다. 현재 값 `2026-07-23 · 111줄`.
- 검증: 세 파일에서 하드 캡 문구 잔존 **0**, `CLAUDE.md`와 `AGENTS.md`의 해당 절 **byte-identical**(diff 0), HANDOFF 113줄.
