...
## Task 7 — 스택 재빌드 + 라이브 관통 (육안 확인 준비 · Slice 2 한 홉 폐쇄)

### User Decisions and Rationale

- **오너 지시(2026-08-10)**: *"재빌드 가자."* 9.1 의 마지막 남은 것(육안 확인)을 위한 선행 작업.

### 재빌드가 **필수**였던 이유 (선택이 아니었다)

`docker images` 실측 — `application` 이미지가 **11일 전**(≈2026-07-30)이었다. 활동 API 는
**2026-08-09**(operation 77)이므로 **그 이미지에는 엔드포인트 자체가 없다.** 재빌드 없이 화면을
열었다면 404 를 보고 *"구현이 안 됐나"* 로 오독했을 자리다. `application`·`admin`·`frontend`
셋을 빌드했다.
...
| 확인 | 결과 |
|---|---|
| 제품앱 `:8520/projects` | **401**(인증 시행이 살아 있다 = 새 이미지) |
| 제품앱 `:8520/projects/x/activity` | **401** — ★ **operation 77 이 배포 이미지에 있다**(옛 이미지면 404) |
| 제품앱 `:8520/admin/users` | **404** — 라우터에 없다(Slice 2 토폴로지) |
| **★ nginx `:5520/api/admin/users`** | **401** — **오너 대기였던 Slice 2 관통 한 홉이 닫혔다** |
| SPA 진입 · 활동 딥링크 | 200 · 200 |
| `docker compose ps` | **healthy 8** + healthcheck 없는 2 |

**게이트웨이 healthcheck 가 liveness 전용**이라 외부 LLM 없이도 스택이 뜬다 — 활동 화면 확인에
LLM 이 필요 없다는 것을 실측으로 확인했다.

### 라이브 관통 — 실제 활동 5건

확인용 계정 `timeline_demo` 를 따로 만들고(기존 `owner`·`probe`·`d8four` 를 안 건드리려고)
제품 API 로 프로젝트 생성 → 개명 → 원고 생성 → 개명 → 버전 저장을 했다. `GET …/activity` 가
**5건을 최신순으로** 돌려주고, **실제 라벨표로 렌더**하면 이렇게 보인다:

```
원고 저장           1
원고 제목 변경        1장 초고 → 1장 — 재회      [원고 열기]
원고 생성           1장 초고                  [원고 열기]
프로젝트 이름 변경      타임라인 확인용 → 타임라인 확인용 — 개명
프로젝트 생성         타임라인 확인용
```

`draft` 행에만 링크가 붙고 `draft_version` 행에는 안 붙는 것(F7)이 실물로 확인된다.

### Issues found — 내 curl 이 낸 422 가 오히려 계약을 증명했다

버전 저장 첫 시도가 **`idempotency_key` 누락으로 422** 였는데, `curl -s` 는 HTTP 4xx 에도 exit 0
이라 `&& echo "version saved"` 가 성공처럼 출력됐다. 타임라인 최신 항목이 `draft_renamed` 인 것을
보고 되짚어 찾았다.

**제품 결함이 아니고, 오히려 라이브 증거가 됐다** — **그 422 는 활동을 남기지 않았다**.
A7=A(*"결과를 안 뒤에 기록한다"*)가 실 스택에서 성립한다는 뜻이다(회귀는 404·409 로 잠가 뒀고,
422 는 그 축의 또 다른 예다).

**교훈**: 라이브 확인에서 `curl` 의 종료코드로 성공을 판단하지 말 것 — `-w '%{http_code}'` 를
항상 붙인다.

### 정정 — 내가 처음에 "DB가 비었다"고 잘못 읽었다

`db.getSiblingDB("ai_writing")` 로 조회해 `users: 0` 을 보고 그렇게 말했으나, **실제 DB 이름은
`ai_writing_system`**(`CORE_SOT_MONGO_DB`)이다. 진짜 값은 users 3(`owner`·`probe`·`d8four`) ·
projects 11 · drafts 11 이고, `activity_events: 0` 은 정상이다(기능이 새것이라 그 뒤로 아무 동작이
없었다). **HANDOFF 베타 관측에 DB 이름을 못박아** 다음 사람이 같은 거짓 판독을 하지 않게 했다.

### Completed work

| 파일 | 변경 |
|---|---|
| `HANDOFF.md` | 베타 관측 재작성(스택 기동·이미지 재빌드·**DB 이름**·LLM 불필요) · **오너 대기에서 nginx 한 홉 제거** · **육안 확인 준비 항목**(주소·계정·데이터·정리 방법) |

코드 변경 **0줄** — 운영 확인 작업이다.

### 아직 안 한 것 (의도)

- **브라우저 렌더 육안 확인** — 사람이 봐야 하는 마지막 한 걸음. 준비는 끝났고 주소·계정이
  HANDOFF 에 있다.
- **확인용 데이터 정리** — 오너가 보고 난 뒤가 맞다(파기하면 활동도 함께 사라진다 = I1 실증 기회).
...
## Task 8 — 오너 정정 반영: 관리자 접근 이력 링크 제거 + IA 방향 기록

### User Decisions and Rationale

- **★ 오너 정정(2026-08-10) — C-4 의 소비자가 누구였는가**: *"그 계약이 음 서로 오해가 있었던 거 같은데,
  내가 생각했던 관리자 기록은 **관리자가 보는 거**였어. 그래서 **문의가 들어올 때 그걸 보고 대답**하는
  걸로였는데."* → **일단 링크만 제거**하고 나머지는 나중에.
- **★ 개인 페이지는 전체 통합이다**: *"개인 페이지잖아? 프로젝트 페이지가 아니고? 그러면 **전체 통합이어야지**.
  … 개인페이지 허브에서 **프로젝트 별로 뻗어나가는** 거지. 그래서 전체 통합이라고 얘기한 거니까 혹시나
  헷갈리지 말고."* → 구현자가 제안한 "프로젝트별 링크만 모으는 허브(backend 0줄)"는 **오너 의도가 아니다**.
- 프론트 전반 재설계 방향(관리자 랜딩 · 활동/관측 이사 · 로그인 화면)은 **방향만 확정, 슬라이스는 미착수**.

### 이 정정이 실제로 뜻하는 것 (다음 사람이 볼 자리)

**오해의 자리가 명확하다.** D8-5f C-4 를 승인할 때 오너가 그린 소비자는 **관리자**였는데, 구현은
[`routers/projects.py:131`](../../../services/application/app/routers/projects.py#L131) 의 근거
(*"알림 채널이 없으므로 '소유자가 알게 된다'의 현실적 형태는 그들이 볼 수 있다는 것"*)를 따라
**소유자 화면에도** 뒀다. 그 근거 자체가 틀린 것은 아니지만 **오너가 승인한 것은 그 용도가 아니었다.**

**능력 손실이 없다는 것을 먼저 확인했다** — 같은 API 를 관리자 콘솔이 이미 소비한다
([`AdminConsole.tsx:252`](../../../frontend/src/admin/AdminConsole.tsx#L252) "접근 이력 보기" →
`listProjectAccessLog`). 즉 오너가 원한 형태는 **이미 구현되어 있었고**, 남는 것은 소유자 쪽 노출뿐이었다.

### Completed work

| 파일 | 변경 |
|---|---|
| [`DraftList.tsx`](../../../frontend/src/drafts/DraftList.tsx) | "관리자 접근 이력 →" 링크 제거. **route·API 는 그대로** |
| `DraftList.test.tsx` | **+1 cell**(over-strict) — 링크가 되살아나면 실패 · 같은 자리 다른 두 링크를 뭉뚱그려 지워도 실패 |
| `docs/system-contract-sot.md` | **v1.7.95** — 노출 결정임을 명시(계약 개정 아님) · 남은 결정과 전제 공유 관계 |
| `HANDOFF.md` | Owner Decisions 에 **IA 방향 5항**(특히 ④ 전체 통합) · 관리자 콘솔 관찰 · 기준선 |

### 왜 API 를 안 없앴나

계약을 축소하면 `schema.d.ts`·tier 가드·경계 행렬이 함께 움직이고 **되돌리기가 비싸진다**.
오너 지시도 *"일단 링크만"* 이었다. **그래서 이것은 계약 개정이 아니라 노출 결정**이고, SoT 행에
그렇게 적었다.

### ★ 남은 결정이 S3·F4 와 전제를 공유한다

C-4 의 *"소유자가 본다"* 근거가 지금 공허한 것은 **관리자 = 오너(1인)** 라는 전제 위에서만이다.
그 전제는 **S3(행위자 열 없음)·F4(공동 작업 시 되살림)와 같은 것**이다. 전제가 바뀌면 셋을 함께
되봐야 하며, SoT v1.7.95 에 그렇게 적었다.

### ★ 개인 페이지 = 전체 통합의 무게 (다음 슬라이스가 알아야 한다)

구현자는 "허브 = 프로젝트별 링크 모음, backend 0줄"을 권고했고 **오너가 그것을 정정했다**.
전체 통합은 **사용자 전역 조회라는 새 표면**을 부른다 — 지금 활동(operation 77)도 관측 KPI 도
**project-scoped** 다. 결정 대상: 무엇을 합칠지 · 상한 · 정렬 · 소유권 필터. 관리자 전역 조회
(A5=C)와 **다른 축**이지만 무게는 비슷하다.

### 뮤테이션 (1종)

| # | 방향 | 적용한 diff | 재실패 셀 |
|---|---|---|---|
| M1 | 되돌림 | `DraftList.tsx` 에 access-log 링크 복원 | `does not offer the admin access log to the project owner` (1) |

커밋(`1d690a0`) 뒤 실행, 원복 후 트리 clean.

### Verification

| 검사 | 결과 |
|---|---|
| frontend 전수 | **`273 passed / 19 files`**(272 → **+1**) |
| build | **701 modules · 진입 417.09 kB**(417.19 → **-0.10**, 링크 하나 분량) · lazy 청크 무변 |
| `tsc --noEmit` | 통과 |
| `test_docs_indexes.py` | `13 cells / 242 subtests` |
| 계약 | operation **77 무변** · `schema.d.ts` 무변 |

### Next steps

1. **IA 슬라이스(9.2) 브리프** — 오너 결정 대기 항목이 HANDOFF 에 정리돼 있다. **개인 페이지 = 전체
   통합**이므로 backend 표면 설계가 브리프의 중심이다.
2. 디자인은 **IA 다음**이다(지금 IA 를 바꾸는 중에 비주얼부터 하면 두 번 한다).

---

## Task 9 — 오너 결정: 접근 이력 열람도 승격·감사 대상 (현행 유지)

### User Decisions and Rationale

- **오너 결정(2026-08-10)**: Task 8 이 남긴 관찰(*"이력만 보고 싶을 때도 grant 를 발급해야 하는 것은
  어긋날 수 있다"*)을 **검토한 뒤 현행 유지**로 정했다: *"개인 자산을 보는 건데 이력만 보려고 하는
  것도 승격이 되어야 하나…? 그냥 둬도 되겠다. **이력만 보는 것도 감사대상으로 보자.**"*
  근거는 **열람 대상이 타인의 개인 자산**이라는 것 — 내용이든 그 내용에 누가 접근했는지든 남의
  프로젝트를 들여다보는 행위라는 점에서 같다.

### ★ 결정이 이미 코드의 동작과 일치한다 (확인함)

관찰을 제기할 때 구현자는 UI 의 `disabled` 만 보고 *"UI 판단일 수 있다"* 고 적었는데, 실제로는
**백엔드 제약을 비추고 있었다**:

- `GET /projects/{id}/access-log` 는 `_REQUIRE_PROJECT_OWNER` 다 — 관리자는 **살아 있는 grant 없이는
  403**. UI 에서 `disabled` 를 풀어도 403 을 받는다.
- grant 로 통과한 요청은
  [`dependencies.py:128`](../../../services/application/app/api/dependencies.py#L128) 의 `record_use` 가
  `access_grant_uses` 에 남긴다. 주석이 그 설계를 명시한다 — *"이 dependency 가 단일 choke point 라
  endpoint 를 추가해도 기록을 우회할 수 없다"* 이고 **격리하지 않는다**(기록 실패 시 요청이 죽는다).

**따라서 오너의 "이력만 보는 것도 감사대상"은 이미 문자 그대로 참이다** — 이력을 본 행위가 그
이력에 한 행으로 남는다. 자기참조지만 감사 로그의 정상 성질이다.

**코드 변경 0줄.** 결정은 "바꾸지 않는다"이고, 다음 사람이 같은 관찰을 다시 제기하지 않도록
HANDOFF 항목을 **관찰 → 폐쇄(현행 유지)** 로 바꾸고 근거를 `file:line` 과 함께 적었다.

### Next steps

- **IA 슬라이스(9.2) 브리프가 다음이다** — 오너 확인 질문(*"브리프 먼저인가?"*)에 구현자 답변은
  **그렇다**. 개인 페이지가 **전체 통합**이라 backend 새 표면 설계가 브리프의 중심이고, 그것을
  정하지 않고 화면부터 만들면 두 번 만든다.

---

## Task 10 — 9.2 브리프 작성 (개인 페이지 = 전체 통합 허브 + IA)

브리프: [`plans/09-2-personal-hub-and-ia-decisions.md`](../../plans/09-2-personal-hub-and-ia-decisions.md)
(**Open — P1~P7 오너 결정 대기**).

### User Decisions and Rationale

- **오너 지시(2026-08-10)**: *"응 바로 작성해줘."* 그 앞의 *"브리프 먼저인가?"* 에 구현자가 **그렇다**고
  답한 것을 승인. 근거는 **개인 페이지가 전체 통합이면 backend 새 표면이 필요**하고, 모양을 정하지
  않고 화면부터 만들면 두 번 만든다는 것(9.1 의 S6 이 작은 규모로 이미 그 형태였다).

### 브리프를 쓰기 전에 코드에서 확인한 것 (추측으로 쓰지 않았다)

| 사실 | 값 | 왜 필요한가 |
|---|---|---|
| 활동·관측이 전부 project-scoped | `GET /projects/{id}/activity` · `…/observability/kpi` | **P1 전체가 이 사실 위에 선다** |
| 사용자 전역 조회 | **존재하지 않는다**(전역은 `GET /admin/observability/kpi` = 관리자 전용) | 통합이 신설이라는 근거 |
| `_fold` 공유 | `aggregate_kpi`·`aggregate_global_kpi` 가 한 곳을 쓴다([`kpi.py:202`](../../../services/application/app/observability/kpi.py#L202)) | P3-ⓑ 를 고르면 구현이 작다는 근거 |
| `GET /me/quota` 응답 | `remaining`·`unlimited`·`status` + 일/주 `limit·used·remaining·resets_at` | **개인 페이지의 첫 내용**이 이미 있다 |
| `GET /projects` | **이미 소유자 필터**(`list_projects_for_owner`) | P1-ⓑ(fan-out)가 가능은 하다는 근거 |
| tier | 인증 전용 **3**(`POST /projects`·`GET /projects`·`GET /me/quota`) | 새 endpoint 가 들어갈 자리 |
| `AuthGate` 범위 | 라우터 **전체**를 감싼다 → 비로그인 화면 0개 | P6 의 "자리만 확보"가 무슨 작업인지 |

### ★ P1 에서 가장 중요한 논거 — fan-out 은 비용이 아니라 **정확성** 문제다

프론트 fan-out(ⓑ)은 backend 0줄이라 매력적으로 보이지만, **프로젝트마다 100 으로 자른 뒤
합치면 "전체에서 최근 20건"이 실제로 최근 20건이 아닐 수 있다** — 오래된 프로젝트의 100번째가
새 프로젝트의 1번째보다 최신일 수 있다.

**9.1 이 계약으로 삼은 것이 정확히 "화면이 상한을 정직하게 말한다"였고**(S2=ⓐ · 상한 연결 가드),
ⓑ 는 **정직하게 말할 수 있는 상한이 없다.** 그래서 권고가 ⓐ 다.

### ★ 확인이 필요한 자리 둘 (브리프에 그렇게 적었다)

1. **P3 — 오너의 "전체 통합"이 KPI 까지인가.** 구현자는 그 말이 **활동 로그 맥락**에서 나왔다고
   읽었고, KPI 는 프로젝트 성격에 종속돼(장편 report vs 짧은 gate) 합치면 해석이 죽는다고 본다.
   그래서 권고는 "진입만"이지만 **오너 뜻이 다르면 ⓑ 다.**
2. **P7 — 프로젝트 화면을 비우는 것이 목적인가.** 권고는 "활동만 남긴다"(저작 중의 질문이므로).
   **비우는 것이 목적이면 ⓐ 다.**

### 유예 신설 F8~F10

9.1 의 F1~F7 은 유효하고 **F1 의 의미가 바뀐다**(통합이 프로젝트별보다 먼저 상한에 찬다).
새로: **F8** 사용자 전역 KPI · **F9** 계정 설정(지금 API 자체가 없다) · **F10** 공개 랜딩
(트리거 = dogfood 를 넘어 남에게 보여줄 때). 전부 **여는 트리거**를 붙였다.

### Completed work

| 파일 | 변경 |
|---|---|
| [`plans/09-2-personal-hub-and-ia-decisions.md`](../../plans/09-2-personal-hub-and-ia-decisions.md) | **신규** — P1~P7 선택지 표 + 권고 + F8~F10 + Deferred + 구현 순서 |
| `HANDOFF.md` | IA 방향 항목 → **브리프 포인터**로 교체(묻는 것 일곱과 핵심 논거 요약) |
| `plans/README.md` · `README.md` | 인덱스 행 + 건수 103 → **104** · 브리프 85 → **86** |
...
1. **오너가 P1~P7 을 정하면** 브리프 §"결정 뒤 구현 순서" 1~7 로 착수. **1번이 회귀 먼저**다.
2. P3·P7 은 **확인 질문**이 붙어 있다 — 답이 권고와 다르면 그 자리만 바뀐다.

---

## Task 11 — 9.2 브리프 검증 반영 (★ 내 핵심 논거가 틀렸다)

### User Decisions and Rationale

- **오너 지시(2026-08-10)**: *"검증기록 확인해서 보강할 부분 보강해줘. 그리고 난 다음 결정할게."*
  **결정 전에** 브리프를 바로잡으라는 것 — 틀린 근거 위에서 결정이 내려지지 않게.

### ★ 반증된 것 — P1 의 "정확성" 논거

**내가 브리프에 적은 중심 문장이 거짓이었다**: *"프로젝트마다 100건으로 자른 뒤 합치면 '전체에서
최근 20건'이 실제로 최근 20건이 아닐 수 있다 … ⓑ 의 치명적인 점은 비용이 아니라 정확성이다."*

**검증 세션이 증명으로 반박했고, 나는 그것을 받아들이기 전에 직접 깨뜨려 봤다**(가이드의 "다른
에이전트 보고를 액면가로 받지 않는다"):

| 조건 | 무작위 20,000회 반례 |
|---|---|
| **N ≤ per-project 상한(100)** | **0** — fan-out 병합이 전역 top-N 과 완전 일치 |
| N > 상한(100) | 3,181 |

스크립트를 커밋했다([`repro_fanout_topn.py`](../../verifications/2026-08-10/repro_fanout_topn.py)) —
memory 규칙(*"검증 재현 스크립트는 커밋한다"*)대로 `/tmp` 에 두지 않았다.

**내 오류의 형태**: *"병합 풀의 나이 폭이 넓다"*(참)와 *"top-N 이 틀린다"*(거짓)를 뒤섞었다. 내가
든 예시(오래된 프로젝트의 100번째가 새 프로젝트의 1번째보다 최신)는 **사실이지만 top-N 을 깨지
않는다** — 그 사건이 최근 창에서 올바르게 빠지는 것일 뿐이다.

**★ 왜 이것이 중요한가**: 그 "정확성"이 **이 브리프가 계약 변경(operation 77→78)을 부르는 유일한
"치명적" 사유**였다. 거짓이면 **오너가 틀린 근거 위에서 계약 비용을 감수하도록 결정이 올라간 것**이
된다. 그리고 **내가 제안한 통합 상한이 50(≤100)이었으므로 내 설계에서는 ⓑ 도 정확했다.**

**결론은 안 바꿨다** — ⓐ 를 여전히 권고한다. 바꾼 것은 **근거**다: 정확성이 아니라 *진짜 통합 표면 ·
전역 총건수 · 서버 단일 상한 · F1 커서가 그 위에서만 자연스러움* 대 *계약 변경 비용*의 저울질이고,
**ⓑ 가 1인·소수 프로젝트에서 훨씬 실행 가능하다는 것을 오너가 알고 고르도록** 표에 적었다.

**남는 진짜 경계도 적었다**: ⓑ 의 정확성은 **상한 100 에 묶여 있고**, 통합 상한을 101 로 올리거나
F1 을 여는 순간 **조용히** 틀려진다.

### 신설 — P8 (검증이 찾아낸 숨은 결정)

`GET /me/activity` 의 범위가 **소유 기준인가 행위자 기준인가**를 나는 결정으로 올리지 않고
§"구현 순서"에서 *"소유 프로젝트 집합"* 이라고 **조용히 골랐다**. 활동 이벤트는 `actor_user_id` 를
가지므로 두 기준이 실재한다.

**오늘은 구분되지 않는다**(관리자=오너, 승격 미사용 → 모든 이벤트가 양쪽을 만족). **그러나 소유
기준은 9.1 S3 의 전제(*"행위자는 항상 본인"*)를 이 화면에서 먼저 깬다** — 승격 관리자가 내
프로젝트에서 한 일이 **행위자 표시 없이** 내 타임라인에 섞인다. 그래서 권고를 **ⓐ 소유 기준 +
계약에 조건 명시**(*"승격이 실사용되면 F4 를 함께 연다"*)로 적었다.

### 나머지 정정 4건

| # | 지적 | 반영 |
|---|---|---|
| P2 | 통합 50 < per-project 100 이면 **"전체"가 "하나"보다 적게 보이는 역설** | 통합 상한을 **per-project 이상**으로 · P1-ⓑ 의 100 경계와의 상호작용 명시 |
| P3 | "해석이 흐려진다"가 과장 — **관리자 전역 KPI 가 이미 더 이질적인 집계**를 한다 | 권고 ⓐ 유지, 이유를 ①얻을 게 적고 ②lazy 경계 ③`_fold` 소비자 증가로 다시 씀 |
| P5 | 로그인 직후 `/admin` 이 **딥링크 도착지를 삼킨다** | **`?next=` 보존**(있으면 그쪽이 이긴다) |
| P7 | "접근 이력 제거"는 부정확 — **링크만** 제거, route·페이지는 살아 있다 | 표기 정정([`App.tsx:33`](../../../frontend/src/App.tsx#L33)) |

### 검증 기록이 파일로 없다 — 그래서 브리프에 담았다

그 검증은 **별도 기록 파일로 발행되지 않았다**(세션 산출물이고 검증자가 발행 여부를 물었다).
`docs/verifications/2026-08-10/` 에는 9.1 기록 둘뿐이다. **load-bearing 한 내용이 사라지지 않도록
브리프 §"부록 — 2026-08-10 독립 검증 노트"에 지적 6건과 반영 내역을 표로 남기고 재현 스크립트를
커밋했다.** (검증 기록 자체의 발행은 검증자·오너 판단 사안이다.)

### Completed work

| 파일 | 변경 |
|---|---|
| [`plans/09-2-…-decisions.md`](../../plans/09-2-personal-hub-and-ia-decisions.md) | **P1 논거 재작성**(반증 + 실측 표 + 저울질) · **P8 신설** · P2·P3·P5·P7 정정 · 요약·구현 순서 갱신 · **부록 검증 노트** |
| [`verifications/2026-08-10/repro_fanout_topn.py`](../../verifications/2026-08-10/repro_fanout_topn.py) | **신규** — 반증 재현 스크립트 |
| `HANDOFF.md` | 묻는 것 일곱 → **여덟** · P1 논거 정정 · P8 요약 |

### Verification

| 검사 | 결과 |
|---|---|
| `repro_fanout_topn.py` | N≤100 반례 **0/20,000** · N>100 반례 3,181/20,000 |
| `test_docs_indexes.py` | `13 cells / 242 subtests` |
| 코드 변경 | **0줄**(제품 코드 무변 — 결정 문서와 재현 스크립트) |

### Next steps

1. **오너가 P1~P8 을 결정한다**(지시: *"그러고 난 다음 결정할게"*).
2. 결정 뒤 브리프 §"결정 뒤 구현 순서" 1~7. **1번이 회귀 먼저**이고 P8 을 셀로 못박는다.
