...
## Task — 다중 사용자(인증·소유권·CMS/관리자) 계획서 작성

### Goals

- 오너 요청: "로그인 기능과 인증, 각 제작물에 대한 아이디별 관리·삭제 같은 CMS 기능, 전체 관리자 기능.
  별도 서버로 해야 되나?" → **계획 문서만** 만들고 구현은 하지 않는다.

### Issues found — 요청이 정본 계약과 충돌한다

- SoT §"제품과 프로젝트 경계"의 두 조항을 뒤집는 요청이다: "MVP는 **계정/인증이 없는** 단일 사용자
  시스템", "향후 다중 사용자를 위해 **`user_id`를 지금 억지로 넣지 않는다**". CLAUDE.md §5에 따라
  임의로 진행하지 않고 **문서 첫 절에서 충돌을 명시하고 D0(유예 해제 여부)로 올렸다**.
- 다만 두 번째 조항은 *금지*가 아니라 **"때가 되기 전에 넣지 말라"는 유예**다. 즉 이 요청은 계약
  위반이 아니라 **유예의 해제**이며, v1.6.93이 "frontend framework 보류"를 해제한 것과 같은 성격이다.
  그 구분을 문서에 적었다 — 오너가 "위반을 승인"하는 게 아니라 "때가 됐다고 선언"하는 결정이다.
- **삭제 정책은 오히려 비어 있었다**: §"Product Shell"이 "보관/삭제 정책 … 미확정"이라고 명시한다.
  CMS의 "삭제"는 계약을 뒤집는 게 아니라 빈 칸을 채우는 일이다.

### 실측 — 이 변경이 건드리는 표면 (추정이 아니라 센 값)

- **공개 operation 62개 / 52 path**(`create_app().openapi()`로 집계). `/health` 외 전부가 인가 대상이고,
  H3 계약상 **401/403을 `responses=`에 선언**해야 하며 트랙별 전수 가드가 빠뜨림을 잡는다.
- `DELETE /projects/{id}`·`/drafts/{id}`는 **이미 있지만 archive**다([`main.py:2242`](../../../services/application/app/main.py#L2242)).
  CMS "삭제"가 archive인지 영구 파기인지가 D5.
- **ES `xpack.security.enabled=false`의 근거가 "인증 slice 선행 필요라 범위 밖"**(v1.6.53 G3=A)이었다 —
  이 슬라이스가 바로 그 선행이다. Mongo 무인증도 같은 묶음.
- worker·script는 request 컨텍스트가 없다(관측 seam이 이미 이 성질에 의존) → **서비스 계정** 개념 필요.

### Completed work

- [`plans/multi-user-auth-cms-decisions.md`](../../plans/multi-user-auth-cms-decisions.md) 작성.
  D0(유예 해제) + D1~D8(별도 서버 여부 · 인증 방식 · 소유권 모델 · 기존 데이터 귀속 · 삭제 의미 ·
  관리자 범위 · 인가 시행 지점 · 슬라이스 순서), 각 항목 options 표 + 추천 + 근거.
- HANDOFF "Owner Decisions Needed" 최상단에 **★★ D0**를 올렸다(dogfood와 선후를 정해야 하는 항목).

### Decisions (구현자 판단 — 문서 안의 추천)

- **오너의 직접 질문("별도 서버로 해야 되나?")에 대한 답은 A(Application 내부 모듈)**다. 근거는
  실측에서 나왔다 — **분리해도 인가는 application에 남는다**. Gateway(GPU·모델 파일)나
  embedding(모델 로딩)이 분리된 동기가 인증에는 없고, 분리하면 토큰 발급만 옮겨가면서 서비스 간
  신뢰·키 배포·장애 결합이 새로 생긴다.
- **세션 쿠키 > JWT**를 추천했다. JWT의 무상태 장점이 단일 origin SPA에서 값을 하지 않는 반면,
  **즉시 무효화 불가**라는 단점은 오너가 요청한 **관리자 기능(계정 정지·삭제)과 직접 충돌**한다.
- **인가는 dependency + 전수 가드**로. 이 저장소가 이미 두 번 쓴 패턴이고(H3 선언 가드·관측 조립
  가드), 관측에서 실측된 교훈("wrapper를 벗겨도 56 passed")이 인가에서 반복되면 결과가 **데이터 유출**이다.
- **영구 삭제를 마지막 슬라이스로 뒀다.** project 하나를 파기하면 snapshot·version·source_ref·memory·
  Chroma 벡터·ES 색인·관측 레코드·scratch가 전부 대상이고, **부분 삭제는 조용한 고아 데이터**를 만든다
  (지워지지 않은 임베딩이 다른 프로젝트 검색에 뜨는 식).

### Next steps

- **D0가 먼저다.** 해제하면 D1~D8을 순서대로, 해제를 미루면 dogfood(★) 뒤로. 어느 쪽이든 이 문서는
  그대로 쓸 수 있게 썼다.
- D0=A일 경우 **SoT 두 조항 개정이 첫 커밋의 일부**여야 한다 — 계약을 고치지 않은 채 구현이 먼저
  들어가면 정본과 코드가 갈라진다.

### D0 결정 — 유예 만료, 정본 개정 (SoT v1.7.49)

**오너 판정(2026-07-26)**: "정본은 MVP용이었고 지금은 어느 정도 완성 단계니까 확장하려는 것 —
정본을 수정하거나 확장하면 된다."

내가 계약을 **바꿀 수 없는 것처럼** 다룬 것이 틀렸다. §5는 충돌 시 "어느 쪽이 canonical인지 묻는다"
이지 "기록된 결정이 이긴다"가 아니고, 그 두 조항은 **스스로 "MVP는"·"지금"이라는 시점 한정어를
달고 있었다**. 즉 만료 조건이 조항 안에 적혀 있었는데 그걸 못 읽었다. 물은 것 자체는 맞았지만
(오너만 단계 전환을 선언할 수 있다) 프레이밍이 "위반 승인 요청"에 가까웠다.

**정본 v1.7.48 → v1.7.49, 본문 4곳 개정**:

1. §"제품과 프로젝트 경계"의 두 조항을 **"다중 사용자로 확장한다"**로 교체. 함께 못박은 것:
   **"아직 구현되지 않았으므로 배포 스택은 여전히 무인증 — 이 조항을 근거로 외부에 노출하면 안 된다."**
   계약이 의도를 먼저 적고 코드가 뒤따라오는 구간에서 가장 위험한 오독이 "계약에 적혀 있으니 있겠지"다.
2. **`project_id` 강제는 살아남는다** — 소유권은 그 위에 얹히는 **두 번째 경계이지 대체가 아니다**.
3. frontend CORS 조항: 종전 근거("인증 없는 단일 사용자 API")가 **사라지는 게 아니라 강해진다**고 정정.
   쿠키 인증에서 CORS를 여는 것은 CSRF 표면을 넓히는 일이다.
4. §"Product Shell"의 "보관/삭제 정책 미확정"을 **보관은 확정 / 영구 삭제만 미확정(D5)**으로 분리하고,
   파기 범위가 Core SOT를 넘어 memory·벡터·ES·관측까지 걸쳐 **부분 삭제는 조용한 고아 데이터를
   만든다**는 사실을 적었다.

인프라 무인증(Mongo·ES `xpack.security=false`)은 v1.6.53 G3=A가 **스스로 "인증 slice 선행 필요"**라고
적어 둔 항목이라, 이 확장이 그 선행임을 명시했다.

**브리프 갱신**: D0 절에 결정을 기록하고, §0 헤더를 "충돌한다"에서 **"착수 시점의 정본과 충돌했다
(v1.7.49로 해소)"**로 고쳤다 — 결정 근거의 기록은 남기되 현행 정본에 대해 거짓을 말하지 않게.

**코드·회귀 무변**(계약 단계 전환만). D1~D8은 승인 대기다.

### 독립 검증 반영 — D0 단계 전환 (합격·조건 없음, 비차단 2건 조치) + 기록문서 정리 (SoT v1.7.50)

오너 요청 독립 검증(`docs/verifications/2026-07-26/multi_user_d0_contract_transition.md`)이
**합격(조건 없음)**. 검증자가 400KB 정본을 시점 한정어로 전수 grep해 "본문 4곳이 전부"를 확인하고,
`project_id` 강제가 실제로 코드에서 cross-project 접근을 거부하는지(`core_sot/service.py` 6개 지점)까지
재도출했다. **검증자가 발견한 것**: 직전 정본(v1.7.48)이 §Core SOT("보관 확정")와 §Product Shell
("보관/삭제 미확정")로 **같은 정본 안에서 충돌하던 기존 부채**를 갖고 있었고, 이번 개정이 단계 전환과
함께 그것도 해소했다.

- **H1 — 규모 숫자의 `/health` 포함/제외 불일치.** "62 operation 전부"와 "`/health` 외"를 함께 써 왔는데
  62/52는 `/health`를 **포함한** 값이다. 재측정: 전체 **62/52**, 제외 **61/51**. D7 전수 가드가 담을
  범위는 제외 쪽이므로 정본·브리프·HANDOFF를 61/51로 통일하고 전체 값을 괄호로 남겼다.
- **H2 — "CSRF 표면" 비유 정정.** 쿠키 인증에서 CORS를 여는 것의 실제 효과는 **다른 origin이 사용자의
  인증 상태로 API 응답을 읽게** 되는 것이고, CSRF는 쿠키 `SameSite`가 막는 **다른 축**이다(CORS는 CSRF를
  막지 않는다). 브리프 D2=A가 이미 `SameSite=Lax`로 정확히 적고 있었으므로 정본을 그 정밀도에 맞췄다.
- **덤으로 잡은 stale 1건**: H3 절의 "503 선언은 `/health` 제외 **60개** 전부"가 v1.7.48의 KPI endpoint
  추가를 반영하지 못한 채 남아 있었다. 재측정으로 **61 operation / 503 선언 61개**를 확인해 정정했다.
  수치를 계약 문장에 박아 두면 endpoint가 늘 때마다 함께 늙는다 — 이번처럼 정정 슬라이스에서
  주변 숫자를 함께 재는 것이 그 부채를 갚는 방법이다.

### 기록문서 정리 (다음 작업자 인계)

오늘 작업이 크게 세 덩어리(관측 증분 C·5 → 대시보드 → 다중 사용자 단계 전환)라 HANDOFF에 stale이
쌓였다. 자가 검수하고 정리했다(124줄 → 129줄; 늘어난 것은 stale 제거보다 인계 정보가 더 많아서다).

- **가장 중요한 수정**: Active Decisions에 **"MVP는 계정/인증 없는 단일 사용자"가 표준 제약으로 남아
  있었다** — 방금 개정한 정본과 정면으로 모순된다. 다중 사용자 확장으로 바꾸고, **"코드는 아직 없으니
  외부 노출 금지"**와 **"소유권은 `project_id` 강제를 대체하지 않는다"**를 함께 적었다.
- **"진행 중 페이즈 — 관측 KPI"를 지웠다.** 그 페이즈는 오늘 계측·집계 API·대시보드까지 닫혔다.
  대신 "진행 중 페이즈 없음 + 다음은 오너가 두 ★ 중 고르는 것"으로 바꾸고, 페이즈 결과물(다음 작업이
  그 위에서 도는 것)만 남겼다.
- **H3 operation 수 60 → 61**(HANDOFF도 정본과 같은 stale을 갖고 있었다).
- **머신 상태 관측치를 2026-07-24 → 2026-07-26으로 갱신**하고, "스택 기동을 기다리는 유일한 미검증
  항목은 화면 육안 확인"임을 명시했다.
- **Next Tasks 재정렬**: 오늘 끝난 항목을 빼고, 화면 육안 확인 2건(관측 화면 + 비동기 패드)을
  "스택을 올리면 바로 할 것"으로 묶었다. 둘 다 **로직은 회귀로 잠겨 있고 렌더만 미검증**이다.
- **CHANGELOG**: 단계 전환을 페이즈 단위 한 행으로 반영(결정 근거·개정 4곳·D1~D8 추천안 포함).

**오늘 남긴 상태**: 코드·회귀는 backend **1556/612**, frontend **207/14**로 커밋돼 있고, 다음 작업의
갈림길은 **D1~D8 승인**과 **dogfood 착수(★)** 두 개다. 어느 쪽을 고르든 브리프와 HANDOFF가
착수 지점을 갖고 있다.
