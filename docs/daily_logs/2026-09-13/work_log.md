...
## 세션 86 — 스크린샷·GIF 전면 재촬영: 한글 두부 해소 + 프런트 재빌드 (오너 지시 "한글폰트 컨테이너 안에 다시 깔고 스크린샷과 gif 수정해야겠는데?")

### Goals

- 오너가 README 에서 발견한 것: **세션 84 촬영본의 한글이 전부 두부(□)** 다. 원인을 잡고 전량 재촬영한다.
- `/mnt/f/devel/entangelk/docs/assets`(포트폴리오 사이트)에도 최신 자산을 옮긴다(GIF 포함).

### 1. 두부의 원인 — 컨테이너가 아니라 **호스트**다

오너 지시는 *"컨테이너 안에 폰트를 깔자"* 였는데, 실측은 다른 곳을 가리켰다: 촬영은 컨테이너가 아니라 **WSL 호스트의 Playwright**(`~/.cache/ms-playwright`)로 한다. 그리고 **앱은 웹폰트를 한 줄도 싣지 않는다** — `frontend/index.html` 에 폰트 링크가 없고 `styles.css` 가 시스템 폰트 스택만 선언한다(`:4` UI 고딕 · `:1808` 원고 명조). 즉 **브라우저가 도는 곳**에 폰트가 있어야 하는데 호스트의 `fc-list :lang=ko` 가 **0** 이었다.

처방은 `~/.config/fontconfig/fonts.conf` 하나(저장소 밖 · sudo 불요). 여기서 **함정 셋을 실측으로 걸렀다** — 이것을 안 걸렀으면 두부만 면하고 다른 방식으로 틀린 그림을 얻었다.

| 함정 | 증상 | 처방 |
|---|---|---|
| `batang.ttc` 가 `<textarea>` 안에서 한글 글리프를 **통째로 떨군다**(`<div>` 는 정상) | 원고 본문이 문장부호만 남고 투명해진다 — 폰트 문제로 안 보이는 모양이라 오진하기 쉽다 | 그 파일을 노출하지 않는다 |
| 윈도 폰트 디렉터리를 통째로 물리면 Chromium 폴백이 **Malgun·Gulim 고딕**으로 튄다 | Chromium 의 글리프 폴백은 `fc-match` 가 아니라 자체 정렬 목록을 쓴다 — fontconfig alias 로 안 잡힌다(실측 반복 확인) | **필요한 것만** 노출해 후보를 결정론으로 만든다 |
| 윈도의 `Noto*-VF.ttf`(가변 폰트) | Sans 는 기본 인스턴스가 **Thin** 으로 잡혀 UI 가 얇아지고, Serif 는 **라틴만 세리프**고 한글은 고딕으로 떨어진다(CDP `getPlatformFontsForNode` 로 확정) | **정적 OTF** 로 교체(Noto Sans/Serif KR Regular·Medium·Bold) |

추가로 원고 스택의 4순위 **`AppleMyungjo`(이 머신에 없음)가 기본 대체로 고딕**이 되어 Chromium 이 거기서 멈추고 뒤의 `"Noto Serif KR"` 까지 가지 못했다 — 두 이름 모두 한국어 명조이므로 명조로 못박았다. 결과: 원고 본문이 **명조**로, UI 가 **Noto Sans KR** 로 나온다. 후자는 오너의 윈도 브라우저가 해석하는 것과 **같은 폰트**다(Inter·Pretendard 는 이 머신 어디에도 없어 스택 3순위가 첫 가용이다).

**남은 흠 하나(정직하게 기록)**: 제목(h1)의 한글은 `serif` 스택인데도 Noto Sans KR 로 떨어진다 — Chromium 의 스크립트별 기본 폰트 결정이라 fontconfig 로 못 돌렸다(alias·prefer·reject 다 시도, CDP 로 매번 재측정). 윈도에서는 바탕(명조)이다. 원고 본문은 명조로 잡히므로 **가장 중요한 표면은 의도대로**다.

### 2. ★ 폰트보다 큰 결함 — **찍힌 UI 자체가 3주 전이었다**

폰트를 고치고 재촬영하다 검토함 목록에 후보 payload 가 안 보이는 것을 발견했다(SoT v1.8.34 *"기본은 펼침"* 계약 위반으로 보였다). 원인은 코드가 아니었다 — **`frontend` 이미지가 2026-08-22 빌드**였다. 세션 84 는 `application` 이미지만 재빌드하고(`ai_writte_system-app`, 09-13 11:07) 프런트를 그대로 뒀다. 그 사이 `frontend/` 에 **커밋 102개**가 쌓여 있었다: writing studio W1~W4 · 루트 랜딩 페이지 · 가입 동의 게이트 · 관리자 잔여 정리 화면 · 약관 v1.1 패널, 그리고 얄궂게도 **`1af6143 원고 한글 명조 폴백`** 까지.

즉 세션 84 의 *"최신 UI로 재촬영"* 서술은 **거짓이었다**. 오너에게 선택지를 올려 **재빌드 승인**을 받고(`docker compose build frontend` → `up -d`) 전량 다시 찍었다.

**드러난 화면들** — 정체성 그룹이 보이는 검토함(*"인물 후보 9건이 같은 대상으로 묶였습니다"* + 근거 + 그룹 승인/거절), 우측 집필 도구 서랍(이어쓰기·분석·검토·메모), 익명 루트의 랜딩 페이지. 전부 세션 84 촬영본에는 **없던 화면**이다.

### 3. 산출

- **스크린샷 8장**(랜딩 신규) — `normalize.py` 규칙 그대로 3:2·1200px.
- 세션 84 의 **중복도 바로잡았다**: `기억후보목록_3x2.png` 와 `기억후보검토_3x2.png` 가 **둘 다 상세 화면**이었다(README "목록" 칸에 목록이 아닌 그림). 이제 목록 칸은 그룹이 보이는 실제 목록이다.
- README 이미지 절 재구성(4행 × 2칸) + 각주 정정.

### 4. 발견 — 데모 스택의 외부 모델이 느리다(기록만)

이어쓰기 생성 GIF 를 찍다 **동기 경로가 504** 로 끝났다(`POST /writing/generate`). 게이트웨이 직접 탐침 3회는 전부 200 이지만 **호출 1건에 34~50초**다(`gemma-4-31b-it`, Gemini OpenAI 호환). 쓰기 파이프라인은 계획·생성·자기보고로 최소 3콜이라 합이 앱 동기 상한(`LONGEST_SYNCHRONOUS_SECONDS = 120`)을 넘는다. 설계 근거였던 실측치는 *"short 23초 · long 91초"*(`quota/lock.py:9`)인데 **지금 환경은 그 전제를 벗어나 있다**. 비동기 경로(medium → 202 + 잡)로 우회했고, 첫 시도는 워커에서 `provider_error` 로 실패했다(재시도 기록은 아래 Verification).

**이 관측 자체가 세션 84 의 nginx 120s 기록과 같은 뿌리**다 — 그쪽은 분석 run, 이쪽은 생성이고, 원인은 프록시가 아니라 **모델 지연**일 가능성이 높다. 상한 재측정이 필요한지는 오너 판단 사안이라 등재만 한다.

### 5. GIF 2종 — 비동기 경로로 바꿔 다시 찍었다

- **이어쓰기 생성**: 동기 504 를 만난 뒤 **비동기 경로(medium)** 로 전환했다. 첫 시도는 워커에서 `provider_error`(잡 `wgj:d64dbdb8…` failed), **재시도가 성공**(`wgj:e692acfd…` succeeded)해 후보가 결과 패드에 도착했다 — 지시문("다음 날 아침, 아린이 경비대청으로 가 헤일라 문즈와 대면하는 장면")에 대해 *"다음 날 아침의 빛은 창백했다. 아린은 여관을 나서 골목 하나를 건넜다…"*. 화면에 `백그라운드 생성 1건 진행 중` → 결과 패드가 그대로 잡혀 **architecture.md §5-3 이 글로 설명한 경로가 그림이 됐다**(README 캡션에서 그 절로 링크).
- **기억 후보 검토**: 목록(정체성 그룹) → 상세 → 원문 근거 → 승인. LLM 무관이라 한 번에 찍혔다.
- 조립은 저장소 관례 그대로(`assemble_gif.py` — mtime 간격으로 대기 프레임 식별 후 5장마다 1장, ffmpeg palette). 폭 720 으로 낮춰 427KB·186KB.
- 후보 도착 뒤 프레임이 스크롤에 잘려 끝나서 **꼬리만 따로 이어 찍었다**(후보 본문이 보이는 상태로 마무리).

### 6. 포트폴리오 사이트 반영 (`/mnt/f/devel/entangelk`)

오너 지시로 사이트 자산도 최신화했다 — 종전 `aiw-editor.png`·`aiw-candidate-review.png` 는 **2026-09-01 촬영본**이고 내용이 *"영희와 철수의 이야기"* 같은 시험 데이터였다.

- `assets/aiw-editor.png` ← 글편집페이지_3x2 · `assets/aiw-candidate-review.png` ← 기억후보검토_3x2
- **GIF 신규 2종** `assets/aiw-generation.gif` · `assets/aiw-review.gif`
- `docs/index.html` 의 AI Writer 카드에 GIF 2칸(`grid2`)을 기존 스틸 아래 추가, 캡션은 그 파일의 영/한 병기 관례 그대로.
- **커밋하지 않았다** — 다른 저장소이고 커밋 권한은 오너의 것이다(작업 트리에 변경만 남겨 둠).

### Verification

- 문서 가드 둘 **29 passed / 962 subtests** — 세션 85 와 동일(이미지·GIF 추가는 이 가드들의 subtest 수를 안 움직인다. 예측이 아니라 양단 실측이고, 그래서 **기준선 3075/1/4239 는 유지**된다 — 백엔드 소스 무변이므로 전수 재측정 조건도 아니다).
- README 신규 링크·앵커 전건 해석 — 이미지 8장·GIF 2종·`architecture.md#5-3` 앵커 포함.
- 촬영본은 **한 장씩 눈으로 확인**했다(대조 시트 + 원고 본문 확대 크롭). 폰트 판정은 눈이 아니라 CDP `CSS.getPlatformFontsForNode` 로 재확인했다 — 가변 폰트가 Thin 으로 잡히던 것을 그렇게 잡아냈다.
- 코드 무변 · 프런트 소스 무접촉(재빌드는 이미지 생성이지 소스 변경이 아니다).

### Next steps

- **오너 확인 요청**: 포트폴리오 사이트 `docs/index.html` 의 **낡은 수치** — *"96 decision briefs · 269 verification records · 2,316 passing tests"* · *"8개 호출부"* · SoT `v1.8.14`. 현재는 **118 · 315 · 3,075 · 9 · v1.8.68** 이다. 사이트 본문은 오너의 글이라 손대지 않았다.
- 촬영 환경 재현 메모: `~/.config/fontconfig/fonts.conf` + `~/.local/share/fonts/eright-shot`(정적 Noto Sans/Serif KR + 윈도 Georgia 심볼릭 링크). **저장소 밖**이라 다른 머신에서 찍으려면 다시 만들어야 한다 — 그 사실과 함정 셋을 이 로그가 정본으로 남긴다.
- 제목(h1) 한글이 명조가 아니라 고딕으로 떨어지는 잔여 흠(§1 마지막 문단) — 촬영 환경 한계이지 제품 결함이 아니다.
