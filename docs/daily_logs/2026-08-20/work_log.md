...
- **다음은 dogfood(GATE-1) 착수다.** 오늘로 **외부 API 확장 세 축이 전부 코드로 섰고**,
  **오너 결정 대기 브리프도 dogfood 하나**만 남았다.
- **★ dogfood 가 두 가지의 트리거를 동시에 당긴다**: 리랭커 **품질 판정**(결정 4-③ — 실사용
  질의에 **오너가** 정답을 붙인 뒤 하네스로) · 임베딩 **배치**(재색인 지연·호출 수 실측).
- **★ 그 둘보다 먼저 오는 것이 외부 키다.** 리랭커는 기본이 꺼짐이라 키 전에는 **no-op 대
  no-op 비교**이고, 재색인 실측도 키가 있어야 한다. **순서: 키 조달 → dogfood 질의·정답
  축적 → 판정.**
- **남은 유예(전부 트리거 있음)**: self-host 리랭커(어댑터 하나 · 붙는 날 profile 뒤 · **모델
  크기·CPU 지연 미측정**) · top-k(지연 실측) · candidate 쪽 리랭킹 제외 여부(조립 한 줄) ·
  차원까지 바뀔 때의 컬렉션 재생성 절차.
- **미검증 = 3커밋**(`a9bca6d` · `7a88ac1` · `f14917b`). 리랭커에서 볼 만한 축 넷은 HANDOFF
  마감 메모에 적었다 — 특히 **② fail-open 이 정말 모든 실패를 덮는가**(지금 `text_of` 가
  던지면 데코레이터가 안 잡는다)가 내가 스스로 의심하는 자리다.
