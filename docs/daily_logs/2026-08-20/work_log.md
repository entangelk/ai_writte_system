...
### Task 17 — 임베딩 슬라이스 검증 조건 폐쇄 (B1) + 비차단 셋

[검증](../../verifications/2026-08-20/embedding_adapter_slice.md) 판정은 **조건부 합격**.
축 넷 중 셋(② 헬퍼 범위 · ③ 경로 안 `v1` 무영향 · ④ 기본값 무영향)은 참으로 확인됐고,
**①이 실존**이었다 — `import RemoteEmbeddingProvider as REP` 뒤의 `REP(…)` 를 **가드가
침묵으로 통과**시킨다.

**★ 지적의 핵심은 또 문언이었다.** 브리프·셀 docstring·HANDOFF 셋 다 *"생성자를 직접 부르는
자리가 **하나라도** 있으면 실패"* 라고 단정하는데, 별칭 호출은 **그 "하나" 이면서 실패하지
않았다.** 검증자 표현대로 **계약 문언이 잠금보다 넓다** — 오늘 아침 mypy 가드의 B1(정본
산출물 문언)·재검 H4(셀 실패 메시지)와 **정확히 같은 모양이고 이번이 세 번째다.**

**폐쇄안 (i)(가드 강화)를 골랐다.** (ii) 문언 축소는 기록을 참으로 만들지만 **계약을 약해진
채 합의**시킨다. **세 번 다 같은 갈래가 나왔고 세 번 다 "검사를 넓힌다" 를 골랐다** — 이제
이건 이 저장소의 규칙으로 굳었다고 본다.

- *구현*: 파일마다 `ImportFrom` 의 `asname` 을 모아 원이름 집합에 더한다. **별칭은 파일
  스코프**라 맵을 파일마다 다시 만든다.
- *잔여 명시*: 할당 별칭(`P = RemoteEmbeddingProvider; P(…)`)은 **안 잡는다.** 이름 재결합
  추적은 타입체커의 일이다. **셀 docstring 이 "잡는 형태 셋 + 잔여 하나" 로 다시 쓰였고,
  "하나라도" 라는 문언이 사라진 것이 이 폐쇄의 절반이다.**
- *확인*: 검증자 재현 스크립트를 **그대로** 돌렸다 — **V1 이 `16 passed` → 가드 실패로
  뒤집혔다**(예고된 그 뒤집힘). V1b(모듈 속성)는 여전히 잡고, V1c 는 문언대로 잔여다.

**비차단 셋도 함께 닫았다** (뮤테이션 페어링)

| # | 뮤테이션 | 재실패한 셀 |
|---|---|---|
| V2 | 새 최상위 파이썬 디렉터리(`tools/`) 등장 | `test_the_scan_covers_every_top_level_code_directory`(H2) |
| V3 | `_SCANNED` 에 없는 디렉터리 이름(반대쪽) | `test_the_scanned_directories_still_exist` subtest |
| V4 | `EMBEDDING_API_FORMAT` 을 **대시**로 | 표기 셀 subtest(H1) |
| V5 | `EMBEDDING_API_MODEL` 을 **콜론**으로 | 〃 |

- **H2 는 "세 번째 compose 파일" 과 같은 계열**이라 경로 하드코딩을 **분류 강요**로 바꿨다 —
  스캔하거나 이유와 함께 범위 밖으로 적거나 둘 중 하나다(`test_compose_exposure` 가 새
  서비스에 쓰는 방식). **반대쪽도 잠갔다**: 목록만 남고 디렉터리가 사라지면 스캔이 0파일이
  되고 위 셀이 조용히 통과한다.
- **H1 은 지금 행동이 같은 자리다**(MODEL/KEY 기본값이 비어 대시·콜론이 중립). 그래도
  잠근 이유는 **다음 사람이 기본값을 넣는 순간 콜론이 "빈 값으로 끄기" 를 조용히 없애기**
  때문이다.

**★ 관측 항목이 가장 조용한 결함이었다.** `if __name__` 블록이 `WireFormatSelectionTest`
**앞에** 있어서, 직접 실행하면 그 **7셀이 아예 수집되지 않았다.** pytest 로는 돌기 때문에
**전수도 CI 도 아무 말을 안 한다** — 오늘 만난 *"green 이 말하지 않은 것"* 의 **네 번째
얼굴**이다(argon2 33건 · mongo 11셀 skip · mypy 88 vs 111 에 이어). 파일 끝으로 옮겨
`PYTHONPATH=. python3 tests/test_embedding_assembly.py` 가 **11 → 18 tests**.

### Task 18 — 리랭커 슬라이스: seam + 외부 어댑터 (결정 2=A)

**로컬 기본이 no-op 이라는 것이 이 결정의 실체다.** `build_rerank_provider_from_env()` 가
주소 없으면 **`None`** 을 준다 — 임베딩 헬퍼가 fake 로 내려가는 것과 **의도적으로 다르다**:
리랭킹에는 *"가짜 재정렬"* 이라는 쓸모 있는 대체물이 없고, **무작위로 섞는 것은 no-op 보다
나쁘다.**

**★ wire 를 정하다 정본과 현실이 어긋나는 것을 발견했다.** D2=A 의 문언은 *"generic
**OpenAI-호환** 어댑터"* 인데 **OpenAI 에는 rerank 엔드포인트가 없다.** D2 가 겨눈 성질은
*"벤더 전용이 아니라 여럿이 공유하는 형식"* 이고, 리랭킹에서 그 자리를 차지한 것은
Cohere 가 낸 `POST /v1/rerank`(Jina·Voyage·TEI·infinity 가 같은 모양)다.
**문언의 글자를 그대로 따랐으면 존재하지 않는 계약을 구현할 뻔했다** — 성질을 따르고
그 판단을 브리프에 적었다.

### Task 19 — 데코레이터 조립 (결정 3=A) + 조립 가드

두 융합 자리의 seam 이 **완전히 같아서**(`retrieve(*, project_id, query, limit)`) 한
데코레이터가 둘 다 덮는다. 항목 타입만 달라 **텍스트 투영을 주입**한다 — **색인이 쓴 것과
같은 투영**(`derive_memory_index_text` / `candidate_index_text`)이라야 리랭커가 색인이 본
것과 같은 글을 읽는다.

- **도메인 코드 0줄.** `_rerank_wrapped` 하나를 두 조립 함수가 부른다.
- **★ 조립 가드가 이 슬라이스에서 임베딩보다 더 필요하다.** `ObservedProvider` 사고는
  *"계측이 사라지면 대시보드가 빈다"* 였는데, **재정렬이 사라지면 아무것도 안 비고 순위만
  예전으로 돌아간다.** 더 조용하다.
- **★ 구현 중 mypy 가드가 실제로 일했다.** 조립에 텍스트 투영을 넣으면서 import 를
  빠뜨렸는데, **`python3 -c "import …main"` 은 통과했다**(참조가 함수 본문 안이라 실행되지
  않는다). `python3 -m mypy` 가 `Name "derive_memory_index_text" is not defined` 로 잡았다 —
  오늘 오전 세운 가드가 오후에 자기 값을 증명한 셈이다.

### Task 20 — 평가 하네스 선작성 (결정 4-②)

[`scripts/eval_retrieval_ranking.py`](../../../scripts/eval_retrieval_ranking.py) — recall@k ·
MRR · nDCG@k.

- **★ 리랭커 전용으로 굳지 않게 포맷을 중립으로 잡았다.** 입력이 *"리랭킹 전/후"* 가 아니라
  **"구성 이름 → 순위 목록"** 이라 **임베딩 모델 교체·RRF `k` 조정·top-k 변경** 비교에도
  그대로 쓰인다(브리프 §후속 고려가 열어 두라고 한 문). **일반화 코드를 미리 쓰지는
  않았다** — 포맷만 중립이다.
- **★ 정답을 채우지 않았다.** 브리프 결정 4-② 의 경계 그대로이고, **그 경계를 셀로 잠갔다** —
  저장소에 `.jsonl` 이 들어오면 실패한다. 하네스가 있다는 사실은 *"평가했다"* 가 아니다.
- *설계 선택 셋*: **MRR 은 k 로 안 자른다**(자르면 3위와 30위가 같아진다) · **nDCG 에 등급을
  안 둔다**(정답을 붙이는 사람이 오너이고 이분법이 훨씬 붙이기 쉽다) · **어떤 구성이 일부
  질의에만 있으면 거부한다**(서로 다른 표본의 평균을 비교하면 숫자는 나오는데 뜻이 없다).

### Task 21 — 뮤테이션 (리랭커 축 7종)

**커밋 후 clean 트리 분기**, 매 회 `git status --short` 공백 확인.

| # | 뮤테이션 | 재실패한 셀 |
|---|---|---|
| R1 | **정본 조립**에서 감싸기 누락(ObservedProvider 사고 형태) | `test_both_assembly_sites_go_through_the_wrapper` subtest |
| R2 | **candidate 조립**에서 감싸기 누락 | 〃(다른 subtest) |
| R3b | fail-open → fail-closed | `test_a_provider_error_returns_the_original_order` |
| R4 | 순열 검사 제거(부분 응답이 결과를 줄이는 방향) | `test_a_response_that_is_not_a_permutation_…` subtest 다수 |
| R5 | 0·1개도 provider 호출(불필요한 요금) | `test_nothing_to_reorder_never_reaches_the_provider` subtest 2 |
| R6b | 동률에서 인덱스 역순(같은 입력의 결과가 뒤집힘) | `test_ties_keep_the_request_order` |
| R7 | bool 인덱스 거부 제거(`True` 가 인덱스 1 로) | `test_malformed_bodies_…` subtest |

- **★ 내 뮤테이션이 두 번 틀렸고 둘 다 다시 했다.** R3 초판은 `except` 절을 통째로 지워
  **`try` 만 남은 문법 오류**를 만들었다(pytest 가 수집 단계에서 죽어 "물지 않았다" 처럼
  보였다). R6 초판은 `(score, -index)` 역정렬이라 **행동상 no-op** 이었다. **뮤테이션이 안
  물면 가드를 의심하기 전에 뮤테이션이 실제로 행동을 바꿨는지부터 본다** — 저장소 규율
  [`verification.md`](../../guides/verification.md) §"When a mutation does not bite" 와 같은 축이다.

### Task 22 — 리랭커 검증 조건 폐쇄 (C1) + 비차단 둘

[검증](../../verifications/2026-08-20/reranker_slice.md) 판정은 **조건부 합격**.
R1~R7·산출물 4건·표기 3분할·전수는 전부 재현됐고, **조건 C1 은 내가 인계 때 스스로
넘긴 축이었다** — *"fail-open 이 정말 모든 실패를 덮는가(`text_of` 가 던지면 데코레이터가
안 잡는다)"*. 검증자가 그것을 **런타임으로 실증**했다. **의심을 넘기는 것이 실제로
작동한 사례로 남긴다.**

**폐쇄안 (i) 단계 전체 fail-open 을 골랐다.** (ii) 문언 스코프 축소(*"모든 실패"* →
*"프로바이더 실패"*)는 기록을 참으로 만들지만 **계약을 약해진 채 합의**시킨다 —
**오늘 같은 갈래가 네 번째였고 네 번 다 검사를 넓혔다.**

- *구현*: 투영·프로바이더 호출·순열 검사를 **한 `try` 안**에 두고 `except Exception`
  (`# noqa: BLE001`)으로 받는다. 순열 위반도 `RerankProviderError` 로 올려 **한 곳에서**
  떨어지게 했다.
- **★ 조용히 삼키지 않는다** — `_log.warning(exc_info=True)`. fail-open 이 조용하면
  **리랭킹이 영원히 no-op 인 채로 아무도 모른다.** 선례는
  [`activity/log.py`](../../../services/application/app/activity/log.py) 의 A4=A 격리 경계이고,
  **오늘 네 번 만난 "green 이 말하지 않은 것" 을 스스로 만들지 않으려고 로그를 붙였다.**
- *확인*: 검증자 스크립트 그대로 — **Part 1 이 "예외 새어나감" → `결과: 정상 반환`
  으로 뒤집혔다.** Part 2 의 RV-B1·RV-B2(모듈 속성·별칭)도 **둘 다 이제 문다.**

**★ R3b 는 "안 문 것" 이 아니라 뮤테이션이 낡은 것이었다.** 스크립트가
`except RerankProviderError:` 리터럴을 찾는데 폐쇄가 그 줄을 바꿔 `count=0` 으로 걸렸다.
같은 의도를 **새 형태로 직접 돌려**(C1-M1) 확인했고, **스크립트 갱신 필요를 기록에
남겼다.** — 이것도 오늘 두 번 겪은 *"뮤테이션이 안 물면 뮤테이션부터 의심한다"* 의
세 번째 사례다(내 R3·R6 초판, 검증자 R6b 첫 시도, 그리고 이것).

**폐쇄 자체의 양방향 뮤테이션 셋**

| # | 뮤테이션 | 재실패한 셀 |
|---|---|---|
| C1-M1 | 경계를 다시 `except RerankProviderError` 로 | `FailOpenScopeTest` 3셀 + subtest |
| C1-M2 | 경고 로그 제거(**조용한** fail-open) | `test_the_fallback_is_logged_rather_than_swallowed` |
| C1-M3 | 정상 경로에서도 경고(로그가 소음) | `test_a_healthy_reranking_logs_nothing` |

**H1 — 같은 날 배운 것을 옆 가드에 안 옮긴 것이 그 자체로 교훈이다.** 임베딩 B1 폐쇄에서
`asname` 인식을 넣어 놓고, **몇 시간 뒤 같은 형태의 리랭커 가드를 쓰면서 원이름만
비교했다.** 규칙: **한 곳에서 배운 것은 같은 형태의 다른 가드에도 옮긴다.** 잔여(할당
별칭)는 임베딩 가드와 같게 문언에 명시했다.

**H2** — 동률 주석 *"요청 순서 유지"* 를 **"응답에 담긴 순서 유지"** 로 정정했다. 안정
정렬이 보존하는 것은 응답 순서이고, 잠그는 성질은 **"같은 응답이 언제나 같은 순서를
낸다"** 이다.

### Task 23 — 마감 점검: 인계 문서의 주장을 하나씩 실측으로 대조

**문서를 다 쓴 뒤 "적힌 것이 참인가" 를 따로 한 번 돌았다.** 오늘 하루의 주제가
*"문언이 실제보다 넓다"* 였으므로 **인계 문서 자신에게 같은 검사를 적용**한 것이다.
**두 개가 틀려 있었고 둘 다 고쳤다.**

| 주장 | 판정 | 근거 |
|---|---|---|
| **미검증 = 1커밋** | **★ 틀림 → 2커밋** | `git log 5d3163a..HEAD` = `92b9b24`(코드) + `90164df`(기록). **기록 커밋을 빼먹었다** |
| **오너 결정 대기 브리프가 dogfood 하나** | **★ 틀림** | `docs/plans/` 의 대기 브리프는 **0건**(오늘 셋 다 확정)이고, dogfood 는 **브리프 없이** §Owner Decisions Needed 에 있다. 게다가 **그 절엔 다른 대기 항목이 여럿**이다 |
| 리랭커 기본 꺼짐 | 참 | `build_rerank_provider_from_env()` → `None` (실행) |
| 임베딩 기본 native | 참 | 주소만 있고 FORMAT 없으면 `RemoteEmbeddingProvider` (실행) |
| mypy 미설치 시 실패 | 참 | 빈 venv 에서 `python -m mypy --version` 실패 확인 |
| 정상 skip = live Chroma 1건 | 참 | 오늘 전수 `-rs` 출력 |
| 리랭커 "있음" 거짓 표기 없음 | 참 | README 에 리랭커 언급 **0건** — 없는 것을 안 적었으니 거짓이 아니다 |

**★ "미검증 = 1커밋" 은 이 저장소가 08-13·08-14·08-15 에 이미 세 번 틀린 항목이다.**
함정 문구까지 있는데(*"인계 문구에서 베끼지 말고 git 에서 다시 유도할 것"*) **오늘 내가
네 번째로 틀렸다.** 원인은 베끼기가 아니라 **기록 커밋을 세는 단위에서 뺀 것**이었다 —
이 저장소의 검증 기록은 `대상 커밋` 에 **기록 커밋도 명시해 덮으므로**, 안 덮인 기록
커밋은 미검증이다. 그 사실을 HANDOFF ③ 항목에 박았다.

**★ 그리고 정정 자체가 곧바로 또 낡았다.** *"2커밋"* 으로 고쳐 커밋하자 **그 정정 커밋이
3번째 미검증**이 됐다. **개수는 구조적으로 낡는다** — 그래서 최종 형태를 숫자가 아니라
**"코드 1커밋 `92b9b24` + 그 기록 계열 · 목록은 `git log 5d3163a..HEAD` 로 유도"** 로 바꿨다.
**기존 §함정 문구를 새로 쌓지 않고 갱신했다**(records-and-handoff 규칙: 새 메모가 옛것을
대체하면 절을 다시 쓴다). **저장소가 이 계열로 다섯 번 틀린 뒤에야 "확인법" 이 아니라
"적는 형태" 를 바꿨다** — 앞의 넷은 전부 확인법을 고쳤는데 같은 병이 또 났다.

**★ 두 오류 다 "코드가 아니라 인계 문서" 에 있었다.** 오늘 세운 가드는 어느 것도 이걸
안 잡는다 — 문서의 주장은 테스트가 없다. **그래서 마감 점검이 필요했고, 다음 사람도
같은 것을 해야 한다.**
...
- **★ 재발했다(2026-08-20 두 번째).** 이번엔 순서를 지켰는데도 틀렸다 — 커밋한 **뒤** 해시를
  적었지만, 그 사이에 **커밋 메시지를 `--amend` 로 고치면서 해시가 바뀌었다**(`7af3a87` →
  `a9bca6d`). 기록 4파일 6곳이 **dangling 커밋**을 가리켰다(`git branch --contains` 가 비어
  있다 — 지금 로컬에서는 `git show` 가 되지만 clone 에서는 안 되고 gc 뒤에는 사라진다).
- *원인*: `--amend` 는 **새 커밋을 만든다.** "커밋 뒤에 적는다" 로는 부족하다.
- *규칙 갱신*: **해시는 그 커밋이 최종이 된 뒤에 적고, 적은 뒤 `git log --oneline -1` 로
  대조한다.** amend·rebase 가 끼면 그 대조가 유일한 방어다. **잘못된 해시는 여전히
  조용하다** — 어떤 가드도 물지 않는다.
- *부수 사고 하나 더*: amend 를 하게 된 이유 자체가 실수였다 — `git commit -m "…"` 의 큰따옴표
  안에 **백틱**을 넣어 `` `as REP` `` 가 **명령 치환으로 실행**됐고(`Assembler messages: can't
  open REP`), 메시지 한 곳이 빈 채로 커밋됐다. **커밋 메시지에 백틱을 쓸 때는 heredoc(`-F -`)을
  쓴다** — 이 세션의 다른 커밋들은 전부 그렇게 했고 그 하나만 `-m` 이었다.

**I-8. 셀 파일에 `if __name__` 블록을 두고 그 뒤에 클래스를 덧붙이면 조용히 사라진다.**

- *문제*: `WireFormatSelectionTest` 7셀이 직접 실행에서 수집되지 않았다.
- *원인*: 나는 그 클래스를 **파일 끝에 append** 했는데, 파일 끝에 이미 `if __name__` 블록이
  있었다. 붙이는 순간 그 블록이 파일 **중간**이 됐다.
- *처리*: 블록을 다시 끝으로 옮겼다.
- *결과*: **pytest 는 영향을 안 받으므로 어떤 가드도 이것을 말하지 않는다.** 규칙:
  **셀을 파일 끝에 붙일 때는 `if __name__` 블록의 위치를 함께 본다.**

**I-9. ★ 인계 문서에 적은 "내가 의심하는 자리" 가 조건이 됐다.**

- 리랭커 슬라이스를 넘기며 **볼 만한 축 넷** 중 ②에 *"fail-open 이 정말 모든 실패를
  덮는가 — 지금 `text_of` 가 던지면 데코레이터가 안 잡는다"* 를 적었고, 검증이 그것을
  런타임으로 실증해 **조건 C1** 이 됐다.
- *왜 기록하는가*: 구현자가 **확신하지 못하는 자리를 그대로 넘기는 것**이 검증을 빠르게
  만든다. 감추면 검증자가 처음부터 찾아야 하고, 못 찾으면 그대로 남는다.
- *규칙*: **미검증 인계에는 "내가 안 잡았을 수 있는 자리" 를 함께 적는다.** 이 저장소가
  이미 쓰던 "볼 만한 축" 칸의 용도가 그것이다 — **자랑이 아니라 의심을 적는 칸이다.**
...
### 전수 회귀 — 검증 조건 폐쇄 뒤 (베타, 2026-08-20)

**`2322 passed · 1 skipped · 2556 subtests` (1342초).** skip 1 = live Chroma.

- **검산 기준은 `2319/1/2545`** — 구현 직후 내 실측 2544 에 **검증 기록 등재분 subtest +1**.
- **셀 +3**(범위 분류 2 · 표기 1) · **subtest +11**(분류 2 + 표기 **9 = 서비스 3 × 변수 3**).
  `2319+3 = 2322` · `2545+11 = 2556` 으로 **양쪽 다 맞는다.**
- mongo 는 `--port 27020` 으로 primary 확인 후 돌렸다.

### 전수 회귀 — 리랭커 슬라이스 뒤 (베타, 2026-08-20)

**`2350 passed · 1 skipped · 2582 subtests` (1182초).** skip 1 = live Chroma.

- **셀 +28**(리랭커 18 · 하네스 10) · **subtest +26**. `2322+28 = 2350` ·
  `2556+26 = 2582` 로 **양쪽 다 맞는다.**
- mongo 는 `--port 27020` 으로 primary 확인 후 돌렸다(오늘 등재한 함정).

### 전수 회귀 — 리랭커 조건 폐쇄 뒤 (베타, 2026-08-20 · 오늘 마지막)

**`2354 passed · 1 skipped · 2586 subtests` (1140초).** skip 1 = live Chroma.

- **검산 기준 `2350/1/2583`**(슬라이스 직후 2582 + 검증 기록 등재분 1)에 **셀 +4 ·
  subtest +3**(`FailOpenScopeTest`). `2350+4 = 2354` · `2583+3 = 2586` — **양쪽 다 맞는다.**
- **하루 전체로는 `2287 collected`(오전 착수 전) → `2354/1/2586`.** 셀 **+67** 이고 전부
  오늘 만든 가드다: mypy 8 · 임베딩 조립 18 + provider 6 + 범위/표기 3 · 리랭커 22 ·
  하네스 10.
...
**★ 오늘은 여기서 마감한다(오너).** 다음 작업자가 바로 이어받을 수 있게
[HANDOFF §다음 작업자가 읽어야 할 것](../../../HANDOFF.md) 블록을 마감 메모 맨 앞에
세웠다 — **그 블록만 읽어도 시작할 수 있다.**

- **다음 작업 = dogfood(GATE-1) 착수.** **기술적 선행 조건이 없다.** `docs/plans/` 의 **대기 브리프는 0건**
  이고(오늘 셋을 다 확정·구현했다), dogfood 는 브리프 없이 HANDOFF §Owner Decisions Needed 에
  있는 항목이다. **그 절에는 다른 대기 항목도 여럿 있으므로** *"대기가 하나"* 가 아니라
  **"다음으로 dogfood 를 권한다"** 가 정확한 서술이다.
- **★ 그런데 dogfood 앞에 외부 키가 있다.** 오늘 만든 것 둘이 dogfood 로 트리거가
  당겨지는데 **둘 다 키가 먼저**다 — 리랭커 품질 판정(기본이 꺼짐이라 키 전에는
  **no-op 대 no-op**)과 임베딩 배치(트리거가 재색인 실측). **순서: 키 조달 → dogfood
  질의·정답 축적 → 판정.**
- **미검증 = 1커밋**(`92b9b24`). 볼 만한 축 셋: ① `except Exception` 경계가 **너무 넓어
  프로그래밍 오류까지 삼키지 않는가**(로그는 남지만 테스트는 초록이다) ② `assertNoLogs`
  셀이 다른 로거의 경고까지 막지는 않는가 ③ **재현 스크립트 R3b 블록의 리터럴이
  낡았다** — 다음 검증자가 갱신한다.
- **남은 유예(전부 트리거 있음)**: self-host 리랭커(모델 크기·CPU 지연 **미측정**) ·
  top-k(지연 실측) · candidate 쪽 리랭킹 제외 여부(조립 한 줄) · 차원까지 바뀔 때의
  컬렉션 재생성 절차 · 배치(`embed_many`).
