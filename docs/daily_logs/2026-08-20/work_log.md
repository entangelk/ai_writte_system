...
- **다음은 리랭커 슬라이스다.** [브리프](../../plans/reranker-slice-decisions.md) Resolved(08-18) ·
  1=A 의 선행 조건(임베딩 어댑터)이 오늘 충족됐다. **검증자 추천대로 B1 폐쇄를 먼저 했다** —
  리랭커 3=A 가 *"데코레이터 + 조립 가드"* 로 **지금 가드의 형태를 그대로 물려받기** 때문이고,
  이제 물려받을 형태가 **별칭까지 잠그고 범위 분류를 강요하는 것**이다.
- **임베딩 축에서 열린 채 남은 것 둘** — 배치(`embed_many`, 트리거 = 재색인 실측 · **외부 키
  필요**) · 차원까지 바뀔 때의 컬렉션 재생성 절차(트리거 = 차원 다른 모델 조달).
- **오너 결정 대기 브리프는 dogfood 하나뿐이다.**
- **미검증 = 1커밋**(`a9bca6d`). 볼 만한 축 둘: ① 별칭 인식이 `ImportFrom` 만 본다 — 다른 `as`
  형태가 남아 있는가 ② `_OUT_OF_SCOPE` 네 항목이 과소·과대하지 않은가.
