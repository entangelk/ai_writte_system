...
**재검 6종** (`4bd1a3d` 커밋 후 트리 clean 분기, 매 회 `git status --short` 공백 확인)
...
- **다음 전수 기대값 `2297 / 1 / 2520`** — 검증 기록 1건이 `test_docs_indexes` 판정 열 전수 셀을
  **subtest +1**(코드 무관 자리), 조건 폐쇄가 **셀 +1**(`test_the_config_cannot_be_quietly_weakened`).
  검증자가 예고한 `2296/1/2520` 에서 셀 하나가 더 는 값이다.
...
- **미검증 = 2커밋**(`4bd1a3d` 조건 폐쇄 · `cd1d82d` 2026-08-16). `0b1c6f3` 과 docs 셋은
  [2026-08-20 검증](../../verifications/2026-08-20/mypy_guard_slice.md)이 덮었고 **조건도 닫혔다.**
  `4bd1a3d` 는 자기 검증뿐이며 볼 만한 축은 **허용 키 집합이 과소한가**다(`[mypy]` 에 정당하게
