 ...
 * 4. **목록이 스타일시트와 같은가** — 아래 목록의 키 집합 ≡ 실제로 `var(--type-*)`
 *    를 쓰는 규칙 집합. 2번이 *"목록에 적힌 것"* 을 검사한다면 이쪽은 **목록 자체**를
 *    검사한다.
 *
 * **양방향**:
 ...
 * - 목록에서 행을 지우면(= 감시를 줄이면) 4번이, 새 규칙을 토큰으로 옮겨 놓고
 *   목록에 안 더하면(= 감시가 안 따라가면) **같은 4번**이 실패한다.
 *
 * ★ **다른 화면은 아직 리터럴이고 그것은 결함이 아니다**(D1=ⓒ 점진). 화면
 * 슬라이스가 자기 차례에 옮기고 **그 화면의 선택자를 아래 목록에 더한다.**
 *
 * ★ **[닫힘 2026-08-13] 종전에는 *"목록에 없는 선택자는 이 파일이 보지 않는다"*
 * 가 이 가드의 한계였다** — 행을 지우면 아무 셀도 실패하지 않았다(뮤테이션 M5
 * 실측, 2026-08-12). 4번이 그 자리를 닫는다: 목록을 **스타일시트에서 유도한 집합과
 * 대조**하므로 행을 지우는 순간 불일치가 된다. 10.4 [`pageLayout.test.ts`](./pageLayout.test.ts)
 * 가 마크업에서 유도해 같은 계열의 한계를 피한 것과 같은 처방이다.
 */
...
describe("타이포 축 (Phase 10 Slice 10.3)", () => {
  ...
  it("keeps the migration list identical to what the stylesheet actually migrated", () => {
    /**
     * ★ **위 셀은 목록이 부른 이름만 본다** — 행을 지우면 그 선택자는 그냥
     * 검사 대상에서 빠지고 아무것도 실패하지 않는다(M5). 그래서 목록을
     * **스타일시트에서 유도한 집합**과 맞대어, 목록이 줄거나 뒤처지는 것 자체를
     * 불일치로 만든다.
     *
     * 유도 기준은 *"font-size 를 `var(--type-*)` 로 선언한 규칙"* 이다 — 이관의
     * 정의가 정확히 그것이고, 그래서 새 화면을 옮기는 사람은 목록에 더하는 것
     * 말고는 이 셀을 통과시킬 방법이 없다.
     */
    const migratedInStylesheet = [...declaredFontSize]
      .filter(([, value]) => value.startsWith("var(--type-"))
      .map(([selector]) => selector)
      .sort();

    expect(Object.keys(MIGRATED).sort()).toEqual(migratedInStylesheet);
  });
});
