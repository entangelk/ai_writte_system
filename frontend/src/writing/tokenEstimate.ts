...
/**
 * 유닛 본문 글자수 상한 — 오너 결정 2026-09-08("하나의 원고치고는 적은 편이라 6000자로";
 * 종전 4000 = D5-2 2026-08-27). 서버 기본값 `DRAFT_RAW_TEXT_MAX_CHARS`(app/env.py)의
 * 미러다: 서버는 이 상한을 저장 스키마(422)와 accept 합성(400, provider 호출 앞) 둘 다에
 * 시행하므로, 여기의 경고·저장 차단은 사용자가 서버 거부를 만나기 *전에* 알게 하는 사전
 * 안내일 뿐 최후 방어가 아니다. ★ 잘라내기로 시행하지 않는다 — textarea maxLength 는
 * 붙여넣기를 몰래 잘라 정본을 손상시킨다.
 *
 * 근거(2026-08-27 실측 정정): 원고는 매 생성마다 검색 조각으로 프롬프트에 실린다 —
 * 현재 장면 문단 전부(제목 없는 유닛이면 유닛 전체)+직전 문단 5개를 꺼내 예산(요청 상한
 * 8192)까지 편집해 넣는다. 유닛이 길어지면 다른 조각이 밀려 이어쓰기가 직전 흐름·기억을
 * 잃는다(조용한 품질 저하). 6000자 ≈ 3,529 tok = 예산의 **~43%** 로, 4000자(~29%) 때보다
 * 현재 장면이 예산을 더 많이 가져간다 — 오너가 알고 택한 트레이드오프다.
 */
export const RAW_TEXT_MAX_CHARS = 6000;
