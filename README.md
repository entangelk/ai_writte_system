...
| 첫 화면(익명) | 로그인 |
|---|---|
| [![랜딩 화면](docs/img/랜딩페이지_3x2.png)](docs/img/랜딩페이지_3x2.png) | [![로그인 화면](docs/img/로그인페이지_3x2.png)](docs/img/로그인페이지_3x2.png) |

| 프로젝트 목록 | 원고 목록(장·장면) |
|---|---|
| [![프로젝트 목록 화면](docs/img/프로젝트목록_3x2.png)](docs/img/프로젝트목록_3x2.png) | [![원고 목록 화면](docs/img/원고목록_3x2.png)](docs/img/원고목록_3x2.png) |

| 원고 작업공간(집필 도구 서랍) | 관측 대시보드 |
|---|---|
| [![원고 작업공간 화면](docs/img/글편집페이지_3x2.png)](docs/img/글편집페이지_3x2.png) | [![관측 대시보드 화면](docs/img/관측대시보드_3x2.png)](docs/img/관측대시보드_3x2.png) |

| 기억 후보 목록(정체성 그룹) | 기억 후보 검토(근거) |
|---|---|
| [![기억 후보 목록 화면](docs/img/기억후보목록_3x2.png)](docs/img/기억후보목록_3x2.png) | [![기억 후보 검토 화면](docs/img/기억후보검토_3x2.png)](docs/img/기억후보검토_3x2.png) |

**움직이는 모습** — 실제 LLM 파이프라인이 돌아가는 녹화(클릭하면 원본 크기). 왼쪽은
긴 출력이 `202 + 잡`으로 바뀌어 워커가 받는 [비동기 경로](docs/architecture.md#5-3-비동기-생성--mediumlong-프리셋)다:

| 이어쓰기 생성 — 지시 → 백그라운드 생성 → 후보 도착 | 기억 후보 검토 — 근거 확인 → 승인 |
|---|---|
| [![이어쓰기 생성 GIF](docs/img/이어쓰기생성.gif)](docs/img/이어쓰기생성.gif) | [![기억 후보 검토 GIF](docs/img/기억후보검토.gif)](docs/img/기억후보검토.gif) |

<sub>스크린샷 8장은 같은 규칙(배경색 패딩 3:2 · 1200px)으로 정규했다 —
[`docs/img/normalize.py`](docs/img/normalize.py). 데모 데이터(검은 태양의 연대기)는 실제
파이프라인 — 분석 추출·후보 그룹화·승인 — 을 돌려 만들었다. 촬영은 이 저장소의 현재
`main`(Phase N 중성 스튜디오 반영)을 빌드한 스택에서 했다(2026-09-14).</sub>
...
| **[개발](#개발--어떻게-만들어졌는가-설계-결정과-검증)** | 결정을 어떻게 내리고 어떻게 검증하는가 | [`docs/system-contract-sot.md`](docs/system-contract-sot.md) · [`docs/plans/README.md`](docs/plans/README.md) · [`docs/verifications/README.md`](docs/verifications/README.md) |
...
| 단계 | 산출물 | 규모 |
|---|---|---|
| **① 결정 브리프** — 선택지 표(`선택지·설명·장점·단점`) + 구현자 추천 + 유예 항목을 적고 **멈춘다**. 추측 구현 금지 | [`docs/plans/`](docs/plans/README.md) | **119개** |
| **② 구현 + 회귀 가드** — 가드는 **양방향**이어야 한다: 원래 결함을 재현하면 실패(under-strict), 과잉 교정으로 정상 경로를 깨도 실패(over-strict) | `tests/` | **3,080 passed / 4,274 subtests** |
| **③ 독립 검증** — 구현자가 아닌 세션이 **뮤테이션**(고친 것을 되돌려 회귀가 다시 실패하는지)으로 반증을 시도한다 | [`docs/verifications/`](docs/verifications/README.md) | **317건 / 74일치** |
| **④ 정본 개정** — 계약이 바뀌면 SoT 버전을 올리고 **변경 이유와 근거 링크**를 남긴다 | [`docs/system-contract-sot.md`](docs/system-contract-sot.md) | **v1.8.72**, 변경이력 전량 보존 |
| **⑤ 인수인계** — 다음 작업자가 시간을 잃지 않도록 **함정**을 기록한다 | [`HANDOFF.md`](HANDOFF.md) · [`docs/daily_logs/`](docs/daily_logs/) | 일자별 |

**검증 판정 분포는 합격 216 · 조건부 합격 96 · 불합격 5**다. **조건부 합격이 30%**라는 것이
...
| 계획 · 결정 브리프 인덱스 (142개) | [`docs/plans/README.md`](docs/plans/README.md) |
| 독립 검증 기록 (317건) | [`docs/verifications/README.md`](docs/verifications/README.md) |
