...
| **② 구현 + 회귀 가드** — 가드는 **양방향**이어야 한다: 원래 결함을 재현하면 실패(under-strict), 과잉 교정으로 정상 경로를 깨도 실패(over-strict) | `tests/` | **3,023 passed / 4,117 subtests** |
| **③ 독립 검증** — 구현자가 아닌 세션이 **뮤테이션**(고친 것을 되돌려 회귀가 다시 실패하는지)으로 반증을 시도한다 | [`docs/verifications/`](docs/verifications/README.md) | **296건 / 70일치** |
...
**검증 판정 분포는 합격 196 · 조건부 합격 95 · 불합격 5**다. **조건부 합격이 32%**라는 것이
...
| 독립 검증 기록 (296건) | [`docs/verifications/README.md`](docs/verifications/README.md) |
