...
| 원고 목록(장·장면) | 관측 대시보드 |
|---|---|
| [![원고 목록 화면](docs/img/원고목록_3x2.png)](docs/img/원고목록_3x2.png) | [![관측 대시보드 화면](docs/img/관측대시보드_3x2.png)](docs/img/관측대시보드_3x2.png) |
...
<sub>스크린샷은 같은 규칙(배경색 패딩 3:2 · 1200px)으로 정규했다 —
[`docs/img/normalize.py`](docs/img/normalize.py). 로그인 화면 포함 7장이 `docs/img/`에 있다.
데모 데이터(검은 태양의 연대기)는 실제 파이프라인 — 분석 추출·후보 그룹화·승인 — 을
돌려 만들었다.</sub>
...
| **② 구현 + 회귀 가드** — 가드는 **양방향**이어야 한다: 원래 결함을 재현하면 실패(under-strict), 과잉 교정으로 정상 경로를 깨도 실패(over-strict) | `tests/` | **3,069 passed / 4,238 subtests** |
