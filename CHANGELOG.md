...
- Gemma Q4 benchmark harness를 추가했다. `scripts/benchmark_llm_provider.py`는 기존 `HttpxJsonTransport` + `LlamaCppProvider` 계약으로 외부 llama.cpp-compatible endpoint에 `short_smoke`, `json_extraction`, `continue_scene` case를 반복 호출하고, latency/token/error summary와 raw run을 JSON으로 출력한다. 실제 live endpoint 수치 없이 budget/retry production 기본값을 추측하지 않기 위해 harness만 추가했으며, 기본값 확정은 benchmark report 이후로 남긴다.
