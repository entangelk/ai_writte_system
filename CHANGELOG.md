...
- project/draft rename API를 추가해 Core SOT CRUD의 "수정"을 완성했다(plan 01 §13 범위 중 마지막 미구현). `PATCH /projects/{id}`(name)와 `PATCH /projects/{id}/drafts/{draft_id}`(title)를 추가하고 service에 `rename_project`/`rename_draft`를 더했다. 없음/cross-project draft는 404, archived project/draft의 rename은 409(기존 save_draft/create_draft와 일관된 archive=쓰기차단 계약). repo/SoT 계약 변경 없음. archive API endpoint는 후속(현재 archive는 service-only). 전체 199개(Mongo 미연결 27 skip / 연결 시 전통과).
