...
class ActivityLogService:
    ...
    격리 경계를 서비스 안에 둔 것은 의도다. 20 개 경로(호출 지점은 21 — ``accept``
    가 성공·502 partial 두 분기에서 부른다)가 각자 ``try/except`` 를 쓰면 한 곳이
    빠지는 순간 그 경로만 fail-closed 가 되고, 그 차이는 저장소가 죽기 전까지 아무
    테스트도 못 본다. 판정 기준은 코드가 이미 문장으로 갖고 있다 —
    ...
    **대가는 조용한 구멍이다**(브리프 §후속 고려): 로그가 비어도 아무도 모른다.
    ...
    def list_for_projects(
        ...
        **빈 집합은 "전부"가 아니라 "없음"이다.** 아래 단락은 **왕복을 아끼는
        것**이지 그 성질의 방어가 아니다 — 성질 자체는 어댑터가 갖고 있고
        (`$in: []` 는 아무것도 안 맞는다) **그것을 잠그는 것은 저장소를 직접
        부르는 셀**이다(`test_an_empty_project_set_reads_nothing_at_the_store`).
        2026-08-10 뮤테이션 실측: 이 단락만 지우거나 어댑터만 무필터로 바꾸면
        **둘 다 상위 셀이 통과한다** — 두 층이 서로를 가리기 때문이다.
        """
        if not project_ids:
            return ()
