...
#
#   항목 7,656 + system 465 + `long` 후보 산문 4,033 + 출력 상한 6,144 = **18,298 > 16,384**
#
# 즉 창 16384에서도 **1,914 넘는다**(종전 `len/4`·4096에서는 19,546으로 3,162 넘었으니 개선이지
# 해소는 아니다). 후보 산문을 빼고 8,121 + 6,144 = 14,265만 보면 "들어간다"로 오독하게 되는데,
# report는 그 산문을 대상으로 하는 호출이라 항상 함께 실린다.
#
# **그래서 이 초과는 조용히 잘리지 않고 K-3 가드가 400으로 거부한다**(실패가 보인다는 것이
# 가드의 값이다). 근본 해결은 보류된 **R-a**(report 전용 예산)이고 오너 결정 대기다.
# 창 8192 배포에서는 더 일찍 걸리므로 알파는 `LLAMA_CTX_SIZE=16384`가 전제다(HANDOFF 함정).
# **지금 베타 프로젝트는 회계 2,876이라 이 선에 닿지 않는다** — 예산을 실제로 꽉 채우는 큰
# 프로젝트에서만 만나는 경계다.
#
