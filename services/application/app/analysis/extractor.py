                ...
                messages=(
                    ChatMessage(
                        ...
                        content=(
                            ...
                            "candidates array. Each source_anchors item must "
                            "contain only a zero-based source_ref_index from "
                            "the supplied request-local catalog."
                        ),
                    ),
                    ChatMessage(
                        role="user",
                        content=json.dumps(
                            {
                                "snapshot_raw_text": snapshot.raw_text,
                                "source_ref_catalog": [
                                    {"source_ref_index": index, "quote": ref.quote}
                                    for index, ref in enumerate(self._source_refs)
                                ],
                            },
                            ensure_ascii=False,
                            separators=(",", ":"),
...
class VersionedPromptAnalysisExtractionAdapter:
    ...
    async def extract(self, snapshot: SnapshotText) -> tuple[AnalysisCandidateDraft, ...]:
        ...
        try:
            # v7: 모델은 요청 로컬 순번만 내고, 영속 id/span/quote/hash는 서버가
            # 원본 SourceRef에서 조립한다. 서버 id는 LLM 요청에 들어가지 않는다.
            return parse_analysis_extraction(
...
def _repair_request(
    *,
    ...
    payload = {
        ...
        # Repair regenerates from server-sanitized inputs. Echoing invalid output
        # could send a model-invented or copied backend identifier back across
        # the boundary that v7 closes.
...
def _source_anchor(
    ...
    # v7: 모델은 요청 로컬 배열 순번만 고른다. 영속 id와 나머지 provenance는
    # 서버가 같은 배열의 SourceRef에서 조립한다. 레거시 id 출력은 거부된다.
    if not isinstance(item, Mapping):
        raise AnalysisExtractionError("source_anchor must be an object")
    _require_fields(item, {"source_ref_index"})
    source_ref_index = item["source_ref_index"]
    if isinstance(source_ref_index, bool) or not isinstance(source_ref_index, int):
        raise AnalysisExtractionError("source_ref_index must be an integer")
    if source_ref_index < 0 or source_ref_index >= len(catalog):
        raise AnalysisExtractionError(
            "source_ref_index must identify an item in the source_ref catalog"
        )
    source_ref = catalog[source_ref_index]
