...
- **공유 심볼**(Pydantic 모델·``_REQUIRE_*``·``_owned``/``_billable`` 헬퍼·에러 dict)
  은 ``from ..main import`` 로 가져온다. ``main`` 이 이 패키지를 ``create_app``
  직전에 import 하므로 순환 import 가 성립하지 않는다(필요한 심볼이 전부 그 앞에
  정의돼 있다).
