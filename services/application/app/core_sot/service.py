...
# 장면 메모 본문 한 글자수 상한 — 오너 결정(2026-08-31). 당시 근거는 원고 본문 상한
# (``env.DRAFT_RAW_TEXT_MAX_CHARS`` = 4000)의 3배 = 3 장면 분량이었는데, 원고 상한이
# 6000 으로 오르며(오너 2026-09-08) 배수는 2 가 됐다. 12000 은 오너가 정한 값 그대로다 —
# 원고 상한의 이어쓰기-예산 근거는 애초에 메모에 물려받지 않았고(메모는 프롬프트·export
# 에 실리지 않는다, D2=A), 값을 정하는 것은 쓰임이다: 메모는 나중에 원고로 옮겨 갈 재료를
# 모아 두는 곳이라 여러 장면 분량을 담을 수 있어야 한다. Mongo 16MB 문서 한계와는 두
# 자릿수 떨어져 있고, 상한의 역할은 창 안전이 아니라 저장 폭주 방지와 UI 가 기댈 명확한
# 경계다(`tests/test_scene_notes.py` 독스트링이 이 서술의 정본이다).
