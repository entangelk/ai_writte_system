...
- 본문 상한은 `SCENE_NOTE_MAX_CHARS`(12000자, 오너 2026-08-31). 당시 근거는 "원고 본문
  상한 4000 의 3배 = 3 장면 분량"이었는데, **원고 상한이 6000 으로 올라(오너 2026-09-08)
  그 배수는 2 가 됐다**. 12000 은 오너가 정한 값 그대로 두었다 — 메모는 프롬프트에
  실리지 않아 이어쓰기-예산 근거를 애초에 물려받지 않았고, 값을 정하는 것은 "여러 장면
  분량의 재료를 담는다"는 쓰임이기 때문이다. 상한을 다시 볼 자리는 여기다.
