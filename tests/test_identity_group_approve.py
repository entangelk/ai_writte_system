...
class GroupApproveOrchestrationTest(unittest.TestCase):
    ...
    def test_steps_are_sorted_by_candidate_id_not_by_membership_order(self):
        ...
        (Slice 4 M8 과 같은 형태의 우연 통과).

        ★ 어긋남을 만들려면 클록을 직접 전진시켜야 한다. `_FixedClock` 은 자동으로
        가지 않으므로 `_open_group` 처럼 한 번에 넣으면 **멤버 added_at 이 전부
        같고**, 그러면 `list_members` 가 동률을 candidate_id 로 깨서 결과가 이미
        id 순이 된다 — 초안 셀이 그 함정에 빠져 정렬 제거 변이를 놓쳤다.
        여기서는 **id 가 큰 멤버를 먼저·이른 시각에** 넣어 두 순서를 반대로 만든다.
        """
        judge = _ScriptedJudge(_no_change())
        w = _build(judge)
        ...
        self.assertGreater(earlier_added.id, later_added.id)
        group = w["groups"].create_group(w["project_id"], CHARACTER)
        for candidate in (earlier_added, later_added):
            w["groups"].add_member(
                project_id=w["project_id"], group_id=group.group_id,
                candidate_id=candidate.id, candidate_type=CHARACTER,
            )
            w["clock"].advance()  # added_at 을 실제로 갈라 놓는다
        listed = [
            member.candidate_id
            for member in w["groups"].list_members(w["project_id"], group.group_id)
        ]
        self.assertEqual(
            listed, [earlier_added.id, later_added.id],
            "가입 순서가 id 순서와 반대여야 이 셀이 의미를 갖는다",
        )
        ...
        self.assertEqual(
            ordering, [later_added.id, earlier_added.id],
